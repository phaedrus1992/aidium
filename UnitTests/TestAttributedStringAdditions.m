...
- (void)testLinkedAttributedString
{
	...
	NSString *linkURLString = @"https://github.com/phaedrus1992/adiumy/";
	...
	STAssertNoThrow(attributedString = [NSAttributedString attributedStringWithLinkLabel:linkLabel
																		 linkDestination:linkURLString],
					@"attributedStringWithLinkLabel:linkDestination: threw an exception");
	STAssertNotNil(attributedString, @"attributedStringWithLinkLabel:linkDestination: returned nil");
	STAssertEqualObjects([attributedString string], linkLabel,
						 @"Attributed string's text is not equal to the original string");
	STAssertEquals([attributedString length], [linkLabel length],
				   @"Attributed string is not the same length (%lu) as the original string (%lu)",
				   [attributedString length], [linkLabel length]);
...
- (void)testAttributedStringWithLinkedSubstring
{
	...
	// First, try a string containing a URL.
	STAssertNoThrow(attributedString = [NSAttributedString attributedStringWithString:linkLabel
																			linkRange:intendedLinkRange
																	  linkDestination:linkURLString],
					@"attributedStringWithLinkLabel:linkDestination: threw an exception");
	STAssertNotNil(attributedString, @"attributedStringWithLinkLabel:linkDestination: returned nil");
	STAssertEqualObjects([attributedString string], linkLabel,
	...
	STAssertEquals([attributedString length], [linkLabel length],
				   @"Attributed string is not the same length (%lu) as the original string (%lu)",
				   [attributedString length], [linkLabel length]);
	linkValue = [attributedString attribute:NSLinkAttributeName
									atIndex:intendedLinkRange.location
...
{
	NSMutableAttributedString *input =
		[[[NSMutableAttributedString alloc] initWithString:@"Adium requires Growl."] autorelease];
	[input addAttribute:NSLinkAttributeName
				  value:[NSURL URLWithString:@"https://github.com/phaedrus1992/adiumy/"]
				  range:[[input string] rangeOfString:@"Adium"]];
	[input addAttribute:NSLinkAttributeName
				  value:[NSURL URLWithString:@"http://growl.info/"]
				  range:[[input string] rangeOfString:@"Growl"]];

	NSAttributedString *result = nil;
	STAssertNoThrow(result = [input attributedStringByConvertingLinksToStrings],
...
- (void)testAttributedStringByConvertingLinksToURLStrings
{
	...
	NSAttributedString *result = nil;
	STAssertNoThrow(result = [input attributedStringByConvertingLinksToURLStrings],
					@"-attributedStringByConvertingLinksToURLStrings threw an exception");
	STAssertNotNil(result, @"-attributedStringByConvertingLinksToURLStrings returned nil");

	STAssertEqualObjects([result string], @"https://github.com/phaedrus1992/adiumy/ requires http://growl.info/.",
						 @"-attributedStringByConvertingLinksToURLStrings did not correctly expand the links");
}
