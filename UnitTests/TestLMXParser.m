 ...
 */
#import "TestLMXParser.h"
#import "AIUnitTestUtilities.h"

#import <LMX/LMXParser.h>

@interface TestLMXParserDelegate : NSObject <LMXParserDelegate> {
...
@implementation TestLMXParserDelegate
...
- (void)testReverseParse
{
	...
	NSData *xmlData = [@"<a><b>c</b></a>" dataUsingEncoding:NSUTF8StringEncoding];
	STAssertNotNil(xmlData, @"Could not encode XML data");

	TestLMXParserDelegate *delegate = [[TestLMXParserDelegate alloc] init];
	LMXParser *parser = [[LMXParser alloc] initWithData:xmlData];
	...
	AISimplifiedAssertEqualObjects([delegate callbackLog], expectedLog,
								   @"LMX callback order should be reverse document order");
