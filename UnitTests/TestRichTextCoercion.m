...
#warning 64BIT: Check formatting arguments
	...
	if ([mgr createDirectoryAtPath:scriptSuitesFolder withIntermediateDirectories:YES attributes:nil error:NULL]) {
		...
		NSTask *sdp = [NSTask launchedTaskWithLaunchPath:@"/usr/bin/sdp" arguments:args];
		[sdp waitUntilExit];
		STAssertEquals([sdp terminationStatus], 0, @"sdp didn't exited with status 0");

		NSString *scriptSuitePath = [scriptSuitesFolder
			stringByAppendingPathComponent:[suiteName stringByAppendingPathExtension:@"scriptSuite"]];
...
#warning 64BIT: Check formatting arguments
	...
	if ([mgr createDirectoryAtPath:scriptTerminologiesFolder
		...
		NSString *scriptTerminologyPath = [scriptTerminologiesFolder
			stringByAppendingPathComponent:[suiteName stringByAppendingPathExtension:@"scriptTerminology"]];
...
- (NSDictionary *)dictionaryByMergingSuiteDictionary:(NSDictionary *)scriptSuite
						   withTerminologyDictionary:(NSDictionary *)scriptTerminology
{
	...
	while ((className = [keyEnum nextObject])) {
		...
		[classDict translate:nil add:[terminologyClasses objectForKey:className] remove:nil];
		[classes setObject:classDict forKey:className];
	}
	NSLog(@" After add: %@", classes);

	[merged setObject:classes forKey:@"Classes"];
...
- (void)setUp
{
	...
	NSDictionary *scriptSuite = [self dictionaryForScriptSuiteNamed:@"NSTextSuite"
													   fromSdefFile:[testBundle pathForResource:@"Adium"
																						 ofType:@"sdef"]];
	NSDictionary *scriptTerminology = [self dictionaryForScriptTerminologyNamed:@"NSTextSuite"
																   fromSdefFile:[testBundle pathForResource:@"Adium"
																									 ofType:@"sdef"]];
	NSDictionary *scriptSuiteAndTerminology = [self dictionaryByMergingSuiteDictionary:scriptSuite
															 withTerminologyDictionary:scriptTerminology];
	//	NSLog(@"scriptSuite: %@", scriptSuite);
	[[NSScriptSuiteRegistry sharedScriptSuiteRegistry] loadSuiteWithDictionary:scriptSuiteAndTerminology
...
- (id)giveMeA:(Class)class ofThisString:(NSString *)str
{
	...
	if ([class isSubclassOfClass:[NSString class]]) {
		...
		NSDictionary *attrs = [NSDictionary dictionaryWithObject:[NSFont fontWithName:@"Helvetica" size:12.0f]
...
- (void)testAttributedStringToPlainText {
	...
	STAssertTrue([output isKindOfClass:[NSString class]], @"Coercion to NSString must result in an NSString; instead, it
resulted in an %@", [output class]); STAssertEqualObjects(output, input, @"Coercion must not change the object's
value");
}
...
- (void)testTextStorageToPlainText
{
	...
	STAssertNotNil(output, @"Coercion returned nil");
	STAssertTrue([output isKindOfClass:[NSString class]],
				 @"Coercion to NSString must result in an NSString; instead, it resulted in an %@", [output class]);
...
/*- (void)testPlainTextToAttributedString {
	...
	STAssertNotNil(output, @"Coercion returned nil");
	STAssertTrue([output isKindOfClass:destClass], @"Coercion to %@ must result in an %@; instead, it resulted in an
%@", destClass, destClass, [output class]); STAssertEqualObjects(output, input, @"Coercion must not change the object's
value");
}
- (void)testPlainTextToMutableAttributedString {
	...
	NSString *input = [self giveMeA:[NSString class] ofThisString:str];
	NSMutableAttributedString *output = [[NSScriptCoercionHandler sharedCoercionHandler] coerceValue:input
toClass:destClass];
...
- (void)testPlainTextToTextStorage
{
	...
	STAssertNotNil(output, @"Coercion returned nil");
	STAssertTrue([output isKindOfClass:destClass],
				 @"Coercion to %@ must result in an %@; instead, it resulted in an %@", destClass, destClass,
				 [output class]);
...
/*- (void)testMutableAttributedStringToPlainTextWithMutations {
	...
	//Mutate the original.
	NSDictionary *attrs = [NSDictionary dictionaryWithObject:[NSFont fontWithName:@"Courier" size:14.0f]
forKey:NSFontAttributeName]; NSAttributedString *replacement = [[[NSAttributedString alloc] initWithString:@" (one of
the slogans from the old Adium X site)" attributes:attrs] autorelease]; [input replaceCharactersInRange:(NSRange){
[input length], 0U } withAttributedString:replacement];

	STAssertNotNil(output, @"Coercion returned nil");
	STAssertTrue([output isKindOfClass:[NSString class]], @"Coercion to NSString must result in an NSString; instead, it
resulted in an %@", [output class]); STAssertEqualObjects(output, str, @"Coercion must not change the object's value");
}
*/
- (void)testTextStorageToPlainTextWithMutations
{
	...
	NSAttributedString *replacement =
		[[[NSAttributedString alloc] initWithString:@" (one of the slogans from the old Adium X site)"
										 attributes:attrs] autorelease];
	[input replaceCharactersInRange:(NSRange){[input length], 0U} withAttributedString:replacement];
...
{
	NSString *source = @"\"Ducks eat for free at Subway!\" as rich text";
