...
{
	return ([[AXCDocumentController sharedDocumentController]
				documentClassForType:[[self documentTypes] objectAtIndex:row]] != Nil);
}
...
- (BOOL)menu:(NSMenu *)menu updateItem:(NSMenuItem *)item atIndex:(int)index shouldCancel:(BOOL)shouldCancel
{
	...
	// Note: If this raises an exception, make sure that all the document types' names are listed in the relevant
...
- (BOOL)validateMenuItem:(NSMenuItem *)item
{
	...
	if ([item tag])
		...
		return ([[AXCDocumentController sharedDocumentController] documentClassForType:[item representedObject]] !=
				Nil);
}
