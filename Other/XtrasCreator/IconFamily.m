...
+ (NSImage *)resampleImage:(NSImage *)image
				toIconWidth:(int)width
	usingImageInterpolation:(NSImageInterpolation)imageInterpolation;
						...
						usingImageInterpolation:(NSImageInterpolation)imageInterpolation
{
	return [[[IconFamily alloc] initWithThumbnailsOfImage:image
								  usingImageInterpolation:imageInterpolation] autorelease];
}

// This is IconFamily's designated initializer.  It creates a new IconFamily that initially has no elements.
//
// The proper way to do this is to simply allocate a zero-sized handle (not to be confused with an empty handle) and
// assign it to hIconFamily.  This technique works on Mac OS X 10.2 as well as on 10.0.x and 10.1.x.  Our previous
// technique of allocating an IconFamily struct with a resourceSize of 0 no longer works as of Mac OS X 10.2.
- init
{
	if ((self = [super init])) {
...
- initWithIconOfFile:(NSString *)path
{
	...
	if (self) {
		...
		result = GetIconRefFromFile(&fileSpec, &iconRef, &label);
		...
		result = IconRefToIconFamily(iconRef, kSelectorAllAvailableData, &hIconFamily);
...
- initWithThumbnailsOfImage:(NSImage *)image usingImageInterpolation:(NSImageInterpolation)imageInterpolation
{
	...
	iconImage32x32 = [IconFamily resampleImage:bitmappedIconImage128x128
								   toIconWidth:32
	...
	iconImage16x16 = [IconFamily resampleImage:bitmappedIconImage128x128
								   toIconWidth:16
					   usingImageInterpolation:imageInterpolation];
...
- (NSBitmapImageRep *)bitmapImageRepWithAlphaForIconFamilyElement:(OSType)elementType;
{
	...
	if (pBitmapImageRepBitmapData) {
		memcpy(pBitmapImageRepBitmapData, *hRawBitmapData, pixelsWide * pixelsWide * 4);
	}
...
{
	NSImage *image = NULL;
	image = [[[NSImage alloc] initWithData:[NSData dataWithBytes:*hIconFamily
														  length:GetHandleSize((Handle)hIconFamily)]] autorelease];
...
- (BOOL)setAsCustomIconForFile:(NSString *)path withCompatibility:(BOOL)compat
{
	...
	// Get an FSRef and an FSSpec for the target file, and an FSRef for its parent directory that we can use in the
	// FNNotify() call below.
	...
	AddResource((Handle)hIconFamilyCopy, kIconFamilyType, kCustomIconResource, "\p");
	if (ResError() != noErr) {
		CloseResFile(file);
		return NO;
	}
	...
	// Notify the system that the directory containing the file has changed, to give Finder the chance to find out about
	// the file's new custom icon.
...
- (BOOL)setAsCustomIconForDirectory:(NSString *)path withCompatibility:(BOOL)compat
{
	...
	result = FSGetCatalogInfo(&targetFolderFSRef, kFSCatInfoFinderInfo, &catInfo, nil, nil, nil);
	if (result != noErr)
		return NO;

	((DInfo *)catInfo.finderInfo)->frFlags =
		(((DInfo *)catInfo.finderInfo)->frFlags | kHasCustomIcon) & ~kHasBeenInited;

	FSSetCatalogInfo(&targetFolderFSRef, kFSCatInfoFinderInfo, &catInfo);
	if (result != noErr)
		return NO;

	// Notify the system that the target directory has changed, to give Finder the chance to find out about its new
	// custom icon.
...
+ (Handle)get32BitDataFromBitmapImageRep:(NSBitmapImageRep *)bitmapImageRep requiredPixelSize:(int)requiredPixelSize
{
	...
	if (bitsPerSample != 8) {
		NSLog(@"get32BitDataFromBitmapImageRep:requiredPixelSize: returning NULL due to bitsPerSample == %d",
			  bitsPerSample);
		return NULL;
	}
	...
	if (((samplesPerPixel == 3) && (bitsPerPixel == 24)) || ((samplesPerPixel == 4) && (bitsPerPixel == 32))) {
		...
		NSLog(@"get32BitDataFromBitmapImageRep:requiredPixelSize: returning NULL due to samplesPerPixel == %d, "
			  @"bitsPerPixel == %",
			  samplesPerPixel, bitsPerPixel);
		return NULL;
	}
...
+ (Handle)get8BitDataFromBitmapImageRep:(NSBitmapImageRep *)bitmapImageRep requiredPixelSize:(int)requiredPixelSize
{
	...
	if (bitsPerSample != 8) {
		NSLog(@"get8BitDataFromBitmapImageRep:requiredPixelSize: returning NULL due to bitsPerSample == %d",
			  bitsPerSample);
		return NULL;
	}
	...
	if (((samplesPerPixel == 3) && (bitsPerPixel == 24)) || ((samplesPerPixel == 4) && (bitsPerPixel == 32))) {
		...
		NSLog(@"get8BitDataFromBitmapImageRep:requiredPixelSize: returning NULL due to samplesPerPixel == %d, "
			  @"bitsPerPixel == %",
			  samplesPerPixel, bitsPerPixel);
		return NULL;
	}
...
+ (Handle)get8BitMaskFromBitmapImageRep:(NSBitmapImageRep *)bitmapImageRep requiredPixelSize:(int)requiredPixelSize
{
	...
	if (bitsPerSample != 8) {
		NSLog(@"get8BitMaskFromBitmapImageRep:requiredPixelSize: returning NULL due to bitsPerSample == %d",
			  bitsPerSample);
		return NULL;
	}

	if (((samplesPerPixel == 1) && (bitsPerPixel == 8)) || ((samplesPerPixel == 3) && (bitsPerPixel == 24)) ||
		...
		NSLog(@"get8BitMaskFromBitmapImageRep:requiredPixelSize: returning NULL due to samplesPerPixel == %d, "
			  @"bitsPerPixel == %",
			  samplesPerPixel, bitsPerPixel);
		return NULL;
	}
...
+ (Handle)get1BitMaskFromBitmapImageRep:(NSBitmapImageRep *)bitmapImageRep requiredPixelSize:(int)requiredPixelSize
{
	...
	if (((bitsPerPixel == 1) && (samplesPerPixel == 1) && (bitsPerSample == 1)) ||
		((bitsPerPixel == 8) && (samplesPerPixel == 1) && (bitsPerSample == 8)) ||
		((bitsPerPixel == 24) && (samplesPerPixel == 3) && (bitsPerSample == 8)) ||
		((bitsPerPixel == 32) && (samplesPerPixel == 4) && (bitsPerSample == 8))) {
		rawDataSize = (pixelsWide * pixelsHigh) / 4;
		hRawData = NewHandle(rawDataSize);
		if (hRawData == NULL)
			return NULL;
		...
		if (bitsPerPixel == 32) {
			for (y = 0; y < pixelsHigh; y++) {
				...
				for (x = 0; x < pixelsWide; x += 8) {
					maskByte = 0;
					maskByte |= (*(unsigned *)pSrc & 0xff) ? 0x80 : 0;
					pSrc += 4;
					maskByte |= (*(unsigned *)pSrc & 0xff) ? 0x40 : 0;
					pSrc += 4;
					maskByte |= (*(unsigned *)pSrc & 0xff) ? 0x20 : 0;
					pSrc += 4;
					maskByte |= (*(unsigned *)pSrc & 0xff) ? 0x10 : 0;
					pSrc += 4;
					maskByte |= (*(unsigned *)pSrc & 0xff) ? 0x08 : 0;
					pSrc += 4;
					maskByte |= (*(unsigned *)pSrc & 0xff) ? 0x04 : 0;
					pSrc += 4;
					maskByte |= (*(unsigned *)pSrc & 0xff) ? 0x02 : 0;
					pSrc += 4;
					maskByte |= (*(unsigned *)pSrc & 0xff) ? 0x01 : 0;
					pSrc += 4;
					*pDest++ = maskByte;
				}
			}
		...
		NSLog(@"get1BitMaskFromBitmapImageRep:requiredPixelSize: returning NULL due to bitsPerPixel == %d, "
			  @"samplesPerPixel== %d, bitsPerSample == %d",
