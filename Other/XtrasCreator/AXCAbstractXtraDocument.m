...
- (void)addResource:(NSString *)path
{
	...
	[displayNames setObject:[[NSFileManager defaultManager] displayNameAtPath:[self absolutePathForFile:path]]
					 forKey:path];
	...
	while ((path = [pathsEnum nextObject])) {
		...
		/*now store the display name as well*/ {
			...
			if (image) {
				...
				displayName = [NSString stringWithFormat:@"%@ (%u%C%u)", displayName, (unsigned)size.width,
														 MULTIPLICATION_SIGN, (unsigned)size.height];
			}
...
- (BOOL)writeToFile:(NSString *)fileName ofType:(NSString *)docType
{
	...
	if (![manager fileExistsAtPath:bundlePath]) {
		...
		while ((resourcePath = [resourceEnu nextObject])) {
			...
			NSString *resourceDestPath =
				[resourcesPath stringByAppendingPathComponent:[resourcePath lastPathComponent]];

			// if these are not the same file...
			if (![resourceSrcPath isEqualToString:resourceDestPath]) {
				[manager copyPath:resourceSrcPath toPath:resourceDestPath handler:nil];
			}
		}
		...
		if ([readmeView isRichText])
			[[readmeView RTFFromRange:readmeRange]
				writeToFile:[resourcesPath stringByAppendingPathComponent:@"ReadMe.rtf"]
				 atomically:YES];
		else
			[[[readmeView string] dataUsingEncoding:NSUTF8StringEncoding]
				writeToFile:[resourcesPath stringByAppendingPathComponent:@"ReadMe.txt"]
				 atomically:YES];
...
{
	return [NSDictionary dictionaryWithObjectsAndKeys:@"English", kCFBundleDevelopmentRegionKey, name, kCFBundleNameKey,
													  [self OSType], @"CFBundlePackageType", bundleID,
													  kCFBundleIdentifierKey, [NSNumber numberWithInt:1],
													  @"XtraBundleVersion", @"1.0", kCFBundleInfoDictionaryVersionKey,
													  version, @"XtraVersion", author, @"XtraAuthors", nil];
}
	   ...
	   proposedDropOperation:(NSTableViewDropOperation)operation
{
	...
	if ((lastDragOperation != NSDragOperationNone) && (operation == NSTableViewDropOn)) {
		...
		if ([filenames count] == 1)
			row = [resources indexForInsortingObject:[filenames objectAtIndex:0U]
									   usingSelector:@selector(caseInsensitiveCompare:)];
		else {
			...
			// compare the y-coord to the height of the row. if below the middle, drop below the row; else, drop above
			// it.
