...
- (BOOL)getFSRef:(FSRef *)fsRef createFileIfNecessary:(BOOL)createFile
{
	...
	urlRef = CFURLCreateWithFileSystemPath(kCFAllocatorDefault, (CFStringRef)self, kCFURLPOSIXPathStyle,
										   FALSE /* isDirectory */);
	if (urlRef == NULL) {
...
+ (NSString *)pathForFSRef:(FSRef *)object
{
	...
	if (pathbuf != NULL) {
		...
		if (err == noErr) {
			path = [(NSString *)CFStringCreateWithCString(kCFAllocatorDefault, (char *)pathbuf, kCFStringEncodingUTF8)
				autorelease];
		}
