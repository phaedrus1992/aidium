...
static BOOL CheckSystemVersion(NSString **requiredVersionPtr, BOOL *sameMajorMinorPtr)
{
	...
	NSString *requiredVersion =
		[[[[NSBundle mainBundle] infoDictionary] objectForKey:@"LSEnvironment"] objectForKey:@"MinimumSystemVersion"];
...
int main(int argc, char *argv[])
{
	...
	if (!canLaunch) {
		...
		NSString *title =
			[NSString stringWithFormat:@"%@ requires Mac OS X %@ or later", applicationName, requiredVersion];
		...
		NSPanel *alertPanel =
			NSGetAlertPanel(title, messageFormat, defaultButton, alternateButton, nil, requiredVersion);
	...
	NSString *path =
		[[NSBundle mainBundle] pathForAuxiliaryExecutable:[NSString stringWithFormat:@"%@.real", executableName]];
