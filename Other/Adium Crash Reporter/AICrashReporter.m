...
- (IBAction)send:(id)sender
{
	if ([[textField_emailAddress stringValue] isEqualToString:@""] &&
		...
		NSBeginCriticalAlertSheet(
			AILocalizedString(@"Contact Information Required", nil), @"OK", nil, nil, window_MainWindow, nil, nil, nil,
			NULL,
			AILocalizedString(@"Please provide either your email address or IM name in case we need to contact you for "
							  @"additional information (or to suggest a solution).",
							  nil));
	} else {
...
- (void)buildAndSendReport
{
	...
	NSString *buildDateAndInfo = [NSString
		stringWithFormat:@"%@	(%@)", [dateFormatter stringForObjectValue:buildDate],
						 (buildUser ? [NSString stringWithFormat:@"%@.%@", buildNumber, buildUser] : buildNumber)];

	NSDictionary *crashReport = [NSDictionary
		dictionaryWithObjectsAndKeys:buildDateAndInfo, @"build", [textField_emailAddress stringValue], @"email",
									 [textField_accountIM stringValue], @"service_name", shortDescription,
									 @"short_desc", [textView_details string], @"desc", crashLog, @"log", nil];
...
- (void)sendReport:(NSDictionary *)crashReport
{
	...
	while (!data || [data length] == 0) {
		...
		if (!data || [data length] == 0) {
			if (NSRunAlertPanel(UNABLE_TO_SEND, [error localizedDescription], AILocalizedString(@"Try Again", nil),
								AILocalizedString(@"Quit", nil), nil) == NSAlertAlternateReturn) {
				break;
			}
		} else {
			sentCrashLog = YES;
...
- (void)windowWillClose:(id)sender
{
	...
	[[NSUserDefaults standardUserDefaults] setObject:[textField_accountIM stringValue] forKey:KEY_CRASH_AIM_ACCOUNT];
}
...
- (void)_loadBuildInformation
{
	...
	FILE *f;
	if ((path = (char *)[[[[NSBundle mainBundle] resourcePath] stringByAppendingString:@"/../../../buildnum"]
		...
		if (*unixDate) {
			buildDate =
				[[NSDate dateWithTimeIntervalSince1970:[[NSString stringWithCString:unixDate] doubleValue]] retain];
		}

		if (*whoami) {
			// If the application was built by one of these people, we assume that it is a release, which means we
			// should not show their username in the crash log. Otherwise, this is somebody's custom build, and
			// including the username marks it as such.
			...
			if ([buildUser isEqualToString:@"adamiser"] || [buildUser isEqualToString:@"evands"] ||
				[buildUser isEqualToString:@"jmelloy"] || [buildUser isEqualToString:@"durin"] ||
				[buildUser isEqualToString:@"rfackler"] || [buildUser isEqualToString:@"david"]) {
				[buildUser release];
				buildUser = nil;
			}
		}
		...
		if (NSRunAlertPanel(
				...
				[NSString
					stringWithFormat:AILocalizedString(
										 @"Your version of Adium is out of date, so crash reporting has been disabled. "
										 @"Your version is %@; the current version is %@. Please update to the latest "
										 @"version, as your crash may have already been fixed.",
										 nil),
									 [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleVersion"],
									 versionString],
				AILocalizedString(@"Update Now", nil), AILocalizedString(@"Cancel", nil),
...
#define UPDATE_TYPE_DICT                                                                                               \
	[NSDictionary dictionaryWithObjectsAndKeys:@"type", @"key", @"Update Type", @"visibleKey", @"beta", @"value",      \
											   @"Beta or Release Versions", @"visibleValue", nil]
#else
#define UPDATE_TYPE_DICT                                                                                               \
	[NSDictionary dictionaryWithObjectsAndKeys:@"type", @"key", @"Update Type", @"visibleKey", @"release", @"value",   \
											   @"Release Versions Only", @"visibleValue", nil]
#endif
