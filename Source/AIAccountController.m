...
- (void)controllerDidLoad
{
	...
	[adium.preferenceController registerDefaults:[NSDictionary dictionaryNamed:ACCOUNT_DEFAULT_PREFS
																	  forClass:[self class]]
...
{
	[adiumPasswords passwordForAccount:inAccount
...
- (void)passwordForProxyServer:(NSString *)server
			   ...
			   notifyingTarget:(id)inTarget
					  selector:(SEL)inSelector
					   context:(id)inContext
{
	[adiumPasswords passwordForProxyServer:server
								  userName:userName
						   notifyingTarget:inTarget
								  selector:inSelector
								   context:inContext];
...
- (void)passwordForType:(AISpecialPasswordType)inType
		...
		notifyingTarget:(id)inTarget
			   selector:(SEL)inSelector
				context:(id)inContext
{
	[adiumPasswords passwordForType:inType
						 forAccount:inAccount
					   promptOption:inOption
							   name:inName
					notifyingTarget:inTarget
						   selector:inSelector
							context:inContext];
...
- (AIAccount *)preferredAccountForSendingContentType:(NSString *)inType toContact:(AIListContact *)inContact
{
	...
	NSAssert([account.service.serviceClass isEqualToString:inContact.service.serviceClass],
			 @"Wrong account for the contact; differing services.");
...
{
	AIEditAccountWindowController *accountWindowController =
		[[AIEditAccountWindowController alloc] initWithAccount:account notifyingTarget:target];
...
- (NSScriptObjectSpecifier *)objectSpecifier
{
	...
	return [[[NSPropertySpecifier alloc] initWithContainerClassDescription:classDescription
														containerSpecifier:container
																	   key:@"accountController"] autorelease];
}
@end
