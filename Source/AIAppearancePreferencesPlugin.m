						 ...
						 firstTime:(BOOL)firstTime
{
	...
	if ([group isEqualToString:PREF_GROUP_APPEARANCE]) {
		if (firstTime || [key isEqualToString:KEY_STATUS_ICON_PACK]) {
			...
			if (!success) {
				NSString *name = [adium.preferenceController defaultPreferenceForKey:KEY_STATUS_ICON_PACK
																			  ...
																			  object:nil];
				path = [adium pathOfPackWithName:name extension:@"AdiumStatusIcons" resourceFolderName:@"Status Icons"];
		...
		if (firstTime || [key isEqualToString:KEY_MENU_BAR_ICONS]) {
			...
			[[NSNotificationCenter defaultCenter] postNotificationName:AIMenuBarIconsDidChangeNotification object:nil];
		}
...
- (void)invalidStatusSetActivated:(NSNotification *)inNotification
{
	[adium.preferenceController setPreference:nil forKey:KEY_STATUS_ICON_PACK group:PREF_GROUP_APPEARANCE];
...
- (void)applySetWithName:(NSString *)setName
			   extension:(NSString *)extension
				inFolder:(NSString *)folder
	   toPreferenceGroup:(NSString *)preferenceGroup
{
	...
	// Apply its values
	if (setDictionary) {
		[adium.preferenceController setPreferences:setDictionary inGroup:preferenceGroup];
	}
}

// Create a layout or theme set
- (BOOL)createSetFromPreferenceGroup:(NSString *)preferenceGroup
							withName:(NSString *)setName
						   extension:(NSString *)extension
							inFolder:(NSString *)folder
{
	NSString *path;
	...
	path = [[[adium applicationSupportDirectory] stringByAppendingPathComponent:folder]
...
- (BOOL)renameSetWithName:(NSString *)setName
				extension:(NSString *)extension
				 inFolder:(NSString *)folder
...
- (BOOL)duplicateSetWithName:(NSString *)setName
				   extension:(NSString *)extension
					inFolder:(NSString *)folder
	...
	for (NSString *filePath in [adium allResourcesForName:folder withExtensions:extension]) {
		...
		if ((themeDict = [NSDictionary dictionaryWithContentsOfFile:filePath])) {
			...
			if (![alreadyAddedArray containsObject:name]) {
				[setArray addObject:[NSDictionary dictionaryWithObjectsAndKeys:name, @"name", filePath, @"path",
																			   themeDict, @"preferences", nil]];
...
{
	return [self availableSetsWithExtension:LIST_LAYOUT_EXTENSION fromFolder:LIST_LAYOUT_FOLDER];
}
- (NSArray *)availableThemeSets
{
	return [self availableSetsWithExtension:LIST_THEME_EXTENSION fromFolder:LIST_THEME_FOLDER];
}
