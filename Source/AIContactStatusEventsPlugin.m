	...
	if (includeSubject) {
		...
		if ([eventID isEqualToString:CONTACT_STATUS_ONLINE_YES]) {
			...
			format = AILocalizedString(
				@"%@ went away", "Event: <A contact's name> went away (is no longer available but is still online)");
		} else if ([eventID isEqualToString:CONTACT_STATUS_AWAY_NO]) {
			...
			format =
				AILocalizedString(@"%@ became active", "Event: <A contact's name> became active (is no longer idle)");
		} else if ([eventID isEqualToString:CONTACT_SEEN_ONLINE_YES]) {
			format = AILocalizedString(
				@"%@ is seen",
			...
			format =
				AILocalizedString(@"%@ is no longer seen",
			...
			format =
				AILocalizedString(@"%@ returned from mobile", "Event: <A contact's name> is no longer mobile (came "
															  "online and is no longer available on a mobile device)");
		}
		...
		if ([eventID isEqualToString:CONTACT_STATUS_ONLINE_YES]) {
			description =
				AILocalizedString(@"connected", "Event: connected (follows a contact's name displayed as a header)");
		} else if ([eventID isEqualToString:CONTACT_STATUS_ONLINE_NO]) {
			description = AILocalizedString(@"disconnected",
											"Event: disconnected (follows a contact's name displayed as a header)");
		} else if ([eventID isEqualToString:CONTACT_STATUS_AWAY_YES]) {
			description =
				AILocalizedString(@"went away", "Event: went away (follows a contact's name displayed as a header)");
		} else if ([eventID isEqualToString:CONTACT_STATUS_AWAY_NO]) {
			description =
				AILocalizedString(@"came back", "Event: came back (follows a contact's name displayed as a header)");
		} else if ([eventID isEqualToString:CONTACT_STATUS_IDLE_YES]) {
			description =
				AILocalizedString(@"went idle", "Event: went idle (follows a contact's name displayed as a header)");
		} else if ([eventID isEqualToString:CONTACT_STATUS_IDLE_NO]) {
			description = AILocalizedString(@"became active",
											"Event: became active (follows a contact's name displayed as a header)");
		} else if ([eventID isEqualToString:CONTACT_SEEN_ONLINE_YES]) {
			description =
				AILocalizedString(@"is seen", "Event: is seen (follows a contact's name displayed as a header)");
		} else if ([eventID isEqualToString:CONTACT_SEEN_ONLINE_NO]) {
			description = AILocalizedString(
				@"is no longer seen", "Event: is no longer seen (follows a contact's name displayed as a header)");
		} else if ([eventID isEqualToString:CONTACT_STATUS_MOBILE_YES]) {
			description = AILocalizedString(@"went mobile",
											"Event: went mobile (follows a contact's name displayed as a header)");
		} else if ([eventID isEqualToString:CONTACT_STATUS_MOBILE_NO]) {
			description = AILocalizedString(
				@"returned from mobile", "Event: is no longer mobile (follows a contact's name displayed as a header)");
		}
	}
...
{
	/* Ignore accounts.
	 * Ignore meta contact children since the actual meta contact provides a better event. The best way to check this is
	 * to verify that the contact's parentContact is itself.*/
	if (([inObject isKindOfClass:[AIListContact class]]) &&
		...
		if ([inObject boolValueForProperty:@"isOnline"]) {
			if ([inModifiedKeys containsObject:@"listObjectStatusMessage"] ||
 ...
 * @param performCompare If NO, we are only concerned about whether any object exists. If YES, a change from one value
 * to another means we've updated.
 *
 * @result YES if the cache changed; NO if it remained the same (event has already occurred on another associated
 * contact)
 */
- (BOOL)updateCache:(NSMutableDictionary *)cache
			 forKey:(NSString *)key
	 ...
	 performCompare:(BOOL)performCompare
{
	...
	if ((newStatus && !oldStatus) || (oldStatus && !newStatus) ||
		((performCompare && newStatus && oldStatus &&
		  [newStatus performSelector:@selector(compare:) withObject:oldStatus] != NSOrderedSame))) {
