...
- (void)setPassword:(NSString *)inPassword forAccount:(AIAccount *)inAccount
{
	...
	if (error) {
		...
		if (inPassword || (err != errSecItemNotFound)) {
			...
			NSLog(@"could not %@ password for account %@: %@ returned %ld (%@)", inPassword ? @"set" : @"remove",
 ...
 * is needed. This allows forcing or suppressing of the prompt dialogue.
				   ...
				   context:(id)inContext
{
	dispatch_async([AdiumPasswords queue], ^{
		NSMutableDictionary *retrievalDict = [NSMutableDictionary
			dictionaryWithObjectsAndKeys:inAccount, @"Account", [NSNumber numberWithInteger:promptOption],
										 @"AIPromptOption", inTarget, @"Target", NSStringFromSelector(inSelector),
										 @"Selector", inContext, @"Context" /* may be nil so should be last */, nil];
...
- (void)setPassword:(NSString *)inPassword forProxyServer:(NSString *)server userName:(NSString *)userName
{
	...
	if (error) {
		...
		if (inPassword || (err != errSecItemNotFound)) {
			NSDictionary *userInfo = [error userInfo];
			NSLog(@"could not %@ password for proxy server %@: %@ returned %ld (%@)", inPassword ? @"set" : @"remove",
				  [self _accountNameForProxyServer:server userName:userName],
				  [userInfo objectForKey:AIKEYCHAIN_ERROR_USERINFO_SECURITYFUNCTIONNAME], (long)err,
				  [userInfo objectForKey:AIKEYCHAIN_ERROR_USERINFO_ERRORDESCRIPTION]);
		}
	}
...
{
	NSError *error = nil;
	[[AIKeychain defaultKeychain_error:&error]
		setInternetPassword:inPassword
				  forServer:[self _serverNameForAccount:inAccount]
...
- (NSString *)passwordForType:(AISpecialPasswordType)inType forAccount:(AIAccount *)inAccount name:(NSString *)inName
{
	...
	AIKeychain *keychain = [AIKeychain defaultKeychain_error:&error];
	NSString *password =
		[keychain internetPasswordForServer:[self _serverNameForAccount:inAccount]
									account:[self _accountNameForSpecialPassword:inType
																		 account:inAccount
...
- (NSString *)_serverNameForAccount:(AIAccount *)inAccount
{
	if (inAccount.useHostForPasswordServerName) {
		...
		return
			[NSString stringWithFormat:@"%@.%@", inAccount.service.serviceID, [self _accountNameForAccount:inAccount]];
	}
}
