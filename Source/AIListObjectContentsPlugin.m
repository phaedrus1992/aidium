...
- (void)installPlugin
{
	...
	contextualMenuItem = [[NSMenuItem alloc] initWithTitle:EXPAND_CONTACT
											 ...
											 keyEquivalent:@""];
	[adium.menuController addContextualMenuItem:contextualMenuItem toLocation:Context_Contact_ListAction];
...
- (NSString *)labelForObject:(AIListObject *)inObject
{
	if ([inObject conformsToProtocol:@protocol(AIContainingObject)] ||
		([inObject isKindOfClass:[AIListContact class]] && ![inObject isKindOfClass:[AIListBookmark class]])) {
...
- (NSAttributedString *)entryForObject:(AIListObject *)inObject
{
	...
	for (AIListContact *contact in listContacts) {
		...
		if (containingObject != nil && [inObject isKindOfClass:[AIListGroup class]] &&
			![[AIContactHidingController sharedController] visibilityOfListObject:contact
																	  inContainer:containingObject]) {
			continue;
		}
		...
		if (++count >= MAX_CONTACTS) {
			[entryString
				appendString:[NSString stringWithFormat:MORE_CONTACTS_STRING, listContacts.count - MAX_CONTACTS]];
			break;
		}

		statusIcon = [[AIStatusIcons statusIconForListObject:contact type:AIStatusIconTab
		...
		} else if ([inObject isKindOfClass:[AIListGroup class]] || [inObject isKindOfClass:[AIListContact class]]) {
			[entryString appendString:contact.displayName];
		}

		serviceIcon = [[AIServiceIcons serviceIconForObject:contact type:AIServiceIconSmall
												  direction:AIIconNormal] imageByScalingToSize:META_TOOLTIP_ICON_SIZE];
		if (serviceIcon) {
			NSTextAttachment *attachment;
			NSTextAttachmentCell *cell;
...
- (void)inspectedObjectDidChange:(NSNotification *)inNotification
{
	...
	if (oldListObject && [oldListObject isKindOfClass:[AIMetaContact class]] &&
		...
		[oldListObject setValue:nil forProperty:@"TemporaryMetaContactExpansion" notify:NotifyNever];
