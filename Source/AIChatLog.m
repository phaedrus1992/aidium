...
- (id)initWithPath:(NSString *)inPath
{
	...
	return [self initWithPath:inPath from:fromUID to:toUID serviceClass:myServiceClass];
}
...
- (NSDate *)date
{
	...
	if (!date) {
		...
		if (!date) {
			// Sometimes the filename doesn't have a date (e.g., “jdoe ((null)).chatlog”). In such cases, if it's a
			// chatlog, parse it and get the date from the first element that has one. We don't do this first because
			// NSXMLParser uses +[NSData dataWithContentsOfURL:], which is painful for large log files.
			if ([[relativePath pathExtension] isEqualToString:@"chatlog"]) {
				NSXMLParser *parser = [[NSXMLParser alloc]
					initWithContentsOfURL:[NSURL fileURLWithPath:[[AILoggerPlugin logBasePath]
