...
- (void)installPlugin
{
	...
	[adium.contactAlertsController registerActionID:SPEAK_TEXT_ALERT_IDENTIFIER withHandler:self];
	[adium.contactAlertsController registerActionID:SPEAK_EVENT_ALERT_IDENTIFIER withHandler:self];
	...
	if ([actionID isEqualToString:SPEAK_TEXT_ALERT_IDENTIFIER]) {
		...
		if ([userText rangeOfString:@"%n"].location != NSNotFound) {
			...
			[userText replaceOccurrencesOfString:@"%n"
									  withString:(replacementText ? replacementText : @"")options:NSLiteralSearch
		...
		if ([userText rangeOfString:@"%t"].location != NSNotFound) {
			[NSDateFormatter
				withLocalizedDateFormatterShowingSeconds:YES
										   showingAMorPM:NO
												 perform:^(NSDateFormatter *timeFormatter) {
													 [userText
														 replaceOccurrencesOfString:@"%t"
																		 withString:[timeFormatter
		...
		if ([userText rangeOfString:@"%m"].location != NSNotFound) {
			...
			if ([adium.contactAlertsController isMessageEvent:eventID] && [userInfo objectForKey:@"AIContentObject"]) {
			...
			[userText replaceOccurrencesOfString:@"%m"
									  withString:(message ? message : @"")options:NSLiteralSearch
										   range:NSMakeRange(0, [userText length])];
		}
		...
		if ([adium.contactAlertsController isMessageEvent:eventID] && [userInfo objectForKey:@"AIContentObject"]) {
			...
			if (speakSender && !isOutgoing) {
				...
				if (!lastSenderString || ![senderString isEqualToString:lastSenderString]) {
					...
					[lastSenderString release];
					lastSenderString = [senderString retain];
