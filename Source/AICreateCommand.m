...
+ (NSScriptObjectSpecifier *)_objectSpecifierFromDescriptor:(NSAppleEventDescriptor *)desc
							   inCommandConstructionContext:(id)context;
@end

@implementation AICreateCommand
/**
 * @brief Overrides default NSCreateCommand behavior when creating an object; rather than calling alloc/init, calls
 * make<Key>WithProperties:
 *
...
#warning This uses a Private API
	...
	id r;
	NSString *methodName =
		[NSString stringWithFormat:@"make%@WithProperties:", [[newObjectDescription className] camelCase]];
