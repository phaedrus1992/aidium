 ...
 *
 * Adium 1.2 and below used a separate plist file on disk for each object. This is a nice memory optimization but a
 * nasty performance hit. This code moves all those plists into a single file when first run and is a no-op after that.
 */
- (void)upgradeToSingleObjectPrefsDictIfNeeded
{
	if (![[self preferenceForKey:LOADED_OBJECT_PREFS_KEY group:PREF_GROUP_GENERAL] boolValue]) {
		...
		while ((file = [enumerator nextObject])) {
			...
			NSMutableDictionary *thisDict = [NSMutableDictionary dictionaryAtPath:dir withName:name create:NO];
			if ([thisDict count]) {
				...
				[prefsDict setObject:thisDict forKey:name];
			}
		}

		[prefsDict asyncWriteToPath:userDirectory withName:@"ByObjectPrefs"];
		...
		while ((file = [enumerator nextObject])) {
			NSString *name = [file stringByDeletingPathExtension];
			NSDictionary *thisDict = [NSDictionary dictionaryAtPath:dir withName:name create:NO];
			if ([thisDict count]) {
				[prefsDict setObject:thisDict forKey:name];
			}
		}

		[prefsDict asyncWriteToPath:userDirectory withName:@"AccountPrefs"];

		[self setPreference:[NSNumber numberWithBool:YES] forKey:LOADED_OBJECT_PREFS_KEY group:PREF_GROUP_GENERAL];
	}
}
...
- (void)dealloc
{
	[delayedNotificationGroups release];
 ...
 *
 * The preference observer will be notified when preferences in group change and passed the preference dictionary for
 * that group The observer must implement:
...
- (void)registerPreferenceObserver:(id)observer forGroup:(NSString *)group
{
	...
	NSParameterAssert(
		[observer respondsToSelector:@selector(preferencesChangedForGroup:key:object:preferenceDict:firstTime:)]);
 ...
 *
 * @param inPrefDict An NSDictionary whose keys are preference keys and objects are the preferences for those keys. All
 * must be plist-encodable.
 * @param group An arbitrary NSString group
 */
 ...
 */
- (void)setPreference:(id)value forKey:(NSString *)key group:(NSString *)group object:(AIListObject *)object
{
 ...
 *
 * Objects inherit from their containing objects, up to the global preference.  If this entire tree has no set
 * preference, defaults are searched, starting against with the object and proceeding up to the global defaults.
 */
- (id)preferenceForKey:(NSString *)key group:(NSString *)group object:(AIListObject *)object
{
	...
	if (!result)
		result = [self defaultPreferenceForKey:key group:group object:object];
...
{
	// We are ignoring inheritance, so we can ignore inherited defaults, too, and use the
	// preferenceContainerForGroup:object: dict
...
- (AIPreferenceContainer *)preferenceContainerForGroup:(NSString *)group object:(AIListObject *)object
{
	...
	if (object) {
		...
		if ((prefContainer = [objectPrefCache objectForKey:cacheKey])) {
			...
			prefContainer = [AIPreferenceContainer preferenceContainerForGroup:group object:object];
...
- (NSString *)userPreferredDownloadFolder
{
	...
	if (!userPreferredDownloadFolder) {
		...
		if (LSGetApplicationForURL((CFURLRef)[NSURL URLWithString:@"http://google.com"], kLSRolesViewer,
								   NULL /*outAppRef*/, &urlToDefaultBrowser) != kLSApplicationNotFoundErr) {
			...
			if ([defaultBrowserName rangeOfString:@"Safari"].location != NSNotFound) {
				/* ICGetPref() for kICDownloadFolder returns any previously set preference, not the default ~/Downloads
				 * or the current Safari setting, in 10.5.0, with Safari the default browser
				 */
				CFPropertyListRef safariDownloadsPath =
					CFPreferencesCopyAppValue(CFSTR("DownloadsPath"), CFSTR("com.apple.Safari"));
