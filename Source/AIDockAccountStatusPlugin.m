...
- (NSSet *)updateListObject:(AIListObject *)inObject keys:(NSSet *)inModifiedKeys silent:(BOOL)silent
{
	if (inObject == nil || [inObject isKindOfClass:[AIAccount class]]) {
		...
		if (inObject == nil || ([inModifiedKeys containsObject:@"isConnecting"] ||
								[inModifiedKeys containsObject:@"waitingToReconnect"])) {
			if ([self _accountsWithBoolProperty:@"isConnecting"] ||
				[self _accountsWithProperty:@"waitingToReconnect"]) {
		...
		if (shouldUpdateStatus || [inModifiedKeys containsObject:@"accountStatus"]) {
			...
			AIStatusType activeStatusType =
				[adium.statusController activeStatusTypeTreatingInvisibleAsAway:!iconSupportsInvisible];
...
- (BOOL)_accountsWithBoolProperty:(NSString *)inKey
{
	for (AIAccount *account in adium.accountController.accounts) {
		if ([account boolValueForProperty:inKey] && account.enabled)
			return YES;
	}
...
{
	for (AIAccount *account in adium.accountController.accounts) {
		if ([account valueForProperty:inKey] && account.enabled)
			return YES;
	}
