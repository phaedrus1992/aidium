...
- (void)dealloc
{
	...
	scriptMenuItem = nil;
	[contextualScriptMenuItem release];
 ...
 *
 * This will clear out and then load from available scripts (external and internal) into flatScriptArray and
 * scriptArray.
 */
- (void)loadScripts
{
	...
	for (NSString *filePath in [adium allResourcesForName:@"Scripts" withExtensions:SCRIPT_BUNDLE_EXTENSION]) {
		...
		if ((scriptBundle = [NSBundle bundleWithPath:filePath])) {
			...
			NSDictionary *infoDict = [NSDictionary
				dictionaryWithContentsOfFile:[[scriptBundle bundlePath] stringByAppendingPathComponent:@"Info.plist"]];
			if (!infoDict)
				infoDict = [scriptBundle infoDictionary];
			...
			for (NSDictionary *scriptDict in [infoDict objectForKey:@"Scripts"]) {
				...
				if ((scriptFileName = [scriptDict objectForKey:@"File"]) &&
					(scriptFilePath = [scriptBundle pathForResource:scriptFileName ofType:SCRIPT_EXTENSION])) {
					...
					if (keyword && [keyword length] && title && [title length]) {
						...
						newInfoDict = [NSMutableDictionary
							dictionaryWithObjectsAndKeys:scriptFilePath, @"Path", keyword, @"Keyword", title, @"Title",
...
NSInteger _scriptTitleSort(id scriptA, id scriptB, void *context)
{
	...
	if (setA && setB) {
		...
		if ((result = [setA caseInsensitiveCompare:setB]) == NSOrderedSame) {
			result =
				[(NSString *)[scriptA objectForKey:@"Title"] caseInsensitiveCompare:[scriptB objectForKey:@"Title"]];
		}
	} else {
...
- (void)_appendScripts:(NSArray *)scripts toMenu:(NSMenu *)menu
{
	...
	for (appendDict in scripts) {
		...
		if ([appendDict objectForKey:@"Title"]) {
			title = [NSString
				stringWithFormat:@"%@ (%@)", [appendDict objectForKey:@"Title"], [appendDict objectForKey:@"Keyword"]];
		} else {
...
- (IBAction)selectScript:(id)sender
{
	...
	if (responder && [responder isKindOfClass:[NSTextView class]]) {
		...
		if (arguments) {
			...
			[italicizedTypingAttributes
				setObject:[[NSFontManager sharedFontManager]
							  convertFont:[originalTypingAttributes objectForKey:NSFontAttributeName]
			...
			for (anArgument in arguments) {
				...
				// Turn on the italics version, insert the argument, then go back to normal for either the comma or the
				// ending
							 ...
							 uniqueID:(unsigned long long)uniqueID
{
	...
	if ((stringMessage = [inAttributedString string])) {
		...
		for (NSMutableDictionary *infoDict in flatScriptArray) {
			...
			if ((prefixOnly &&
				 ([stringMessage rangeOfString:keyword options:(NSCaseInsensitiveSearch | NSAnchoredSearch)].location ==
				  0)) ||
				(!prefixOnly &&
				 [stringMessage rangeOfString:keyword options:NSCaseInsensitiveSearch].location != NSNotFound)) {
				...
				if ((shouldSendNumber) && (![shouldSendNumber boolValue]) &&
			   ...
			   uniqueID:(unsigned long long)uniqueID
{
	NSDictionary *userInfo = [NSDictionary
		dictionaryWithObjectsAndKeys:attributedString, @"Mutable Attributed String", NSStringFromRange(keywordRange),
									 @"Range", [NSNumber numberWithUnsignedLongLong:uniqueID], @"uniqueID",
									 (context ? context : [NSNull null]), @"context", nil];
...
- (void)applescriptDidRun:(id)userInfo resultString:(NSString *)resultString
{
	...
	if (NSMaxRange(keywordRange) <= [attributedString length]) {
		if (([resultString hasPrefix:@"<HTML>"])) {
			...
			NSAttributedString *attributedScriptResult = [AIHTMLDecoder
						   decodeHTML:resultString
				withDefaultAttributes:[attributedString attributesAtIndex:keywordRange.location effectiveRange:nil]];
			[attributedString replaceCharactersInRange:keywordRange withAttributedString:attributedScriptResult];

		} else {
			[attributedString replaceCharactersInRange:keywordRange withString:resultString];
		}
	}
