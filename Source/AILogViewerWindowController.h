...
typedef enum { LOG_SEARCH_FROM = 0, LOG_SEARCH_TO, LOG_SEARCH_DATE, LOG_SEARCH_CONTENT } LogSearchMode;
...
@interface AILogViewerWindowController
	: AIWindowController <NSToolbarDelegate, NSOutlineViewDelegate, NSTableViewDelegate> {
	...
	BOOL ignoreSelectionChange; // Hack to prevent automatic table selection changes from clearing the automaticSearch
								// flag
