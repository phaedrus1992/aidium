...
- (IBAction)blockContact:(id)sender
{
	...
	object = ((sender == blockContactMenuItem) ? adium.interfaceController.selectedListObject
			...
			if ([object isKindOfClass:[AIMetaContact class]]) {
				...
				while ((containedContact = [enumerator nextObject])) {
					...
					if ([account conformsToProtocol:@protocol(AIAccount_Privacy)]) {
						...
						NSLog(@"Account %@ does not support blocking (contact %@ not blocked on this account)", account,
							  containedContact);
					}
				}
			} else {
	...
	if ([object isKindOfClass:[AIListContact class]]) {
		...
		if ([object isKindOfClass:[AIMetaContact class]]) {
			...
			while ((contact = [enumerator nextObject])) {
				...
				if ([account conformsToProtocol:@protocol(AIAccount_Privacy)]) {
					AIPrivacyType privType =
						(([account privacyOptions] == AIPrivacyOptionAllowUsers) ? AIPrivacyTypePermit
																				 : AIPrivacyTypeDeny);
			...
			if ([account conformsToProtocol:@protocol(AIAccount_Privacy)]) {
				AIPrivacyType privType =
					(([account privacyOptions] == AIPrivacyOptionAllowUsers) ? AIPrivacyTypePermit : AIPrivacyTypeDeny);
				if ([[account listObjectsOnPrivacyList:privType] containsObject:contact]) {
					switch (privType) {
					case AIPrivacyTypePermit:
						/* He's on a permit list. The action would remove him, blocking him */
...
- (void)_setContact:(AIListContact *)contact isBlocked:(BOOL)isBlocked
{
	...
	NSEnumerator *enumerator =
		[[adium.accountController accountsCompatibleWithService:contact.service] objectEnumerator];
	...
	while ((account = [enumerator nextObject])) {
		...
		if ([account conformsToProtocol:@protocol(AIAccount_Privacy)]) {
			...
			if (sameContact) {
				/* If the account is in AIPrivacyOptionAllowUsers mode, blocking a contact means removing it from the
				 * allow list. Similarly, in allow mode, unblocking a contact means adding it to the allow list.
				 *
				 * In AIPrivacyOptionDenyUsers mode, blocking a contact means adding it to the block list.
				 *
				 * In all other modes, we can't block specific contacts... so we first switch to
				 * AIPrivacyOptionDenyUsers, the more lenient of the two possibilities, then add the contact to the
				 * block list.
				 */
	...
	if ([contact isKindOfClass:[AIMetaContact class]]) {
		...
		for (containedContact in [meta uniqueContainedObjects]) {
			...
			if ([acct conformsToProtocol:@protocol(AIAccount_Privacy)]) {
				...
				NSLog(@"Account %@ does not support blocking (contact %@ not blocked on this account)", acct,
					  containedContact);
			}
		}
	} else {
...
- (BOOL)contactIsBlocked:(AIListContact *)chkContact
{
	...
	if ([chkContact isKindOfClass:[AIMetaContact class]]) {
		...
		for (contact in [meta uniqueContainedObjects]) {
			...
			if ([acct conformsToProtocol:@protocol(AIAccount_Privacy)]) {
				AIPrivacyType privType =
					(([(AIAccount<AIAccount_Privacy> *)acct privacyOptions] == AIPrivacyOptionAllowUsers)
						 ? AIPrivacyTypePermit
						 : AIPrivacyTypeDeny);
	...
	while ((currentWindow = [windowEnumerator nextObject])) {
		...
		if ((windowToolbar = [currentWindow toolbar])) {
			...
			if (windowToolbar == senderToolbar) {
				...
				NSString *format = (shouldBlock ? AILocalizedString(@"Are you sure you want to block %@?", nil)
												: AILocalizedString(@"Are you sure you want to unblock %@?", nil));

				NSString *questionQualifier =
					[NSString stringWithFormat:AILocalizedString(@"%lu contacts", nil),
...
{
	[self updateToolbarIconOfChat:[notification object] inWindow:[[notification userInfo] objectForKey:@"NSWindow"]];
}
...
- (void)updateToolbarItemForObject:(AIListObject *)inObject
{
	...
	if (([chatToolbarItems count] > 0) && [inObject isKindOfClass:[AIListContact class]] &&
		...
		[self updateToolbarIconOfChat:chat inWindow:window];
	}
}
