...
- (NSAttributedString *)filterAttributedString:(NSAttributedString *)inAttributedString context:(id)context
{
	...
	if (inAttributedString &&
		 ...
		 */
		if (([[inAttributedString string] rangeOfCharacterFromSet:[self emoticonHintCharacterSet]].location !=
			 NSNotFound) ||
			([context isKindOfClass:[AIContentObject class]] &&
			 ([[(AIContentObject *)context chat] customEmoticons]))) {
 ...
 *
 * @result The location in messageString of the beginning of the emoticon replaced, or NSNotFound if no replacement was
 * made
 */
										...
										isMessage:(BOOL)isMessage
{
	...
	*currentLocation =
		[messageString rangeOfCharacterFromSet:emoticonStartCharacterSet
									   options:NSLiteralSearch
										 range:NSMakeRange(*currentLocation, messageStringLength - *currentLocation)]
			.location;
	if (*currentLocation != NSNotFound) {
		...
		if ([candidateEmoticons count]) {
			 ...
			 *		It is bordered by emoticons on both sides or by an emoticon on the left and a period, space, or line
			 * break on the right
			 */
			BOOL acceptable = NO;
			...
			if (!acceptable) {
				/* Bordered by spaces or line breaks or quotes, or by a period on the left and a space or a line break
				 * or quote on the right If we're being called recursively, we have a potential emoticon to our left;
				 * we only need to check the right. This is also true if we're not being called recursively but there's
				 * an NSAttachmentAttribute to our left. That will happen if, for example, the string is ":):) ". The
				 * first emoticon is at the start of the line and so is immediately acceptable. The second should be
				 * acceptable because it is to the right of an emoticon and the left of a space.
				 */
			...
			if (!acceptable) {
				/* If the emoticon would end the string except for whitespace, newlines, or punctionation at the end, or
				 * it begins the string after removing whitespace, newlines, or punctuation at the beginning, it is
				 * acceptable even if the previous conditions weren't met.
				 */
			...
			if (!acceptable) {
				 ...
				 * If the return value is not NSNotFound, an emoticon was found and replaced ahead of us. Discontinuous
			...
			if (acceptable) {
				...
				originalAttributes = [originalAttributes
					dictionaryWithDifferenceWithSetOfKeys:[NSSet setWithObject:NSAttachmentAttributeName]];

				// grab the original attributes, to ensure that the background is not lost in a message consisting only
				// of an emoticon
				[replacement addAttributes:originalAttributes range:NSMakeRange(0, 1)];

				// insert the emoticon
				if (!(*newMessage))
					*newMessage = [originalAttributedString mutableCopy];
				[*newMessage replaceCharactersInRange:emoticonRangeInNewMessage withAttributedString:replacement];
...
- (NSAttributedString *)_convertEmoticonsInMessage:(NSAttributedString *)inMessage context:(id)context
{
	...
	// we can avoid loading images if the emoticon is headed for the wkmv, since it will just load from the original
	// path anyway
	BOOL isMessage = NO;
	...
	if ([context isKindOfClass:[AIContentObject class]]) {
		...
		if (customEmoticons && !((AIContentObject *)context).isOutgoing) {
			/* XXX Note that we only display custom emoticons for incoming messages; we can not set our own custom
			 * emotcions at this time
			 */
		...
		serviceClassContext = [[[adium.accountController preferredAccountForSendingContentType:CONTENT_MESSAGE_TYPE
																					 toContact:(AIListContact *)context]
...
// Returns all active emoticons, categoriezed by starting character, using a dictionary, with each value containing an
// array of characters
- (NSDictionary *)emoticonIndex
{
	if (!_emoticonIndexDict)
		[self _buildCharacterSetsAndIndexEmoticons];
...
- (void)setEmoticon:(AIEmoticon *)inEmoticon inPack:(AIEmoticonPack *)inPack enabled:(BOOL)enabled
{
	...
	if (!packDict)
		packDict = [[NSMutableDictionary alloc] init];
	if (!disabledArray)
		disabledArray = [[NSMutableArray alloc] init];
...
- (NSArray *)availableEmoticonPacks
{
	if (!_availableEmoticonPacks) {
		...
		for (NSString *path in
			 [adium allResourcesForName:EMOTICONS_PATH_NAME
						 withExtensions:[NSArray arrayWithObjects:EMOTICON_PACK_PATH_EXTENSION,
																  ADIUM_EMOTICON_SET_PATH_EXTENSION,
																  PROTEUS_EMOTICON_SET_PATH_EXTENSION, nil]]) {
...
- (AIEmoticonPack *)emoticonPackWithName:(NSString *)inName
{
	for (AIEmoticonPack *emoticonPack in self.availableEmoticonPacks) {
		if ([emoticonPack.name isEqualToString:inName])
			return emoticonPack;
	}
...
- (void)moveEmoticonPacks:(NSArray *)inPacks toIndex:(NSUInteger)idx
{
	...
	for (AIEmoticonPack *pack in inPacks) {
		if ([_availableEmoticonPacks indexOfObject:pack] < idx)
			idx--;
...
- (NSCharacterSet *)emoticonStartCharacterSet
{
	if (!_emoticonStartCharacterSet)
		[self _buildCharacterSetsAndIndexEmoticons];
	return _emoticonStartCharacterSet;
}

// For optimization, we build a list of characters that could possibly be an emoticon and will require additional
// scanning. We also build a dictionary categorizing the emoticons by their first character to quicken lookups.
- (void)_buildCharacterSetsAndIndexEmoticons
{
		...
		if (emoticon.isEnabled) {
			for (NSString *text in emoticon.textEquivalents) {
				...
				if ([text length] != 0) { // Invalid emoticon files may let empty text equivalents sneak in
					...
					// If any letter in this text equivalent already exists in the quick scan character set, we can skip
					// it
					if ([text rangeOfCharacterFromSet:tmpEmoticonHintCharacterSet].location == NSNotFound) {
						// Potential for optimization!: Favor punctuation characters ( :();- ) over letters (especially
						// vowels).
...
- (void)resetActiveEmoticons
{
	[_activeEmoticonPacks release];
	_activeEmoticonPacks = nil;

	[_activeEmoticons release];
	_activeEmoticons = nil;
