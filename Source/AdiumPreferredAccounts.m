...
- (AIAccount *)fallbackAccountForSendingToContact:(AIListContact *)inContact strictChecking:(BOOL)strictChecking
{
	...
	for (AIAccount *account in adium.accountController.accounts) {
		if (inContact.service == account.service && (account.online || (account.enabled && !strictChecking))) {
			return account;
		}
	}
...
- (AIAccount *)preferredAccountForSendingContentType:(NSString *)inType
										   toContact:(AIListContact *)inContact
									  strictChecking:(BOOL)strictChecking
{
	AIAccount *account;
	...
	if (!accountID)
		accountID = [inContact.parentContact preferenceForKey:KEY_PREFERRED_SOURCE_ACCOUNT
	...
	if (accountID) {
		if (![accountID isKindOfClass:[NSString class]]) {
			...
			[inContact setPreference:accountID forKey:KEY_PREFERRED_SOURCE_ACCOUNT group:PREF_GROUP_PREFERRED_ACCOUNTS];
		}
...
- (AIAccount *)preferredAccountForSendingContentType:(NSString *)inType toContact:(AIListContact *)inContact
{
	...
	if (!account) {
		...
		if (!account) {
			AILogWithSignature(@"Could not find a good choice to talk to %@; will return first available account",
							   inContact);
...
- (void)didSendContent:(NSNotification *)notification
{
	...
	if (chat && destObject) {
		...
		if (![[destObject preferenceForKey:KEY_PREFERRED_SOURCE_ACCOUNT group:PREF_GROUP_PREFERRED_ACCOUNTS]
				isEqualToString:sourceAccount.internalObjectID]) {
