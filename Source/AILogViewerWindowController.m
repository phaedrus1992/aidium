...
- (void)dealloc
{
	...
	[logFromGroupDict release];
	logFromGroupDict = nil;
	[logToGroupDict release];
	logToGroupDict = nil;
...
- (void)initLogFiltering
{
	...
	for (NSString *folderName in [[[NSFileManager defaultManager] contentsOfDirectoryAtPath:basePath error:NULL]
		...
		if (![folderName isEqualToString:@".DS_Store"]) { // avoid the directory info
			 ...
			 * Check against count to guard in case of old, malformed or otherwise odd folders & whatnot sitting in log
			 * base
			 */
...
- (void)rebuildContactsList
{
		...
		for (AILogToGroup *currentToGroup in [logFromGroup toGroupArray]) {
			...
			if ((currentTo = [currentToGroup to])) {
				...
				AIListObject *listObject =
					((serviceClass && currentTo)
						 ? [adium.contactController
							   existingListObjectWithUniqueID:[AIListObject internalObjectIDForServiceID:serviceClass
																									 UID:currentTo]]
						 : nil);
	...
	if (!isOpeningForContact) {
		...
		[self outlineViewSelectionDidChange:[NSNotification notificationWithName:@"outlineViewSelectionDidChange"
																		  object:nil]];
	}
}
...
- (void)windowDidLoad
{
	...
	[[toolbarItems objectForKey:@"toggleemoticons"]
		setImage:[NSImage imageNamed:(showEmoticons ? IMAGE_EMOTICONS_ON : IMAGE_EMOTICONS_OFF) forClass:[self class]]];
	...
	[[toolbarItems objectForKey:@"toggletimestamps"]
		setImage:[NSImage imageNamed:(showTimestamps ? IMAGE_TIMESTAMPS_ON : IMAGE_TIMESTAMPS_OFF)
							forClass:[self class]]];
...
- (void)updateProgressDisplay
{
	...
	if (activeSearchString && [activeSearchString length]) {
		[textField_resultCount
			setStringValue:[NSString
							   stringWithFormat:((count != 1) ? AILocalizedString(@"%lu matching transcripts", nil)
															  : AILocalizedString(@"1 matching transcript", nil)),
												count]];
	} else {
		[textField_resultCount
			setStringValue:[NSString stringWithFormat:((count != 1) ? AILocalizedString(@"%lu transcripts", nil)
	...
	if (activeSearchString && [activeSearchString length]) {
		...
		if (searching || indexing) {
			[progress appendString:[NSString stringWithFormat:AILocalizedString(@"Searching for '%@'", nil),
															  activeSearchString]];
		} else {
			[progress appendString:[NSString stringWithFormat:AILocalizedString(@"Search for '%@' complete.", nil),
															  activeSearchString]];
		}
	}
	...
	if (indexing) {
		...
		if (plugin.indexIsFlushing) {
			...
			[progress
				appendString:[NSString stringWithFormat:AILocalizedString(@"Indexing %qi of %qi transcripts", nil),
														plugin.logsIndexed, plugin.logsToIndex]];
		}
	}
	...
	if (!windowIsClosing) {
		...
		if (searchMode == LOG_SEARCH_CONTENT && (activeSearchString && [activeSearchString length])) {
			if (searching) {
				...
				// We're not searching - restart the search immediately every 10 updates to utilize the newly indexed
				// logs
...
- (void)refreshResultsSearchIsComplete:(BOOL)searchIsComplete
{
	...
	if (searchIsComplete && ((activeSearchID == searchIDToReattemptWhenComplete) && !windowIsClosing)) {
...
- (void)searchComplete
{
	...
	[refreshResultsTimer release];
	refreshResultsTimer = nil;
...
- (void)openChatOnDoubleAction:(id)sender
{
	...
	if ([item isKindOfClass:[AIListContact class]]) {
		...
		[adium.interfaceController setActiveChat:[adium.chatController openChatWithContact:(AIListContact *)item
...
- (void)displayLogs:(NSArray *)logArray
{
	...
	if (logArray) {
		displayOperation = [[NSInvocationOperation alloc] initWithTarget:self
																selector:@selector(_displayLogs:)
																  object:logArray];
...
#define HORIZONTAL_RULE_LENGTH 18
			...
			[NSDateFormatter withLocalizedDateFormatterPerform:^(NSDateFormatter *headerDateFormatter) {
				[displayText appendString:[NSString stringWithFormat:@"%@%@\n%@ - %@\n%@\n\n",
																	 ...
																	 [theLog to], horizontalRule]
						   withAttributes:[[AITextAttributes textAttributesWithFontFamily:@"Helvetica"
																				   traits:NSBoldFontMask
																					 size:12] dictionary]];
			}];
		}

		if ([[theLog relativePath] hasSuffix:@".AdiumHTMLLog"] || [[theLog relativePath] hasSuffix:@".html"] ||
			[[theLog relativePath] hasSuffix:@".html.bak"]) {
			// HTML log
			...
			if ([[NSFileManager defaultManager] fileExistsAtPath:logFullPath isDirectory:&isDir]) {
				...
				if (isDir)
					logFullPath = [logFullPath
						stringByAppendingPathComponent:[[[logFullPath lastPathComponent] stringByDeletingPathExtension]
														   stringByAppendingPathExtension:@"xml"]];
			}

			NSDictionary *options = [NSDictionary
				dictionaryWithObjectsAndKeys:[NSNumber numberWithBool:showTimestamps], @"showTimestamps",
											 [NSNumber numberWithBool:showEmoticons], @"showEmoticons", nil];
			NSAttributedString *attributedLogFileText = [AIXMLChatlogConverter readFile:logFullPath
																			withOptions:options];
			...
			NSString *logFileText = [NSString stringWithContentsOfURL:logURL encoding:NSUTF8StringEncoding error:NULL];
			if (logFileText) {
				AITextAttributes *textAttributes = [AITextAttributes textAttributesWithFontFamily:@"Helvetica"
																						   traits:0
																							 size:12];
				NSAttributedString *attributedLogFileText =
					[[[NSAttributedString alloc] initWithString:logFileText
	...
	if (displayText && [displayText length] && ![thisOperation isCancelled]) {
		...
		if ((searchMode == LOG_SEARCH_CONTENT) && [activeSearchString length]) {
			...
			while (![scanner isAtEnd]) {
				...
				// And a closing one
				if (![scanner isAtEnd] && [scanner scanUpToString:@"\"" intoString:&quotedString]) {
					// Scan past the quote
					[scanner scanString:@"\"" intoString:NULL];
					/* If a string within quotes is found, remove the words from the quoted string and add the full
					 * string to what we'll be highlighting.
					 *
					 * We'll use indexOfObject: and removeObjectAtIndex: so we only remove _one_ instance. Otherwise,
					 * this string: "killer attack ninja kittens" OR ninja wouldn't highlight the word ninja by itself.
					 */
			...
			for (searchWord in searchWordsArray) {
				...
				// Check against and/or.  We don't just remove it from the array because then we couldn't check case
				// insensitively.
	...
	if (![thisOperation isCancelled]) {
		...
		[self performSelectorOnMainThread:@selector(_displayLogText:) withObject:finalDisplayText waitUntilDone:YES];
...
- (void)selectFirstLog
{
	...
	// We need a little trickery here.  When we change the row, the table view will call our
	// tableViewSelectionDidChange: method. This method will clear the automaticSearch flag, and break any
	// scroll-to-bottom behavior we have going on for the custom search.  As a quick hack, I've added an
	// ignoreSelectionChange flag that can be set to inform our selectionDidChange method that we instantiated this
	// selection change, and not the user.
				   ...
				   firstOccurrence:(NSRange *)outRange
{
	...
	while (location != NSNotFound && location < plainBigStringLength) {
		...
		if (foundRange.location != NSNotFound) {
			if (outRange->location == NSNotFound)
				*outRange = foundRange;

			if (!attributeDictionary) {
				attributeDictionary = [NSMutableDictionary
					dictionaryWithObjectsAndKeys:[NSFont boldSystemFontOfSize:14], NSFontAttributeName,
												 [NSColor yellowColor], NSBackgroundColorAttributeName, nil];
			}
			[bigString addAttributes:attributeDictionary range:foundRange];
...
- (void)hilightNextPrevious
{
	...
	[textField_findCount setStringValue:[NSString stringWithFormat:@"%lu/%lu", (unsigned long)currentMatch,
...
- (void)resortLogs
{
	...
	if ([identifier isEqualToString:@"To"]) {
		...
		[currentSearchResults
			sortUsingSelector:(sortDirection ? @selector(compareFromReverse:) : @selector(compareFrom:))];

	} else if ([identifier isEqualToString:@"Date"]) {
		[currentSearchResults
			sortUsingSelector:(sortDirection ? @selector(compareDateReverse:) : @selector(compareDate:))];

	} else if ([identifier isEqualToString:@"Rank"]) {
		[currentSearchResults
			sortUsingSelector:(sortDirection ? @selector(compareRankReverse:) : @selector(compareRank:))];

	} else if ([identifier isEqualToString:@"Service"]) {
		[currentSearchResults
			sortUsingSelector:(sortDirection ? @selector(compareServiceReverse:) : @selector(compareService:))];
	}
...
- (void)sortCurrentSearchResultsForTableColumn:(NSTableColumn *)tableColumn direction:(BOOL)direction
{
	...
	[tableView_results
		setIndicatorImage:[NSImage imageNamed:(direction ? @"NSDescendingSortIndicator" : @"NSAscendingSortIndicator")]
...
- (void)startSearchingClearingCurrentResults:(BOOL)clearCurrentResults
{
	...
	if (clearCurrentResults) {
		[resultsLock lock];
		// Stop any existing searches inside of resultsLock so we won't get any additions results added that we don't
		// want
		[self stopSearching];
	...
	searchDict = [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithInteger:activeSearchID], @"ID",
															[NSNumber numberWithInteger:searchMode], @"Mode",
															activeSearchString, @"String", nil];
...
- (void)filterForContact:(AIListContact *)inContact
{
	...
	[outlineView_contacts
		selectItemsInArray:[NSArray arrayWithObject:(parentContact ? (id)parentContact : (id)allContactsIdentifier)]];
...
- (void)filterForChatName:(NSString *)chatName withAccount:(AIAccount *)account
{
	...
	AILogToGroup *logToGroup = [logToGroupDict
		objectForKey:[[NSString stringWithFormat:@"%@.%@", account.service.serviceID, account.UID.safeFilenameString]
 ...
 */
- (BOOL)searchShouldDisplayDocument:(SKDocumentRef)inDocument
					 pathComponents:(NSArray *)pathComponents
						   testDate:(BOOL)testDate
{
	...
	if ([contactIDsToFilter count]) {
		...
		if (!pathComponents)
			pathComponents = pathComponentsForDocument(inDocument);

		NSUInteger numPathComponents = [pathComponents count];

		NSArray *serviceAndFromUIDArray =
			[[pathComponents objectAtIndex:numPathComponents - 3] componentsSeparatedByString:@"."];
		NSString *serviceClass =
			(([serviceAndFromUIDArray count] >= 2) ? [serviceAndFromUIDArray objectAtIndex:0] : @"");

		NSString *contactName = [pathComponents objectAtIndex:(numPathComponents - 2)];

		shouldDisplayDocument = [contactIDsToFilter
			containsObject:[[NSString stringWithFormat:@"%@.%@", serviceClass, contactName] compactedString]];
	}
	...
	if (shouldDisplayDocument && testDate && (filterDateType != AIDateTypeAnyDate)) {
		...
		NSUInteger numPathComponents = [pathComponents count];
		NSString *toPath = [NSString stringWithFormat:@"%@/%@", [pathComponents objectAtIndex:numPathComponents - 3],
													  [pathComponents objectAtIndex:numPathComponents - 2]];
		NSString *relativePath =
			[NSString stringWithFormat:@"%@/%@", toPath, [pathComponents objectAtIndex:numPathComponents - 1]];
		AIChatLog *theLog;
...
- (void)_logContentFilter:(NSString *)searchString searchID:(NSInteger)searchID onSearchIndex:(SKIndexRef)logSearchIndex
{
	...
	thisSearch = SKSearchCreate(logSearchIndex, (CFStringRef)wildcardedSearchString, kSKSearchOptionDefault);
...
#define BATCH_NUMBER 100
		...
		more = SKSearchFindMatches(thisSearch, BATCH_NUMBER, foundDocIDs, foundScores,
								   0.5, // maximum time before func returns, in seconds
								   &foundCount);

		totalCount += foundCount;

		SKIndexCopyDocumentRefsForDocumentIDs(logSearchIndex, foundCount, foundDocIDs, foundDocRefs);
		for (i = 0; ((i < foundCount) && (searchID == activeSearchID)); i++) {
			...
			/*
			 * Nasty implementation note: As of 10.4.7 and all previous versions, a path longer than 1024 bytes
			 * (PATH_MAX) will cause CFURLCopyFileSystemPath() to crash [ultimately in CFGetAllocator()].  This is the
			 * case for all Cocoa applications...
			 */
			NSString *logPath = [(NSURL *)url path];
			if (!logPath)
				AILogWithSignature(@"Could not get path for %@. ", url);
			...
			if ([self searchShouldDisplayDocument:document pathComponents:pathComponents testDate:NO]) {
				NSUInteger numPathComponents = [pathComponents count];
				NSString *toPath =
					[NSString stringWithFormat:@"%@/%@", [pathComponents objectAtIndex:numPathComponents - 3],
											   [pathComponents objectAtIndex:numPathComponents - 2]];
				NSString *path =
					[NSString stringWithFormat:@"%@/%@", toPath, [pathComponents objectAtIndex:numPathComponents - 1]];
				AIChatLog *theLog;
				...
				if (!theLog) {
					AILog(@"_logContentFilter: %p's key %@ yields %@; logAtPath:%@ gives %@", logToGroupDict, toPath,
						  [logToGroupDict objectForKey:toPath], path, theLog);
				}
				[resultsLock lock];
				if ((theLog != nil) && (![currentSearchResults containsObjectIdenticalTo:theLog]) &&
					[self chatLogMatchesDateFilter:theLog] && (searchID == activeSearchID)) {
					[theLog setRankingValueOnArbitraryScale:foundScores[i]];

					// SearchKit does not normalize ranking scores, so we track the largest we've found and use it
					// as 1.0
					if (foundScores[i] > largestRankingValue)
						largestRankingValue = foundScores[i];
		...
		[self performSelectorOnMainThread:@selector(updateProgressDisplay) withObject:nil waitUntilDone:NO];
	...
	if (searchID == activeSearchID) { // If we're still supposed to go
		...
		if (searchString && [searchString length]) {
			...
			case LOG_SEARCH_DATE:
				...
				break;
			case LOG_SEARCH_CONTENT:
				[self _logContentFilter:searchString searchID:searchID onSearchIndex:[plugin logContentIndex]];
				break;
			}
		} else {
...
- (void)_logFilter:(NSString *)searchString searchID:(NSInteger)searchID mode:(LogSearchMode)mode
{
	...
	if ((mode == LOG_SEARCH_DATE) && (searchString != nil)) {
		searchStringDate = [[NSDate dateWithNaturalLanguageString:searchString] dateWithCalendarFormat:nil
																							  timeZone:nil];
	}
	...
	for (AILogFromGroup *fromGroup in [logFromGroupDict objectEnumerator]) {
		...
		if ((mode != LOG_SEARCH_FROM) || (!searchString) ||
			...
			for (AILogToGroup *toGroup in [fromGroup toGroupArray]) {
				 ...
				 */
				if ((![contactIDsToFilter count] ||
					 [contactIDsToFilter containsObject:[[NSString stringWithFormat:@"%@.%@", [toGroup serviceClass],
																					[toGroup to]] compactedString]]) &&
					((mode != LOG_SEARCH_TO) || (!searchString) ||
					 ([[toGroup to] rangeOfString:searchString options:NSCaseInsensitiveSearch].location !=
					  NSNotFound))) {
					...
					for (AIChatLog *theLog in [toGroup logEnumerator]) {
						 ...
						 */
						if ((mode != LOG_SEARCH_DATE) || (!searchString) ||
...
- (id)tableView:(NSTableView *)tableView objectValueForTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row
{
	...
	if (row < 0 || row >= [currentSearchResults count]) {
		...
		if ([identifier isEqualToString:@"To"]) {
			...
			AIListObject *listObject = [adium.contactController
				existingListObjectWithUniqueID:[AIListObject internalObjectIDForServiceID:[theLog serviceClass]
																					  UID:[theLog to]]];
			if (listObject) {
				// Use the longDisplayName, following the user's contact list preferences as this is presumably how she
				// wants to view contacts' names.
			...
			image =
				[AIServiceIcons serviceIconForService:[adium.accountController firstServiceWithServiceID:serviceClass]
...
- (void)tableViewSelectionDidChange:(NSNotification *)notification
{
	...
	[self performSelector:@selector(tableViewSelectionDidChangeDelayed) withObject:nil afterDelay:0.05];
}
...
- (void)tableViewSelectionDidChangeDelayed
{
	if (!ignoreSelectionChange) {
		...
		@try {
			 ...
			 * Catching it is far more straightforward than preventing that possibility without breaking our
			 * re-selection of selected search results as the table view reloads when new results come in.
			 */
...
#define NUMBER_DATE_STYLES 4
		NSDateFormatterStyle timeFormatterStyles[NUMBER_TIME_STYLES] = {NSDateFormatterShortStyle,
																		NSDateFormatterNoStyle};
		NSDateFormatterStyle formatterStyles[NUMBER_DATE_STYLES] = {
			NSDateFormatterFullStyle, NSDateFormatterLongStyle, NSDateFormatterMediumStyle, NSDateFormatterShortStyle};
		...
		for (NSInteger i = 0; (i < NUMBER_TIME_STYLES) && (requiredWidth > width); i++) {
			...
			for (NSInteger j = 0; (j < NUMBER_DATE_STYLES) && (requiredWidth > width); j++) {
				...
				// Require a bit of space so the date looks comfortable. Very long dates relative to the current date
				// can still overflow...
				requiredWidth += 3;
			}
		}
...
- (IBAction)toggleTimestampFiltering:(id)sender
{
	...
	[sender setImage:[NSImage imageNamed:(showTimestamps ? IMAGE_TIMESTAMPS_ON : IMAGE_TIMESTAMPS_OFF)
								forClass:[self class]]];
...
{
	return (!item ||
			([item isKindOfClass:[AIMetaContact class]] &&
...
- (id)outlineView:(NSOutlineView *)outlineView objectValueForTableColumn:(NSTableColumn *)tableColumn byItem:(id)item
{
	...
	if (itemClass == [AIMetaContact class]) {
		...
		return [NSString
			stringWithFormat:AILocalizedString(@"All (%@)", nil),
							 ((contactCount == 1)
			   ...
			   item:(id)item
{
	if ([item isKindOfClass:[AIMetaContact class]] &&
		[[(AIMetaContact *)item listContactsIncludingOfflineAccounts] count] > 1) {
		/* If the metacontact contains a single contact, fall through (isKindOfClass:[AIListContact class]) and allow
		 * using of a service icon. If it has multiple contacts, use no icon unless a user icon is present.
		 */
		...
		NSImage *image = [AIUserIcons listUserIconForContact:(AIListContact *)item size:NSMakeSize(16, 16)];
		if (!image)
			image = [AIServiceIcons serviceIconForObject:(AIListContact *)item
													type:AIServiceIconSmall
											   direction:AIIconNormal];
		[cell setImage:image];

	} else if ([item isKindOfClass:[AILogToGroup class]]) {
		[cell setImage:[AIServiceIcons
						   serviceIconForService:[adium.accountController
													 firstServiceWithServiceID:[(AILogToGroup *)item serviceClass]]
											type:AIServiceIconSmall
									   direction:AIIconNormal]];
		...
		if ([[outlineView arrayOfSelectedItems] containsObjectIdenticalTo:item] &&
			...
			if (!adiumIconHighlighted) {
				adiumIconHighlighted = [[NSImage imageNamed:@"adiumHighlight" forClass:[self class]] retain];
			}
			...
			if (!adiumIcon) {
				adiumIcon = [[NSImage imageNamed:@"adium" forClass:[self class]] retain];
			}
...
- (void)outlineViewSelectionDidChange:(NSNotification *)notification
{
	...
	[self performSelector:@selector(outlineViewSelectionDidChangeDelayed) withObject:nil afterDelay:0.05];
}
		...
		for (item in selectedItems) {
			if ([item isKindOfClass:[AIMetaContact class]]) {
				for (AIListContact *contact in [(AIMetaContact *)item listContactsIncludingOfflineAccounts]) {
					[contactIDsToFilter
						addObject:[[[NSString stringWithFormat:@"%@.%@", contact.service.serviceID, contact.UID]
									  compactedString] safeFilenameString]];
				}

			} else if ([item isKindOfClass:[AIListContact class]]) {
				[contactIDsToFilter
					addObject:[[[NSString stringWithFormat:@"%@.%@", ((AIListContact *)item).service.serviceID,
														   ((AIListContact *)item).UID] compactedString]
								  safeFilenameString]];

			} else if ([item isKindOfClass:[AILogToGroup class]]) {
				[contactIDsToFilter addObject:[[NSString stringWithFormat:@"%@.%@", [(AILogToGroup *)item serviceClass],
																		  [(AILogToGroup *)item to]] compactedString]];
			}
		}
	}
...
- (NSMenu *)outlineView:(NSOutlineView *)outlineView menuForEvent:(NSEvent *)theEvent;
{
	if (outlineView == outlineView_contacts) {
		NSInteger clickedRow = [outlineView_contacts
			rowAtPoint:[outlineView_contacts convertPoint:[theEvent locationInWindow] fromView:nil]];
		...
		if ([item isKindOfClass:[AILogToGroup class]]) {
			if ([(AILogToGroup *)item to] && [(AILogToGroup *)item serviceClass]) {
				...
				AIService *service =
					[adium.accountController firstServiceWithServiceID:[(AILogToGroup *)item serviceClass]];
				if (service) {
		...
		if ([item isKindOfClass:[AIListContact class]]) {
			NSArray *locationsArray =
				[NSArray arrayWithObjects:[NSNumber numberWithInteger:Context_Contact_Message],
...
- (void)installToolbar
{
	...
	// Toggle Emoticons
	[AIToolbarUtilities
		addToolbarItemToDictionary:toolbarItems
					withIdentifier:@"toggleemoticons"
							 label:(showEmoticons ? HIDE_EMOTICONS : SHOW_EMOTICONS)paletteLabel
								  :AILocalizedString(@"Show/Hide Emoticons", nil)
						   toolTip:AILocalizedString(@"Show or hide emoticons in logs", nil)
							target:self
				   settingSelector:@selector(setImage:)
					   itemContent:[NSImage imageNamed:(showEmoticons ? IMAGE_EMOTICONS_ON : IMAGE_EMOTICONS_OFF)
											  forClass:[self class]]
	...
	[AIToolbarUtilities
		addToolbarItemToDictionary:toolbarItems
					withIdentifier:@"toggletimestamps"
							 label:(showTimestamps ? HIDE_TIMESTAMPS : SHOW_TIMESTAMPS)paletteLabel
								  :AILocalizedString(@"Show/Hide Timestamps", nil)
						   toolTip:AILocalizedString(@"Show or hide timestamps in logs", nil)
							target:self
				   settingSelector:@selector(setImage:)
					   itemContent:[NSImage imageNamed:(showTimestamps ? IMAGE_TIMESTAMPS_ON : IMAGE_TIMESTAMPS_OFF)
											  forClass:[self class]]
...
{
	return [NSArray arrayWithObjects:DATE_ITEM_IDENTIFIER, NSToolbarFlexibleSpaceItemIdentifier, @"delete",
									 @"toggleemoticons", @"toggletimestamps", NSToolbarPrintItemIdentifier,
...
- (NSMenuItem *)_menuItemForDateType:(AIDateType)dateType dict:(NSDictionary *)dateTypeTitleDict
{
	NSMenuItem *menuItem = [[NSMenuItem allocWithZone:[NSMenu menuZone]]
		initWithTitle:[dateTypeTitleDict objectForKey:[NSNumber numberWithInteger:dateType]]
...
{
	NSInteger todayDayOfWeek = [[[NSCalendar currentCalendar] components:NSWeekdayCalendarUnit fromDate:today] weekday];
	NSInteger firstDayOfWeek = [[NSCalendar currentCalendar] firstWeekday];

	return ((todayDayOfWeek >= firstDayOfWeek) ? (todayDayOfWeek - firstDayOfWeek)
											   : ((todayDayOfWeek + 7) - firstDayOfWeek));
}
...
- (void)openLogAtPath:(NSString *)inPath
{
	...
	[self filterForContact:[adium.contactController
							   contactWithService:[adium.accountController firstServiceWithServiceID:serviceID]
										  account:nil
											  UID:contactName]];
	...
	if ([canonicalInPath hasPrefix:[canonicalBasePath stringByAppendingString:@"/"]]) {
		AILogToGroup *logToGroup =
			[logToGroupDict objectForKey:[serviceAndAccountName stringByAppendingPathComponent:contactName]];
		 ...
		 *	/Users/evands/Application Support/Adium 2.0/Users/OtherUser/Logs/AIM.Tekjew/HotChick001/HotChick001
		 * (3-30-2005).AdiumLog and we want to make it relative to our current user's logs folder, which might be
		 *  /Users/evands/Application Support/Adium 2.0/Users/Default/Logs
		 *
		 * To achieve this, add a "/.." for each directory in our current user's logs folder, then add the full path to
		 * the log.
		 */
		...
		chatLog = [[[AIChatLog alloc]
			initWithPath:fakeRelativePath
...
- (NSAlert *)alertForDeletionOfLogCount:(NSUInteger)logCount
{
	...
	[alert
		setInformativeText:[NSString stringWithFormat:AILocalizedString(
														  @"Are you sure you want to send %lu logs to the Trash?", nil),
													  logCount]];
...
- (void)restoreDeletedLogs:(NSArray *)deletedLogs
{
	...
	for (aLog in deletedLogs) {
		...
		[fileManager createDirectoryAtPath:[logPath stringByDeletingLastPathComponent]
...
- (void)deleteLogsAlertDidEnd:(NSAlert *)alert returnCode:(NSInteger)returnCode contextInfo:(void *)contextInfo;
{
	...
	if (returnCode == NSAlertFirstButtonReturn) {
		...
		for (aLog in selectedLogs) {
			...
			AILogToGroup *logToGroup =
				[logToGroupDict objectForKey:[[aLog relativePath] stringByDeletingLastPathComponent]];
			...
			if ([logToGroup logCount] == 0) {
				AILogFromGroup *logFromGroup =
					[logFromGroupDict objectForKey:[[[aLog relativePath] stringByDeletingLastPathComponent]
		...
		[undoManager registerUndoWithTarget:self selector:@selector(restoreDeletedLogs:) object:selectedLogs];
...
- (void)deleteLogs:(NSArray *)selectedLogs
{
	if ([selectedLogs count] > 1) {
		...
		[self deleteLogsAlertDidEnd:nil returnCode:NSAlertFirstButtonReturn contextInfo:[selectedLogs retain]];
	}
}
...
- (void)restoreDeletedToGroups:(NSArray *)toGroups
{
	...
	for (toGroup in toGroups) {
		...
		[fileManager createDirectoryAtPath:[toGroupPath stringByDeletingLastPathComponent]
			   withIntermediateDirectories:YES
								attributes:nil
									 error:NULL];
		if ([fileManager fileExistsAtPath:toGroupPath]) {
			AILog(@"Removing path %@ to make way for %@", toGroupPath,
				  [trashPath stringByAppendingPathComponent:[toGroupPath lastPathComponent]]);
			[fileManager removeItemAtPath:toGroupPath error:NULL];
		}
...
- (void)deleteSelectedContactsFromSourceListAlertDidEnd:(NSAlert *)alert
											...
											contextInfo:(void *)contextInfo;
{
	...
	if (returnCode == NSAlertFirstButtonReturn) {
		...
		for (logToGroup in allSelectedToGroups) {
			...
			AILogFromGroup *logFromGroup = [logFromGroupDict
				objectForKey:[NSString stringWithFormat:@"%@.%@", [logToGroup serviceClass], [logToGroup from]]];
			[logFromGroup removeToGroup:logToGroup];
		}
...
- (void)swipeWithEvent:(NSEvent *)inEvent
{
	...
	[targetTableView selectRowIndexes:[NSIndexSet indexSetWithIndex:nextSelected] byExtendingSelection:NO];
...
NSString *handleSpecialCasesForUIDAndServiceClass(NSString *contactUID, NSString *serviceClass)
{
	...
	if ([serviceClass isEqualToString:@"Jabber"]) {
		...
		if ([contactUID hasSuffix:@"@gmail.com"] || [contactUID hasSuffix:@"@googlemail.com"] ||
...
- (NSMenu *)dateTypeMenu
{
	NSDictionary *dateTypeTitleDict = [NSDictionary
		dictionaryWithObjectsAndKeys:AILocalizedString(@"Any Date", nil),
									 [NSNumber numberWithInteger:AIDateTypeAnyDate], AILocalizedString(@"Today", nil),
									 [NSNumber numberWithInteger:AIDateTypeToday],
									 AILocalizedString(@"Since Yesterday", nil),
									 [NSNumber numberWithInteger:AIDateTypeSinceYesterday],
									 AILocalizedString(@"This Week", nil),
									 [NSNumber numberWithInteger:AIDateTypeThisWeek],
									 AILocalizedString(@"Within Last 2 Weeks", nil),
									 [NSNumber numberWithInteger:AIDateTypeWithinLastTwoWeeks],
									 AILocalizedString(@"This Month", nil),
									 [NSNumber numberWithInteger:AIDateTypeThisMonth],
									 AILocalizedString(@"Within Last 2 Months", nil),
									 [NSNumber numberWithInteger:AIDateTypeWithinLastTwoMonths], nil];
	...
	dateTypeTitleDict = [NSDictionary
		dictionaryWithObjectsAndKeys:AILocalizedString(@"Exactly", nil), [NSNumber numberWithInteger:AIDateTypeExactly],
