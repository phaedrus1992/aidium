...
- (void)installPlugin
{
	...
	menuItem_addContactContext =
		[[NSMenuItem allocWithZone:[NSMenu menuZone]] initWithTitle:ADD_CONTACT_TO_GROUP_ELLIPSIS
															 target:self
															 action:@selector(addContact:)
													  keyEquivalent:@""];
...
- (BOOL)validateMenuItem:(NSMenuItem *)menuItem
{
	if (menuItem == menuItem_delete) {
		...
		if (!selectedListObject) {
			...
			[menuItem setTitle:([selectedListObject isKindOfClass:[AIListGroup class]] ? DELETE_GROUP_ELLIPSIS
																					   : DELETE_CONTACT_ELLIPSIS)];
			return YES;
		}
...
- (void)promptForNewContactOnWindow:(NSWindow *)inWindow selectedListObject:(AIListObject *)inListObject
{
	...
	if (![inListObject isKindOfClass:[AIListContact class]] || [inListObject isKindOfClass:[AIListBookmark class]] ||
	...
	AINewContactWindowController *newContactWindowController = [[AINewContactWindowController alloc]
		initWithContactName:(inListObject ? inListObject.UID : nil)
					service:(inListObject ? [(AIListContact *)inListObject service] : nil)account:nil];
...
- (void)addContactRequest:(NSNotification *)notification
{
	...
	if (userInfo) {
		AINewContactWindowController *newContactWindowController =
			[[AINewContactWindowController alloc] initWithContactName:[userInfo objectForKey:@"UID"]
	...
	if (selectedObjects) {
		...
		if ((currentContextMenuObject = adium.menuController.currentContextMenuObject)) {
			...
			for (AIListGroup *group in currentContextMenuObject.groups) {
				[contactInstances
					addObject:[NSDictionary dictionaryWithObjectsAndKeys:currentContextMenuObject, @"ListObject", group,
...
- (void)deleteFromArray:(NSArray *)array
{
	...
	/* XXX Should allow the account to determine if the message will indicate the group is just hidden, not deleted, á
	 * la Twitter */
	if (array.count == 1) {
		...
		if ([listObject isKindOfClass:[AIListGroup class]]) {
			message = [NSString
				stringWithFormat:
					AILocalizedString(
						@"This will remove the group \"%@\" from the contact lists of your online accounts. The %lu "
						@"contacts within this group will also be removed.\n\nThis action can not be undone.",
						nil),
					listObject.displayName, ((AIListGroup *)listObject).countOfContainedObjects];
		} else {
			if (listObject.containingObjects.count == 1) {
				message = [NSString
					stringWithFormat:AILocalizedString(
										 @"This will remove %@ from the contact lists of your online accounts.", nil),
									 listObject.displayName];
			} else {
				message = [NSString
					stringWithFormat:AILocalizedString(
										 @"This will remove %@ from the group \"%@\" of your online accounts.", nil),
									 listObject.displayName, containingObject.displayName];
			}
		}
	} else {
		...
		if (containsGroup) {
			message = [NSString
				stringWithFormat:
					AILocalizedString(
						@"This will remove %lu items from the contact lists of your online accounts. Contacts in any "
						@"deleted groups will also be removed.\n\nThis action can not be undone.",
						nil),
					array.count];
		} else {
			message =
				[NSString stringWithFormat:AILocalizedString(@"This will remove %lu contacts from the contact lists of "
