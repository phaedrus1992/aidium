...
- (void)importAccountsForService:(NSString *)serviceName
{
	...
	for (NSInteger accountLoop = 0; accountLoop < [accountsFromRaw count]; accountLoop++) {
		if (![serviceName isEqual:@"SubNet"]) {
			...
			[NSApp beginSheet:bonjourNamePromptWindow
...
{
	NSAlert *warningBeforehand = [NSAlert
			 alertWithMessageText:AILocalizedString(@"Are you sure you want to delete all of your iChat Transcripts?",
													nil)
		...
		informativeTextWithFormat:
			AILocalizedString(@"All of the iChat transcripts that were imported into Adium will be moved to the Trash.",
							  nil)];
