...
- (void)controllerDidLoad
{
	...
	[adium.contactAlertsController registerEventID:CONTENT_MESSAGE_SENT
									   withHandler:self
										   inGroup:AIMessageEventHandlerGroup
										globalOnly:NO];
	[adium.contactAlertsController registerEventID:CONTENT_MESSAGE_SENT_GROUP
									   withHandler:self
										   inGroup:AIMessageEventHandlerGroup
										globalOnly:NO];
	[adium.contactAlertsController registerEventID:CONTENT_MESSAGE_RECEIVED
									   withHandler:self
										   inGroup:AIMessageEventHandlerGroup
										globalOnly:NO];
	[adium.contactAlertsController registerEventID:CONTENT_MESSAGE_RECEIVED_FIRST
									   withHandler:self
										   inGroup:AIMessageEventHandlerGroup
										globalOnly:NO];
	[adium.contactAlertsController registerEventID:CONTENT_MESSAGE_RECEIVED_BACKGROUND
									   withHandler:self
										   inGroup:AIMessageEventHandlerGroup
										globalOnly:NO];
	[adium.contactAlertsController registerEventID:CONTENT_MESSAGE_RECEIVED_GROUP
									   withHandler:self
										   inGroup:AIMessageEventHandlerGroup
										globalOnly:NO];
	[adium.contactAlertsController registerEventID:CONTENT_MESSAGE_RECEIVED_BACKGROUND_GROUP
									   withHandler:self
										   inGroup:AIMessageEventHandlerGroup
										globalOnly:NO];
	[adium.contactAlertsController registerEventID:CONTENT_MESSAGE_RECEIVED_AWAY
									   withHandler:self
										   inGroup:AIMessageEventHandlerGroup
										globalOnly:NO];
	[adium.contactAlertsController registerEventID:CONTENT_MESSAGE_RECEIVED_AWAY_GROUP
									   withHandler:self
										   inGroup:AIMessageEventHandlerGroup
										globalOnly:NO];
	[adium.contactAlertsController registerEventID:CONTENT_GROUP_CHAT_MENTION
									   withHandler:self
										   inGroup:AIMessageEventHandlerGroup
										globalOnly:NO];
...
{
	if (inModifiedKeys == nil || [inModifiedKeys containsObject:KEY_CHAT_TIMED_OUT] ||
		[inModifiedKeys containsObject:KEY_CHAT_CLOSED_WINDOW] || [inModifiedKeys containsObject:KEY_CHAT_ERROR]) {
		...
		if ([inChat valueForProperty:KEY_CHAT_ERROR] != nil) {
			...
			case AIChatMessageSendingUserNotAvailable:
				message =
					[NSString stringWithFormat:AILocalizedString(@"Could not send because %@ is not available.", nil),
											   listObject.formattedUID];
				break;

			case AIChatMessageSendingUserIsBlocked:
				message = [NSString stringWithFormat:AILocalizedString(@"Could not send because %@ is blocked.", nil),
													 listObject.formattedUID];
				break;
			...
			case AIChatMessageSendingMissedRateLimitExceeded:
				message = AILocalizedString(@"Could not send the last message because the rate limit has been "
											@"exceeded. Please wait a moment and then try again.",
											nil);
				break;
			...
			case AIChatMessageReceivingMissedRateLimitExceeded:
				message = AILocalizedString(@"Could not receive the last message because the rate limit has been "
											@"exceeded. Please wait a moment and then try again.",
											nil);
				break;

			case AIChatMessageReceivingMissedRemoteIsTooEvil:
				message = [NSString stringWithFormat:AILocalizedString(@"Could not receive; %@ is too evil.", nil),
													 listObject.formattedUID];
			...
			message = [NSString
				stringWithFormat:AILocalizedString(@"%@ closed the conversation window.", nil), listObject.displayName];
			type = @"closed";
		} else if ([inChat boolValueForProperty:KEY_CHAT_TIMED_OUT] && listObject) {
			message = [NSString stringWithFormat:AILocalizedString(@"The conversation with %@ timed out.", nil),
												 listObject.displayName];
			type = @"timed_out";
		}

		if (message) {
			[adium.contentController displayEvent:message ofType:type inChat:inChat];
		}
	}
	...
	if (includeSubject) {
		...
		if ([eventID isEqualToString:CONTENT_MESSAGE_SENT] || [eventID isEqualToString:CONTENT_MESSAGE_SENT_GROUP]) {
			...
			if (messageText && messageText.length) {
				description =
					[NSString stringWithFormat:AILocalizedString(@"You said %@ to %@", "You said Message to Contact"),
											   messageText, displayName];

			} else {
				description = [NSString
					stringWithFormat:AILocalizedString(@"You sent a message to %@", "You sent a message to Contact"),
									 displayName];
			}
			...
			if (messageText && [messageText length]) {
				description = [NSString stringWithFormat:AILocalizedString(@"%@ said %@", "Contact said Message"),
														 displayName, messageText];

			} else {
				description = [NSString
					stringWithFormat:AILocalizedString(@"%@ sent you a message", "Contact sent you a message"),
									 displayName];
			}
		}
...
- (NSString *)stringFromMessageAttributedString:(NSAttributedString *)attributedString
{
	NSMutableAttributedString *mutableMessage =
		[[[attributedString attributedStringByConvertingAttachmentsToStrings] mutableCopy] autorelease];
...
- (NSImage *)imageForEventID:(NSString *)eventID
{
	...
	if (!eventImage)
		eventImage = [[NSImage imageNamed:@"events-message"] retain];
	return eventImage;
}
