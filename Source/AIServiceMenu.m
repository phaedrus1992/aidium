 ...
 *
 * Each menu item's represented object is the AIService it represents.  Services are grouped by 'importance' and sorted
 ...
 * @param activeServicesOnly If YES, only services for enabled accounts are included. If NO, all possible services are
 * included.
 * @param longDescription If YES, use the service's longer (more verbose) description -- for example, AOL Instant
 * Messenger rather than AIM
 * @param format Allows the description to be placed within a format string. If it is nil, the description alone will be
 * used.
 */
							  ...
							  format:(NSString *)format
{
	...
	for (importance = AIServicePrimary; importance <= AIServiceUnsupported; importance++) {
		...
		while ((service = [enumerator nextObject])) {
			if (([service serviceImportance] == importance) && ![service isHidden] &&
