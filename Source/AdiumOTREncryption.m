...
#define PRIVKEY_PATH                                                                                                   \
	[[[adium.loginController userDirectory] stringByAppendingPathComponent:@"otr.private_key"] UTF8String]
#define STORE_PATH                                                                                                     \
	[[[adium.loginController userDirectory] stringByAppendingPathComponent:@"otr.fingerprints"] UTF8String]
 ...
 */
#define OTRL_POLICY_MANUAL_AND_RESPOND_TO_WHITESPACE                                                                   \
...
- (void)prepareEncryption
{
	...
	err = otrl_privkey_read_fingerprints(otrg_plugin_userstate, STORE_PATH, NULL, NULL);
	if (err) {
		const char *errMsg = gpg_strerror(err);
...
static NSDictionary *details_for_context(ConnContext *context)
{
	...
	otrl_privkey_fingerprint(otrg_get_userstate(), our_hash, context->accountname, context->protocol);
	...
	for (NSUInteger i = 0; i < idhalflen; ++i)
		sprintf(sess1 + (2 * i), "%02x", sessionid[i]);
	for (NSUInteger i = 0; i < idhalflen; ++i)
		sprintf(sess2 + (2 * i), "%02x", sessionid[i + idhalflen]);

	account =
		[adium.accountController accountWithInternalObjectID:[NSString stringWithUTF8String:context->accountname]];

	securityDetailsDict = [NSDictionary
		dictionaryWithObjectsAndKeys:[NSString stringWithUTF8String:their_hash], @"Their Fingerprint",
									 [NSString stringWithUTF8String:our_hash], @"Our Fingerprint",
									 [NSNumber numberWithInteger:encryptionStatus], @"EncryptionStatus", account,
									 @"AIAccount", [NSString stringWithUTF8String:context->username], @"who",
									 [NSString stringWithUTF8String:sess1],
									 (sess1_outgoing ? @"Outgoing SessionID" : @"Incoming SessionID"),
									 [NSString stringWithUTF8String:sess2],
									 (sess1_outgoing ? @"Incoming SessionID" : @"Outgoing SessionID"), nil];
...
static ConnContext *contextForChat(AIChat *chat)
{
	...
	context = otrl_context_find(otrg_plugin_userstate, username, accountname, proto, 0, NULL, NULL, NULL);
...
void otrg_plugin_create_privkey(const char *accountname, const char *protocol)
{
	...
	/* Generate the key */
	otrl_privkey_generate(otrg_plugin_userstate, PRIVKEY_PATH, accountname, protocol);
	otrg_ui_update_keylist();
 ...
 */
static int display_otr_message(const char *accountname, const char *protocol, const char *username, const char *msg)
{
	NSString *message;
	...
	if (([message rangeOfString:@"<b>The following message received from"].location != NSNotFound) &&
		 ...
		 * <b>The following message received from tekjew was <i>not</i> encrypted: [</b>MESSAGE_HERE - POTENTIALLY
		 * HTML<b>]</b>
		 */
		NSString *OTRMessage =
			[adiumOTREncryption localizedOTRMessage:@"The following message was <b>not encrypted</b>: "
									   withUsername:nil
							 isWorthOpeningANewChat:NULL];
		message = [OTRMessage
			stringByAppendingString:[message substringWithRange:NSMakeRange(NSMaxRange(endRange),
		...
		if (isWorthOpeningANewChat) {
			...
			/* It's not worth opening a new chat. If we found a chat but it's not open, which can happen if the chat is
			 * still being used by some delayed process, don't display a message thereby opening it.
			 */
			if (![chat isOpen])
				chat = nil;
		}
...
static void notify_cb(void *opdata, OtrlNotifyLevel level, const char *accountname, const char *protocol,
					  const char *username, const char *title, const char *primary, const char *secondary)
{
	...
	[adiumOTREncryption
		notifyWithTitle:[adiumOTREncryption localizedOTRMessage:[NSString stringWithUTF8String:title]
 ...
 * callback. */
static int display_otr_message_cb(void *opdata, const char *accountname, const char *protocol, const char *username,
								  const char *msg)
{
	NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
							   ...
							   const char *username, unsigned char fingerprint[20])
{
	...
	context = otrl_context_find(us, username, accountname, protocol, 0, NULL, NULL, NULL);
...
int max_message_size_cb(void *opdata, ConnContext *context)
{
	...
	if (!maxSizeByServiceClassDict) {
		maxSizeByServiceClassDict = [[NSDictionary alloc]
			initWithObjectsAndKeys:[NSNumber numberWithInteger:2343], @"AIM-compatible",
								   [NSNumber numberWithInteger:1409], @"MSN", [NSNumber numberWithInteger:832],
								   @"Yahoo!", [NSNumber numberWithInteger:1999], @"Gadu-Gadu",
								   [NSNumber numberWithInteger:417], @"IRC", nil];
	}
...
- (void)willSendContentMessage:(AIContentMessage *)inContentMessage
{
	...
	err = otrl_message_sending(otrg_plugin_userstate, &ui_ops, /* opData */ NULL, accountname, protocol, username,
	...
	if (err && fullOutgoingMessage == NULL) {
		...
		otrl_message_fragment_and_send(&ui_ops, /* opData */ NULL, context, fullOutgoingMessage,
									   OTRL_FRAGMENT_SEND_ALL_BUT_LAST, &lastFragmentOfMessage);
...
void otrg_plugin_start_smp(ConnContext *context, const unsigned char *secret, size_t secretlen)
{
	otrl_message_initiate_smp(otrg_plugin_userstate, &ui_ops, NULL, context, secret, secretlen);
}

/* Continue the Socialist Millionaires' Protocol over the current connection,
 * using the given initial secret (ie finish step 2). */
void otrg_plugin_continue_smp(ConnContext *context, const unsigned char *secret, size_t secretlen)
{
	otrl_message_respond_smp(otrg_plugin_userstate, &ui_ops, NULL, context, secret, secretlen);
}
						   ...
						   onAccount:(AIAccount *)inAccount
{
	 ...
	 */
	res = otrl_message_receiving(otrg_plugin_userstate, &ui_ops, NULL, accountname, protocol, username, message,
	 ...
	 * Protocol. */
	ConnContext *context =
		otrl_context_find(otrg_plugin_userstate, username, accountname, protocol, 0, NULL, NULL, NULL);
	if (context) {
...
void update_security_details_for_chat(AIChat *inChat)
{
	...
	[adiumOTREncryption setSecurityDetails:details_for_context(context) forChat:inChat];
}
	...
	if (inChat) {
		...
		if (securityDetailsDict) {
			...
			description = [NSString
				stringWithFormat:
					format, AILocalizedString(@"Encrypted by Off-the-Record Messaging", nil),
					[[inChat listObject] formattedUID], [securityDetailsDict objectForKey:@"Their Fingerprint"],
					AILocalizedString(@"Secure ID for this session:", nil),
					AILocalizedString(@"Incoming:",
									  "This is shown before the Off-the-Record Session ID (a series of numbers and "
									  "letters) sent by the other party with whom you are having an encrypted chat."),
					[securityDetailsDict objectForKey:@"Incoming SessionID"],
					AILocalizedString(
						@"Outgoing:",
						"This is shown before the Off-the-Record Session ID (a series of numbers and letters) sent by "
						"you to the other party with whom you are having an encrypted chat."),
					[securityDetailsDict objectForKey:@"Outgoing SessionID"], nil];

			[fullSecurityDetailsDict setObject:description forKey:@"Description"];
		} else {
...
void send_default_query_to_chat(AIChat *inChat)
{
	...
	char *msg =
		otrl_proto_default_query_msg([inChat.account.formattedUID UTF8String], policyForContact([inChat listObject]));
...
void disconnect_from_context(ConnContext *context)
{
	otrl_message_disconnect(otrg_plugin_userstate, &ui_ops, NULL, context->accountname, context->protocol,
							context->username);
...
void otrg_ui_forget_fingerprint(Fingerprint *fingerprint)
{
	...
	if (context && (context->msgstate == OTRL_MSGSTATE_ENCRYPTED && context->active_fingerprint == fingerprint))
		return;
 ...
 * @param isWorthOpeningANewChat On return, YES if display of this message should open a chat if one doesn't exist. Pass
		   ...
		   isWorthOpeningANewChat:(BOOL *)isWorthOpeningANewChat
{
	...
	if (([message rangeOfString:@"You sent unencrypted data to"].location != NSNotFound) &&
		...
		localizedOTRMessage = [NSString
			stringWithFormat:AILocalizedString(@"You sent an unencrypted message, but %@ was expecting encryption.",
											   "Message when sending unencrypted messages to a contact expecting "
											   "encrypted ones. %s will be a name."),
							 username];

	} else if (([message rangeOfString:@"You sent encrypted data to"].location != NSNotFound) &&
			   ([message rangeOfString:@"who wasn't expecting it"].location != NSNotFound)) {
		localizedOTRMessage = [NSString
			stringWithFormat:AILocalizedString(@"You sent an encrypted message, but %@ was not expecting encryption.",
											   "Message when sending encrypted messages to a contact expecting "
											   "unencrypted ones. %s will be a name."),
							 username];
		if (isWorthOpeningANewChat)
		...
		localizedOTRMessage = [NSString
			stringWithFormat:AILocalizedString(
								 @"%@ is no longer using encryption; you should cancel encryption on your side.",
								 "Message when the remote contact cancels his half of an encrypted conversation. %s "
								 "will be a name."),
							 username];
		...
		localizedOTRMessage = [NSString
			stringWithFormat:AILocalizedString(@"%@'s private connection to you is closed.",
		...
		localizedOTRMessage = AILocalizedString(
			@"Your message was not sent. You should end the encrypted chat on your side or re-request encryption.",
			nil);
		if (isWorthOpeningANewChat)
			*isWorthOpeningANewChat = YES;
		...
		localizedOTRMessage = [NSString
			stringWithFormat:AILocalizedString(@"An encrypted message from %@ could not be decrypted.", nil), username];
		if (isWorthOpeningANewChat)
			*isWorthOpeningANewChat = YES;
	}
...
- (void)notifyWithTitle:(NSString *)title primary:(NSString *)primary secondary:(NSString *)secondary
{
	...
	[adium.interfaceController handleMessage:primary withDescription:secondary withWindowTitle:title];
}

#pragma mark Upgrading gaim-otr --> Adium-otr
/*!
 * @brief Construct a dictionary converting libpurple prpl names to Adium serviceIDs for the purpose of fingerprint
 * upgrading
 */
- (NSDictionary *)prplDict
{
	return [NSDictionary
		dictionaryWithObjectsAndKeys:@"libpurple-OSCAR-AIM", @"prpl-oscar", @"libpurple-Gadu-Gadu", @"prpl-gg",
									 @"libpurple-Jabber", @"prpl-jabber", @"libpurple-Sametime", @"prpl-meanwhile",
									 @"libpurple-MSN", @"prpl-msn", @"libpurple-GroupWise", @"prpl-novell",
									 @"libpurple-Yahoo!", @"prpl-yahoo", @"libpurple-zephyr", @"prpl-zephyr", nil];
}
...
- (NSString *)upgradedFingerprintsFromFile:(NSString *)inPath
{
	...
	if (!sourceFingerprints || ![sourceFingerprints length])
		return nil;
	...
	while (![scanner isAtEnd]) {
		...
		if (username && accountname && protocol && key) {
			for (AIAccount *account in adium.accountController.accounts) {
				...
				if ([[NSSet setWithObjects:account.UID, account.formattedUID, [account.UID compactedString], nil]
						containsObject:accountname]) {
					if ([account.service.serviceCodeUniqueID isEqualToString:[prplDict objectForKey:protocol]]) {
						[outFingerprints
							appendString:[NSString stringWithFormat:@"%@\t%@\t%@\t%@", username,
																	account.internalObjectID,
																	account.service.serviceCodeUniqueID, key]];
						if (trusted) {
...
- (NSString *)upgradedPrivateKeyFromFile:(NSString *)inPath
{
	...
	if (!sourcePrivateKey || ![sourcePrivateKey length])
		return nil;
	...
	for (AIAccount *account in adium.accountController.accounts) {
		...
		for (NSString *accountName in
			 [NSSet setWithObjects:account.UID, account.formattedUID, [account.UID compactedString], nil]) {
			NSRange accountNameRange = NSMakeRange(0, 0);
			NSRange searchRange = NSMakeRange(0, [sourcePrivateKey length]);

			while (accountNameRange.location != NSNotFound && (NSMaxRange(searchRange) <= [sourcePrivateKey length])) {
				...
				if (accountNameRange.location != NSNotFound) {
					...
					// Make sure that this account name actually begins and finishes a name; otherwise (name TekJew2)
					// matches (name TekJew)
					if ((![[sourcePrivateKey substringWithRange:NSMakeRange(accountNameRange.location - 6, 6)]
							 isEqualToString:@"(name "] &&
						 ![[sourcePrivateKey substringWithRange:NSMakeRange(accountNameRange.location - 7, 7)]
							 isEqualToString:@"(name \""]) ||
						(![[sourcePrivateKey substringWithRange:NSMakeRange(NSMaxRange(accountNameRange), 1)]
							 isEqualToString:@")"] &&
						 ![[sourcePrivateKey substringWithRange:NSMakeRange(NSMaxRange(accountNameRange), 2)]
							 isEqualToString:@"\")"])) {
						continue;
					}
					...
					if (protocolRange.location != NSNotFound) {
						...
						if ([account.service.serviceCodeUniqueID isEqualToString:uniqueServiceID]) {
							...
							[sourcePrivateKey replaceCharactersInRange:protocolNameRange withString:uniqueServiceID];
							...
							if ([sourcePrivateKey characterAtIndex:(accountNameRange.location +
																	accountNameRange.length + 1)] == '\"') {
								accountNameRange.length += 1;
							}
...
- (void)upgradeOTRIfNeeded
{
	if (![[adium.preferenceController preferenceForKey:@"GaimOTR_to_AdiumOTR_Update" group:@"OTR"] boolValue]) {
		...
		NSString *privateKey =
			[self upgradedPrivateKeyFromFile:[sourcePath stringByAppendingPathComponent:@"otr.private_key"]];
		...
		NSString *fingerprints =
			[self upgradedFingerprintsFromFile:[sourcePath stringByAppendingPathComponent:@"otr.fingerprints"]];
	...
	if (![[adium.preferenceController preferenceForKey:@"Libgaim_to_Libpurple_Update" group:@"OTR"] boolValue]) {
		NSString *destinationPath = [adium.loginController userDirectory];
		...
		[privateKeys writeToFile:privateKeyPath atomically:YES encoding:NSUTF8StringEncoding error:NULL];
		[privateKeys release];
		...
		[fingerprints writeToFile:fingerprintsPath atomically:YES encoding:NSUTF8StringEncoding error:NULL];
