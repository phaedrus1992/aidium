...
- (void)chat:(AIChat *)chat addedListContact:(AIListContact *)inContact
{
	[adium.contactAlertsController generateEvent:CONTENT_CONTACT_JOINED_CHAT
								   forListObject:inContact
										userInfo:[NSDictionary dictionaryWithObject:chat forKey:@"AIChat"]
									...
									includeSubject:(BOOL)includeSubject
{
	...
	if (includeSubject) {
		if ([eventID isEqualToString:CONTENT_CONTACT_JOINED_CHAT]) {
			description = [NSString stringWithFormat:AILocalizedString(@"%@ joined %@", "Contact joined Chat Name"),
													 listObject.displayName, chat.displayName];

		} else if ([eventID isEqualToString:CONTENT_CONTACT_LEFT_CHAT]) {
			description = [NSString stringWithFormat:AILocalizedString(@"%@ left %@", "Contact left Chat Name"),
													 listObject.displayName, chat.displayName];
		} else if ([eventID isEqualToString:CONTENT_CONTACT_LEFT_CHAT]) {
			description = [NSString stringWithFormat:AILocalizedString(@"%@ left %@", "Contact left Chat Name"),
													 listObject.displayName, chat.displayName];
		} else if ([eventID isEqualToString:CONTENT_CONTACT_LEFT_CHAT]) {
			description = [NSString stringWithFormat:AILocalizedString(@"%@ invites you to a group chat %@",
																	   "Contact invites you to a group chat"),
													 listObject.displayName, chat.displayName];
		} else if ([eventID isEqualToString:CONTENT_GROUP_CHAT_MENTION]) {
			description = [NSString stringWithFormat:AILocalizedString(@"%@ mentioned you in %@",
		...
		if ([eventID isEqualToString:CONTENT_CONTACT_JOINED_CHAT]) {
			description = [NSString
				stringWithFormat:AILocalizedString(@"joined %@", "Contact joined Chat Name"), chat.displayName];

		} else if ([eventID isEqualToString:CONTENT_CONTACT_LEFT_CHAT]) {
			description =
				[NSString stringWithFormat:AILocalizedString(@"left %@", "Contact left Chat Name"), chat.displayName];
		} else if ([eventID isEqualToString:CONTENT_GROUP_CHAT_INVITE]) {
			description =
				[NSString stringWithFormat:AILocalizedString(@"invites you to a group chat", "Contact left Chat Name"),
										   chat.displayName];
		} else if ([eventID isEqualToString:CONTENT_GROUP_CHAT_MENTION]) {
			description = [NSString stringWithFormat:AILocalizedString(@"you were mentioned in %@",
																	   "Someone mentions your name in a group chat"),
													 chat.displayName];
		}
	}
