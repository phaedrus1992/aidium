...
static OSStatus systemOutputDeviceDidChange(AudioObjectID inObjectID, UInt32 inNumberAddresses,
											const AudioObjectPropertyAddress inAddresses[], void *refcon);
...
- (id)init
{
	if ((self = [super init])) {
		...
		// Sign up for notification when the user changes the system output device in the Sound pane of System
		// Preferences.
		AudioObjectPropertyAddress audioAddress = {kAudioHardwarePropertyDefaultSystemOutputDevice,
												   kAudioObjectPropertyScopeGlobal, kAudioObjectPropertyElementMaster};
		OSStatus err =
			AudioObjectAddPropertyListener(kAudioObjectSystemObject, &audioAddress, systemOutputDeviceDidChange, self);

		if (err != noErr)
			NSLog(@"%s: Couldn't sign up for system-output-device-changed notification, because "
				  @"AudioHardwareAddPropertyListener returned %ld. Adium will not know when the default system audio "
				  @"device changes.",
				  __PRETTY_FUNCTION__, (long)err);
	}
...
- (void)controllerDidLoad
{
	...
	[adium.preferenceController registerDefaults:[NSDictionary dictionaryNamed:SOUND_DEFAULT_PREFS
																	  forClass:[self class]]
...
- (void)dealloc
{
	...
	[soundCacheDict release];
	soundCacheDict = nil;
	[soundCacheArray release];
	soundCacheArray = nil;
	[soundCacheCleanupTimer invalidate];
	[soundCacheCleanupTimer release];
	soundCacheCleanupTimer = nil;
...
- (void)cachedPlaySound:(NSString *)inPath
{
	...
	if (!sound) {
		...
		// Move this sound to the front of the cache (This will naturally move lesser used sounds to the back for
		// removal)
	...
	if (sound) {
		...
		// This only has an effect if the movie is not already playing. It won't stop it, and it won't start it over
		// (the latter is what setCurrentTime: is for).
...
- (NSString *)systemAudioDeviceID
{
	...
	AudioObjectPropertyAddress theAddress = {kAudioHardwarePropertyDefaultSystemOutputDevice,
											 kAudioObjectPropertyScopeGlobal, kAudioObjectPropertyElementMaster};
	err = AudioObjectGetPropertyData(kAudioObjectSystemObject, &theAddress, 0, NULL, &dataSize, &systemOutputDevice);
	if (err != noErr) {
		NSLog(@"%s: Could not get the system output device: AudioHardwareGetProperty returned error %ld",
			  __PRETTY_FUNCTION__, (long)err);
		return NULL;
	}
	...
	AudioObjectPropertyAddress uidAddress = {kAudioDevicePropertyDeviceUID, kAudioDevicePropertyScopeOutput,
											 /*channel*/ 0};

	err = AudioObjectGetPropertyData(systemOutputDevice, &uidAddress, 0, NULL, &dataSize, &deviceUID);
	if (err != noErr) {
		NSLog(@"%s: Could not get the device UID for device %ld: AudioDeviceGetProperty returned error %ld",
			  __PRETTY_FUNCTION__, (unsigned long)systemOutputDevice, (long)err);
		return NULL;
	}
...
#pragma unused(inAddresses)
	...
	NSCAssert1(self,
			   @"AudioHardware property listener function %s called with nil refcon, which we expected to be the "
			   @"AdiumSound instance",
			   __PRETTY_FUNCTION__);

	[self performSelectorOnMainThread:@selector(systemOutputDeviceDidChange) withObject:nil waitUntilDone:NO];
	[pool release];
