...
- (void)installPlugin
{
	...
	advancedPreferences =
		[(ESContactListAdvancedPreferences *)[ESContactListAdvancedPreferences preferencePane] retain];

	attachSubmenu = [[NSMenu alloc] init];
	[attachSubmenu setDelegate:self];

	// Context submenu
	attachMenuItem = [[NSMenuItem alloc]
		initWithTitle:AILocalizedString(@"Attach To Window", "Menu item for attaching groups to detachable windows")
			   target:self
	...
	// Context submenu
	detachMenuItem = [[NSMenuItem alloc]
		initWithTitle:AILocalizedString(@"Detach From Window", "Menu item for detaching groups from their window")
			   target:self
	...
	menuItem_nextDetached =
		[[NSMenuItem alloc] initWithTitle:AILocalizedString(@"Next Detached Group", "menu item title")
								   target:self
	...
	menuItem_previousDetached =
		[[NSMenuItem alloc] initWithTitle:AILocalizedString(@"Previous Detached Group", "menu item title")
								   target:self
...
- (id)detachContactList:(AIContactList *)contactList
{
	...
	AIListWindowController *newContactList =
		[AIBorderlessListWindowController listWindowControllerForContactList:contactList];
...
{
	return ([self contactListIsVisible] && [[defaultController window] isMainWindow]);
}
...
{
	return (defaultController && [[defaultController window] isVisible] &&
...
- (void)menuNeedsUpdate:(NSMenu *)menu
{
	...
	if (selectedObject.contactList != adium.contactController.contactList) {
		[menu addItemWithTitle:AILocalizedString(@"Main Window",
...
- (void)detachFromWindow:(id)sender
{
	...
	[self moveListGroup:(AIListGroup *)adium.menuController.currentContextMenuObject toContactList:destinationGroup];
...
- (BOOL)validateMenuItem:(NSMenuItem *)menuItem
{
	...
	if ((menuItem == menuItem_nextDetached) || (menuItem == menuItem_previousDetached) ||
		(menuItem == menuItem_consolidate) || (menuItem == attachMenuItem)) {
		...
		if (firstTime || !key || [key isEqualToString:KEY_LIST_LAYOUT_WINDOW_STYLE]) {
			...
			if (newWindowStyle != windowStyle) {
				...
				if (defaultController) {
					...
					[self performSelector:@selector(closeAndReopencontactList) withObject:nil afterDelay:0];
				}
			}
		}
	}
...
- (void)saveAndCloseDetachedGroups
{
	...
	for (AIListWindowController *windowController in [[contactLists copy] autorelease]) {
		NSMutableDictionary *dict = [NSMutableDictionary
			dictionaryWithObject:[[[windowController contactList] containedObjects] valueForKey:@"UID"]
						  forKey:DETACHED_WINDOW_GROUPS];
		[detachedWindowsDicts addObject:dict];
		[self closeContactList:windowController];
	}

	[adium.preferenceController setPreference:detachedWindowsDicts forKey:DETACHED_WINDOWS group:PREF_DETACHED_GROUPS];
