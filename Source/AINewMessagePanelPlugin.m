...
- (BOOL)validateMenuItem:(id)menuItem
{
	if ([menuItem action] == @selector(newMessage:)) {
		...
		for (AIAccount *account in [adium.accountController
				 accountsCompatibleWithService:adium.menuController.currentContextMenuObject.service]) {
			if (account.online) {
				enable = YES;
				break;
			}
...
- (void)contextualOpenChat:(id)sender
{
	...
	[adium.interfaceController
		setActiveChat:[adium.chatController
						  openChatWithContact:(AIListContact *)adium.menuController.currentContextMenuObject
