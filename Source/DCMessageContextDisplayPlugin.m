...
- (void)addContextDisplayToWindow:(NSNotification *)notification
{
	...
	if (context && [context count] > 0 && shouldDisplay) {
		...
		for (contextMessage in context) {
			...
			[adium.contentController displayContentObject:contextMessage usingContentFilters:YES immediately:YES];
		}
...
- (NSArray *)contextForChat:(AIChat *)chat lines:(NSInteger)linesLeftToFind alsoStatus:(BOOL)alsoStatus
{
	...
	if (!logPaths || linesLeftToFind == 0)
		return nil;

	NSString *logObjectUID = chat.name;
	if (!logObjectUID)
		logObjectUID = chat.listObject.UID;
	logObjectUID = [logObjectUID safeFilenameString];

	AIHTMLDecoder *decoder = [AIHTMLDecoder decoder];

	NSString *baseLogPath = [[AILoggerPlugin logBasePath]
		stringByAppendingPathComponent:[AILoggerPlugin relativePathForLogWithObject:logObjectUID
																		  onAccount:chat.account]];
	...
	while (linesLeftToFind > 0 && (logPath = [pathsEnumerator nextObject])) {
		...
		// By default, the xmlFilePath is the chat log file/bundle... if we find that the chatlog is a bundle, we'll use
		// the xml file inside.
		...
		if ([[NSFileManager defaultManager] fileExistsAtPath:logPath isDirectory:&isDir]) {
			...
			if (isDir) {
				baseURL = logPath;
				xmlFilePath =
					[logPath stringByAppendingPathComponent:[[[logPath lastPathComponent] stringByDeletingPathExtension]
		...
		NSMutableDictionary *contextInfo = nil;
		{
			...
			NSString *serviceName = [[[[[logPath stringByDeletingLastPathComponent] stringByDeletingLastPathComponent]
				lastPathComponent] componentsSeparatedByString:@"."] objectAtIndex:0U];

			AIListObject *account = chat.account;
			NSString *accountID = [NSString stringWithFormat:@"%@.%@", account.service.serviceID, account.UID];

			contextInfo = [NSMutableDictionary
				dictionaryWithObjectsAndKeys:serviceName, @"Service name", account, @"Account", accountID,
											 @"Account ID", chat, @"Chat", decoder, @"AIHTMLDecoder",
											 [NSValue valueWithPointer:&linesLeftToFind], @"LinesLeftToFindValue",
											 foundMessages, @"FoundMessages", elementStack, @"ElementStack",
											 [NSNumber numberWithBool:alsoStatus], @"AlsoAllowStatus", nil];
		...
		@try {
			do {
				...
				// Continue to parse as long as we need more elements, we have data to read, and LMX doesn't think we're
				// done.
			} while ([foundMessages count] < linesLeftToFind && offset > 0 && result != LMXParsedCompletely);

		} @catch (id theException) {
			AILogWithSignature(
				@"Error \"%@\" while parsing %@; foundMessages at that point was %@, and the chunk to be parsed was %@",
				theException, logPath, foundMessages, chunk);
			...
			AILog(@"Context: %lu messages from %@: %@", (unsigned long)foundMessages.count,
				  [xmlFilePath lastPathComponent], foundMessages);
	...
	if (linesLeftToFind > 0) {
		AILogWithSignature(@"Unable to find %lu logs for %@; we needed %lu more", (unsigned long)linesToDisplay, chat,
						   (unsigned long)linesLeftToFind);
	}
...
- (void)parser:(LMXParser *)parser elementStarted:(NSString *)elementName attributes:(NSDictionary *)inAttributes
{
	...
	if ([elementStack count]) {
		...
		if ([elementName isEqualToString:@"message"]) {
			...
			// This means that we have all of this message now, and therefore can create a single content object from
			// the AIXMLElement tree and then throw away that tree. This saves memory when a message element contains
			// many elements (since each one is represented by an AIXMLElement sub-tree in the AIXMLElement tree, as
			// opposed to a simple NSAttributeRun in the NSAttributedString of the content object).
			...
			if (timeString) {
				...
				NSString *sender =
					[NSString stringWithFormat:@"%@.%@", serviceName, [attributes objectForKey:@"sender"]];
				 ...
				 *Why the class trickery? Less code duplication, clearer what is actually different between the two
				 * cases.
				 */
				Class messageClass =
					(-[timeVal timeIntervalSinceNow] > 300.0) ? [AIContentContext class] : [AIContentMessage class];
				...
				AIContentMessage *message = [messageClass
					messageInChat:chat
					   withSource:(sentByMe ? account : listContact)destination
								 :(sentByMe ? (chat.isGroupChat ? nil : chat.listObject) : account)date:timeVal
						  message:[[contextInfo objectForKey:@"AIHTMLDecoder"] decodeHTML:[element contentsAsXMLString]]
						autoreply:(autoreplyAttribute &&
								   [autoreplyAttribute caseInsensitiveCompare:@"true"] == NSOrderedSame)];
		...
		} else if ([[contextInfo valueForKey:@"AlsoAllowStatus"] boolValue] &&
			...
			if (timeString) {
				...
				AIContentStatus *status = [[AIContentStatus alloc] initWithChat:chat
																		 source:nil
																	destination:nil
																		   date:timeVal];
