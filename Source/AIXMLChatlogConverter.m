...
+ (NSAttributedString *)readFile:(NSString *)filePath withOptions:(NSDictionary *)options
{
	...
	@try {
		...
		return [[[NSAttributedString alloc]
			initWithString:AILocalizedString(@"Sorry, there was an error parsing this transcript. It may be corrupt.",
											 nil)] autorelease];
	}
	return result;
}
...
- (id)init
{
	if ((self = [super init])) {
		...
		statusLookup = [[NSDictionary alloc]
			initWithObjectsAndKeys:
				AILocalizedString(@"Online", nil), @"online", AILocalizedString(@"Idle", nil), @"idle",
				...
				[adium.statusController localizedDescriptionForCoreStatusName:STATUS_NAME_EXTENDED_AWAY],
				@"extendedAway", [adium.statusController localizedDescriptionForCoreStatusName:STATUS_NAME_BRB],
				@"beRightBack",
				[adium.statusController localizedDescriptionForCoreStatusName:STATUS_NAME_NOT_AVAILABLE],
				@"notAvailable", [adium.statusController localizedDescriptionForCoreStatusName:STATUS_NAME_NOT_AT_DESK],
				@"notAtMyDesk",
				[adium.statusController localizedDescriptionForCoreStatusName:STATUS_NAME_NOT_IN_OFFICE],
				@"notInTheOffice",
...
- (NSAttributedString *)readData:(NSData *)xmlData withOptions:(NSDictionary *)options retrying:(BOOL)reentrancyFlag
{
	...
	NSError *err = nil;
	NSXMLDocument *xmlDoc = [[[NSXMLDocument alloc] initWithData:xmlData options:NSXMLNodePreserveCDATA
		...
		if ([type isEqualToString:@"message"]) {
			...
			if (sentMessage) {
				...
				dispatch_sync(dispatch_get_main_queue(), ^{
					listObject = [adium.contactController
						existingListObjectWithUniqueID:[AIListObject internalObjectIDForServiceID:service UID:sender]];
				});
			...
			[NSDateFormatter withLocalizedDateFormatterShowingSeconds:YES
														showingAMorPM:YES
															  perform:^(NSDateFormatter *dateFormatter) {
																  timestampStr =
																	  [[dateFormatter stringFromDate:date] retain];
															  }];
					...
					[htmlDecoder
						decodeHTML:[NSString
									   stringWithFormat:
										   ...
										   (showTimestamps
												? [NSString stringWithFormat:@"<span class=\"timestamp\">%@</span> ",
																			 timestampStr]
												: @""),
										   shownSender,
			...
			if (showEmoticons) {
				attributedMessage = [adium.contentController
					filterAttributedString:attributedMessage
						   usingFilterType:AIFilterMessageDisplay
								 direction:(sentMessage ? AIFilterOutgoing : AIFilterIncoming)context:nil];
			}
			...
			// Note: I am diverging from what the AILoggerPlugin logs in this case.  It can't handle every case we can
			// have here
			if ([messageXML length]) {
				if ([statusLookup objectForKey:status]) {
					displayMessage = [NSString stringWithFormat:AILocalizedString(@"Changed status to %@: %@", nil),
																[statusLookup objectForKey:status], messageXML];
				} else {
				...
				displayMessage = [NSString stringWithFormat:AILocalizedString(@"Changed status to %@", nil),
															[statusLookup objectForKey:status]];
			}
			...
			if ([displayMessage length]) {
				[output
					appendAttributedString:[htmlDecoder
											   decodeHTML:[NSString
															  stringWithFormat:@"<div class=\"status\">%@ (%@)</div>\n",
																			   displayMessage, timestampStr]]];
			}
			[timestampStr release];
		}
	}
