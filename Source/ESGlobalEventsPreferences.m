...
#define VOLUME_SOUND_PATH                                                                                              \
	[NSString                                                                                                          \
		pathWithComponents:[NSArray arrayWithObjects:@"/", @"System", @"Library", @"LoginPlugins",                     \
													 [@"BezelServices" stringByAppendingPathExtension:@"loginPlugin"], \
													 @"Contents", @"Resources",                                        \
													 [@"volume" stringByAppendingPathExtension:@"aiff"], nil]]
...
- (NSString *)paneName
{
	return AILocalizedString(@"Events", "Name of preferences and tab for specifying what Adium should do when events "
										"occur - for example, display a Growl alert when John signs on.");
}
/*!
 * @brief Nib name
 */
...
- (void)localizePane
{
	[[button_minvolume cell]
		accessibilitySetOverrideValue:AILocalizedString(
										  @"Set minimum volume",
										  "Accessibility label for button to set to the minimum sound volume")
						 forAttribute:NSAccessibilityDescriptionAttribute];
	[[button_maxvolume cell]
		accessibilitySetOverrideValue:AILocalizedString(
										  @"Set maximum volume",
										  "Accessibility label for button to set to the maximum sound volume")
						 forAttribute:NSAccessibilityDescriptionAttribute];
	[[slider_volume cell]
		accessibilitySetOverrideValue:AILocalizedString(@"Volume", "Accessibility label for the sound volume slider")
						 forAttribute:NSAccessibilityDescriptionAttribute];
...
- (void)viewWillClose
{
	...
	[contactAlertsViewController release];
...
- (void)popUp:(NSPopUpButton *)inPopUp shouldShowCustom:(BOOL)showCustom
{
	...
	BOOL customIsShowing =
		(lastItem && (![lastItem representedObject] && [[lastItem title] isEqualToString:CUSTOM_TITLE]));
	if (showCustom && !customIsShowing) {
		// Add 'custom' then select it
		[[inPopUp menu] addItem:[NSMenuItem separatorItem]];
		[[inPopUp menu] addItemWithTitle:CUSTOM_TITLE target:nil action:nil keyEquivalent:@""];
...
- (NSMenu *)eventPresetsMenu
{
	...
	menuItem = [[[NSMenuItem allocWithZone:[NSMenu menuZone]]
		initWithTitle:[AILocalizedString(@"Add New Preset", nil) stringByAppendingEllipsis]
			   target:self
	...
	menuItem = [[[NSMenuItem allocWithZone:[NSMenu menuZone]]
		initWithTitle:[AILocalizedString(@"Edit Presets", nil) stringByAppendingEllipsis]
			   target:self
...
- (void)selectActiveEventInPopUp
{
	...
	if (![popUp_eventPreset selectedItem])
		[popUp_eventPreset selectItemWithTitle:activeEventSetName];
	// And if that fails, select the first item (something went wrong, we should at least have a selection)
	if (![popUp_eventPreset selectedItem])
		[popUp_eventPreset selectItemAtIndex:0];
}
...
- (void)addNewPreset:(id)sender
{
	...
	ESPresetNameSheetController *presetNameSheetController =
		[[ESPresetNameSheetController alloc] initWithDefaultName:defaultName
...
- (void)editPresets:(id)sender
{
	ESPresetManagementController *presentManagementController =
		[[ESPresetManagementController alloc] initWithPresets:[plugin storedEventPresetsArray]
												   namedByKey:@"Name"
												 withDelegate:self];
			...
			renamedPreset:(id *)renamedPreset
{
	...
	NSString *localizedCurrentName =
		[self _localizedTitle:[adium.preferenceController preferenceForKey:KEY_ACTIVE_EVENT_SET
																	 group:PREF_GROUP_EVENT_PRESETS]];
	[newPreset setObject:newName forKey:@"Name"];
	...
	if ([localizedCurrentName isEqualToString:oldPresetName]) {
		[adium.preferenceController setPreference:newName forKey:KEY_ACTIVE_EVENT_SET group:PREF_GROUP_EVENT_PRESETS];
	}
...
- (NSArray *)duplicatePreset:(NSDictionary *)preset inPresets:(NSArray *)presets createdDuplicate:(id *)duplicatePreset
{
	...
	[newEventPreset setObject:newName forKey:@"Name"];
	...
	if (duplicatePreset != NULL)
		*duplicatePreset = [[newEventPreset retain] autorelease];
		...
		presetAfterMove:(id *)presetAfterMove
{
	...
	[newEventPreset setObject:[NSNumber numberWithDouble:newOrderIndex] forKey:@"OrderIndex"];

	// Now save the new preset
	[plugin saveEventPreset:newEventPreset];
	if (presetAfterMove != NULL)
		*presetAfterMove = [[newEventPreset retain] autorelease];
	[newEventPreset release];
...
- (void)contactAlertsDidChangeForActionID:(NSString *)actionID
{
	if (!actionID || [actionID isEqualToString:SOUND_ALERT_IDENTIFIER]) {
		...
		if (![alertsArray count]) {
			...
			if ([alertsArray count] && ![sounds count]) {
				// If we have one or more sound alerts and there are no sounds in this sound set ("None" sound set),
				// there's no matching soundSetMenuitem.
				soundMenuItem = nil;
					...
					for (alertDict in alertsArray) {
						if ([[alertDict objectForKey:KEY_ACTION_ID] isEqualToString:SOUND_ALERT_IDENTIFIER]) {
							...
							if (![sounds objectForKey:englishEvent] ||
								![[[alertDict objectForKey:KEY_ACTION_DETAILS] objectForKey:KEY_ALERT_SOUND_PATH]
									isEqualToString:[sounds objectForKey:englishEvent]]) {
								soundMenuItem = nil;
								break;
							}
						}
 ...
 *
 * Also called after the user changes sounds manually, by -[ESGlobalEventsPreferences
 * contactAlertsDidChangeForActionID].
 */
- (IBAction)selectSoundSet:(id)sender
{
...
- (NSMutableDictionary *)currentEventSetForSaving
{
	...
	if (soundSetName) {
		[currentEventSetForSaving setObject:soundSetName forKey:KEY_EVENT_SOUND_SET];
	} else {
		[currentEventSetForSaving removeObjectForKey:KEY_EVENT_SOUND_SET];
	}

	// Get and store the alerts array
	NSArray *alertsArray = [adium.contactAlertsController alertsForListObject:nil withEventID:nil actionID:nil];
...
- (void)showPresetCopySheet:(NSString *)originalPresetName
{
	...
	explanatoryText = AILocalizedString(
		@"You are editing a default event set.  Please enter a unique name for your modified set.", nil);
...
- (void)updateSoundSetSelection
{
	...
	while ((soundSet = [enumerator nextObject])) {
		if ([[soundSet name] isEqualToString:name])
			break;
	}
