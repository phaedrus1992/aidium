...
- (void)xtrasChanged:(NSNotification *)notification
{
	...
	// Convert our filename extension into a Uniform Type Identifier so that we can robustly determine what type of Xtra
	// this is.
	CFStringRef type = (CFStringRef)[(NSString *)UTTypeCreatePreferredIdentifierForTag(
		kUTTagClassFilenameExtension, (CFStringRef)filenameExtension,
						 ...
						 firstTime:(BOOL)firstTime
{
	...
	if ([group isEqualToString:PREF_GROUP_APPEARANCE]) {
		if (firstTime) {
			...
			[checkBox_verticalAutosizing
				setState:[[prefDict objectForKey:KEY_LIST_LAYOUT_VERTICAL_AUTOSIZE] boolValue]];
			[checkBox_horizontalAutosizing
				setState:[[prefDict objectForKey:KEY_LIST_LAYOUT_HORIZONTAL_AUTOSIZE] boolValue]];
			[slider_windowOpacity
				setDoubleValue:([[prefDict objectForKey:KEY_LIST_LAYOUT_WINDOW_OPACITY] doubleValue] * 100.0)];
			[slider_horizontalWidth
				setIntegerValue:[[prefDict objectForKey:KEY_LIST_LAYOUT_HORIZONTAL_WIDTH] integerValue]];
			[self _updateSliderValues];
		}

		// Horizontal resizing label
		if (firstTime || [key isEqualToString:KEY_LIST_LAYOUT_WINDOW_STYLE] ||
			...
			// Configure vertical autosizing's appearance. AIListWindowController must match this behavior for this to
			// make sense.
			switch (windowStyle) {
			...
			case AIContactListWindowStyleGroupChat:
				...
				[checkBox_verticalAutosizing
					setState:[[adium.preferenceController preferenceForKey:KEY_LIST_LAYOUT_VERTICAL_AUTOSIZE
		...
		if (firstTime || [key isEqualToString:KEY_STATUS_ICON_PACK]) {
			...
			if (![popUp_statusIcons selectedItem]) {
				[popUp_statusIcons
					selectItemWithTitle:[adium.preferenceController defaultPreferenceForKey:KEY_STATUS_ICON_PACK
																					  group:PREF_GROUP_APPEARANCE
																					 object:nil]];
			}
		}
		if (firstTime || [key isEqualToString:KEY_SERVICE_ICON_PACK]) {
			...
			if (![popUp_serviceIcons selectedItem]) {
				[popUp_serviceIcons
					selectItemWithTitle:[adium.preferenceController defaultPreferenceForKey:KEY_SERVICE_ICON_PACK
																					  group:PREF_GROUP_APPEARANCE
																					 object:nil]];
			}
		}
		if (firstTime || [key isEqualToString:KEY_MENU_BAR_ICONS]) {
			...
			if (![popUp_menuBarIcons selectedItem]) {
				[popUp_menuBarIcons
					selectItemWithTitle:[adium.preferenceController defaultPreferenceForKey:KEY_MENU_BAR_ICONS
																					  group:PREF_GROUP_APPEARANCE
																					 object:nil]];
			}
		}
...
- (IBAction)changePreference:(id)sender
{
	if (sender == popUp_statusIcons) {
		...
		if ([[sender selectedItem] tag] != AIEmoticonMenuMultiple) {
			...
			if (selectedPack)
				[adium.emoticonController setEmoticonPack:selectedPack enabled:YES];
...
- (void)_updateSliderValues
{
	[textField_windowOpacity
		setStringValue:[NSString stringWithFormat:@"%ld%%", (NSInteger)[slider_windowOpacity doubleValue]]];
	[textField_horizontalWidthIndicator
		setStringValue:[NSString stringWithFormat:@"%ldpx", [slider_horizontalWidth integerValue]]];
}
...
- (NSMenu *)_emoticonPackMenu
{
	...
	if ([[adium.emoticonController activeEmoticonPacks] count] > 1) {
		menuItem = [[[NSMenuItem allocWithZone:[NSMenu menuZone]]
			initWithTitle:AILocalizedString(@"Multiple Packs Selected", nil)
				   target:nil
				   action:nil
			keyEquivalent:@""] autorelease];
		[menuItem setImage:[NSImage imageNamed:@"emoticonBlank" forClass:[self class]]];
...
- (IBAction)createListTheme:(id)sender
{
	...
	ESPresetNameSheetController *presetNameSheetController = [[ESPresetNameSheetController alloc]
		initWithDefaultName:[[theme stringByAppendingString:@" "]
								stringByAppendingString:AILocalizedString(@"(Copy)", nil)]
...
- (void)listThemeEditorWillCloseWithChanges:(BOOL)saveChanges forThemeNamed:(NSString *)name
{
	if (saveChanges) {
		...
		if ([plugin createSetFromPreferenceGroup:PREF_GROUP_LIST_THEME
			...
			[adium.preferenceController setPreference:name forKey:KEY_LIST_THEME_NAME group:PREF_GROUP_APPEARANCE];
		}
		...
		[popUp_colorTheme selectItemWithTitle:[adium.preferenceController preferenceForKey:KEY_LIST_THEME_NAME
																					 group:PREF_GROUP_APPEARANCE]];
	}
}
...
- (void)manageListThemes:(id)sender
{
	...
	ESPresetManagementController *presetManagementController =
		[[ESPresetManagementController alloc] initWithPresets:_listThemes namedByKey:@"name" withDelegate:self];
...
- (IBAction)createListLayout:(id)sender
{
	...
	ESPresetNameSheetController *presetNameSheetController = [[ESPresetNameSheetController alloc]
		initWithDefaultName:[[layout stringByAppendingString:@" "]
								stringByAppendingString:AILocalizedString(@"(Copy)", nil)]
...
- (void)listLayoutEditorWillCloseWithChanges:(BOOL)saveChanges forLayoutNamed:(NSString *)name
{
	if (saveChanges) {
		...
		if ([plugin createSetFromPreferenceGroup:PREF_GROUP_LIST_LAYOUT
			...
			[adium.preferenceController setPreference:name forKey:KEY_LIST_LAYOUT_NAME group:PREF_GROUP_APPEARANCE];
		}

	} else {
		// Revert back to selected layout
		NSString *layout = [adium.preferenceController preferenceForKey:KEY_LIST_LAYOUT_NAME
																  group:PREF_GROUP_APPEARANCE];
...
- (void)manageListLayouts:(id)sender
{
	...
	ESPresetManagementController *presetManagementController =
		[[ESPresetManagementController alloc] initWithPresets:_listLayouts namedByKey:@"name" withDelegate:self];
	[presetManagementController showOnWindow:[[self view] window]];
						 ...
						 userInfo:(id)userInfo
{
	...
	while ((presetDict = [enumerator nextObject])) {
		if ([newName isEqualToString:[presetDict objectForKey:@"name"]])
			return NO;
	}
 ...
 *
 */
- (NSArray *)renamePreset:(NSDictionary *)preset
				   toName:(NSString *)newName
				inPresets:(NSArray *)presets
			renamedPreset:(id *)renamedPreset
{
	NSArray *newPresets;
...
{
	NSString *newName =
		[NSString stringWithFormat:@"%@ (%@)", [preset objectForKey:@"name"], AILocalizedString(@"Copy", nil)];
...
- (NSMenu *)_listLayoutMenu
{
	...
	// Preset management
	menuItem = [[[NSMenuItem allocWithZone:[NSMenu menuZone]]
		initWithTitle:[AILocalizedString(@"Add New Layout", nil) stringByAppendingEllipsis]
			   target:self
	...
	menuItem = [[[NSMenuItem allocWithZone:[NSMenu menuZone]]
		initWithTitle:[AILocalizedString(@"Edit Layouts", nil) stringByAppendingEllipsis]
			   target:self
...
- (NSMenu *)_colorThemeMenu
{
	...
	// Preset management
	menuItem = [[[NSMenuItem allocWithZone:[NSMenu menuZone]]
		initWithTitle:[AILocalizedString(@"Add New Theme", nil) stringByAppendingEllipsis]
			   target:self
	...
	menuItem = [[[NSMenuItem allocWithZone:[NSMenu menuZone]]
		initWithTitle:[AILocalizedString(@"Edit Themes", nil) stringByAppendingEllipsis]
			   target:self
...
- (NSMenuItem *)meuItemForDockIconPackAtPath:(NSString *)packPath
{
	...
	[adium.dockController getName:&name previewState:&preview forIconPackAtPath:packPath];
 ...
 *
 * Initially, the dock icon menu just has the currently selected icon; the others will be generated lazily if the icon
 * is displayed, in menuNeedsUpdate:
 */
- (void)configureDockIconMenu
{
	...
	NSString *activePackName = [adium.preferenceController preferenceForKey:KEY_ACTIVE_DOCK_ICON
																	  group:PREF_GROUP_APPEARANCE];
	iconPath = [adium pathOfPackWithName:activePackName extension:@"AdiumIcon" resourceFolderName:FOLDER_DOCK_ICONS];
