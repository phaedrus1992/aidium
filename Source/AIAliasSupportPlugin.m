 ...
 * The userInfo is a dictionary with an NSNumber on key @"Notify" indicating if a 'changed' notification should be sent
 * out as a result. If this is NO, it is equivalent to a 'silent' update. The user info dictionary also has the desired
 * NSString alias on the key @"Alias". If this is not specified, the object's preference is reloaded.
 */
- (void)applyAliasRequested:(NSNotification *)notification
{
	...
	if (!alias) {
		alias = [object preferenceForKey:@"Alias" group:PREF_GROUP_ALIASES];
	}

	[self _applyAlias:alias toObject:object notify:(shouldNotifyNumber ? [shouldNotifyNumber boolValue] : NO)];
}
...
- (NSSet *)_applyAlias:(NSString *)inAlias toObject:(AIListObject *)inObject notify:(BOOL)notify
{
	...
	// Apply the alias
	[[inObject displayArrayForKey:@"Display Name" create:(inAlias != nil)] setObject:inAlias
																		   withOwner:self
	...
	if ([inObject isKindOfClass:[AIListContact class]]) {
		...
		[[inObject displayArrayForKey:@"Long Display Name"
							   create:(longDisplayName && ![longDisplayName isEqualToString:displayName])]
			setObject:longDisplayName
			withOwner:self];
	}
	...
	if (notify) {
		[[AIContactObserverManager sharedManager] listObjectAttributesChanged:inObject modifiedKeys:modifiedAttributes];
	}
