	...
	if ([inObject isKindOfClass:[AIListContact class]] && !inObject.isStranger) {
		if ([inModifiedKeys containsObject:@"isOnline"]) {
			if (inObject.online) {
				...
				[inObject setPreference:[NSDate date] forKey:KEY_LAST_SEEN_DATE group:PREF_GROUP_LAST_SEEN];
...
{
	return AILocalizedString(@"Last Seen",
	...
	if (!inObject.online) {
		...
		if (lastSeenStatus && lastSeenDate) {
			...
			[NSDateFormatter withLocalizedShortDateFormatterPerform:^(NSDateFormatter *dateFormatter) {
				[sinceDateFormatter
					setDateFormat:[NSString
									  stringWithFormat:@"%@, %@", [dateFormatter dateFormat],
													   [NSDateFormatter localizedDateFormatStringShowingSeconds:NO
																								  showingAMorPM:YES]]];
			}];

			// stringForTimeIntervalSinceDate may return @"" if it's too short of an interval.
			timeElapsed = [NSDateFormatter stringForTimeIntervalSinceDate:lastSeenDate
														   showingSeconds:NO
															  abbreviated:NO];
			if (timeElapsed && [timeElapsed length]) {
				timeElapsedWithDesignation = [NSString
					stringWithFormat:
						AILocalizedString(
							@"%@ ago",
							"%@ will be replaced by an amount of time such as '1 day, 4 hours'. This string is used in "
							"the 'Last Seen:' information shown when hovering over an offline contact."),
						timeElapsed];
			} else {
			...
			entry = [[NSAttributedString alloc]
				initWithString:[NSString stringWithFormat:@"%@\n%@%@%@", lastSeenStatus, timeElapsedWithDesignation,
