...
#define KEY_CONTACT_LIST_DOCKED_TO_BOTTOM_OF_SCREEN                                                                    \
	[NSString stringWithFormat:@"Contact List Docked To Bottom:%@", [[self contactList] contentsBasedIdentifier]]
				 ...
				 delegate:(id<AIListControllerDelegate>)inDelegate
{
	...
	if ((self = [self initWithContactListView:inContactListView
		...
		[[NSNotificationCenter defaultCenter] addObserver:self
												 selector:@selector(reloadListObject:)
...
{
	if (!object)
		[(AIAnimatingListOutlineView *)contactListView
			setEnableAnimation:[[prefDict objectForKey:KEY_CL_ANIMATE_CHANGES] boolValue]];
}
...
- (void)contactListDesiredSizeChanged
{
	...
	if ((self.autoResizeVertically || self.autoResizeHorizontally) && (theWindow = [contactListView window]) &&
		...
		NSRect desiredFrame =
			[self _desiredWindowFrameUsingDesiredWidth:(self.autoResizeHorizontally || (forcedWindowWidth != -1))
		...
		if (!NSEqualRects(currentFrame, desiredFrame)) {
			...
			[theWindow
				setMinSize:NSMakeSize((self.autoResizeHorizontally ? desiredFrame.size.width : minWindowSize.width),
									  (self.autoResizeVertically ? (desiredFrame.size.height - toolbarHeight)
 ...
 * The window is actually off-screen and should remain as such; we therefore perform sizing but maintain an appropriate
 * origin such that the window won't be seen.
 */
- (void)contactListWillSlideOnScreen
{
	...
	if ((self.autoResizeVertically || self.autoResizeHorizontally) && (theWindow = [contactListView window])) {
		...
		/* Pretend, for autosizing purposes, we're where we'll be once we're done sliding on screen. This allows sizing
		 * relative to screen edges and the dock to work properly. We'll return to our previous origin after performing
		 * size checking.
		 */
...
- (void)windowDidMove:(NSNotification *)notification
{
	...
	if ((windowFrame.origin.y < boundingFrame.origin.y + EDGE_CATCH_Y) &&
		((windowFrame.origin.y + windowFrame.size.height) <
		 (boundingFrame.origin.y + boundingFrame.size.height - EDGE_CATCH_Y))) {
		...
		if ((windowFrame.origin.y < visibleBoundingFrame.origin.y + EDGE_CATCH_Y) &&
			((windowFrame.origin.y + windowFrame.size.height) <
			 (visibleBoundingFrame.origin.y + visibleBoundingFrame.size.height - EDGE_CATCH_Y))) {
...
- (NSRect)_desiredWindowFrameUsingDesiredWidth:(BOOL)useDesiredWidth desiredHeight:(BOOL)useDesiredHeight
{
	...
	if (!currentScreen)
		currentScreen = [(AIListWindowController *)[theWindow windowController] windowLastScreen];
	if (!currentScreen)
	...
	if (useDesiredWidth) {
		...
		anchorToRightEdge =
			((currentScreen && ((NSMaxX(windowFrame) + EDGE_CATCH_X) >= NSMaxX(visibleScreenFrame))) ||
	 ...
	 *
	 * If the window is against the left or right edges of the screen AND the user did not dock to the visibleFrame
	 * last, we use the full screenFrame as our bound. The edge check is used since most users' docks will not extend to
	 * the edges of the screen. Alternately, if the user docked to the total frame last, we can safely use the full
	 * screen even if we aren't on the edge.
	 */
	...
	if ((windowOnEdge && (dockToBottomOfScreen != AIDockToBottom_VisibleFrame)) ||
		...
		if (([NSApp isOnMavericksOrNewer] && [NSScreen screensHaveSeparateSpaces]) ||
			((screens = [NSScreen screens]) && ([screens count]) && (currentScreen == [screens objectAtIndex:0]))) {
	...
	if (useDesiredHeight) {
		...
		if (NSHeight(newWindowFrame) >= NSHeight(boundingFrame)) {
			...
			if (dockToBottomOfScreen == AIDockToBottom_No) {
				...
				// If the user did dock (either to the full screen or the visible screen), the origin should remain in
				// place.
		...
		if (newWindowFrame.size.height == 0)
			newWindowFrame.size.height = 1;

		// Keep the window from hanging off any Y screen edge (This is optional and could be removed if this annoys
		// people)
		if (NSMaxY(newWindowFrame) > NSMaxY(boundingFrame))
			newWindowFrame.origin.y = NSMaxY(boundingFrame) - newWindowFrame.size.height;
		if (NSMinY(newWindowFrame) < NSMinY(boundingFrame))
			newWindowFrame.origin.y = NSMinY(boundingFrame);
	}

	if (useDesiredWidth) {
		/* If the desired height plus any toolbar height exceeds the height we determined, we will be showing a
		 * scroller; expand horizontally to take that into account.
		 */
		if (desiredHeight + (NSHeight(windowFrame) - NSHeight(viewFrame)) > NSHeight(newWindowFrame)) {
			CGFloat scrollerWidth =
				[NSScroller scrollerWidthForControlSize:[[scrollView_contactList verticalScroller] controlSize]
		...
		if (newWindowFrame.size.width == 0)
			newWindowFrame.size.width = 1;

		// Keep the window from hanging off any X screen edge (This is optional and could be removed if this annoys
		// people)
		if (NSMaxX(newWindowFrame) > NSMaxX(boundingFrame))
			newWindowFrame.origin.x = NSMaxX(boundingFrame) - NSWidth(newWindowFrame);
		if (NSMinX(newWindowFrame) < NSMinX(boundingFrame))
			newWindowFrame.origin.x = NSMinX(boundingFrame);
	}

	return newWindowFrame;
}

@synthesize autoResizeHorizontally, autoResizeVertically, autoResizeHorizontallyWithIdleTime, minWindowSize,
			...
			proposedChildIndex:(NSInteger)idx
{
	...
	if ([types containsObject:@"AIListObject"]) {
		...
		for (dragItem in dragItems) {
			...
			if (![dragItem isKindOfClass:[AIListGroup class]])
				hasNonGroup = YES;
			if (hasGroup && hasNonGroup)
				break;
		}

		// Don't allow a drop within the contact list or within a group if we contain a mixture of groups and non-groups
		// (e.g. contacts)
		if (hasGroup && hasNonGroup)
		...
		if (!item && adium.contactController.useContactListGroups) {
			 ...
			 * If groups are shown at all, assuming we have any items in the list at all, she is hovering near but not
			 * in a group. If (index > 0), the drag is below the end of a group. That group is at (index - 1) in the
			 * outline view's root. If (index == 0), the drag is at the very top of the contact list. Do this right by
			 * shifting the drop to that group.
			 */
			AIProxyListObject *itemAboveProposedIndex =
				(AIProxyListObject *)[[outlineView dataSource] outlineView:outlineView
																	 child:((idx > 0) ? (idx - 1) : 0)
																	ofItem:nil];
			...
			idx = ((idx > 0) ? [[outlineView dataSource] outlineView:outlineView
											  numberOfChildrenOfItem:itemAboveProposedIndex]
		...
		if ((idx == NSOutlineViewDropOnItemIndex) && [proposedListObject isKindOfClass:[AIListContact class]] &&
			...
			if (([contactListView rowForItem:primaryDragItem] == -1) ||
				...
				if ([primaryDragItem isKindOfClass:[AIListContact class]] &&
					 ...
					 */
					[outlineView
						   setDropItem:[AIProxyListObject
										   existingProxyListObjectForListObject:[(AIListContact *)item parentContact]
																   inListObject:[(AIListContact *)item parentContact]
																					.containingObjects.anyObject]
			...
			// Figure out where we would insert the dragged item if the sort controller manages the location and it's
			// going into an expandable item

			// XXX If we can sort manually but the sort controller also has some control (e.g. status sort with manual
			// ordering), we should get a hint and make use of it.

			AISortController *sortController = [AISortController activeSortController];
			AIListObject<AIContainingObject> *container = proposedListObject
															  ? (AIListObject<AIContainingObject> *)proposedListObject
			...
			if (!sortController.canSortManually && [container containsObject:[dragItems objectAtIndex:0]]) {
				...
				if (proposedListObject && [proposedListObject isKindOfClass:[AIMetaContact class]]) {
					...
					if ((isExpanded && (idx == [[outlineView dataSource] outlineView:outlineView
						...
						[outlineView setDropItem:parentObject
								  dropChildIndex:[parentObject visibleIndexOfObject:proposedListObject] + 1];
					}
				}
	...
	} else if ([types containsObject:NSFilenamesPboardType] || [types containsObject:NSRTFPboardType] ||
			   [types containsObject:NSURLPboardType] || [types containsObject:NSStringPboardType] ||
			   [types containsObject:AIiTunesTrackPboardType]) {
		retVal =
			((proposedListObject && [proposedListObject isKindOfClass:[AIListContact class]]) ? NSDragOperationLink
																							  : NSDragOperationNone);
...
- (NSArray *)arrayOfAllContactsFromArray:(NSArray *)inArray
{
	...
	for (aDragItem in inArray) {
		if ([aDragItem isKindOfClass:[AIMetaContact class]]) {
			...
			[realDragItems
				addObjectsFromArray:[[adium.contactController allContactsWithService:aDragItem.service
	...
	if ((availableType = [draggingPasteboard availableTypeFromArray:[NSArray arrayWithObject:@"AIListObject"]])) {
		...
		if (!dragItems) {
			...
			for (uniqueID in dragItemsUniqueIDs) {
				[arrayOfDragItems
					addObject:[AIProxyListObject
								  proxyListObjectForListObject:[adium.contactController
																   existingListObjectWithUniqueID:uniqueID]
												  inListObject:nil]];
			}
		...
		if ([item.listObject isKindOfClass:[AIListGroup class]]) {
			...
			if (item.listObject != adium.contactController.offlineGroup) {
				...
				for (AIProxyListObject *proxyObject in dragItems) {
					...
					NSAssert2([group canContainObject:listObject], @"BUG: Attempting to drop %@ into %@", listObject,
							  group);
		...
		[[AIContactObserverManager sharedManager] endListObjectNotificationsDelay];
					...
					availableTypeFromArray:[NSArray arrayWithObjects:NSFilenamesPboardType, AIiTunesTrackPboardType,
																	 nil]])) {
