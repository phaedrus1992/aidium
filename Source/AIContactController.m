...
- (void)clearAllMetaContactData
{
	...
	[adium.preferenceController setPreference:nil forKey:KEY_FLAT_METACONTACTS group:PREF_GROUP_CONTACT_LIST];
	[adium.preferenceController setPreference:nil forKey:KEY_METACONTACT_OWNERSHIP group:PREF_GROUP_CONTACT_LIST];

	// Clear out old metacontact files
	[[NSFileManager defaultManager]
		removeFilesInDirectory:[[adium.loginController userDirectory] stringByAppendingPathComponent:OBJECT_PREFS_PATH]
...
- (void)saveContactList
{
	...
	[adium.preferenceController setPreference:bookmarks forKey:KEY_BOOKMARKS group:PREF_GROUP_CONTACT_LIST];
}
	...
	for (NSData *data in [adium.preferenceController preferenceForKey:KEY_BOOKMARKS group:PREF_GROUP_CONTACT_LIST]) {
		...
		if (bookmark) {
			if ([bookmarkDict objectForKey:bookmark.internalObjectID]) {
				// In case we end up with two bookmarks with the same internalObjectID; this should be almost
				// impossible.
...
- (void)_loadMetaContactsFromArray:(NSArray *)array
{
	for (NSString *identifier in array) {
		...
		NSNumber *objectID =
			[NSNumber numberWithInteger:[[[identifier componentsSeparatedByString:@"-"] objectAtIndex:1] integerValue]];
...
- (void)_addContactLocally:(AIListContact *)listContact toGroup:(id<AIContainingObject>)localGroup
{
	...
	if (listContact.canJoinMetaContacts) {
		...
		if (existingObject && (existingObject != listContact)) {
			...
			AILogWithSignature(@"%@ and %@ match; grouped into %@", listContact, existingObject, metaContact);
 ...
 *
 * This will remove listContact from our local tracking within each of oldGroups, which must be
 * AIListObject<AIContainingObject> objects. It will then add them to groups, which must also be
 * AIListObject<AIContainingObject> objects.
 *
...
- (IBAction)toggleShowGroups:(id)sender
{
	...
	// Update the contact list.  Do it on the next run loop for better menu responsiveness, as it may be a lengthy
	// procedure.
	[self performSelector:@selector(_performChangeOfUseContactListGroups) withObject:nil afterDelay:0];
}
...
- (BOOL)_restoreContactsToMetaContact:(AIMetaContact *)metaContact
{
	...
	if (containedContactsArray.count) {
		[self _restoreContactsToMetaContact:metaContact fromContainedContactsArray:containedContactsArray];
 ...
 *
 * This does not actually place any AIListContacts within the metaContact.  Instead, it updates the
 * contactToMetaContactLookupDict dictionary to have metaContact associated with the list contacts specified by
 * containedContactsArray. This allows us to add them lazily to the metaContact (in contactWithService:account:UID:) as
 * necessary.
 *
 * @param metaContact The metaContact to which contact referneces are added
 * @param containedContactsArray An array of NSDictionary objects, each of which has SERVICE_ID_KEY and UID_KEY which
 * together specify an internalObjectID of an AIListContact
 */
- (void)_restoreContactsToMetaContact:(AIMetaContact *)metaContact
		   fromContainedContactsArray:(NSArray *)containedContactsArray
{
	for (NSDictionary *containedContactDict in containedContactsArray) {
		...
		if (![[containedContactDict objectForKey:KEY_IS_METACONTACT] boolValue]) {
			/* Assign this metaContact to the appropriate internalObjectID for containedContact's represented
			 * listObject.
			 *
			...
			NSString *internalObjectID =
				[AIListObject internalObjectIDForServiceID:[containedContactDict objectForKey:SERVICE_ID_KEY]
													   UID:[containedContactDict objectForKey:UID_KEY]];
			[contactToMetaContactLookupDict setObject:metaContact forKey:internalObjectID];
		}
	}
}
...
- (void)_storeListObject:(AIListObject *)listObject inMetaContact:(AIMetaContact *)metaContact
{
	...
	if (!containedContactsArray)
		containedContactsArray = [[NSMutableArray alloc] init];
	containedContactDict = nil;

	// Create the dictionary describing this list object
	containedContactDict = [NSDictionary
		dictionaryWithObjectsAndKeys:listObject.service.serviceID, SERVICE_ID_KEY, listObject.UID, UID_KEY, nil];
	...
	if (containedContactDict && ([containedContactsArray indexOfObject:containedContactDict] == NSNotFound)) {
		...
		[adium.contactAlertsController mergeAndMoveContactAlertsFromListObject:listObject intoListObject:metaContact];
...
- (void)removeContact:(AIListContact *)inContact fromMetaContact:(AIMetaContact *)metaContact
{
	...
	if (containedContactDict) {
		...
		[newAllMetaContactsDict setObject:newContainedContactsArray forKey:metaContactInternalObjectID];
 ...
 *
 * @result Either the existing AIMetaContact -[self groupUIDs:forServices:usingMetaContactHint:] would return if passed
 * a nil metaContactHint, or nil (if no existing metacontact would be used).
 */
- (AIMetaContact *)knownMetaContactForGroupingUIDs:(NSArray *)UIDsArray forServices:(NSArray *)servicesArray
{
	...
	for (NSInteger i = 0; i < count; i++) {
		if ((metaContact = [contactToMetaContactLookupDict
				 objectForKey:[AIListObject internalObjectIDForServiceID:[servicesArray objectAtIndex:i]
 ...
 * @param UIDsArray NSArray of UIDs
 * @param servicesArray NSArray of serviceIDs corresponding to entries in UIDsArray
 * @param metaContactHint If passed, an AIMetaContact to use for the grouping if an existing one isn't found. If nil, a
 * new metacontact will be craeted in that case.
 */
- (AIMetaContact *)groupUIDs:(NSArray *)UIDsArray
				 forServices:(NSArray *)servicesArray
		usingMetaContactHint:(AIMetaContact *)metaContactHint
{
	...
	for (NSUInteger i = 0; i < count; i++) {
		...
		internalObjectID = [AIListObject internalObjectIDForServiceID:serviceID UID:UID];
		if (!metaContact) {
	...
	if ([internalObjectIDs count] > 1) {
		...
		for (internalObjectID in internalObjectIDs) {
			...
			if ((existingObject = [self existingListObjectWithUniqueID:internalObjectID])) {
				 ...
				 */
				NSAssert([metaContact canContainObject:existingObject],
						 @"Attempting to add something metacontacts can't hold to a metacontact");
				[self addContact:(id)existingObject toMetaContact:metaContact];
			} else {
...
- (void)explodeMetaContact:(AIMetaContact *)metaContact
{
	...
	NSMutableDictionary *allMetaContactsDict =
		[[adium.preferenceController preferenceForKey:KEY_METACONTACT_OWNERSHIP
												group:PREF_GROUP_CONTACT_LIST] mutableCopy];
...
{
	if (key && ![key isEqualToString:KEY_HIDE_CONTACTS] && ![key isEqualToString:KEY_SHOW_OFFLINE_CONTACTS] &&
		![key isEqualToString:KEY_USE_OFFLINE_GROUP] && ![key isEqualToString:KEY_HIDE_CONTACT_LIST_GROUPS]) {
		return;
	}
...
{
	if ((!inModifiedKeys || [inModifiedKeys containsObject:@"isOnline"]) &&
...
- (NSArray *)allContacts
{
	...
	for (AIListContact *contact in self.contactEnumerator) {
		/* We want only contacts, not metacontacts. For a given contact, -[contact parentContact] could be used to
		 * access the meta. */
	...
	for (AIListObject *object in inGroup) {
		if ([object conformsToProtocol:@protocol(AIContainingObject)]) {
			[contactArray addObjectsFromArray:[self allContactsInObject:(id<AIContainingObject>)object
				   ...
				   (!inAccount || ([(AIListContact *)object account] == inAccount)))
...
- (NSMenu *)groupMenuWithTarget:(id)target
{
	...
	// Now traverse the contactLists array in order and build a menu showing the groups in each list with separators in
	// between.
	NSInteger i = 0;
...
{
	if (!(inUID && [inUID length] && inService))
		return nil; // Ignore invalid requests
...
- (AIListContact *)existingContactWithService:(AIService *)inService
...
- (NSSet *)allContactsWithService:(AIService *)service UID:(NSString *)inUID
{
	...
	for (AIAccount *account in [adium.accountController accountsCompatibleWithService:service]) {
		AIListContact *listContact = [self existingContactWithService:service account:account UID:inUID];
...
- (AIListObject *)existingListObjectWithUniqueID:(NSString *)uniqueID
{
	...
	for (AIListObject *listObject in contactDict.objectEnumerator) {
		if ([listObject.internalObjectID isEqualToString:uniqueID])
			return listObject;
	}
...
- (AIListContact *)preferredContactWithUID:(NSString *)inUID
							  andServiceID:(NSString *)inService
					 forSendingContentType:(NSString *)inType
{
	AIService *theService = [adium.accountController firstServiceWithServiceID:inService];
	AIListContact *tempListContact = [[AIListContact alloc] initWithUID:inUID service:theService];
 ...
 *
 * If the destination contact's parent contact differs from the destination contact itself, the chat is with a
 * metaContact. If that metaContact's preferred destination for messaging isn't the same as the contact which was just
 * messaged, update the preference so that a new chat with this metaContact would default to the proper contact.
 */
- (void)didSendContent:(NSNotification *)notification
{
...
- (AIListGroup *)groupWithUID:(NSString *)groupUID
{
	...
	if (!(group = [groupDict objectForKey:[groupUID lowercaseString]])) {
		...
		[contactPropertiesObserverManager performSelector:@selector(_updateAllAttributesOfObject:)
											   withObject:group
											   afterDelay:0.0];
...
{
	NSMutableDictionary *userInfo = [NSMutableDictionary dictionaryWithObject:contactUID forKey:UID_KEY];
	if (inService)
		[userInfo setObject:inService forKey:@"AIService"];
	if (inAccount)
		[userInfo setObject:inAccount forKey:@"AIAccount"];

	[[NSNotificationCenter defaultCenter] postNotificationName:Contact_AddNewContact object:nil userInfo:userInfo];
}

- (void)moveContact:(AIListContact *)contact fromGroups:(NSSet *)oldGroups intoGroups:(NSSet *)groups
{
	AILogWithSignature(@"moveContact %@ [meta=%p; contained by %@; %@] from %@ into %@", contact, contact.metaContact,
					   contact.containingObjects,
					   (contact.existsServerside ? @"exists serverside" : @"does NOT exist serverside"), oldGroups,
					   groups);
		...
		if ([contact conformsToProtocol:@protocol(AIContainingObject)]) {
			...
			for (AIListContact *child in (id<AIContainingObject>)contact) {
				...
				if (child.account.online && !child.isStranger)
					[child.account moveListObjects:[NSArray arrayWithObject:child]
										fromGroups:oldGroups
										  toGroups:groups];
			}
		}
	}
...
- (void)moveGroup:(AIListGroup *)group
	fromContactList:(AIContactList *)oldContactList
	  toContactList:(AIContactList *)newContactList
{
	...
	[[NSNotificationCenter defaultCenter] postNotificationName:Contact_ListChanged object:newContactList userInfo:nil];
