...
- (void)installPlugin
{
	...
	if (!storedEventPresets)
		storedEventPresets = [[NSMutableDictionary alloc] init];

	/* If there is no active event set, or the active event set is not present in our built in or stored event sets
	 * then we are in one of two conditions: either this is a first-launch, or the user has deleted the event
	 * preferences. Either way, we want to set ourselves to the default notification set before proceeding.
	 */
	activeEventSet = [adium.preferenceController preferenceForKey:KEY_ACTIVE_EVENT_SET group:PREF_GROUP_EVENT_PRESETS];
	if (!activeEventSet ||
		(![builtInEventPresets objectForKey:activeEventSet] && ![storedEventPresets objectForKey:activeEventSet])) {
...
+ (NSDictionary *)soundAlertForKey:(NSString *)key inSoundsDict:(NSDictionary *)sounds
{
	...
	if (key && (event = [adium.contactAlertsController eventIDForEnglishDisplayName:key])) {
		soundAlert =
			[NSDictionary dictionaryWithObjectsAndKeys:event, KEY_EVENT_ID, SOUND_ALERT_IDENTIFIER, KEY_ACTION_ID,
													   [NSDictionary dictionaryWithObject:[sounds objectForKey:key]
																				   forKey:KEY_ALERT_SOUND_PATH],
													   KEY_ACTION_DETAILS, nil];
	}
...
- (void)applySoundSet:(AISoundSet *)soundSet
{
	...
	for (NSString *key in sounds) {
		NSDictionary *soundAlert = [ESGlobalEventsPreferencesPlugin soundAlertForKey:key inSoundsDict:sounds];
		if (soundAlert) {
...
- (void)setEventPreset:(NSDictionary *)eventPreset
{
	...
	if ([eventPreset objectForKey:@"Built In"] && [[eventPreset objectForKey:@"Built In"] boolValue]) {
		...
		[self applySoundSet:(soundSet ? [AISoundSet soundSetWithContentsOfFile:[soundSet stringByExpandingBundlePath]]
									  : nil)];
	}
...
- (void)saveEventPreset:(NSMutableDictionary *)eventPreset
{
	...
	if (![eventPreset objectForKey:KEY_ORDER_INDEX]) {
		[eventPreset setObject:[NSNumber numberWithDouble:[self nextOrderIndex]] forKey:KEY_ORDER_INDEX];
	}
	...
	if (!name) {
		...
		[eventPreset setObject:name forKey:KEY_EVENT_SET_NAME];
	}

	NSAssert(eventPreset != nil, @"eventPreset is nil");
	[storedEventPresets setObject:eventPreset forKey:name];
...
NSInteger eventPresetsSort(id eventPresetA, id eventPresetB, void *context)
{
	...
	if (orderIndexA > orderIndexB) {
		...
		return [[eventPresetA objectForKey:KEY_EVENT_SET_NAME]
			caseInsensitiveCompare:[eventPresetB objectForKey:KEY_EVENT_SET_NAME]];
	}
}

- (NSArray *)storedEventPresetsArray
{
	return [[storedEventPresets allValues] sortedArrayUsingFunction:eventPresetsSort context:nil];
}

- (NSArray *)builtInEventPresetsArray
{
	return [[builtInEventPresets allValues] sortedArrayUsingFunction:eventPresetsSort context:nil];
}
