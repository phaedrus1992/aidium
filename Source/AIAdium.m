...
@synthesize accountController, chatController, contactController, contentController, dockController, emoticonController,
	interfaceController, loginController, menuController, preferenceController, soundController, statusController,
	toolbarController, contactAlertsController, fileTransferController, applescriptabilityController, debugController;
...
- (void)handleURLEvent:(NSAppleEventDescriptor *)event withReplyEvent:(NSAppleEventDescriptor *)replyEvent
{
	if (!completedApplicationLoad) {
		...
		[[NSNotificationCenter defaultCenter] postNotificationName:AIURLHandleNotification
															object:[[event descriptorAtIndex:1] stringValue]];
	}
}
...
- (void)completeLogin
{
	...
	[accountController
		controllerDidLoad]; //** Before contactController so accounts and services are available for contact creation
	...
	[contactController
		controllerDidLoad]; //** Before interfaceController so the contact list is available to the interface
	...
	[[NSDistributedNotificationCenter defaultCenter] postNotificationName:AIApplicationDidFinishLoadingNotification
																   object:nil];
...
{
	if (![[preferenceController preferenceForKey:@"Confirm Quit" group:@"Confirmations"] boolValue]) {
	...
	case AIQuitConfirmSelective:
		if ([chatController unviewedContentCount] > 0 && confirmUnreadMessages) {
			questionToAsk = (([chatController unviewedContentCount] > 1)
								 ? [NSString stringWithFormat:AILocalizedString(@"You have %llu unread messages.",
																				@"Quit Confirmation"),
															  (unsigned long long)[chatController unviewedContentCount]]
								 : AILocalizedString(@"You have an unread message.", @"Quit Confirmation"));
			...
			questionToAsk =
				(([fileTransferController activeTransferCount] > 1)
					 ? [NSString stringWithFormat:AILocalizedString(@"You have %llu file transfers in progress.",
																	@"Quit Confirmation"),
												  (unsigned long long)[fileTransferController activeTransferCount]]
					 : AILocalizedString(@"You have a file transfer in progress.", @"Quit Confirmation"));
			...
			questionToAsk =
				(([[chatController openChats] count] > 1)
					 ? [NSString stringWithFormat:AILocalizedString(@"You have %llu open chats.", @"Quit Confirmation"),
												  (unsigned long long)[[chatController openChats] count]]
					 : AILocalizedString(@"You have an open chat.", @"Quit Confirmation"));
	...
	if (allowQuit == NSTerminateLater) {
		[self.interfaceController
			displayQuestion:AILocalizedString(@"Confirm Quit", nil)
			withDescription:[questionToAsk
								stringByAppendingFormat:@"%@%@", ([questionToAsk length] > 0 ? @"\n" : @""),
														AILocalizedString(@"Are you sure you want to quit Adium?",
																		  @"Quit Confirmation")]
			withWindowTitle:nil
...
- (void)applicationWillTerminate:(NSNotification *)notification
{
	...
	[statusController
		controllerWillClose]; //** Before accountController so account states are saved before being set to offline
	...
	[preferenceController
		controllerWillClose]; //** Last since other controllers may want to write preferences as they close
...
- (void)deleteTemporaryFiles
{
	[[NSFileManager defaultManager] removeFilesInDirectory:[self cachesPath] withPrefix:@"TEMP" movingToTrash:NO];
}
...
{
	/* We may have received a message or begun a file transfer while the menu was open, if this is reached via a menu
	 * item. Wait one last run loop before beginning to quit so that activity can be registered, since menus run in a
	 * different run loop mode, NSEventTrackingRunLoopMode.
	 */
...
- (BOOL)application:(NSApplication *)theApplication openFile:(NSString *)filename
{
	...
	if (([extension caseInsensitiveCompare:@"AdiumLog"] == NSOrderedSame) ||
		...
		if (completedApplicationLoad) {
			...
			[[NSNotificationCenter defaultCenter] postNotificationName:AIShowLogAtPathNotification object:filename];
		} else {
			// Queue the request until Adium is done launching if Adium is not ready
			[queuedLogPathToShow release];
			queuedLogPathToShow = [filename retain];
		}
	...
	[prefsCategory release];
	prefsCategory = nil;
	[advancedPrefsName release];
	advancedPrefsName = nil;

	/* Specify a file extension and a human-readable description of what the files of this type do
	 * We reassign the extension so that regardless of its original case we end up with the case we want; this allows
	 * installation of xtras to proceed properly on case-sensitive file systems.
	 */
	if ([extension caseInsensitiveCompare:@"AdiumPlugin"] == NSOrderedSame) {
		...
		// Plugins haven't been loaded yet if the application isn't done loading, so only request a restart if it has
		...
		*/
		...
		if (![packName isEqualToString:defaultPackName]) {
			...
			errorMessage = [NSString
				stringWithFormat:
					AILocalizedString(
						@"%@ is the name of the default status icon pack; this pack therefore can not be installed.",
						nil),
					packName];
		}
	}
	...
	if (destination) {
		...
		destinationFilePath =
			[destination stringByAppendingPathComponent:[[filename lastPathComponent] stringByDeletingPathExtension]];
		...
		if ([filename caseInsensitiveCompare:destinationFilePath] == NSOrderedSame) {
			...
			format = AILocalizedString(
				@"Installation of the %@ %@ was successful because the file was already in the correct location.",
				"Installation introduction, like 'Installation of the message style Fiat was successful...'.");

			alertMsg = [NSString
				stringWithFormat:format, fileDescription, [[filename lastPathComponent] stringByDeletingPathExtension]];
			...
			if ([[NSFileManager defaultManager] copyItemAtPath:filename toPath:destinationFilePath error:NULL]) {

				alertTitle = AILocalizedString(@"Installation Successful", "Title of installation successful window");
				alertMsg = [NSString
					stringWithFormat:
						AILocalizedString(
							@"Installation of the %@ %@ was successful.",
							"Installation sentence, like 'Installation of the message style Fiat was successful.'."),
						fileDescription, [[filename lastPathComponent] stringByDeletingPathExtension]];
				...
				alertMsg = [NSString
					stringWithFormat:AILocalizedString(@"Installation of the %@ %@ was unsuccessful.",
													   "Installation failed sentence, like 'Installation of the "
													   "message style Fiat was unsuccessful.'."),
									 fileDescription, [[filename lastPathComponent] stringByDeletingPathExtension]];
			}
		}
		...
		NSRunAlertPanel(AILocalizedString(@"Installation Failed", "Title of installation failed window"), @"%@", nil,
...
- (NSString *)applicationSupportDirectory
{
	...
	if (!_preferencesFolderPath) {
		_preferencesFolderPath = [[[[[NSBundle mainBundle] infoDictionary] objectForKey:PORTABLE_ADIUM_KEY]
...
- (NSString *)createResourcePathForName:(NSString *)name
{
	...
	if ([existingResourcePaths indexOfObject:targetPath] == NSNotFound) {
		...
		if (![defaultManager createDirectoryAtPath:targetPath
					   withIntermediateDirectories:YES
										attributes:nil
											 error:NULL]) {
			BOOL error;
			...
			if (!error)
				error = ![defaultManager createDirectoryAtPath:targetPath
								   withIntermediateDirectories:YES
													attributes:nil
														 error:NULL];
			...
			if (error) {
				...
				NSInteger result = NSRunCriticalAlertPanel(
					[NSString stringWithFormat:AILocalizedString(@"Could not create the %@ folder.", nil), name],
					AILocalizedString(@"Try running Repair Permissions from Disk Utility.", nil),
					AILocalizedString(@"OK", nil), AILocalizedString(@"Launch Disk Utility", nil), nil);
...
- (NSArray *)resourcePathsForName:(NSString *)name
{
	...
	// Find Library directories in all domains except /System (as of Panther, that's ~/Library, /Library, and
	// /Network/Library)
	...
	/* Check our application support directory directly. It may have been covered by the
	 * NSSearchPathForDirectoriesInDomains() search, or it may be distinct via the Portable Adium preference.
	 */
	NSString *path = [self applicationSupportDirectory];
	if (name)
		path = [path stringByAppendingPathComponent:name];
	if (![pathArray containsObject:path] && ([defaultManager fileExistsAtPath:path isDirectory:&isDir]) && (isDir)) {
	...
	if (name) {
		path =
			[[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:name] stringByExpandingTildeInPath];
...
/*!
 * @brief Returns an array of the paths to all of the resources for a given name, filtering out those without a certain
 * extension
 * @param name The full name (including extension as appropriate) of the resource for which to search
...
- (NSArray *)allResourcesForName:(NSString *)name withExtensions:(id)extensions
{
	...
	for (NSString *resourceDir in [self resourcePathsForName:name]) {
		for (NSString *resourcePath in [[NSFileManager defaultManager] contentsOfDirectoryAtPath:resourceDir
																						   error:NULL]) {
...
- (NSString *)cachesPath
{
	...
	if (!cachesPath) {
		...
		if ([defaultManager createDirectoryAtPath:cachesPath
					  withIntermediateDirectories:YES
									   attributes:nil
											error:NULL]) {
...
- (NSString *)pathOfPackWithName:(NSString *)name
					   extension:(NSString *)extension
			  resourceFolderName:(NSString *)folderName
{
	...
	for (NSString *resourcePath in [self resourcePathsForName:folderName]) {
		...
		if ([fileManager fileExistsAtPath:packPath])
			return [packPath stringByExpandingTildeInPath];
	}
	...
	if (![self earliestLaunchedAdiumVersion]) {
		...
		if ([[NSUserDefaults standardUserDefaults] boolForKey:@"Adium 1.3.3:Reimported Spotlight Logs"]) {
			...
			[[NSUserDefaults standardUserDefaults]
				setObject:[[NSBundle mainBundle] objectForInfoDictionaryKey:(NSString *)kCFBundleVersionKey]
...
- (BOOL)application:(NSApplication *)sender delegateHandlesKey:(NSString *)key
{
	...
	if ([key isEqualToString:@"applescriptabilityController"] || [key isEqualToString:@"interfaceController"]) {
		handleKey = YES;
	}
...
// properly handle the sparkle:minimumSystemVersion element. The second generation began with 1.0.6 and 1.1.1, with a
// Sparkle Plus that can handle that element.
...
- (NSArray *)feedParametersForUpdater:(SUUpdater *)updater sendingSystemProfile:(BOOL)sendProfileInfo
{
	...
	// return just the type of update we're looking for and the generation number.
	...
	[profileInfo
		addObject:[NSDictionary dictionaryWithObjectsAndKeys:@"revision", @"key", @"Revision", @"visibleKey", buildId,
															 @"value", buildId, @"visibleValue", nil]];
	...
	if (sendProfileInfo) {
		...
		entry = [NSDictionary dictionaryWithObjectsAndKeys:@"IMServices", @"key", accountInfoString, @"value", nil];
		[profileInfo addObject:entry];
		...
		if (error == 0) {
			...
			[profileInfo
				addObject:[NSDictionary
							  dictionaryWithObjects:[NSArray arrayWithObjects:@"detailedcputype", @"Detailed CPU Type",
																			  visibleCPUType, visibleCPUType, nil]
											forKeys:[NSArray arrayWithObjects:@"key", @"displayKey", @"value",
																			  @"displayValue", nil]]];
		}
	}
...
typedef enum { kNumberType, kStringType, kPeriodType } SUCharacterType;
