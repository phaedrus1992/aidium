...
+ (NSString *)logBasePath
{
	...
	dispatch_once(&didResolveLogBaseAlias, ^{
		...
		if (noErr != err) {
			NSLog(@"Warning: Couldn't obtain FSRef for transcripts folder: %s (%ld)", GetMacOSStatusCommentString(err),
				  (long)err);
		} else if (!isDir) {
			Boolean wasAliased_nobodyCares;
			err = FSResolveAliasFile(&ref, /*resolveAliasChains*/ true, &isDir, &wasAliased_nobodyCares);
			if (noErr != err) {
				NSLog(@"Warning: Couldn't resolve alias to transcripts folder: %s (%ld)",
					  GetMacOSStatusCommentString(err), (long)err);
			} else {
		...
		searchIndexQueue, blockWithAutoreleasePool(^{
			if (!logIndex) {
				...
				if (!_index) {
					...
					_index = SKIndexCreateWithURL((CFURLRef)logIndexURL, (CFStringRef) @"Content", kSKIndexInverted,
...
- (void)_configureMenuItems
{
	...
	viewContactLogsMenuItem =
		[[[NSMenuItem allocWithZone:[NSMenu menuZone]] initWithTitle:VIEW_LOGS_WITH_CONTACT
															  target:self
		...
		dispatch_group_async(
			logAppendingGroup, dispatch_get_main_queue(), blockWithAutoreleasePool(^{
				...
				if ([contentType isEqualToString:CONTENT_MESSAGE_TYPE] ||
					...
					[messageElement
						addEscapedObject:[xhtmlDecoder encodeHTML:[content message]
...
- (void)chatOpened:(NSNotification *)notification
{
	...
	if (appender) {
		...
		[eventElement
			setAttributeNames:[NSArray arrayWithObjects:@"type", @"sender", @"time", nil]
					   values:[NSArray arrayWithObjects:@"windowOpened", chat.account.UID,
														[formatter
															stringFromDate:[[NSDate date] dateWithCalendarFormat:nil
																										timeZone:nil]],
														nil]];
						...
						dispatch_group_async(
							logIndexingGroup, ioQueue, blockWithAutoreleasePool(^{
								...
								if (document && bself.indexingAllowed) {
									 ...
									 *	1. Slower, especially to start initial indexing, which is the most common use
									 * case since the log viewer indexes recently-modified ("dirty") logs when it opens.
									 *  2. Sometimes logs don't appear to be associated with the right URI type and
									 * therefore don't get indexed.
									 *  3. On 10.3, this means that logs' markup is indexed in addition to their text,
													...
													if (documentText && CFStringGetLength(documentText) > 0 &&
														...
														dispatch_once(&onceToken, ^{
															skQueue = dispatch_queue_create(
																"im.adium.AILoggerPlugin._cleanDirtyLogs.skQueue", 0);
														});
														...
														dispatch_group_async(logIndexingGroup, skQueue, ^{
															...
															OSAtomicIncrement64Barrier(
																(int64_t *)&(bself->logsIndexed));
															OSAtomicDecrement64Barrier((int64_t *)&_remainingLogs);

															if (lastUpdate == 0 ||
																TickCount() > lastUpdate + LOG_INDEX_STATUS_INTERVAL ||
																_remainingLogs == 0) {
																...
																OSAtomicCompareAndSwap32Barrier(lastUpdate, tick,
																								(int32_t *)&lastUpdate);
															}
															...
															if (unsavedChanges > LOG_CLEAN_SAVE_INTERVAL) {
																...
																OSAtomicCompareAndSwap32Barrier(
																	unsavedChanges, 0, (int32_t *)&unsavedChanges);
															}
...
- (void)_didCleanDirtyLogs
{
	...
	__block __typeof__(self) bself = self;
	dispatch_sync(dirtyLogSetMutationQueue, ^{
		OSAtomicCompareAndSwap64Barrier(bself->logsToIndex, [bself->dirtyLogSet count],
										(int64_t *)&(bself->logsToIndex));
	});
