...
- (IBAction)promptForMultiples:(id)sender
{
	...
	NSAlert *multipleImportPrompt =
		[NSAlert alertWithMessageText:AILocalizedString(@"Have you used other chat clients?",
														"Title which introduces import assistants during setup")
						defaultButton:AILocalizedStringFromTable(@"Continue", @"Buttons", nil)
					  alternateButton:AILocalizedString(@"Import from iChat",
														"iChat is the OS X instant messaging client which ships with "
														"OS X; the name probably should not be localized")
						  otherButton:nil
			informativeTextWithFormat:
				AILocalizedString(
					@"Adium includes assistants to import your accounts, settings, and transcripts from other clients. "
					@"Choose a client below to open its assistant, or press Continue to skip importing.",
					nil)];
...
- (BOOL)didCompleteTabViewItemWithIdentifier:(NSString *)identifier
{
	...
	if ([identifier isEqualToString:ACCOUNT_SETUP_IDENTIFIER]) {
		...
		if (UID && [UID length]) {
			...
			AIAccount *account = [adium.accountController createAccountWithService:service UID:UID];
			...
			// Successful without having a UID entered if they already added at least one account; unsuccessful
			// otherwise.
		...
		setFormatter:[AIStringFormatter
						 stringFormatterAllowingCharacters:[service allowedCharactersForAccountName]
											 ...
											 caseSensitive:[service caseSensitive]
											  errorMessage:AILocalizedString(
															   @"The characters you're entering are not valid for an "
															   @"account name on this service.",
															   nil)]];
...
- (void)tabView:(NSTabView *)inTabView willSelectTabViewItem:(NSTabViewItem *)tabViewItem
{
	...
	if ([identifier isEqualToString:ACCOUNT_SETUP_IDENTIFIER]) {
		...
		if (!setupAccountTabViewItem) {
			...
			NSAttributedString *accountMessage = [AIHTMLDecoder
						   decodeHTML:
							   AILocalizedString(
		...
		[button_alternate
			setLocalizedString:AILocalizedString(@"Add Another",
