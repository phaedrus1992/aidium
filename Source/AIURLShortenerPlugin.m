...
{
	NSDictionary *shorteners =
		[NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithInteger:AITinyURL], @"tinyurl.com",
												   [NSNumber numberWithInteger:AIisgd], @"is.gd",
												   [NSNumber numberWithInteger:AIMetamark], @"xrl.us", nil];
...
/*!
 * @brief Our menu item is valid if we have a text view to replace in, the text view has some selected text in it, and
 * the selected text is a valid URL.
 */
	...
	if (textView) {
		...
		if ((text.length > 0) && (selectedRange.location < text.length)) {
			...
			if (selectedRange.length > 0) {
				...
				return [AHHyperlinkScanner isStringValidURI:[text.string substringWithRange:selectedRange]
												usingStrict:NO
												  fromIndex:NULL
												 withStatus:NULL
											   schemeLength:NULL];
...
- (void)shortenLink
{
	...
	if (linkURL.length) {
		...
		[self shortenAddress:linkURL withService:shortener inTextView:textView];
	} else {
		NSBeep();
	}
}
...
- (void)shortenAddress:(NSString *)address withService:(AIShortenLinkService)service inTextView:(NSTextView *)textView
{
	...
	switch (service) {
	case AITinyURL:
		request = [NSString stringWithFormat:@"http://tinyurl.com/api-create.php?url=%@",
											 [address stringByAddingPercentEscapesForAllCharacters]];
		break;

	case AIisgd:
		request = [NSString stringWithFormat:@"http://is.gd/api.php?longurl=%@",
											 [address stringByAddingPercentEscapesForAllCharacters]];
		break;

	case AIMetamark:
		request = [NSString stringWithFormat:@"http://metamark.net/api/rest/simple?long_url=%@",
											 [address stringByAddingPercentEscapesForAllCharacters]];
		break;
...
- (void)insertResultFromURL:(NSURL *)inURL intoTextView:(NSTextView *)textView
{
	...
	if (shortenedURL) {
		...
		NSMutableDictionary *attrs = [NSMutableDictionary
			dictionaryWithDictionary:[textView.attributedString attributesAtIndex:selectedRange.location
																   effectiveRange:nil]];
