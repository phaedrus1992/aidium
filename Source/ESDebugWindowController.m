...
- (void)performFilter
{
	...
	for (aDebugString in fullDebugLogArray) {
		if (!filter || [aDebugString rangeOfString:filter options:NSCaseInsensitiveSearch].location != NSNotFound) {
...
{
	if (sharedDebugWindowInstance)
		[sharedDebugWindowInstance addedDebugMessage:aDebugString];
}
...
- (void)windowDidLoad
{
	...
	[scrollView_debug performSelector:@selector(scrollToBottom) withObject:nil afterDelay:0];
...
- (void)windowWillClose:(id)sender
{
	...
	// Close down
	[mutableDebugString release];
	mutableDebugString = nil;
	[fullDebugLogArray release];
	fullDebugLogArray = nil;
	[debugParagraphStyle release];
