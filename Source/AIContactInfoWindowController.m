...
+ (void)closeInfoWindow
{
	if (sharedContactInfoInstance) {
		...
		[sharedContactInfoInstance release];
...
- (void)windowWillClose:(NSNotification *)inNotification
{
	...
	[sharedContactInfoInstance autorelease];
	sharedContactInfoInstance = nil;
...
- (void)setupToolbarSegments
{
	...
	for (i = 0; i < [inspectorToolbar numberOfColumns]; i++) {
		...
		case CONTACT_INFO_SEGMENT:
			segmentLabel =
				AILocalizedString(@"Status and Profile",
		...
		case CONTACT_ADDRESSBOOK_SEGMENT:
			segmentLabel =
				AILocalizedString(@"Contact Information",
		...
		case CONTACT_EVENTS_SEGMENT:
			segmentLabel = AILocalizedString(
				@"Events", "This segment displays controls for a user to set up events for this contact.");
			segmentImage = [NSImage imageNamed:EVENTS_SEGMENT_IMAGE];
			break;
		case CONTACT_ADVANCED_SEGMENT:
			segmentLabel =
				AILocalizedString(@"Advanced Settings", "This segment displays the advanced settings for a contact, "
														"including encryption details and account information.");
...
- (void)setDisplayedListObject:(AIListObject *)inObject
{
	...
	if (inObject != displayedObject) {
		...
		;
		if (displayedObject)
			[notificationUserInfo setObject:displayedObject forKey:KEY_PREVIOUS_INSPECTED_OBJECT];
		if (inObject)
			[notificationUserInfo setObject:inObject forKey:KEY_NEW_INSPECTED_OBJECT];
		[displayedObject release];
...
- (void)configureForDisplayedObject
{
	...
	if (displayedObject) {
		[[self window]
			setTitle:[NSString stringWithFormat:AILocalizedString(@"%@'s Info", nil), displayedObject.displayName]];
	} else {
...
- (void)animateViewIn:(NSView *)aView;
{
	...
	NSViewAnimation *viewAnim =
		[[NSViewAnimation alloc] initWithViewAnimations:[NSArray arrayWithObjects:animationDict, nil]];
