...
- (id)initWithName:(NSString *)inName notifyingTarget:(id)inTarget
{
	if ((self = [super initWithWindowNibName:@"ListLayoutSheet"])) {
		NSParameterAssert(
			inTarget && [inTarget respondsToSelector:@selector(listLayoutEditorWillCloseWithChanges:forLayoutNamed:)]);
...
- (void)configureControls
{
	...
	[popUp_extendedStatusPosition
		selectItemWithTag:[[prefDict objectForKey:KEY_LIST_LAYOUT_EXTENDED_STATUS_POSITION] integerValue]];

	// Window style
	[popUp_extendedStatusStyle setMenu:[self extendedStatusStyleMenu]];
	[popUp_extendedStatusStyle
		selectItemWithTag:[[prefDict objectForKey:KEY_LIST_LAYOUT_EXTENDED_STATUS_STYLE] integerValue]];
	...
	[slider_contactLeftIndent
		setIntegerValue:[[prefDict objectForKey:KEY_LIST_LAYOUT_CONTACT_LEFT_INDENT] integerValue]];
	[slider_contactRightIndent
		setIntegerValue:[[prefDict objectForKey:KEY_LIST_LAYOUT_CONTACT_RIGHT_INDENT] integerValue]];
...
- (void)updateSliderValues
{
	...
	[textField_contactSpacing
		setStringValue:[NSString stringWithFormat:@"%ldpx", [slider_contactSpacing integerValue]]];
	[textField_groupTopSpacing
		setStringValue:[NSString stringWithFormat:@"%ldpx", [slider_groupTopSpacing integerValue]]];
	[textField_contactLeftIndent
		setStringValue:[NSString stringWithFormat:@"%ldpx", [slider_contactLeftIndent integerValue]]];
	[textField_contactRightIndent
		setStringValue:[NSString stringWithFormat:@"%ldpx", [slider_contactRightIndent integerValue]]];
	[textField_outlineWidthIndicator
		setStringValue:[NSString stringWithFormat:@"%ldpx", [slider_outlineWidth integerValue]]];
}
...
{
	NSDictionary *prefDict = [adium.preferenceController preferencesForGroup:PREF_GROUP_LIST_LAYOUT];
	NSInteger windowStyle = [[adium.preferenceController preferenceForKey:KEY_LIST_LAYOUT_WINDOW_STYLE
	...
	[popUp_statusIconPosition
		setEnabled:([checkBox_statusIconsVisible state] && [checkBox_statusIconsVisible isEnabled] &&
					([popUp_statusIconPosition numberOfItems] > 0))];
	[popUp_serviceIconPosition
		setEnabled:([checkBox_serviceIconsVisible state] && [checkBox_serviceIconsVisible isEnabled] &&
...
- (void)updateStatusAndServiceIconMenusFromPrefDict:(NSDictionary *)prefDict
{
	...
	if ([[adium.preferenceController preferenceForKey:KEY_LIST_LAYOUT_WINDOW_STYLE group:PREF_GROUP_APPEARANCE]
			integerValue] != AIContactListWindowStyleContactBubbles_Fitted) {
	...
	/* If we can't select an item in the status icon or service icon menu, that means our previous selection is no
	 * longer available; set the preference to whichever option we just 'selected' by default.
	 */
	...
	if ([popUp_statusIconPosition numberOfItems] &&
		![popUp_statusIconPosition
			selectItemWithTag:[[prefDict objectForKey:KEY_LIST_LAYOUT_STATUS_ICON_POSITION] integerValue]]) {
		[popUp_statusIconPosition selectItemAtIndex:0];
		[adium.preferenceController
			setPreference:[NSNumber numberWithInteger:[[popUp_statusIconPosition selectedItem] tag]]
				   forKey:KEY_LIST_LAYOUT_STATUS_ICON_POSITION
					group:PREF_GROUP_LIST_LAYOUT];
	}
	...
	if ([popUp_serviceIconPosition numberOfItems] &&
		![popUp_serviceIconPosition
			selectItemWithTag:[[prefDict objectForKey:KEY_LIST_LAYOUT_SERVICE_ICON_POSITION] integerValue]]) {
		[popUp_serviceIconPosition selectItemAtIndex:0];
		[adium.preferenceController
			setPreference:[NSNumber numberWithInteger:[[popUp_serviceIconPosition selectedItem] tag]]
				   forKey:KEY_LIST_LAYOUT_SERVICE_ICON_POSITION
					group:PREF_GROUP_LIST_LAYOUT];
	}
}
...
- (void)updateUserIconMenuFromPrefDict:(NSDictionary *)prefDict
{
	...
	[popUp_userIconPosition
		selectItemWithTag:[[prefDict objectForKey:KEY_LIST_LAYOUT_USER_ICON_POSITION] integerValue]];
}
...
- (NSMenu *)alignmentMenuWithChoices:(NSInteger[])alignmentChoices
{
	...
	while (alignmentChoices[i] != -1) {
		...
		case NSLeftTextAlignment:
			...
			break;
		case NSCenterTextAlignment:
			menuTitle = AILocalizedString(@"Center", nil);
			break;
		case NSRightTextAlignment:
...
- (NSMenu *)positionMenuWithChoices:(NSInteger[])positionChoices
{
	...
	while (positionChoices[i] != -1) {
		...
		case LIST_POSITION_FAR_LEFT:
			menuTitle = AILocalizedString(@"Far Left", nil);
			break;
		case LIST_POSITION_FAR_RIGHT:
			menuTitle = AILocalizedString(@"Far Right", nil);
			break;
		case LIST_POSITION_BADGE_LEFT:
			menuTitle = AILocalizedString(@"Badge (Lower Left)", nil);
			break;
		case LIST_POSITION_BADGE_RIGHT:
			menuTitle = AILocalizedString(@"Badge (Lower Right)", nil);
			break;
		}
		menuItem = [[[NSMenuItem allocWithZone:[NSMenu menuZone]] initWithTitle:menuTitle
																		 target:nil
...
- (NSMenu *)extendedStatusPositionMenu
{
	...
	menuItem = [[[NSMenuItem allocWithZone:[NSMenu menuZone]]
		initWithTitle:AILocalizedString(@"Idle Beside, Status Below", nil)
			   target:nil
			   action:nil
		keyEquivalent:@""] autorelease];
