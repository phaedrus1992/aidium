						 ...
						 firstTime:(BOOL)firstTime
{
	...
	EXTENDED_STATUS_STYLE statusStyle =
		[(NSNumber *)[prefDict objectForKey:KEY_LIST_LAYOUT_EXTENDED_STATUS_STYLE] intValue];
	...
	EXTENDED_STATUS_POSITION statusPosition =
		[(NSNumber *)[prefDict objectForKey:KEY_LIST_LAYOUT_EXTENDED_STATUS_POSITION] intValue];
...
- (NSSet *)updateListObject:(AIListObject *)inObject keys:(NSSet *)inModifiedKeys silent:(BOOL)silent
{
	...
	/* Work at the parent contact (metacontact, etc.) level for extended status, since that's what's displayed in the
	 * contact list. We completely ignore status updates sent for an object which isn't the highest-level up (e.g. is
	 * within a metacontact).
	 */
	if ((inModifiedKeys == nil || (showIdle && [inModifiedKeys containsObject:@"idle"]) ||
		...
		if (showStatus) {
			...
			filteredMessage =
				[adium.contentController filterAttributedString:[(AIListContact *)inObject contactListStatusMessage]
												usingFilterType:AIFilterContactList
													  direction:AIFilterIncoming
														context:inObject];
			statusMessage = [[[[filteredMessage string]
				stringByTrimmingCharactersInSet:whitespaceAndNewlineCharacterSet] mutableCopy] autorelease];
