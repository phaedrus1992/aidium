...
- (void)installPlugin
{
	...
	if (LSGetApplicationForURL((CFURLRef)[NSURL URLWithString:@"http://google.com"], kLSRolesViewer, NULL /*outAppRef*/,
		...
		NSEnumerator *enumerator = [[NSArray arrayWithObjects:@"Safari", @"Firefox", @"OmniWeb", @"Camino", @"Shiira",
															  @"NetNewsWire", @"Google Chrome", nil] objectEnumerator];
		NSString *aSupportedBrowser;

		while ((aSupportedBrowser = [enumerator nextObject])) {
			if ([defaultBrowserName rangeOfString:aSupportedBrowser options:(NSCaseInsensitiveSearch | NSLiteralSearch)]
					.location != NSNotFound) {
	...
	toolbarItem = [AIToolbarUtilities
		toolbarItemWithIdentifier:SAFARI_LINK_IDENTIFER
					 ...
					 paletteLabel:[NSString stringWithFormat:AILocalizedString(@"Insert %@ Link", nil), browserName]
						  toolTip:[NSString
									  stringWithFormat:AILocalizedString(@"Insert link to active page in %@", nil),
													   browserName]
						   target:self
...
- (IBAction)insertSafariLink:(id)sender
{
	...
	if (earliestTextView) {
		NSArray *arguments =
			[NSArray arrayWithObject:AILocalizedString(@"Multiple browsers are open. Please select one link:",
													   "Prompt when more than one web browser is available when "
													   "inserting a link from the active browser.")];
...
- (void)applescriptDidRun:(id)userInfo resultString:(NSString *)resultString
{
	...
	if (resultString && [resultString length]) {
		...
		if (attributes)
			[earliestTextView setTypingAttributes:attributes];
		[attributes release];
