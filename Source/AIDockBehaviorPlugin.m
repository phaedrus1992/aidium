...
#define AIDockBehavior_ALERT_LONG                                                                                      \
	AILocalizedString(@"Bounce the dock icon %@", "%@ will be repalced with a string like 'one time' or "              \
												  "'repeatedly'.")
...
- (NSString *)longDescriptionForActionID:(NSString *)actionID withDetails:(NSDictionary *)details
{
	...
	return [NSString stringWithFormat:AIDockBehavior_ALERT_LONG,
									  [[adium.dockController descriptionForBehavior:behavior] lowercaseString]];
}
 ...
 *
 * We remove all observers because no matter how many chats we were watching, we will stop bouncing; subsequently,
 * stopping a bounce would be inappropriate as it would not be associated with the chat or event which triggered a later
 * bounce.
 */
- (void)stopBouncing:(NSNotification *)inNotification
{
	[[NSNotificationCenter defaultCenter] removeObserver:self name:Chat_WillClose object:nil];
	[[NSNotificationCenter defaultCenter] removeObserver:self name:Chat_BecameActive object:nil];
