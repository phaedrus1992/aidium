...
- (void)configureForActionDetails:(NSDictionary *)inDetails listObject:(AIListObject *)inObject
{
	...
	if (!inDetails)
		inDetails = [adium.preferenceController preferenceForKey:[self defaultDetailsKey] group:PREF_GROUP_ANNOUNCER];
...
- (NSDictionary *)actionDetails
{
	...
	[adium.preferenceController setPreference:actionDetails forKey:[self defaultDetailsKey] group:PREF_GROUP_ANNOUNCER];
...
- (NSDictionary *)actionDetailsFromDict:(NSMutableDictionary *)actionDetails
{
	...
	if (!actionDetails)
		actionDetails = [NSMutableDictionary dictionary];
	...
	if (voice) {
		[actionDetails setObject:voice forKey:KEY_VOICE_STRING];
	}

	if ([pitch floatValue] != [adium.soundController defaultPitch]) {
		[actionDetails setObject:pitch forKey:KEY_PITCH];
	}

	if ([rate floatValue] != [adium.soundController defaultRate]) {
		[actionDetails setObject:rate forKey:KEY_RATE];
	}
	[actionDetails setObject:[NSNumber numberWithBool:[checkBox_customRate state]] forKey:KEY_RATE_CUSTOM];
	[actionDetails setObject:[NSNumber numberWithBool:[checkBox_customPitch state]] forKey:KEY_PITCH_CUSTOM];

	[actionDetails setObject:speakTime forKey:KEY_ANNOUNCER_TIME];
	[actionDetails setObject:speakContactName forKey:KEY_ANNOUNCER_SENDER];
...
- (NSMenu *)voicesMenu
{
	...
	NSMenuItem *menuItem =
		[[[NSMenuItem allocWithZone:[NSMenu menuZone]] initWithTitle:AILocalizedString(@"Use System Default", nil)
															  target:nil
															  action:nil
