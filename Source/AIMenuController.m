...
- (void)awakeFromNib
{
	...
	locationArray = [[NSMutableArray alloc]
		initWithObjects:menu_Adium_About, menu_Adium_Preferences, menu_Adium_Other, menu_File_New, menu_File_Close,
						menu_File_Save, menu_File_Accounts, menu_File_Additions, menu_Edit_Bottom, menu_Edit_Links,
						menu_Edit_Additions, menu_View_General, menu_View_Sorting, menu_View_Toggles,
						menu_View_Counting_Toggles, menu_View_Appearance_Toggles, menu_View_Additions,
						menu_Display_General, menu_Display_Jump, menu_Display_MessageControl, menu_Contact_Manage,
						menu_Contact_Info, menu_Contact_Action, menu_Contact_NegativeAction, menu_Contact_Additions,
						menu_Status_State, menu_Status_SocialNetworking, menu_Status_Accounts, menu_Status_Additions,
						menu_Format_Styles, menu_Format_Palettes, menu_Format_Additions, menu_Window_Top,
						menu_Window_Commands, menu_Window_Auxiliary, menu_Window_Fixed, menu_Help_Local, menu_Help_Web,
						menu_Help_Additions, menu_Dock_Status, nil];
}
...
- (void)removeMenuItem:(NSMenuItem *)targetItem
{
	...
	if (!menuItemProcessingDelays) {
		// Remove any double dividers by removing the upper divier. Also, remove dividers at the top or bottom of the
		// menu
		for (loop = 0; loop < [targetMenu numberOfItems]; loop++) {
			if (([[targetMenu itemAtIndex:loop] isSeparatorItem]) &&
				((loop == [targetMenu numberOfItems] - 1) || (loop == 0) ||
				 ([[targetMenu itemAtIndex:loop - 1] isSeparatorItem]))) {
		 ...
		 * It's not currently needed in that situation so this is a very small performance hack... it could move outside
		 * the conditional if necessary. -evands
		 */
 ...
 * @param location The AIContextMenuLocation associated with this menu item.  Ordering within a given location is not
 * defined.
 */
 ...
 * with these locations' items in order, separated by NSMenuItemSeparators.
...
- (NSMenu *)contextualMenuWithLocations:(NSArray *)inLocationArray forListObject:(AIListObject *)inObject
{
	...
	if ([inObject isKindOfClass:[AIMetaContact class]]) {
		for (AIListContact *aListContact in ((AIMetaContact *)inObject).uniqueContainedObjects) {
			[self addMenuItemsForContact:aListContact toMenu:workingMenu separatorItem:&separatorItem];
		}

	} else if ([inObject isKindOfClass:[AIListContact class]] && ![inObject isKindOfClass:[AIListBookmark class]]) {
		[self addMenuItemsForContact:(AIListContact *)inObject toMenu:workingMenu separatorItem:&separatorItem];
	}
...
- (NSMenu *)contextualMenuWithLocations:(NSArray *)inLocationArray forChat:(AIChat *)inChat
{
	...
	[self addMenuItemsForChat:inChat toMenu:workingMenu separatorItem:&separatorItem];
 ...
 * @param seperatorItem Pointer to a BOOL which can be YES to indicate that a separator item should be inserted before
 * the menu items if any are added. It will then be set to NO.
 */
- (void)addMenuItemsForContact:(AIListContact *)inContact
						toMenu:(NSMenu *)workingMenu
				 separatorItem:(BOOL *)separatorItem
 ...
 *
 * This is useful since the menu is generated by many disparate classes; a single class as delegate can't handle all
 * items.
 */
