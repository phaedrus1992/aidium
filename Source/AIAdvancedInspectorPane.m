...
- (BOOL)accountMenu:(AIAccountMenu *)inAccountMenu shouldIncludeAccount:(AIAccount *)inAccount
{
	...
	if ([displayedObject isKindOfClass:[AIMetaContact class]]) {
		NSArray *services =
			[((AIMetaContact *)displayedObject).uniqueContainedObjects valueForKeyPath:@"service.serviceClass"];
...
- (void)accountMenu:(AIAccountMenu *)inAccountMenu didRebuildMenuItems:(NSArray *)menuItems
{
	...
	[self accountMenu:inAccountMenu
		didSelectAccount:([popUp_accounts numberOfItems] ? [[popUp_accounts selectedItem] representedObject] : nil)];
}
...
- (void)contactMenuDidRebuild:(AIContactMenu *)inContactMenu
{
	...
	[self contactMenu:inContactMenu
		didSelectContact:([popUp_contact numberOfItems] ? [[popUp_contact selectedItem] representedObject] : nil)];
}
...
- (void)removeGroup
{
	for (AIListGroup *group in
		 [currentSelectedContact.remoteGroups.allObjects objectsAtIndexes:tableView_groups.selectedRowIndexes]) {
