...
- (id)init
{
	if ((self = [super init])) {
		...
		[NSTimer scheduledTimerWithTimeInterval:2.0
										 target:self
									   selector:@selector(reportResponderChain:)
									   userInfo:nil
										repeats:YES];
#endif
	}
...
- (void)controllerDidLoad
{
	...
	menuItem_toggleUserlist =
		[[NSMenuItem allocWithZone:[NSMenu menuZone]] initWithTitle:AILocalizedString(@"Toggle User List", nil)
															 target:self
	...
	menuItem_toggleUserlistSide =
		[[NSMenuItem allocWithZone:[NSMenu menuZone]] initWithTitle:AILocalizedString(@"Toggle User List Side", nil)
															 target:self
	...
	// Clear display
	menuItem_clearDisplay =
		[[NSMenuItem allocWithZone:[NSMenu menuZone]] initWithTitle:AILocalizedString(@"Clear Display", nil)
															 target:self
	...
	menuItem = [[NSMenuItem allocWithZone:[NSMenu menuZone]]
		initWithTitle:AILocalizedString(@"Contact List", "Name of the window which lists contacts")
			   target:self
	...
	menuItem = [[NSMenuItem allocWithZone:[NSMenu menuZone]]
		initWithTitle:AILocalizedString(@"Close Chat", "Title for the close chat menu item")
			   target:self
	...
	menuItem = [[NSMenuItem alloc]
		initWithTitle:AILocalizedStringFromTableInBundle(
						  @"Authorization Requests", nil,
						  [NSBundle bundleForClass:[AIAuthorizationRequestsWindowController class]], nil)
			   target:self
	...
	if (!object) {
		...
		if (firstTime) {
			if (saveContainers) {
				 ...
				 */
				[adium.preferenceController setPreference:nil forKey:KEY_CONTAINERS group:PREF_GROUP_INTERFACE];
			}
		}
	}
}
	...
	if (![self contactListIsVisibleAndMain] && [[interfacePlugin openContainerIDs] count] == 0) {
		...
		if ((mostRecentUnviewedChat = [adium.chatController mostRecentUnviewedChat])) {
			...
			for (NSWindow *window in [NSApp windows]) {
				// Check stylemask to rule out the system menu's window (Which reports itself as visible like a real
				// window)
...
- (void)restoreSavedContainers
{
	...
	for (NSDictionary *dict in [NSKeyedUnarchiver unarchiveObjectWithData:savedData]) {
		...
		[[windowController window] setFrame:[windowController savedFrameFromString:[dict objectForKey:@"Frame"]]
									display:YES];

		for (NSDictionary *chatDict in [dict objectForKey:@"Content"]) {
			AIChat *chat = nil;
			AIService *service =
				[adium.accountController firstServiceWithServiceID:[chatDict objectForKey:@"serviceID"]];
			AIAccount *account =
				[adium.accountController accountWithInternalObjectID:[chatDict objectForKey:@"AccountID"]];
			...
			[chat setValue:[NSNumber numberWithBool:YES] forProperty:@"Restored Chat" notify:NotifyNow];
...
- (void)saveContainers
{
	...
	for (NSDictionary *dict in [interfacePlugin openContainersAndChats]) {
				...
				[newContainerDict
					addEntriesFromDictionary:[NSDictionary
												 dictionaryWithObjectsAndKeys:
													 ...
													 [NSNumber
														 numberWithBool:([dict objectForKey:@"ActiveChat"] == chat)],
													 @"ActiveChat", chat.name, @"Name", [chat chatCreationDictionary],
				...
				[newContainerDict
					addEntriesFromDictionary:[NSDictionary
												 dictionaryWithObjectsAndKeys:
													 [NSNumber
														 numberWithBool:([dict objectForKey:@"ActiveChat"] == chat)],
													 @"ActiveChat", chat.listObject.UID, @"UID",
		...
		[saveDict setObject:containerContents forKey:@"Content"];
...
// Messaging
// ------------------------------------------------------------------------------------------------------------ Methods
// for instructing the interface to provide a representation of chats, and to determine which chat has user focus
...
- (void)closeChat:(AIChat *)inChat
{
	if (inChat) {
		if ([adium.chatController closeChat:inChat]) {
			...
			if (inChat.isGroupChat) {
				// -chatCreationDictionary may be nil, so put it last.
				[newRecentlyClosedChat
					addEntriesFromDictionary:[NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithBool:YES],
																						@"IsGroupChat", inChat.name,
																						@"Name",
																						[inChat chatCreationDictionary],
																						@"ChatCreationInfo", nil]];
			} else {
				[newRecentlyClosedChat
					addEntriesFromDictionary:[NSDictionary
												 dictionaryWithObjectsAndKeys:inChat.listObject.UID, @"UID",
																			  inChat.account.service.serviceID,
																			  @"serviceID",
																			  inChat.account.internalObjectID,
																			  @"AccountID", nil]];
			}
...
- (void)consolidateChats
{
	...
	while ((containerID = [containerEnumerator nextObject])) {
		...
		while ((chat = [chatEnumerator nextObject])) {
			[interfacePlugin moveChat:chat toContainerWithID:firstContainerID index:-1];
		}
...
- (IBAction)reopenChat:(id)sender
{
	...
	if (!chat) {
		NSRunAlertPanel(
			AILocalizedString(@"Restoring chat failed", nil),
...
// by the interface to let us know what's going on.  We're informed of chats opening, closing, changing order, etc.
...
- (void)chatDidBecomeActive:(AIChat *)inChat
{
	...
	if (inChat && (inChat != mostRecentActiveChat)) {
		[mostRecentActiveChat release];
		mostRecentActiveChat = nil;
		mostRecentActiveChat = [inChat retain];
	}

	[[NSNotificationCenter defaultCenter]
		postNotificationName:Chat_BecameActive
					  object:inChat
					userInfo:(previouslyActiveChat ? [NSDictionary dictionaryWithObject:previouslyActiveChat
																				 forKey:@"PreviouslyActiveChat"]
												   : nil)];

	if (inChat) {
		/* Clear the unviewed content on the next event loop so other methods have a chance to react to the chat
		 * becoming active. Specifically, this lets the handleReopenWithVisibleWindows: method have a chance to know
		 * that this chat had unviewed content.
		 */
		[inChat performSelector:@selector(clearUnviewedContentCount) withObject:nil afterDelay:0];
	}
...
- (void)updateActiveWindowMenuItem
{
	...
	for (item in windowMenuArray) {
		if ([item representedObject])
			[item setState:([item representedObject] == activeChat ? NSOnState : NSOffState)];
	}
}
	...
	for (NSDictionary *containerDict in [interfacePlugin openContainersAndChats]) {
		...
		if (contentArray.count > 1) {
			item = [[NSMenuItem allocWithZone:[NSMenu menuZone]]
				initWithTitle:([containerName length] ? containerName : AILocalizedString(@"Chats", nil))
					   target:nil
					   action:nil
				keyEquivalent:@""];
...
- (AIListObject *)selectedListObjectInContactList
{
	return [self _performSelectorOnFirstAvailableResponder:@selector(listObject)
									  conformingToProtocol:@protocol(ContactListOutlineView)];
}
- (NSArray *)arrayOfSelectedListObjectsInContactList
{
	return [self _performSelectorOnFirstAvailableResponder:@selector(arrayOfListObjects)
									  conformingToProtocol:@protocol(ContactListOutlineView)];
}
- (NSArray *)arrayOfSelectedListObjectsWithGroupsInContactList
{
	return [self _performSelectorOnFirstAvailableResponder:@selector(arrayOfListObjectsWithGroups)
									  conformingToProtocol:@protocol(ContactListOutlineView)];
}

// Message View
// --------------------------------------------------------------------------------------------------------- Message
// view is abstracted from the containing interface, since they're not directly related to eachother
...
- (void)handleMessage:(NSString *)inTitle withDescription:(NSString *)inDesc withWindowTitle:(NSString *)inWindowTitle;
{
	...
	errorDict = [NSDictionary
		dictionaryWithObjectsAndKeys:inTitle, @"Title", inDesc, @"Description", inWindowTitle, @"Window Title", nil];
...
- (void)displayQuestion:(NSString *)inTitle
	withAttributedDescription:(NSAttributedString *)inDesc
			  withWindowTitle:(NSString *)inWindowTitle
					 ...
					 userInfo:(id)inUserInfo
{
	...
	[[NSNotificationCenter defaultCenter] postNotificationName:Interface_ShouldDisplayQuestion
														object:nil
													  userInfo:questionDict];
}

- (void)displayQuestion:(NSString *)inTitle
		withDescription:(NSString *)inDesc
		withWindowTitle:(NSString *)inWindowTitle
		  defaultButton:(NSString *)inDefaultButton
		alternateButton:(NSString *)inAlternateButton
			otherButton:(NSString *)inOtherButton
			   ...
			   userInfo:(id)inUserInfo
{
	[self displayQuestion:inTitle
		withAttributedDescription:[[[NSAttributedString alloc]
									  initWithString:inDesc
...
- (void)showTooltipForListObject:(AIListObject *)object atScreenPoint:(NSPoint)point onWindow:(NSWindow *)inWindow
{
	if (object) {
		if (object == tooltipListObject) { // If we already have this tooltip open
			...
			if (!tooltipImage)
				tooltipImage = [[AIServiceIcons serviceIconForObject:tooltipListObject
			...
			if (tooltipImage) {
				...
				tabArray = [[NSArray alloc]
					initWithObjects:[[[NSTextTab alloc] initWithType:NSRightTabStopType
			...
			[tooltipBody addAttribute:NSParagraphStyleAttributeName
								value:paragraphStyle
								range:NSMakeRange(0, [tooltipBody length])];
...
- (NSMutableAttributedString *)_tooltipTitleForObject:(AIListObject *)object
{
	...
	NSMutableDictionary *titleDict = [NSMutableDictionary
		dictionaryWithObject:[fontManager convertFont:[NSFont toolTipsFontOfSize:12] toHaveTrait:NSBoldFontMask]
					  forKey:NSFontAttributeName];
	NSMutableDictionary *labelDict = [NSMutableDictionary
		dictionaryWithObject:[fontManager convertFont:[NSFont toolTipsFontOfSize:9] toHaveTrait:NSBoldFontMask]
					  forKey:NSFontAttributeName];
	NSMutableDictionary *labelEndLineDict = [NSMutableDictionary dictionaryWithObject:[NSFont toolTipsFontOfSize:2]
																			   forKey:NSFontAttributeName];
	NSMutableDictionary *entryDict = [NSMutableDictionary dictionaryWithObject:toolTipsFont forKey:NSFontAttributeName];
	...
	if ([object isKindOfClass:[AIListGroup class]]) {
		[titleString appendString:[NSString stringWithFormat:@" (%ld/%ld)", [(AIListGroup *)object visibleCount],
															 [(AIListGroup *)object countOfContainedObjects]]
	...
	for (tooltipEntry in contactListTooltipEntryArray) {
		...
		if (entryString && [entryString length]) {
			...
			if (labelString && [labelString length]) {
				...
				NSAttributedString *labelAttribString =
					[[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@:", labelString]
													attributes:labelDict];

				// The largest size should be the label's size plus the distance to the next tab at least a space past
				// its end
	...
	for (entryString in entryArray) {
		NSAttributedString *labelAttribString = [[NSAttributedString alloc]
			initWithString:[NSString stringWithFormat:@"\t%@:\t", [labelEnumerator nextObject]]
				attributes:labelDict];
...
- (NSMutableAttributedString *)_tooltipBodyForObject:(AIListObject *)object
{
	...
	for (NSMutableAttributedString *entryString in entryArray) {
		NSMutableAttributedString *labelString = [[NSMutableAttributedString alloc]
			initWithString:[NSString stringWithFormat:@"\t%@:\t", [labelEnumerator nextObject]]
				attributes:labelDict];
		...
		[entryString adjustColorsToShowOnBackground:[NSColor colorWithCalibratedRed:1.000f
																			  green:1.000f
																			   blue:0.800f
																			  alpha:1.0f]];

		// headIndent doesn't apply to the first line of a paragraph... so when new lines are in the entry, we need to
		// tab over to the proper location
		if ([entryString replaceOccurrencesOfString:@"\r"
										 withString:@"\r\t\t"
...
- (IBAction)adiumPrint:(id)sender
{
	...
	if ([keyWindowController respondsToSelector:@selector(adiumPrint:)]) {
		[keyWindowController performSelector:@selector(adiumPrint:) withObject:sender];
	}
}
		...
		if (!fontPanelAccessoryView) {
			...
			[button_fontPanelSetAsDefault
				setLocalizedString:AILocalizedString(
									   @"Save This Setting As My Default Font",
									   "Appears in the Format > Show Fonts window. You are limited for horizontal "
									   "space, so try to keep it at most the length of the English string.")];
		}
...
- (BOOL)validateMenuItem:(NSMenuItem *)menuItem
{
	...
	if (menuItem == menuItem_bold || menuItem == menuItem_italic) {
		...
		return NO;
			   ...
			   menuItem == menuItem_pasteWithImagesAndColors) {

		// The user can paste if the pasteboard contains an image, some text, one or more files, or one or more URLs.
		NSPasteboard *pboard = [NSPasteboard generalPasteboard];
		NSArray *nonImageTypes =
			[NSArray arrayWithObjects:NSStringPboardType, NSRTFPboardType, NSURLPboardType, NSFilenamesPboardType,
									  NSFilesPromisePboardType, NSRTFDPboardType, nil];
		...
		[menuItem_showToolbar setTitle:([[keyWin toolbar] isVisible] ? AILocalizedString(@"Hide Toolbar", nil)
																	 : AILocalizedString(@"Show Toolbar", nil))];
		return [keyWin toolbar] != nil;

	} else if (menuItem == menuItem_customizeToolbar) {
		return ([keyWin toolbar] != nil && [[keyWin toolbar] isVisible] &&
				[[keyWin windowController] canCustomizeToolbar]);
...
- (NSMenu *)menuForWindowLevelsNotifyingTarget:(id)target
{
	...
	menuItem =
		[[NSMenuItem allocWithZone:[NSMenu menuZone]] initWithTitle:AILocalizedString(@"Above other windows", nil)
															 target:target
	...
	menuItem =
		[[NSMenuItem allocWithZone:[NSMenu menuZone]] initWithTitle:AILocalizedString(@"Below other windows", nil)
															 target:target
															 action:@selector(selectedWindowLevel:)
													  keyEquivalent:@""];
	[menuItem setEnabled:YES];
