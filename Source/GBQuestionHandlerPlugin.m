						  ...
						  userInfo:(id)userInfo
{
	...
	if (target != nil || selectorString != nil) {
		...
		if ([target respondsToSelector:selector]) {
			[target performSelector:selector
						 ...
						 withObject:[userInfo objectForKey:@"Userinfo"]
						 withObject:[NSNumber numberWithBool:suppression]];
		}
	}
	if ([self displayNextAlert])
		...
		// Note: Explicitly not released here: ESTextAndButtonsWindowController will autorelease itself in
		// -windowWillClose:
		[currentAlert close];
		currentAlert = nil;
	}
