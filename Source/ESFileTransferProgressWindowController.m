...
- (void)_removeFileTransfer:(ESFileTransfer *)inFileTransfer
{
	...
	if ((row = [self existingRowForFileTransfer:inFileTransfer]))
...
- (ESFileTransferProgressRow *)existingRowForFileTransfer:(ESFileTransfer *)inFileTransfer
{
	...
	for (row in progressRows) {
		if ([row fileTransfer] == inFileTransfer)
			break;
	}
...
- (void)updateStatusBar
{
	...
	if (downloads > 0) {
		if (downloads == 1)
			...
			downloadsString =
				[NSString stringWithFormat:AILocalizedString(@"%lu downloads", "(number) downloads"), downloads];
	}
...
- (NSMenu *)outlineView:(NSOutlineView *)inOutlineView menuForEvent:(NSEvent *)inEvent
{
	...
	location = [inOutlineView convertPoint:[inEvent locationInWindow] fromView:nil];
...
- (NSRect)windowWillUseStandardFrame:(NSWindow *)inWindow defaultFrame:(NSRect)defaultFrame
{
	...
	if (windowFrame.size.width < minWinSize.width)
		windowFrame.size.width = minWinSize.width;
	if (windowFrame.size.height < minWinSize.height)
		windowFrame.size.height = minWinSize.height;
	if (windowFrame.size.width > maxWinSize.width)
		windowFrame.size.width = maxWinSize.width;
	if (windowFrame.size.height > maxWinSize.height)
		windowFrame.size.height = maxWinSize.height;
