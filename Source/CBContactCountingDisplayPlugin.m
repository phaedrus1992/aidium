						 ...
						 firstTime:(BOOL)firstTime
{
	...
	if ([group isEqualToString:PREF_GROUP_CONTACT_LIST] &&
		([key isEqualToString:KEY_COUNT_ONLINE_CONTACTS] || [key isEqualToString:KEY_COUNT_ALL_CONTACTS] ||
		 firstTime)) {
...
- (NSSet *)updateListObject:(AIListObject *)inObject keys:(NSSet *)inModifiedKeys silent:(BOOL)silent
{
	...
	if ([inObject isKindOfClass:[AIListContact class]] && [inModifiedKeys containsObject:@"isOnline"]) {
		...
		[groups unionSet:inObject.groups];
			   ...
			   (!inModifiedKeys || ([inModifiedKeys containsObject:@"ObjectCount"] ||
									[inModifiedKeys containsObject:@"VisibleObjectCount"])) &&
			   ![inObject.UID isEqualToString:ADIUM_ROOT_GROUP_NAME]) {

		/* We check against a nil inModifiedKeys so we can remove our Counting information from the display when the
		 * user toggles it off.
		 *
	...
	for (AIListGroup *inGroup in groups) {
		...
		if (countOnlineObjects && countAllObjects && (onlineObjects != totalObjects) &&
			...
			countString =
				[NSString stringWithFormat:AILocalizedString(@"%lu of %lu",
															 "Used in the display for the contact list for the number "
															 "of online contacts out of the number of total contacts"),
		...
		[inGroup setValue:countString forProperty:@"countText" notify:NotifyNever];
