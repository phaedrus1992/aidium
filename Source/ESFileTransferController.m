...
#define SAFE_FILE_EXTENSIONS_SET                                                                                       \
	[NSSet setWithObjects:@"jpg", @"jpeg", @"gif", @"png", @"tif", @"tiff", @"psd", @"pdf", @"txt", @"rtf", @"html",   \
						  @"htm", @"swf", @"mp3", @"wma", @"wmv", @"ogg", @"ogm", @"mov", @"mpg", @"mpeg", @"m1v",     \
						  @"m2v", @"mp4", @"avi", @"vob", @"avi", @"asx", @"asf", @"pls", @"m3u", @"rmp", @"aif",      \
						  @"aiff", @"aifc", @"wav", @"wave", @"m4a", @"m4p", @"m4b", @"dmg", @"udif", @"ndif",         \
						  @"dart", @"sparseimage", @"cdr", @"dvdr", @"iso", @"img", @"toast", @"rar", @"sit", @"sitx", \
						  @"bin", @"hqx", @"zip", @"gz", @"tgz", @"tar", @"bz", @"bz2", @"tbz", @"z", @"taz", @"uu",   \
						  @"uue", @"colloquytranscript", @"torrent", @"AdiumIcon", @"AdiumSoundset", @"AdiumEmoticon", \
...
- (void)controllerDidLoad
{
	...
	menuItem_sendFileContext = [[NSMenuItem alloc] initWithTitle:SEND_FILE_WITH_ELLIPSIS
														  target:self
														  action:@selector(contextualMenuSendFile:)
												   keyEquivalent:@""];
	...
	[contactAlertsController registerEventID:FILE_TRANSFER_REQUEST
								 withHandler:self
									 inGroup:AIFileTransferEventHandlerGroup
								  globalOnly:YES];
	[contactAlertsController registerEventID:FILE_TRANSFER_WAITING_REMOTE
								 withHandler:self
									 inGroup:AIFileTransferEventHandlerGroup
								  globalOnly:YES];
	[contactAlertsController registerEventID:FILE_TRANSFER_BEGAN
								 withHandler:self
									 inGroup:AIFileTransferEventHandlerGroup
								  globalOnly:YES];
	[contactAlertsController registerEventID:FILE_TRANSFER_CANCELLED
								 withHandler:self
									 inGroup:AIFileTransferEventHandlerGroup
								  globalOnly:YES];
	[contactAlertsController registerEventID:FILE_TRANSFER_COMPLETE
								 withHandler:self
									 inGroup:AIFileTransferEventHandlerGroup
								  globalOnly:YES];
	[contactAlertsController registerEventID:FILE_TRANSFER_FAILED
								 withHandler:self
									 inGroup:AIFileTransferEventHandlerGroup
								  globalOnly:NO];
	...
	toolbarItem = [AIToolbarUtilities
		toolbarItemWithIdentifier:SEND_FILE_IDENTIFIER
							label:SEND_FILE
...
- (void)dealloc
{
	...
	[safeFileExtensions release];
	safeFileExtensions = nil;
	[fileTransferArray release];
	fileTransferArray = nil;
}

#pragma mark Access to file transfer objects
- (ESFileTransfer *)newFileTransferWithContact:(AIListContact *)inContact
									forAccount:(AIAccount *)inAccount
										  type:(AIFileTransferType)t
{
	ESFileTransfer *fileTransfer;

	fileTransfer = [ESFileTransfer fileTransferWithContact:inContact forAccount:inAccount type:t];
...
- (NSUInteger)activeTransferCount
{
	...
	for (t in fileTransferArray) {
		...
		if ((status == Unknown_Status_FileTransfer) || (status == Not_Started_FileTransfer) ||
			(status == Checksumming_Filetransfer) || (status == Waiting_on_Remote_User_FileTransfer) ||
			(status == Connecting_FileTransfer) || (status == Accepted_FileTransfer) ||
...
- (void)receiveRequestForFileTransfer:(ESFileTransfer *)fileTransfer
{
	...
	if ((autoAcceptType == AutoAccept_All) ||
		...
		if ([remoteFilename hasPrefix:@"."])
			remoteFilename = [@"_" stringByAppendingString:remoteFilename];
		...
		[ESFileTransferRequestPromptController
			displayPromptForFileTransfer:fileTransfer
 ...
 * @param localFilename Full path at which to save the file.  If anything exists at this path it will be overwritten
 * without further confirmation.  Pass nil to deny the transfer.
 */
- (void)_finishReceiveRequestForFileTransfer:(ESFileTransfer *)fileTransfer localFilename:(NSString *)localFilename
{
...
- (NSOpenPanel *)sendFilePanelForListContact:(AIListContact *)listContact
{
	...
	[openPanel
		setTitle:[NSString stringWithFormat:AILocalizedString(@"Send File to %@", nil), listContact.displayName]];
...
- (NSString *)pathToArchiveOfFolder:(NSString *)inPath
{
	...
	if (tmpDir) {
		NSString *launchPath = [[[@"/" stringByAppendingPathComponent:@"usr"] stringByAppendingPathComponent:@"bin"]
		...
		if ([defaultManager fileExistsAtPath:launchPath]) {
			...
			pathToArchive = [[NSFileManager defaultManager]
				uniquePathForPath:[tmpDir stringByAppendingPathComponent:[folderName
																			 stringByAppendingPathExtension:@"zip"]]];

			arguments = [NSArray arrayWithObjects:@"-r", // we'll want to store recursively
												  @"-1", // use the fastest level of compression that isn't storage; the
														 // user can compress manually to do better
												  ...
												  nil];
			AILog(
				@"-[ESFileTransferController pathToArchiveOfFolder:]: Will launch %@ with arguments %@ in directory %@",
...
- (void)sendFile:(NSString *)inPath toListContact:(AIListContact *)listContact
{
	...
	if ((account = [adium.accountController preferredAccountForSendingContentType:CONTENT_FILE_TRANSFER_TYPE
		...
		if ([defaultManager fileExistsAtPath:inPath isDirectory:&isDir]) {
			...
			if (isDir && ![(AIAccount<AIAccount_Files> *)account canSendFolders]) {
...
- (IBAction)contextualMenuSendFile:(id)sender
{
	...
	AIListContact *listContact =
		[adium.contactController preferredContactForContentType:CONTENT_FILE_TRANSFER_TYPE
												 forListContact:(AIListContact *)selectedObject];
...
- (BOOL)shouldOpenCompleteFileTransfer:(ESFileTransfer *)fileTransfer
{
	...
	if (autoOpenSafe && ([fileTransfer fileTransferType] == Incoming_FileTransfer)) {

		if (!safeFileExtensions)
			safeFileExtensions = [SAFE_FILE_EXTENSIONS_SET retain];
