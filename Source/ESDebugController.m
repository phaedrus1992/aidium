...
- (id)init
{
	if ((self = [super init])) {
		...
		NSUInteger handlingMask = NSLogUncaughtExceptionMask | NSLogUncaughtSystemExceptionMask |
								  NSLogUncaughtRuntimeErrorMask | NSLogTopLevelExceptionMask | NSLogOtherExceptionMask;
...
- (void)start:(NSNotification *)dummy
{
	...
	NSMenuItem *menuItem =
		[[NSMenuItem allocWithZone:[NSMenu menuZone]] initWithTitle:AILocalizedString(@"Debug Window", nil)
															 target:self
	...
	if ([[adium.preferenceController preferenceForKey:KEY_DEBUG_WINDOW_OPEN group:GROUP_DEBUG] boolValue]) {
...
- (void)controllerWillClose
{
	...
	[adium.preferenceController
		setPreference:([ESDebugWindowController debugWindowIsOpen] ? [NSNumber numberWithBool:YES] : nil)
...
- (void)addMessage:(NSString *)actualMessage
{
	...
	if ([debugLogArray count] > CACHED_DEBUG_LOGS)
		[debugLogArray removeObjectAtIndex:0];
