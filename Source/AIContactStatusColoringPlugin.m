...
- (void)installPlugin
{
	...
	interestedKeysSet =
		[[NSSet alloc] initWithObjects:KEY_TYPING, KEY_UNVIEWED_CONTENT, @"listObjectStatusType", @"isIdle",
...
- (void)_applyColorToContact:(AIListContact *)inContact
{
	...
	if ((!color && !labelColor)) {
		if (offlineEnabled && (!inContact.online && ![inContact boolValueForProperty:@"signedOff"])) {
			color = offlineColor;
			invertedColor = offlineInvertedColor;
			labelColor = offlineLabelColor;
			if (offlineImageFading)
				opacity = OFFLINE_IMAGE_OPACITY;
	...
	[inContact setValue:color forProperty:@"textColor" notify:NotifyNever];
	[inContact setValue:invertedColor forProperty:@"invertedTextColor" notify:NotifyNever];
	[inContact setValue:labelColor forProperty:@"labelColor" notify:NotifyNever];
	[inContact setValue:[NSNumber numberWithDouble:opacity] forProperty:@"imageOpacity" notify:NotifyNever];
	[inContact setValue:[NSNumber numberWithBool:isEvent] forProperty:@"isEvent" notify:NotifyNever];

	if (applyToMetaToo && [inContact metaContact]) {
		[[inContact metaContact] setValue:color forProperty:@"textColor" notify:NotifyNever];
		[[inContact metaContact] setValue:invertedColor forProperty:@"invertedTextColor" notify:NotifyNever];
		[[inContact metaContact] setValue:labelColor forProperty:@"labelColor" notify:NotifyNever];
		[[inContact metaContact] setValue:[NSNumber numberWithDouble:opacity]
							  forProperty:@"imageOpacity"
								   notify:NotifyNever];
		[[inContact metaContact] setValue:[NSNumber numberWithBool:isEvent] forProperty:@"isEvent" notify:NotifyNever];
	}
}
...
- (void)flash:(int)value
{
	...
	for (object in flashingListObjects) {
		...
		[[AIContactObserverManager sharedManager] listObjectAttributesChanged:object modifiedKeys:modifiedKeys];
	}
}
