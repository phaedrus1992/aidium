...
- (id)initWithName:(NSString *)inName notifyingTarget:(id)inTarget
{
	if ((self = [super initWithWindowNibName:@"ListThemeSheet"])) {
		NSParameterAssert(inTarget &&
						  [inTarget respondsToSelector:@selector(listThemeEditorWillCloseWithChanges:forThemeNamed:)]);
...
- (void)configureControls
{
	...
	[colorWell_groupBackgroundGradient
		setColor:[[prefDict objectForKey:KEY_LIST_THEME_GROUP_BACKGROUND_GRADIENT] representedColor]];
	...
	[checkBox_useBackgroundImage
		setToolTip:AILocalizedString(@"Background images are only applicable to normal and borderless window styles",
									 nil)];
	...
	[popUp_displayImageStyle
		selectItemWithTag:[[prefDict objectForKey:KEY_LIST_THEME_BACKGROUND_IMAGE_STYLE] integerValue]];
...
- (void)updateSliderValues
{
	[textField_backgroundFade
		setStringValue:[NSString stringWithFormat:@"%ld%%", (NSInteger)([slider_backgroundFade doubleValue] * 100.0)]];
}
