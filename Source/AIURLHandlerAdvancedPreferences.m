...
- (NSArray *)applicationDictionaryArrayForScheme:(NSString *)scheme
{
	...
	if (!applications) {
		...
		for (NSString *bundleID in applicationArray) {
			...
			FSRef fileSystemRef;
			OSStatus err =
				LSFindApplicationForInfo(kLSUnknownCreator, (CFStringRef)bundleID, NULL, &fileSystemRef, NULL);
			...
			// Application Image
			IconRef iconRef;
			err = GetIconRefFromFileInfo(&fileSystemRef, 0, NULL, kFSCatInfoNone, NULL, kIconServicesNoBadgeFlag,
										 &iconRef, NULL);
			if (err) {
				return nil;
			}
