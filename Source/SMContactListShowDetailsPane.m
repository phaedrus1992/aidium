...
- (void)viewDidLoad
{
	...
	[label_secondsToShow setLocalizedString:AILocalizedString(@"Seconds To Show:",
															  "Label before a slider which sets the number of seconds "
															  "to show the contact list when an action is triggerred")];
	[label_note setLocalizedString:AILocalizedString(
									   @"Note: This behavior is only available if the contact list is set to hide.",
...
{
	return [NSDictionary dictionaryWithObject:[NSNumber numberWithDouble:[slider_secondsToShow doubleValue]]
									   forKey:KEY_SECONDS_TO_SHOW_LIST];
}
