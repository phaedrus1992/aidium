...
- (void)requestUserNotifyingTarget:(id)inTarget selector:(SEL)inSelector
{
	...
	if (!userName) {
		...
		if (shouldShowWindow) {
			...
			if (userRequestedShowWindow)
				[loginWindowController disableLoginTimeout];
		}
	}
...
- (NSArray *)userArray
{
	...
	for (NSString *path in [[NSFileManager defaultManager] contentsOfDirectoryAtPath:userPath error:NULL]) {
		...
		if ([[NSFileManager defaultManager] fileExistsAtPath:[userPath stringByAppendingPathComponent:path]
...
- (void)deleteUser:(NSString *)inUserName
{
	...
	sourcePath = [[[adium applicationSupportDirectory] stringByAppendingPathComponent:PATH_USERS]
		stringByAppendingPathComponent:inUserName];
...
- (void)renameUser:(NSString *)oldName to:(NSString *)newName
{
	...
	// Create the source and dest paths
	sourcePath = [[[adium applicationSupportDirectory] stringByAppendingPathComponent:PATH_USERS]
		stringByAppendingPathComponent:oldName];
	destPath = [[[adium applicationSupportDirectory] stringByAppendingPathComponent:PATH_USERS]
