...
- (void)awakeFromNib
{
	[[self window] setTitle:AILocalizedString(@"Connect Guest Account",
...
- (void)windowWillClose:(id)sender
{
	...
	[sharedGuestAccountWindowController autorelease];
...
- (NSString *)UID
{
	...
	if (!UID || ![UID length])
		UID = [self.service defaultUserName];
...
{
	if (!account) {
		account = [[adium.accountController createAccountWithService:self.service UID:self.UID] retain];
	} else {
		if ((self.service != account.service) || (![self.UID isEqualToString:account.UID])) {
			[account release];
...
- (IBAction)selectServiceType:(id)sender
{
	...
	[label_name setStringValue:[[service userNameLabel]
	...
	if (!placeholder || ![placeholder length])
		placeholder = [service UIDPlaceholder];
