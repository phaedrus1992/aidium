 ...
 *
 * Random Things I Remembered note: Unless you have a good reason, you always want to use -[NSBezierPath addClip], not
 * -[NSBezierPath setClip], so you take into account the existing clip rect.
 */
- (void)drawRect:(NSRect)rect
{
	...
	if (backgroundImage && NSIntersectsRect(imageDrawingRect, rect)) {
		...
		if (NSIntersectsRect(transparentRect, rect)) {
			...
			[backgroundImage drawInRect:imageDrawingRect atSize:imageSize position:IMAGE_POSITION_LEFT fraction:.30f];
		...
		[backgroundImage drawInRect:imageDrawingRect atSize:imageSize position:IMAGE_POSITION_LEFT fraction:1.0f];
		[NSGraphicsContext restoreGraphicsState];
	}
}
