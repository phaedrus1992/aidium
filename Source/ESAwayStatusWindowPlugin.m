						 ...
						 firstTime:(BOOL)firstTime
{
	...
	if (showStatusWindow != oldShowStatusWindow) {
		if (showStatusWindow) {
			/* Register as a list object observer, which will update all objects for us immediately leading to the
			 * proper status window toggling. */
	...
	if (showStatusWindow) {
		[ESAwayStatusWindowController
			setAlwaysOnTop:[[prefDict objectForKey:KEY_STATUS_STATUS_WINDOW_ON_TOP] boolValue]];
		[ESAwayStatusWindowController
			setHideInBackground:[[prefDict objectForKey:KEY_STATUS_STATUS_WINDOW_HIDE_IN_BACKGROUND] boolValue]];
	}
}
...
- (NSSet *)updateListObject:(AIListObject *)inObject keys:(NSSet *)inModifiedKeys silent:(BOOL)silent
{
	if ([inObject isKindOfClass:[AIAccount class]] &&
		(!inModifiedKeys || [inModifiedKeys containsObject:@"accountStatus"] ||
		 ...
		 */
		[NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(processStatusUpdate) object:nil];
		[self performSelector:@selector(processStatusUpdate) withObject:nil afterDelay:0];
	}
