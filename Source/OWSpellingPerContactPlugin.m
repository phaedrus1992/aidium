...
- (void)chatBecameActive:(NSNotification *)notification
{
	@try {
		...
		if (previousChat) {
			...
			if (language && ![[languageDict objectForKey:chatID] isEqualToString:language]) {
				// If this chat is not known to be in the current language, store its setting in our languageDict
				[languageDict setObject:language forKey:chatID];
			}
		}
		...
		if (newChat) {
			...
			if (!newChatLanguage) {
				...
				if (listObject) {
					...
					newChatLanguage = [listObject preferenceForKey:KEY_LAST_USED_SPELLING
				...
				[languageDict setObject:newChatLanguage forKey:chatID];
			}
...
- (void)chatWillClose:(NSNotification *)notification
{
	...
	if (listObject) {
		...
		// If we didn't cache a language for this chat, or the chat is currently the active chat, use the spell
		// checker's value.
		...
		if ((previousLanguage && ![previousLanguage isEqualToString:chatLanguage]) ||
			...
			[listObject setPreference:chatLanguage forKey:KEY_LAST_USED_SPELLING group:GROUP_LAST_USED_SPELLING];
		}
