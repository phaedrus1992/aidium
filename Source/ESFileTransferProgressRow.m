...
- (void)dealloc
{
	...
	[bytesSentQueue release];
	bytesSentQueue = nil;
	[updateTickQueue release];
	updateTickQueue = nil;
...
- (void)fileTransfer:(ESFileTransfer *)inFileTransfer didSetLocalFilename:(NSString *)inLocalFilename
{
	...
	if (!filename)
		filename = [[inFileTransfer remoteFilename] lastPathComponent];
...
- (void)gotUpdateForFileTransfer:(ESFileTransfer *)inFileTransfer
{
	...
	if (lastUpdateTick && (((updateTick - lastUpdateTick) / 60.0) < 0.2) && (status == In_Progress_FileTransfer) &&
		!forceUpdate) {
		return;
	}
	...
	case Checksumming_Filetransfer:
		...
		[view setProgressAnimation:YES];
		transferSpeedStatus =
			[AILocalizedString(@"Preparing file", "waiting to begin a file transfer status") stringByAppendingEllipsis];
		break;
	...
	if (type == Unknown_FileTransfer) {
		...
		case Checksumming_Filetransfer:
			transferBytesStatus =
				[AILocalizedString(@"Preparing file transfer", "File transfer preparing status description")
					stringByAppendingEllipsis];
			break;
		case Waiting_on_Remote_User_FileTransfer:
			transferBytesStatus = [AILocalizedString(@"Waiting for transfer to be accepted",
													 "File transfer waiting on remote user status description")
				stringByAppendingEllipsis];
			break;
		case Connecting_FileTransfer:
			transferBytesStatus =
				[AILocalizedString(@"Establishing file transfer connection",
								   "File transfer connecting status description") stringByAppendingEllipsis];
			break;
		case Accepted_FileTransfer:
			transferBytesStatus = [AILocalizedString(@"Accepted file transfer", nil) stringByAppendingEllipsis];
			break;
		case In_Progress_FileTransfer: {
			NSString *bytesString = [adium.fileTransferController stringForSize:bytesSent of:size ofString:sizeString];
		...
		case Cancelled_Remote_FileTransfer:
			transferBytesStatus =
				AILocalizedString(@"Remote contact cancelled", "File transfer cancelled remotely status description");
			break;
		case Failed_FileTransfer:
	...
	if ((status == In_Progress_FileTransfer) && lastUpdateTick && lastBytesSent) {
		if (updateTick != lastUpdateTick) {
			...
			transferSpeedStatus = [NSString
				stringWithFormat:AILocalizedString(@"%@/sec", "Rate of transfer phrase. %@ will be replaced by an "
															  "abbreviated data amount such as 4 KB or 1 MB"),
								 [adium.fileTransferController stringForSize:rate]];

			if (rate > 0) {
				unsigned long long secsRemaining = ((size - bytesSent) / rate);
				transferRemainingStatus =
					[NSString stringWithFormat:
								  AILocalizedString(
									  @"%@ remaining",
									  "Time remaining for a file transfer to be completed phrase. %@ will be replaced "
									  "by an amount of time such as '5 seconds' or '4 minutes and 30 seconds'."),
								  [self readableTimeForSecs:secsRemaining inLongFormat:YES]];
			} else {
...
- (NSMenu *)menuForEvent:(NSEvent *)theEvent
{
	...
	if (([fileTransfer status] == Complete_FileTransfer) ||
		...
		menuItem =
			[[[NSMenuItem allocWithZone:[NSMenu menuZone]] initWithTitle:AILocalizedString(@"Show in Finder", nil)
																  target:self
	...
	if ([fileTransfer isStopped]) {
		menuItem =
			[[[NSMenuItem allocWithZone:[NSMenu menuZone]] initWithTitle:AILocalizedString(@"Remove from List", nil)
																  target:self
...
{
	[owner fileTransferProgressRow:self heightChangedFrom:oldHeight to:newHeight];
}
...
- (NSString *)readableTimeForSecs:(NSTimeInterval)secs inLongFormat:(BOOL)longFormat
{
	...
	NSDictionary *desc = [NSDictionary dictionaryWithObjectsAndKeys:AILocalizedString(@"second", "singular second"),
																	[NSNumber numberWithUnsignedInteger:1],
																	AILocalizedString(@"minute", "singular minute"),
																	[NSNumber numberWithUnsignedInteger:60],
																	AILocalizedString(@"hour", "singular hour"),
																	[NSNumber numberWithUnsignedInteger:3600],
																	AILocalizedString(@"day", "singular day"),
																	[NSNumber numberWithUnsignedInteger:86400],
																	AILocalizedString(@"week", "singular week"),
																	[NSNumber numberWithUnsignedInteger:604800],
																	AILocalizedString(@"month", "singular month"),
																	[NSNumber numberWithUnsignedInteger:2628000],
																	AILocalizedString(@"year", "singular year"),
																	[NSNumber numberWithUnsignedInteger:31536000], nil];
	NSDictionary *plural =
		[NSDictionary dictionaryWithObjectsAndKeys:AILocalizedString(@"seconds", "plural seconds"),
												   [NSNumber numberWithUnsignedInteger:1],
												   AILocalizedString(@"minutes", "plural minutes"),
												   [NSNumber numberWithUnsignedInteger:60],
												   AILocalizedString(@"hours", "plural hours"),
												   [NSNumber numberWithUnsignedInteger:3600],
												   AILocalizedString(@"days", "plural days"),
												   [NSNumber numberWithUnsignedInteger:86400],
												   AILocalizedString(@"weeks", "plural weeks"),
												   [NSNumber numberWithUnsignedInteger:604800],
												   AILocalizedString(@"months", "plural months"),
												   [NSNumber numberWithUnsignedInteger:2628000],
												   AILocalizedString(@"years", "plural years"),
												   [NSNumber numberWithUnsignedInteger:31536000], nil];
