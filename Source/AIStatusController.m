...
- (void)controllerDidLoad
{
	...
	for (AIAccount *account in adium.accountController.accounts) {
		NSData *lastStatusData = [account preferenceForKey:@"LastStatus" group:GROUP_ACCOUNT_STATUS];
...
- (void)controllerWillClose
{
	...
	[adium.preferenceController
		setPreference:[NSKeyedArchiver archivedDataWithRootObject:[[self rootStateGroup] containedStatusItems]]
...
- (void)dealloc
{
	[_rootStateGroup release];
	_rootStateGroup = nil;
	[_sortedFullStateArray release];
	_sortedFullStateArray = nil;
	[super dealloc];
}
 ...
 *
 * @param statusName A name which will be passed back to accounts of this service.  Internal use only.  Use the
 * AIStatusController.h #defines where appropriate.
 * @param description A human-readable localized description which will be shown to the user.  Use the
 * AIStatusController.h #defines where appropriate.
			...
			forService:(AIService *)service
{
	...
	if (!statusDictsByServiceCodeUniqueID[type])
		statusDictsByServiceCodeUniqueID[type] = [[NSMutableDictionary alloc] init];
	if (!(statusDicts = [statusDictsByServiceCodeUniqueID[type] objectForKey:serviceCodeUniqueID])) {
		statusDicts = [NSMutableSet set];
		[statusDictsByServiceCodeUniqueID[type] setObject:statusDicts forKey:serviceCodeUniqueID];
	}

	// Create a dictionary for this status entry
	NSDictionary *statusDict =
		[NSDictionary dictionaryWithObjectsAndKeys:statusName, KEY_STATUS_NAME, description, KEY_STATUS_DESCRIPTION,
 ...
 * @param inServiceCodeUniqueID The service for which to return active statuses.  If nil, return all statuses for online
 * services.
							  ...
							  withTarget:(id)target
{
	...
	// First, add our built-in items (so they will be at the top of the array and service-specific 'copies' won't
	// replace them)
 ...
 * @param alreadyAddedTitles NSMutableSet of NSString titles which have already been added and should not be duplicated.
 * Will be updated as items are added.
 */
- (void)_addMenuItemsForStatusOfType:(AIStatusType)type
						  withTarget:(id)target
				  ...
				  alreadyAddedTitles:(NSMutableSet *)alreadyAddedTitles
{
	...
	for (statusDict in statusDicts) {
		...
		/*
		 * Only add if it has not already been added by another service.... Services need to use unique titles if they
		 * have unique state names, but are welcome to share common name/description combinations, which is why the
		 * #defines exist.
		 */
...
- (NSString *)localizedDescriptionForCoreStatusName:(NSString *)statusName
{
	...
	if (!coreLocalizedStatusDescriptions) {
		coreLocalizedStatusDescriptions = [[NSDictionary
			dictionaryWithObjectsAndKeys:
				AILocalizedStringFromTable(@"Available", @"Statuses", "Name of a status"), STATUS_NAME_AVAILABLE,
				AILocalizedStringFromTable(@"Free for chat", @"Statuses", "Name of a status"),
				STATUS_NAME_FREE_FOR_CHAT,
				AILocalizedStringFromTable(@"Available for friends only", @"Statuses", "Name of a status"),
				STATUS_NAME_AVAILABLE_FRIENDS_ONLY,
				AILocalizedStringFromTable(@"Away", @"Statuses", "Name of a status"), STATUS_NAME_AWAY,
				AILocalizedStringFromTable(@"Extended away", @"Statuses", "Name of a status"),
				STATUS_NAME_EXTENDED_AWAY,
				AILocalizedStringFromTable(@"Away for friends only", @"Statuses", "Name of a status"),
				STATUS_NAME_AWAY_FRIENDS_ONLY,
				AILocalizedStringFromTable(@"Do not disturb", @"Statuses", "Name of a status"), STATUS_NAME_DND,
				AILocalizedStringFromTable(@"Not available", @"Statuses", "Name of a status"),
				STATUS_NAME_NOT_AVAILABLE, AILocalizedStringFromTable(@"Occupied", @"Statuses", "Name of a status"),
				STATUS_NAME_OCCUPIED, AILocalizedStringFromTable(@"Be right back", @"Statuses", "Name of a status"),
				STATUS_NAME_BRB, AILocalizedStringFromTable(@"Busy", @"Statuses", "Name of a status"), STATUS_NAME_BUSY,
				...
				AILocalizedStringFromTable(@"Not in the office", @"Statuses", "Name of a status"),
				STATUS_NAME_NOT_IN_OFFICE, AILocalizedStringFromTable(@"On vacation", @"Statuses", "Name of a status"),
				STATUS_NAME_VACATION, AILocalizedStringFromTable(@"Stepped out", @"Statuses", "Name of a status"),
				STATUS_NAME_STEPPED_OUT, AILocalizedStringFromTable(@"Invisible", @"Statuses", "Name of a status"),
				STATUS_NAME_INVISIBLE, AILocalizedStringFromTable(@"Offline", @"Statuses", "Name of a status"),
				STATUS_NAME_OFFLINE, nil] retain];
	}
...
{
	return [self localizedDescriptionForStatusName:statusState.statusName statusType:statusState.statusType];
}
...
- (void)_resetActiveStatusState
{
	...
	[_activeStatusState release];
	_activeStatusState = nil;
	[_allActiveStatusStates release];
	_allActiveStatusStates = nil;
...
- (NSSet *)updateListObject:(AIListObject *)inObject keys:(NSSet *)inModifiedKeys silent:(BOOL)silent
{
	if ([inObject isKindOfClass:[AIAccount class]]) {
		if ([inModifiedKeys containsObject:@"isOnline"] || [inModifiedKeys containsObject:@"idleSince"] ||
			[inModifiedKeys containsObject:@"accountStatus"] || [inModifiedKeys containsObject:KEY_ENABLED]) {
...
- (void)applyState:(AIStatus *)statusState toAccounts:(NSArray *)accountArray
{
	...
	if (![adium.accountController oneOrMoreConnectedAccounts]) {
		...
		for (AIAccount *account in accountArray) {
			if (account.enabled && ([accountsToConnect containsObject:account] || noAccountsToConnectCount)) {
	...
	// If this is not an offline status, we've now made use of accountsToConnect and should clear it so it isn't used
	// again.
	if (!isOfflineStatus) {
		[accountsToConnect removeAllObjects];
	}
	...
	if (![[self flatStatusSet] containsObject:statusState] && ![temporaryStateArray containsObject:statusState]) {
...
- (AIStatusGroup *)rootStateGroup
{
	if (!_rootStateGroup) {
		...
		if (!_rootStateGroup)
			_rootStateGroup = [[AIStatusGroup statusGroup] retain];
...
- (void)buildBuiltInStatusTypes
{
	...
	statusDict = [NSDictionary
		dictionaryWithObjectsAndKeys:STATUS_NAME_AVAILABLE, KEY_STATUS_NAME,
									 [self localizedDescriptionForCoreStatusName:STATUS_NAME_AVAILABLE],
									 KEY_STATUS_DESCRIPTION, [NSNumber numberWithInt:AIAvailableStatusType],
									 KEY_STATUS_TYPE, nil];
	[builtInStatusTypes[AIAvailableStatusType] addObject:statusDict];

	builtInStatusTypes[AIAwayStatusType] = [[NSMutableSet alloc] init];
	statusDict =
		[NSDictionary dictionaryWithObjectsAndKeys:STATUS_NAME_AWAY, KEY_STATUS_NAME,
												   [self localizedDescriptionForCoreStatusName:STATUS_NAME_AWAY],
												   KEY_STATUS_DESCRIPTION, [NSNumber numberWithInt:AIAwayStatusType],
												   KEY_STATUS_TYPE, nil];
...
{
	return [[[self builtInStateArray]
		filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"statusType == %i", AIAvailableStatusType]]
		objectAtIndex:0];
}
/**
...
{
	return [[[self builtInStateArray]
		filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"statusType == %i", AIAwayStatusType]]
		objectAtIndex:0];
}
/**
...
{
	return [[[self builtInStateArray]
		filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"statusType == %i", AIInvisibleStatusType]]
		objectAtIndex:0];
}
/**
...
{
	return [[[self builtInStateArray]
		filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"statusType == %i", AIOfflineStatusType]]
		objectAtIndex:0];
}
 ...
 *
 * @result A cached NSArray which is sorted by status type (available, away), built-in vs. user-made, and then original
 * ordering.
 */
- (NSArray *)sortedFullStateArray
{
	if (!_sortedFullStateArray) {
		...
		originalStateArray =
			[[self builtInStateArray] arrayByAddingObjectsFromArray:[[self rootStateGroup] containedStatusItems]];
		...
		if (adium.accountController.oneOrMoreConnectedAccounts) {
			...
			for (AIAccount *account in adium.accountController.accounts) {
				if (account.online) {
					...
					[statusCounts addObject:(accountStatusState ? accountStatusState : self.defaultInitialStatusState)];
				}
			}
 ...
 * @param activeUnvailableStatusName Pointer to an NSString*; returns by reference a status name if all states are in
 * the same name, or nil if they differ
 * @param allOnlineAccountsAreUnvailable Pointer to a BOOL; returns by reference YES is all online accounts are
 * unavailable, NO if one or more is available
 */
		...
		if (account.online || [account boolValueForProperty:@"isConnecting"]) {
			...
			if ((statusType == AIAwayStatusType) || (statusType == AIInvisibleStatusType)) {
				...
				if (foundStatusName) {
					...
					if ((activeUnvailableStatusName != NULL) && (*activeUnvailableStatusName != nil) &&
...
- (void)addStatusState:(AIStatus *)statusState
{
	...
	if ((mutabilityType == AILockedStatusState) || (mutabilityType == AISecondaryLockedStatusState)) {
...
- (void)saveStatusAsLastUsed:(AIStatus *)statusState
{
	...
	lastStatusStates =
		[[[adium.preferenceController preferenceForKey:@"LastStatusStates"
