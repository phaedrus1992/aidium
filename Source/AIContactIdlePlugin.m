 ...
 *
 * Don't calculate an idle time for a metacontact; its "idle time" should be determined dynamically based on its
 * contained contacts.
 */
...
- (void)setIdleForObject:(AIListObject *)inObject silent:(BOOL)silent
{
	...
	[inObject setValue:idleNumber forProperty:@"idle" notify:NotifyLater];
...
- (NSAttributedString *)entryForObject:(AIListObject *)inObject
{
	...
	if ((idleMinutes > 599400) || (idleMinutes == -1)) { // Cap idle at 999 Hours (999*60 minutes)
		...
		entry =
			[[NSAttributedString alloc] initWithString:[NSDateFormatter stringForTimeInterval:(idleMinutes * 60.0)]];
	}
