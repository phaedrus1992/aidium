...
- (void)viewDidLoad
{
	...
	if ([[adium.preferenceController preferenceForKey:KEY_USE_USER_ICON group:GROUP_ACCOUNT_STATUS] boolValue]) {
...
- (void)changePreference:(id)sender
{
	if (sender == textField_displayName) {
		...
		[adium.preferenceController
			setPreference:((displayName && [displayName length])
...
- (void)configureProfile
{
	...
	NSData *profileData = [adium.preferenceController preferenceForKey:@"textProfile" group:GROUP_ACCOUNT_STATUS];
...
- (void)textDidChange:(NSNotification *)aNotification
{
	if ([aNotification object] == textView_profile) {
		[NSObject cancelPreviousPerformRequestsWithTarget:self
												 selector:@selector(changePreference:)
												   object:textView_profile];
		[self performSelector:@selector(changePreference:) withObject:textView_profile afterDelay:1.0];
	}
}
...
{
	[adium.preferenceController setPreference:nil forKey:KEY_USER_ICON group:GROUP_ACCOUNT_STATUS];
...
- (void)configureImageView
{
	NSData *imageData = [adium.preferenceController preferenceForKey:KEY_USER_ICON group:GROUP_ACCOUNT_STATUS];
	if (!imageData) {
		imageData = [adium.preferenceController preferenceForKey:KEY_DEFAULT_USER_ICON group:GROUP_ACCOUNT_STATUS];
	}
...
#define DISPLAY_NAME_TOOLTIP                                                                                           \
	AILocalizedString(@"Your name, which on supported services will be sent to remote contacts. Substitutions from "   \
					  @"the Edit->Scripts and Edit->iTunes menus may be used here.",                                   \
					  nil)
	[label_remoteAlias setToolTip:DISPLAY_NAME_TOOLTIP];
	[textField_displayName setToolTip:DISPLAY_NAME_TOOLTIP];

#define PROFILE_TOOLTIP                                                                                                \
	AILocalizedString(@"Profile to display when contacts request information about you (not supported by all "         \
					  @"services). Text may be formatted using the Edit and Format menus.",                            \
					  nil)
