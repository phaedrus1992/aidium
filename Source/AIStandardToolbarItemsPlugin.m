...
{
	// New Message
	NSToolbarItem *toolbarItem = [AIToolbarUtilities
		toolbarItemWithIdentifier:@"SourceDestination"
					 ...
					 paletteLabel:AILocalizedString(@"Change Source or Destination", nil)
						  toolTip:AILocalizedString(
									  @"If multiple accounts can send to this contact or this is a combined contact, "
									  @"change the source and/or destination of this chat",
									  nil)
						   target:self
...
- (IBAction)showSourceDestinationPicker:(NSToolbarItem *)toolbarItem
{
	...
	if ([object isKindOfClass:[AIListContact class]]) {
		AIChat *chat = [adium.chatController openChatWithContact:(AIListContact *)object onPreferredAccount:YES];
