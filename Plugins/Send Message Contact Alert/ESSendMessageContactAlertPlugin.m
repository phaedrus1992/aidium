...
- (NSString *)longDescriptionForActionID:(NSString *)actionID withDetails:(NSDictionary *)details
{
	NSString *messageText =
		[[NSAttributedString stringWithData:[details objectForKey:KEY_MESSAGE_SEND_MESSAGE]] string];
			   ...
			   userInfo:(id)userInfo
{
	...
	AILogWithSignature(@"actionID %@, listObject %@, details %@, eventID %@, userInfo %@", actionID, listObject,
					   details, eventID, userInfo);
	...
	if ([contact isKindOfClass:[AIMetaContact class]]) {
		...
		contact = [adium.contactController contactWithService:contact.service account:account UID:contact.UID];
	}
	...
	if (![account availableForSendingContentType:CONTENT_MESSAGE_TYPE toContact:contact]) {
	...
	if (account && contact) {
		...
		chat = [adium.chatController openChatWithContact:contact onPreferredAccount:NO];
		if (([chat messageSendingAbility] == AIChatCanSendMessageNow) ||
			...
			AILogWithSignature(@"%@ %@ to %@ from %@ in %@", (success ? @"Sent" : @"Failed to send"), content, contact,
							   account, chat);

			// Display an error message if the message was not delivered
			if (!success) {
				[adium.interfaceController
					  handleMessage:AILocalizedString(@"Contact Alert Error", nil)
					withDescription:[NSString stringWithFormat:AILocalizedString(@"Unable to send message to %@.", nil),
															   contact.displayName]
					withWindowTitle:@""];
			}
		}
