...
- (void)configureMenuItems
{
	...
	[adium.menuController addMenuItem:menuItem_encryption toLocation:LOC_Contact_Additions];

	menuItem_encryptionContext = [menuItem_encryption copy];

	[adium.menuController addContextualMenuItem:menuItem_encryptionContext toLocation:Context_Contact_ChatAction];
}
...
- (void)registerToolbarItem
{
	...
	toolbarItem = [AIToolbarUtilities
		toolbarItemWithIdentifier:@"Encryption"
							label:TITLE_ENCRYPTION
					 paletteLabel:AILocalizedString(@"Encrypted Messaging", nil)
						  toolTip:AILocalizedString(@"Toggle encrypted messaging. Shows a closed lock when secure and "
													@"an open lock when insecure.",
													nil)
						   target:self
...
// A chat became visible in a window.  Update the item with the @"Encryption" identifier to show the IsSecure state for
// this chat
- (void)chatDidBecomeVisible:(NSNotification *)notification
{
...
- (NSSet *)updateChat:(AIChat *)inChat keys:(NSSet *)inModifiedKeys silent:(BOOL)silent
{
	if ([inModifiedKeys containsObject:@"securityDetails"]) {
		[self _updateToolbarIconOfChat:inChat inWindow:[adium.interfaceController windowForChat:inChat]];
		...
		if (chatIsSecure != [inChat boolValueForProperty:@"secureMessagingLastEncryptedState"]) {
			...
			if (chatIsSecure) {
				if ([inChat encryptionStatus] == EncryptionStatus_Unverified) {
					...
					NSString *displayName = (listObject ? listObject.formattedUID : inChat.displayName);
...
- (IBAction)toggleSecureMessaging:(id)sender
{
	...
	[chat.account requestSecureMessaging:!chat.isSecure inChat:chat];
}
	...
	if ([[[menuItem menu] title] isEqualToString:ENCRYPTION_MENU_TITLE]) {
		...
		case EncryptedChat_Default: {
			if (listContact) {
				...
				[menuItem
					setState:(tag == userPreference || ![listContact preferenceForKey:KEY_ENCRYPTED_CHAT_PREFERENCE
																				group:GROUP_ENCRYPTION])];
			}
			return YES;
			break;
