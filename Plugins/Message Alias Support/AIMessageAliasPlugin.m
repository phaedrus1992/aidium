...
- (NSMutableAttributedString *)replaceKeywordsInString:(NSAttributedString *)attributedString context:(id)context
{
	...
	if ([self string:str containsValidKeyword:@"%n"]) {
		...
		if ([context isKindOfClass:[AIContentObject class]]) {
			replacement = [[context destination]
				UID]; // This exists primarily for AIM compatibility; AIM uses the UID (no formatting).
	...
	if ([self string:str containsValidKeyword:@"%t"]) {
		...
		[NSDateFormatter
			withLocalizedDateFormatterShowingSeconds:YES
									   showingAMorPM:YES
											 perform:^(NSDateFormatter *localDateFormatter) {
												 [newAttributedString
													 replaceOccurrencesOfString:@"%t"
																	 withString:[localDateFormatter
																					stringFromDate:currentDate]
																		options:NSLiteralSearch
																		  range:NSMakeRange(
																					0, [newAttributedString length])];
											 }];
	}
