			  ...
			  containerName:(NSString *)inName
{
	if ((self = [super initWithWindowNibName:windowNibName])) {
		...
		[myWindow registerForDraggedTypes:[NSArray arrayWithObject:@"NSTabViewItem"]];
	}
...
- (void)windowDidLoad
{
	...
	[tabView_tabBar setAllowsBackgroundTabClosing:[[NSUserDefaults standardUserDefaults]
													  boolForKey:@"AIAllowBackgroundTabClosing"]];
...
{
	if (!windowIsClosing && self.containedChats.count > 1 &&
		[[adium.preferenceController preferenceForKey:KEY_CONFIRM_MSG_CLOSE
		...
		switch ([[adium.preferenceController preferenceForKey:KEY_CONFIRM_MSG_CLOSE_TYPE
														group:PREF_GROUP_CONFIRMATIONS] integerValue]) {
		...
		if (unreadCount) {
			if (unreadCount == 1) {
				question = [NSString
					stringWithFormat:AILocalizedString(@"%lu chats are open in this window, 1 of which has unviewed "
													   @"messages. Do you want to close this window anyway?",
													   nil),
									 (unsigned long)self.containedChats.count];
			} else {
				question = [NSString
					stringWithFormat:AILocalizedString(@"%lu chats are open in this window, %lu of which have unviewed "
													   @"messages. Do you want to close this window anyway?",
													   nil),
									 (unsigned long)self.containedChats.count, (unsigned long)unreadCount];
			}
		} else {
			question = [NSString
				stringWithFormat:AILocalizedString(
									 @"%lu chats are open in this window. Do you want to close this window anyway?",
									 nil),
								 (unsigned long)self.containedChats.count];
		}
						 ...
						 firstTime:(BOOL)firstTime
{
	if ([group isEqualToString:PREF_GROUP_DUAL_WINDOW_INTERFACE]) {
		...
		[[tabView_tabBar overflowPopUpButton]
			setAlternateImage:[AIStatusIcons statusIconForStatusName:@"content"
...
- (void)_relayoutWindow
{
	MMTabBarOrientation orientation =
		((tabPosition == AdiumTabPositionBottom || tabPosition == AdiumTabPositionTop) ? MMTabBarHorizontalOrientation
	...
	case MMTabBarHorizontalOrientation: {
		...
		if (tabPosition == AdiumTabPositionBottom) {
			...
			tabViewMessagesFrame.origin.y =
				NSHeight(tabBarFrame) + (isTabBarHidden ? 0 : (HORIZONTAL_TAB_BAR_TO_VIEW_SPACING - 1));
			tabViewMessagesFrame.size.height = NSHeight(contentRect) - NSHeight(tabBarFrame) -
											   (isTabBarHidden ? 0 : HORIZONTAL_TAB_BAR_TO_VIEW_SPACING) + 3;
		...
		[tabView_tabBar setButtonMinWidth:60];
		[tabView_tabBar setButtonMaxWidth:250];

		tabBarFrame.origin.x = 0;
		tabViewMessagesFrame.origin.x = 0;

		if (!isTabBarHidden) {
			NSRect horzLineFrame =
				NSMakeRect(tabBarFrame.origin.x,
						   (tabPosition == AdiumTabPositionBottom) ? NSMinY(tabViewMessagesFrame) - 1
																   : NSMaxY(tabViewMessagesFrame) - 2,
...
- (void)updateIconForTabViewItem:(AIMessageTabViewItem *)tabViewItem
{
	...
	if ([[tabView_tabBar visibleTabViewItems] indexOfObject:tabViewItem] >=
		[tabView_tabBar numberOfVisibleTabViewItems]) {
		// The chat is in the overflow menu. If any chat has unviewed content, it should be animating to demonstrate
		// that.
...
- (void)addTabViewItem:(AIMessageTabViewItem *)inTabViewItem atIndex:(NSInteger)idx silent:(BOOL)silent
{
	...
	if (![tabView_messages selectedTabViewItem])
...
{
	/* When a tab isn't selected, its views are not within any window. We want the tab to be able to remove tracking
	 * rects from the window before closing, so if it isn't selected we need to select it briefly to let this happen.
	 * Since this is all within the same run loop, as long as code in the tab view's delegate is well-behaved and uses
	 * setNeedsDisplay: rather than display if it does drawing, the UI shouldn't change at all.
	 */
	 ...
	 * This must be done last so that the NSTabView and its delegate (MMTabBarView) can still make use of the
	 * idenfitier.
	 */
...
- (void)windowDidBecomeKey:(NSNotification *)notification
{
	[adium.interfaceController
		chatDidBecomeActive:[(AIMessageTabViewItem *)[tabView_messages selectedTabViewItem] chat]];
}
...
- (void)windowDidResignKey:(NSNotification *)notification
{
	[((AIMessageTabViewItem *)
		  tabView_messages.selectedTabViewItem).messageViewController.messageDisplayController markForFocusChange];
	[adium.interfaceController chatDidBecomeActive:nil];
}
...
- (CGFloat)splitView:(NSSplitView *)sender constrainMinCoordinate:(CGFloat)proposedMin ofSubviewAt:(NSInteger)offset
{
	...
	if (sender == tabView_splitView) {
		...
		case AdiumTabPositionRight:
			min = ([tabView_tabBar isTabBarHidden] ? NSWidth([tabView_splitView frame])
												   : (NSWidth([tabView_splitView frame]) -
													  MAXIMUM_WIDTH_FOR_VERTICAL_TABS - [sender dividerThickness]));
			break;
		}
	} else {
...
- (void)splitView:(NSSplitView *)sender resizeSubviewsWithOldSize:(NSSize)oldSize
{
	...
	messageFrame.size = NSMakeSize([sender frame].size.width - tabBarFrame.size.width - [sender dividerThickness],
...
// MMTabBarView Delegate
// -------------------------------------------------------------------------------------------------
#pragma mark MMTabBarView Delegate
...
- (BOOL)tabView:(NSTabView *)tabView
	shouldDragTabViewItem:(NSTabViewItem *)tabViewItem
			   fromTabBar:(MMTabBarView *)tabBarView
{
	return YES;
}
...
- (BOOL)tabView:(NSTabView *)tabView
	shouldDropTabViewItem:(NSTabViewItem *)tabViewItem
				 inTabBar:(MMTabBarView *)tabBarView
{
	return YES;
}
...
- (NSMenu *)tabView:(NSTabView *)tabView menuForTabViewItem:(NSTabViewItem *)tabViewItem
{
	...
	if (selectedObject) {
		...
		if ([selectedObject isIntentionallyNotAStranger]) {
			locations = [NSMutableArray arrayWithObjects:[NSNumber numberWithInteger:Context_Contact_Manage],
			...
			locations =
				[NSMutableArray arrayWithObjects:[NSNumber numberWithInteger:Context_Contact_Manage],
												 [NSNumber numberWithInteger:Context_Contact_Action],
												 [NSNumber numberWithInteger:Context_Contact_NegativeAction],
												 [NSNumber numberWithInteger:Context_Contact_ChatAction],
		...
		tmp = [adium.menuController contextualMenuWithLocations:locations forListObject:selectedObject inChat:chat];

	} else if (chat.isGroupChat) {
		NSArray *locations = [NSArray arrayWithObjects:[NSNumber numberWithInteger:Context_GroupChat_Manage],
													   [NSNumber numberWithInteger:Context_GroupChat_Action],
													   [NSNumber numberWithInteger:Context_Tab_Action], nil];

		tmp = [adium.menuController contextualMenuWithLocations:locations forChat:chat];
	}
...
- (NSImage *)tabView:(NSTabView *)tabView
			  ...
			  styleMask:(NSUInteger *)styleMask
{
	...
	switch (tabPosition) {
	case AdiumTabPositionBottom:
		offset->width = [style leftMarginForTabBarView:tabView_tabBar];
	...
	case AdiumTabPositionLeft:
		offset->width = 0;
		offset->height = 21 + [style topMarginForTabBarView:tabView_tabBar];
		break;
	case AdiumTabPositionRight:
...
- (MMTabBarView *)tabView:(NSTabView *)tabView
						   ...
						   atPoint:(NSPoint)point
{
	...
	switch (tabPosition) {
	case AdiumTabPositionBottom:
		point.x -= [style leftMarginForTabBarView:tabView_tabBar];
		point.y -= 22;
		break;
	case AdiumTabPositionTop:
		point.x -= [style leftMarginForTabBarView:tabView_tabBar];
	...
	case AdiumTabPositionLeft:
		point.y -=
			NSHeight([[[newController window] contentView] frame]) - [style topMarginForTabBarView:tabView_tabBar] + 1;
		break;
	case AdiumTabPositionRight:
...
- (CGFloat)desiredWidthForVerticalTabBar:(MMTabBarView *)tabBarView
{
...
- (NSString *)tabView:(NSTabView *)tabView toolTipForTabViewItem:(NSTabViewItem *)tabViewItem
{
	...
	if (chat.isGroupChat) {
		tooltip = [NSString stringWithFormat:AILocalizedString(@"%@ in %@", "AccountName on ChatRoomName"),
											 chat.account.formattedUID, chat.name];
	} else {
		...
		AILog(@"Displaying tooltip for %@ --> %@ (%@) --> %@ (%@)", chat, chat.account, chat.account.formattedUID,
			  destination, destinationFormattedUID);
		if (includeDestination && includeSource) {
			tooltip =
				[NSString stringWithFormat:AILocalizedString(@"%@ talking to %@", "AccountName talking to Username"),
										   chat.account.formattedUID, destinationFormattedUID];
...
- (void)tabView:(NSTabView *)aTabView tabViewItem:(NSTabViewItem *)tabViewItem isInOverflowMenu:(BOOL)inOverflowMenu
{
	...
	[self performSelector:@selector(updateOverflowMenuUnviewedContentIcon) withObject:nil afterDelay:0];
}
...
- (void)windowWillMiniaturize:(NSNotification *)notification
{
	...
	{
		// Draw the chat image with space around it (the dock will do ugly scaling if we don't make a transparent
		// border)
...
- (BOOL)window:(NSWindow *)sender
	shouldDragDocumentWithEvent:(NSEvent *)mouseEvent
						   from:(NSPoint)startPoint
