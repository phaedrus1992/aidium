...
- (void)gotFilteredMessageToSendLater:(NSAttributedString *)filteredMessage
					 receivingContext:(NSMutableDictionary *)alertDict;
...
- (id)initForChat:(AIChat *)inChat
{
	if ((self = [super init])) {
		 ...
		 * Call chatParticipatingListObjectsChanged first, which will set up the user list. This allows other sizing to
		 * match.
		 */
...
- (void)saveUserListMinimumSize
{
	[adium.preferenceController
		setPreference:[NSNumber numberWithBool:[self userListVisible]]
			   forKey:[KEY_USER_LIST_VISIBLE_PREFIX
						  stringByAppendingFormat:@"%@.%@", chat.account.internalObjectID, chat.name]
				group:PREF_GROUP_DUAL_WINDOW_INTERFACE];
...
- (IBAction)sendMessage:(id)sender
{
	...
	if ([attributedString length] != 0) {
		...
		if ([[attributedString string]
				caseInsensitiveCompare:AILocalizedString(@"/clear",
														 "Command which will clear the message area of a chat. Please "
														 "include the '/' at the front of your translation.")] ==
			NSOrderedSame) {
		...
		if (suppressSendLaterPrompt || (messageSendingAbility == AIChatCanSendMessageNow) ||
			((messageSendingAbility == AIChatCanSendViaServersideOfflineMessage) &&
			 chat.account.sendOfflineMessagesWithoutPrompting)) {
			...
			NSImage *icon = ([listObject userIcon] ? [listObject userIcon]
				...
				[alert setInformativeText:
						   [NSString
							   stringWithFormat:AILocalizedString(
													@"Send Now will deliver your message to the server immediately. %@ "
				...
				[alert
					setInformativeText:
						[NSString stringWithFormat:AILocalizedString(
													   @"Send Later will send the message the next time both you and "
													   @"%@ are online. Send Now may work if %@ is invisible or is not "
													   @"on your contact list and so only appears to be offline.",
			...
			case AIChatCanNotSendMessage: {
				[alert setInformativeText:[NSString stringWithFormat:AILocalizedString(
																		 @"Send Later will send the message the next "
																		 @"time both you and %@ are online.",
																		 "Send Later dialogue explanation text"),
																	 formattedUID]];
				[alert addButtonWithTitle:AILocalizedString(@"Send Later", nil)];
			...
			NSButton *dontSendButton =
				((messageSendingAbility == AIChatCanNotSendMessage) ? [[alert buttons] objectAtIndex:1]
...
- (void)updateFramesForAccountSelectionView
{
	...
	[view_accountSelection
		setFrameOrigin:NSMakePoint(NSMinX(splitView_verticalSplit.frame), NSMaxY(splitView_verticalSplit.frame))];
...
- (void)_configureTextEntryView
{
	...
	if ([[textView_outgoing enclosingScrollView] respondsToSelector:@selector(setVerticalScrollElasticity:)]) {
		[[textView_outgoing enclosingScrollView]
			setVerticalScrollElasticity:1]; // Swap 1 with NSScrollElasticityNone on 10.7+
	}
}
...
- (BOOL)canTabCompleteForPartialWord:(NSString *)partialWord
{
	return ([self contactsMatchingBeginningString:partialWord].count > 0 ||
			[self.chat.displayName
				rangeOfString:partialWord
					  options:(NSDiacriticInsensitiveSearch | NSCaseInsensitiveSearch | NSAnchoredSearch)]
					.location != NSNotFound);
}
...
- (NSRange)textView:(NSTextView *)inTextView rangeForCompletion:(NSRange)charRange
{
	if (self.chat.isGroupChat && charRange.location > 0) {
		...
		NSRange whitespacePosition = [allText rangeOfCharacterFromSet:[NSCharacterSet whitespaceCharacterSet]
		...
		if (whitespacePosition.location == NSNotFound) {
			...
			partialWord = [allText substringWithRange:NSMakeRange(NSMaxRange(whitespacePosition),
																  allText.length - NSMaxRange(whitespacePosition))];
		}
...
- (NSArray *)contactsMatchingBeginningString:(NSString *)partialWord
{
	...
	for (AIListContact *listContact in self.chat) {
		...
		if ([[self.chat displayNameForContact:listContact]
				rangeOfString:partialWord
					  options:(NSDiacriticInsensitiveSearch | NSCaseInsensitiveSearch | NSAnchoredSearch)]
					.location != NSNotFound ||
			...
			AILogWithSignature(@"Added match %@ with nick %@; UID: %@; formattedUID: %@; displayName: %@", listContact,
							   [self.chat aliasForContact:listContact], listContact.UID, listContact.formattedUID,
							   listContact.displayName);
		}
	}
	...
	indexOfSelectedItem:(NSInteger *)idx
{
	...
	if (self.chat.isGroupChat) {
		...
		if (charRange.location != 0 && charRange.location >= prefix.length) {
			prefix = [[textView.textStorage.string
						 substringWithRange:NSMakeRange(charRange.location - prefix.length, prefix.length)]
		...
		if (charRange.location + charRange.length + suffix.length <= textView.textStorage.string.length) {
			suffix = [[textView.textStorage.string
						 substringWithRange:NSMakeRange(charRange.location + charRange.length, suffix.length)]
...
- (BOOL)userListInitiallyVisible
{
	NSNumber *visibility = [adium.preferenceController
		preferenceForKey:[KEY_USER_LIST_VISIBLE_PREFIX
							 stringByAppendingFormat:@"%@.%@", chat.account.internalObjectID, chat.name]
				   group:PREF_GROUP_DUAL_WINDOW_INTERFACE];
...
- (void)setUserListVisible:(BOOL)inVisible
{
	...
	[adium.preferenceController
		setPreference:[NSNumber numberWithBool:inVisible]
			   forKey:[KEY_USER_LIST_VISIBLE_PREFIX
						  stringByAppendingFormat:@"%@.%@", chat.account.internalObjectID, chat.name]
				group:PREF_GROUP_DUAL_WINDOW_INTERFACE];
}
...
- (void)_showUserListView
{
	...
	[splitView_verticalSplit setPosition:[self _userListViewDividerPositionIgnoringUserMinimum:NO] ofDividerAtIndex:0];
	[splitView_verticalSplit adjustSubviews];
}
...
- (void)chatParticipatingListObjectsChanged:(NSNotification *)notification
{
	...
	/* Even if we're not viewing the user list, we can't risk it keeping stale information about potentially released
	 * objects */
	[userListController reloadData];
	...
	if ([aNotification object] == splitView_verticalSplit) {
		if (NSWidth(view_userList.frame) > 0) {
			...
			[NSObject cancelPreviousPerformRequestsWithTarget:self
													 selector:@selector(saveUserListMinimumSize)
													   object:nil];
			[self performSelector:@selector(saveUserListMinimumSize) withObject:nil afterDelay:0.5];
			   ...
			   [splitView_textEntryHorizontal inLiveResize]) {
...
- (void)splitView:(NSSplitView *)splitView resizeSubviewsWithOldSize:(NSSize)oldSize
{
	if ([splitView inLiveResize] || adium.interfaceController.activeChat != chat) {
		...
		if (splitView == splitView_verticalSplit) {
			...
			if (userListVisible) {
				if (userListOnRight) {
					userFrame.size.width =
						currentFrame.size.width - [self _userListViewDividerPositionIgnoringUserMinimum:NO];
				} else
