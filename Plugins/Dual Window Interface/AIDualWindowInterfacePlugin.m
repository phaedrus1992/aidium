			  ...
			  atIndex:(NSUInteger)idx
{
	...
	if (!messageTab) {
		...
		[chat setValue:messageTab forProperty:@"messageTabViewItem" notify:NotifyNever];
...
- (void)closeChat:(AIChat *)chat
{
	...
	[chat setValue:nil forProperty:@"messageTabViewItem" notify:NotifyNever];
}
...
- (NSArray *)openContainersAndChats
{
	...
	for (AIMessageWindowController *container in [containers objectEnumerator]) {
		[openContainersAndChats
			addObject:[NSDictionary dictionaryWithObjectsAndKeys:container.containerID, @"ID",
																 NSStringFromRect(container.window.frame), @"Frame",
																 container.containedChats, @"Content",
																 container.activeChat, @"ActiveChat", container.name,
																 @"Name", nil]];
	}
...
- (NSWindow *)windowForChat:(AIChat *)chat
{
	...
	return (([windowController activeChat] == chat) ? [windowController window] : nil);
}
...
- (AIMessageWindowController *)openContainerWithID:(NSString *)containerID name:(NSString *)containerName
{
	...
	AIMessageWindowController *windowController = [containers objectForKey:containerID];
	if (!windowController) {
		windowController = [AIMessageWindowController messageWindowControllerForInterface:self
																				   withID:containerID
																					 name:containerName];
		...
		if (applicationIsHidden)
			[delayedContainerShowArray addObject:windowController];
	}
...
- (void)applicationDidUnhide:(NSNotification *)notification
{
	...
	for (container in delayedContainerShowArray)
		[container showWindowInFrontIfAllowed:YES];
