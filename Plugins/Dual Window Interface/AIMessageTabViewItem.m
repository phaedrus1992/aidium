		...
		if (!chat.isGroupChat) {
			...
			[[NSNotificationCenter defaultCenter] addObserver:self
													 selector:@selector(chatParticipatingListObjectsChanged:)
													   ...
													   object:chat];
			[[NSNotificationCenter defaultCenter] addObserver:self
													 selector:@selector(chatAttributesChanged:)
...
- (void)chatParticipatingListObjectsChanged:(NSNotification *)notification
{
	...
	// We fake a ListObject_AttributesChanged notification with our new list object. Our list object may be nil, but
	// that's fine.
	notification = [NSNotification notificationWithName:ListObject_AttributesChanged object:listObject];
...
- (void)listObjectAttributesChanged:(NSNotification *)notification
{
	...
	AIListContact *messageViewContact =
		[messageViewController listObject]; // Is the contact in the message view part of a metacontact?
...
- (NSImage *)icon
{
	...
	if (!image && messageViewController.chat.isGroupChat) {
		BOOL displayOnline = messageViewController.chat.account.online &&
							 [messageViewController.chat boolValueForProperty:@"accountJoined"];
	...
	if (!image)
		image = [AIStatusIcons statusIconForUnknownStatusWithIconType:AIStatusIconTab direction:AIIconNormal];
...
- (NSColor *)countColor
{
	return self.chat.unviewedMentionCount ? [NSColor colorWithCalibratedRed:1.0f green:0.3f blue:0.3f alpha:0.6f]
										  : [NSColor colorWithCalibratedWhite:0.3f alpha:0.6f];
}
...
- (void)updateTabStatusIcon
{
	/* Really, we should be observing for the icon changing and posting a dependent key change notification when it
	 * does... Pretending to have changed our icon key is a path of much less resistance to note that -[self icon] has
	 * changed.
	 */
