...
- (id)init
{
	if ((self = [super init])) {
		...
		[notificationCenter addObserver:self selector:@selector(updateOpenChats) name:Chat_DidOpen object:nil];
		[notificationCenter addObserver:self selector:@selector(updateOpenChats) name:Chat_WillClose object:nil];
		[notificationCenter addObserver:self selector:@selector(updateOpenChats) name:Chat_OrderDidChange object:nil];
...
- (void)updateUnreadCount
{
	NSUInteger unreadCount = (showConversationCount ? [adium.chatController unviewedConversationCount]
...
- (NSImage *)badgeDuck:(NSImage *)duckImage withImage:(NSImage *)badgeImage
{
	...
	if (badgeImage) {
		...
		NSRect destRect = {{.x = srcRect.size.width, .y = 0.0f}, [duckImage size]};
		destRect.size.width *= 0.5f;
		destRect.size.height *= 0.5f;

		// If the badge is bigger than that portion, resize proportionally. Otherwise, leave it alone and adjust the
		// destination origin appropriately.
...
- (NSSet *)updateListObject:(AIListObject *)inObject keys:(NSSet *)inModifiedKeys silent:(BOOL)silent
{
	if ([inObject isKindOfClass:[AIAccount class]]) {
		if ([inModifiedKeys containsObject:@"isConnecting"] || [inModifiedKeys containsObject:@"waitingToReconnect"]) {
			[self updateMenuIcons];
		}
	}
...
- (void)menuNeedsUpdate:(NSMenu *)menu
{
	...
	if (menu == mainMenu && mainMenuNeedsUpdate) {
		...
		for (menuItem in stateMenuItemsArray) {
			...
			// Validate the menu items as they are added since they weren't previously validated when the menu was
			// clicked
		...
		for (menuItem in accountMenuItemsArray) {
			...
			if ((submenu = [menuItem submenu])) {
				for (NSMenuItem *submenuItem in submenu.itemArray) {
					// Validate the submenu items as they are added since they weren't previously validated when the
					// menu was clicked
