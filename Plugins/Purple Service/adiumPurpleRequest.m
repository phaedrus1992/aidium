 ...
 * and then the account disconnects.  All window controllers created from adiumPurpleRequest.m should return
 * non-autoreleased instances of themselves.  They then release themselves when their window closes.  Rather than
 * calling
 * [[self window] close], they should use purple_request_close_with_handle(self) to ensure proper bookkeeping
 * purpleside.
 */
...
static void *adiumPurpleRequestInput(const char *title, const char *primary, const char *secondary,
									 const char *defaultValue, gboolean multiline, gboolean masked, gchar *hint,
...
static void *adiumPurpleRequestChoice(const char *title, const char *primary, const char *secondary, gint defaultValue,
									  const char *okText, GCallback okCb, const char *cancelText, GCallback cancelCb,
									  PurpleAccount *account, const char *who, PurpleConversation *conv, void *userData,
									  va_list choices)
{
	AILogWithSignature(@"%s\n%s\n%s ", (title ? title : ""), (primary ? primary : ""), (secondary ? secondary : ""));

	return nil;
}

static void *adiumPurpleRequestActionWithIcon(const char *title, const char *primary, const char *secondary,
...
static void *adiumPurpleRequestAction(const char *title, const char *primary, const char *secondary, int default_action,
									  PurpleAccount *account, const char *who, PurpleConversation *conv, void *userData,
									  size_t actionCount, va_list actions)
{
	return adiumPurpleRequestActionWithIcon(title, primary, secondary, default_action, account, who, conv,
											/* iconData */ NULL, /* iconSize */ 0, userData, actionCount, actions);
}

static void *adiumPurpleRequestFields(const char *title, const char *primary, const char *secondary,
									  ...
									  const char *who, PurpleConversation *conv, void *userData)
{
	@autoreleasepool {
		...
		if (title && (strcmp(title, _("Register New XMPP Account")) == 0)) {
			/* Jabber registration request. Instead of displaying a request dialogue, we fill in the information
			 * automatically. And by that, I mean that we accept all the default empty values, since the username and
			 * password are preset for us. */
			...
			[adium.accountController
				passwordForAccount:adiumAccount
				   ...
				   notifyingTarget:[ESPurpleRequestAdapter class]
						  selector:@selector(returnedPassword:returnCode:context:)
						   context:[NSDictionary
									   dictionaryWithObjectsAndKeys:adiumAccount, @"Account",
...
static void *adiumPurpleRequestFile(const char *title, const char *filename, gboolean savedialog, GCallback ok_cb,
 ...
 * @param uiHandle must be an id; it should either be NSNull or an object which can respond to close, such as
 * NSWindowController.
 */
