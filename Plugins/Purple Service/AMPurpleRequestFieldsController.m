...
- (NSXMLElement *)xhtml
{
	...
	if (isMultiline) {
		...
		if (defaultvalue)
			[textinput addAttribute:[NSXMLNode attributeWithName:@"value"
													 stringValue:[NSString stringWithUTF8String:defaultvalue]]];
	}
...
- (NSXMLElement *)xhtml
{
	...
	[textinput addAttribute:[NSXMLNode attributeWithName:@"value"
											 stringValue:[NSString stringWithFormat:@"%ld", defaultvalue]]];
	[textinput addAttribute:[NSXMLNode attributeWithName:@"name" stringValue:[self key]]];
...
- (NSXMLElement *)xhtml
{
	...
	purple_request_field_bool_set_value(field,
										FALSE); // since we won't get an -applyValue: message when the checkbox isn't
												// checked, assume false for now. This might be changed later.
	return result;
}
	...
	if (len > 5) {
		...
		for (label = labels; label; label = g_list_next(label), ++i) {
			...
			NSXMLElement *option = [NSXMLNode elementWithName:@"option"
												  stringValue:[NSString stringWithUTF8String:labelstr]];
			[option addAttribute:[NSXMLNode attributeWithName:@"value"
												  stringValue:[NSString stringWithFormat:@"%lu", i]]];
			if (i == defaultvalue)
		...
		for (label = labels; label; label = g_list_next(label), ++i) {
			...
			[radiobutton addAttribute:[NSXMLNode attributeWithName:@"value"
													   stringValue:[NSString stringWithFormat:@"%lu", i]]];
...
- (NSXMLElement *)xhtml
{
	...
	[list addAttribute:[NSXMLNode attributeWithName:@"size"
										stringValue:[NSString stringWithFormat:@"%u", (len > 10) ? 10 : len]]];
...
- (NSXMLElement *)xhtml
{
	...
	NSString *filename = [[[NSString stringWithFormat:@"TEMP-Image_%@", [self key]]
		stringByAppendingPathExtension:extension] safeFilenameString];
	...
	if ([data writeToFile:imagePath atomically:YES]) {
		[imageElement addAttribute:[NSXMLNode attributeWithName:@"src"
													stringValue:[[NSURL fileURLWithPath:imagePath] absoluteString]]];
		   ...
		   userData:(void *)_userData
{
	if ((self = [super initWithWindowNibName:@"AMPurpleRequestFieldsWindow"])) {
		...
		[head
			addChild:[NSXMLNode
						 elementWithName:@"style"
								children:[NSArray
											 arrayWithObject:[NSXMLNode
																 textWithStringValue:@"body {"
																					 ...
																					 @"	border-style: solid;"
																					 @"}"]]
							  attributes:[NSArray arrayWithObject:[NSXMLNode attributeWithName:@"type"
		...
		for (; gl != NULL; gl = gl->next) {
			...
			if (len > 1) {
				...
				if (fieldtitle)
					[fieldset addChild:[NSXMLNode elementWithName:@"legend"
													  stringValue:[NSString stringWithUTF8String:fieldtitle]]];
			} else
				fieldset = formdiv;
														  ...
														  attributes:[NSArray
																		 arrayWithObjects:
																			 ...
																			 [NSXMLNode attributeWithName:@"id"
																							  stringValue:@"submit"],
																			 [NSXMLNode attributeWithName:@"value"
																							  stringValue:okText],
																			 nil]],
												 nil]
							  attributes:[NSArray arrayWithObject:[NSXMLElement attributeWithName:@"id"
																					  stringValue:@"submitbuttons"]]]];
		...
		/*
		 //Code here originally made the webview transparent; the result is an all-black window. I don't think this is
		desired. if ([webview respondsToSelector:@selector(setBackgroundColor:)]) {
	...
	if ([[[request URL] scheme] isEqualToString:@"applewebdata"] || [[[request URL] scheme] isEqualToString:@"about"])
		...
		if ([[[request URL] absoluteString] isEqualToString:@"http://www.adium.im/XMPP/form"]) {
			...
			for (field in formfields) {
				...
				key = (NSString *)CFURLCreateStringByReplacingPercentEscapesUsingEncoding(
				...
				value = (NSString *)CFURLCreateStringByReplacingPercentEscapesUsingEncoding(
