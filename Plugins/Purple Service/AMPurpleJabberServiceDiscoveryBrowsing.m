...
- (IBAction)browse:(id)sender
{
	if (!rootnode) {
		...
		rootnode = [[AMPurpleJabberNode alloc]
			initWithJID:(user->domain ? [NSString stringWithUTF8String:user->domain] : nil)
				   node:nil
				   name:(user->domain ? [NSString stringWithUTF8String:user->domain] : nil)connection:gc];
	}

	AMPurpleJabberServiceDiscoveryBrowserController *browser =
		[[AMPurpleJabberServiceDiscoveryBrowserController alloc] initWithAccount:account
																purpleConnection:gc
																			node:rootnode];
