...
- (id)initWithDict:(NSDictionary *)infoDict
{
	if ((self = [super init])) {
		...
		// Message or message header may be in HTML. If it's plain text, we'll just be getting an attributed string out
		// of this.
 ...
 * AILocalizedString(@"Do you want to allow or disallow people on your buddy list to send you MSN Mobile pages to your
 * cell phone or other mobile device?", nil) AILocalizedString(@"Allow","Button title to allow an action")
...
- (void)processBuddyListSynchronizationForTitle:(NSString **)title message:(NSString **)message
{
	/*
	 ...
	 message is ("%s on the local list is inside the group \"%s\" but not on the server list. Do you want this buddy to
	be added?")
	...
	*title = [NSString stringWithFormat:AILocalizedString(@"Contact list synchronization error for %@", nil),
										[*title substringFromIndex:NSMaxRange(startRange)]];

	startRange = [*message rangeOfString:@" "];
	*message = [NSString stringWithFormat:AILocalizedString(@"%@ is on your local contact list but not on the server "
															@"list. Do you want to add this contact to your list?",
															nil),
										  [*message substringToIndex:startRange.location]];
}
...
- (NSDictionary *)translatedInfoDict:(NSDictionary *)inDict
{
	...
	if (title && [title rangeOfString:@"Buddy list synchronization issue in"].location != NSNotFound) {
		...
		[translatedDict setObject:title forKey:@"TitleString"];
		[translatedDict setObject:message forKey:@"Message"];

	} else {
		// Replace each string with a translated version if possible
		[translatedDict setObject:[thisBundle localizedStringForKey:title value:title table:nil] forKey:@"TitleString"];
		[translatedDict setObject:[thisBundle localizedStringForKey:message value:message table:nil] forKey:@"Message"];
	}

	enumerator = [[inDict objectForKey:@"Button Names"] objectEnumerator];
	while ((buttonName = [enumerator nextObject])) {
		[buttonNamesArray addObject:[thisBundle localizedStringForKey:buttonName value:buttonName table:nil]];
	}
	[translatedDict setObject:buttonNamesArray forKey:@"Button Names"];
