 ...
 */
+ (BOOL)performDnsRequestWithData:(PurpleDnsQueryData *)inData
					   resolvedCB:(PurpleDnsQueryResolvedCallback)inResolved
						 failedCB:(PurpleDnsQueryFailedCallback)inFailed
{
		  ...
		  failedCB:(PurpleDnsQueryFailedCallback)failed
{
	...
	// Retained by dictionary reference under ARC; released when removed from dictionary
...
- (void)lookupSucceededWithAddresses:(NSArray *)addresses
{
	...
	AILog(@"DNS resolve complete for %s:%d; %lu addresses returned", purple_dnsquery_get_host(query_data),
		  purple_dnsquery_get_port(query_data), (unsigned long)[addresses count]);
...
- (void)lookupFailedWithError:(const CFStreamError *)streamError
{
	AILogWithSignature(@"Failed lookup for %s. Error domain %lu, error %i", purple_dnsquery_get_host([self queryData]),
					   (long)(streamError ? streamError->domain : 0), (streamError ? streamError->error : 0));
	...
	g_snprintf(message, sizeof(message), _("Error resolving %s:\n%s"), purple_dnsquery_get_host(query_data),
			   (streamError ? gai_strerror(streamError->error) : _("Unknown")));
...
- (BOOL)startLookup
{
	...
	AILogWithSignature(@"Performing DNS resolve: %s:%d", purple_dnsquery_get_host(query_data),
					   purple_dnsquery_get_port(query_data));

	host = CFHostCreateWithName(
		kCFAllocatorDefault,
...
- (void)_finishDnsRequest
{
	...
	// Under ARC, released when removed from the dictionary
}
...
gboolean adiumPurpleDnsRequestResolve(PurpleDnsQueryData *query_data, PurpleDnsQueryResolvedCallback resolved_cb,
									  PurpleDnsQueryFailedCallback failed_cb)
{
...
static PurpleDnsQueryUiOps adiumPurpleDnsRequestOps = {adiumPurpleDnsRequestResolve, adiumPurpleDnsRequestDestroy,
