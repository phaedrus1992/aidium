...
- (void)didConnect
{
	...
	for (__strong NSString *command in [[self preferenceForKey:KEY_IRC_COMMANDS group:GROUP_ACCOUNT_STATUS]
		...
		if (command.length) {
			...
			if (cmdStatus == PURPLE_CMD_STATUS_NOT_FOUND) {
				...
				AILogWithSignature(@"Command (%@) failed: %d - %@", command, cmdStatus,
...
- (NSString *)encodedAttributedStringForSendingContentMessage:(AIContentMessage *)inContentMessage
{
	...
	BOOL hasSlashMe =
		([messageString rangeOfString:@"/me " options:(NSCaseInsensitiveSearch | NSAnchoredSearch)].location == 0);
	 ...
	 */
	BOOL hasSlashSay =
		([messageString rangeOfString:@"/say " options:(NSCaseInsensitiveSearch | NSAnchoredSearch)].location == 0);
	...
	if (!didCommand || hasSlashMe) {
		...
		/* If we're sending a message on an encrypted direct msg, we can encode the HTML normally, as links will go
		 * through fine. However, in all other cases, IRC will drop the title of any link, so we preprocess it to be in
		 * the form "title (link)"
		 */
		...
		if (hasSlashSay)
			messageAttributedString = [messageAttributedString
				attributedSubstringFromRange:NSMakeRange([@"/say " length],
														 messageAttributedString.length - [@"/say " length])];

		encodedString =
			[AIHTMLDecoder encodeHTML:(inContentMessage.chat.isSecure
...
- (void)identifyForName:(NSString *)name password:(NSString *)inPassword
{
	if ([self.host rangeOfString:@"quakenet" options:NSCaseInsensitiveSearch].location != NSNotFound) {
		[self
			sendRawCommand:[NSString stringWithFormat:@"PRIVMSG Q@CServe.quakenet.org :AUTH %@ %@", name, inPassword]];
	} else if ([self.host rangeOfString:@"undernet" options:NSCaseInsensitiveSearch].location != NSNotFound) {
		[self sendRawCommand:[NSString
								 stringWithFormat:@"PRIVMSG X@channels.undernet.org :LOGIN %@ %@", name, inPassword]];
	} else if ([self.host rangeOfString:@"gamesurge" options:NSCaseInsensitiveSearch].location != NSNotFound) {
		[self sendRawCommand:[NSString stringWithFormat:@"PRIVMSG AuthServ@Services.GameSurge.net :AUTH %@ %@", name,
														inPassword]];
	} else {
...
BOOL contactUIDIsServerContact(NSString *contactUID)
{
	return (([contactUID caseInsensitiveCompare:@"nickserv"] == NSOrderedSame) ||
			...
			([contactUID rangeOfString:@"-connect"
							   options:(NSBackwardsSearch | NSCaseInsensitiveSearch | NSAnchoredSearch)]
				 .location != NSNotFound));
}
...
{
	NSString *ourUID =
		[NSString stringWithUTF8String:purple_normalize(self.purpleAccount, [self.displayName UTF8String])];
...
- (BOOL)validateMenuItem:(NSMenuItem *)menuItem
{
	...
	case AIRequiresHalfop:
		return (anySelected &&
				((flags & AIGroupChatOp) == AIGroupChatOp || (flags & AIGroupChatHalfOp) == AIGroupChatHalfOp));
		break;
	...
	for (NSUInteger x = 0; x < objects.count; x++) {
		...
		if ((x + 1) % 4 == 0 || x + 1 == objects.count) {
			if ([operation isEqualToString:@"MODE"]) {
				[self sendRawCommand:[NSString stringWithFormat:@"MODE %@ %@%@ %@", chat.name, (apply ? @"+" : @"-"),
																[@"" stringByPaddingToLength:(x + 1) % 4 ?: 4
																				  withString:flag
																			 startingAtIndex:0],
																names]];
			} else if ([operation isEqualToString:@"KICK"]) {
				[self sendRawCommand:[NSString stringWithFormat:@"KICK %@ %@", chat.name,
																[names stringByReplacingOccurrencesOfString:@" "
																								 withString:@","]]];
			}
