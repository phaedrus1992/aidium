...
- (void)_receivedMessage:(NSAttributedString *)attributedMessage
				  inChat:(AIChat *)chat
		 fromListContact:(AIListContact *)sourceContact
...
- (void)updateContact:(AIListContact *)theContact toAlias:(NSString *)purpleAlias
{
	if (![[purpleAlias compactedString] isEqualToString:[theContact.UID compactedString]]) {
		...
		[theContact setServersideAlias:purpleAlias silently:silentAndDelayed];

	} else {
		// If it's the same characters as the UID, apply it as a formatted UID
		if (![purpleAlias isEqualToString:theContact.formattedUID] && ![purpleAlias isEqualToString:theContact.UID]) {
			[theContact setFormattedUID:purpleAlias notify:NotifyLater];
...
{
	[theContact setOnline:YES notify:NotifyLater silently:silentAndDelayed];
...
{
	[theContact setOnline:NO notify:NotifyLater silently:silentAndDelayed];
...
{
	[theContact setSignonDate:signonDate notify:NotifyLater];
...
{
	[theContact setStatusWithName:statusName statusType:[statusTypeNumber intValue] notify:NotifyLater];
	[theContact setStatusMessage:inStatusMessage notify:NotifyLater];
...
{
	[theContact setIdle:NO sinceDate:nil notify:NotifyLater];
...
{
	[theContact setWarningLevel:[evilNumber integerValue] notify:NotifyLater];
...
{
	[theContact setServersideIconData:nil notify:NotifyLater];
...
{
	[NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(clearIconForContact:) object:theContact];
	if (userIconData) {
		[theContact setServersideIconData:userIconData notify:NotifyLater];
		 ...
		 * We therefore cancel empty icon updates when we receive a new icon, and we do the actual clearing on a delay
		 * in case this is what is about to happen.
		 */
		[self performSelector:@selector(clearIconForContact:) withObject:theContact afterDelay:10.0];
	}
}
...
{
	return [NSString stringWithFormat:NSLocalizedString(@"View %@'s %@ web profile", nil), contact.formattedUID,
									  [contact.service shortDescription]];
}
													 ...
													 forContact:(AIListContact *)contact
{
	...
	for (l = purple_notify_user_info_get_entries(user_info); l != NULL; l = l->next) {
		...
		case PURPLE_NOTIFY_USER_INFO_ENTRY_SECTION_HEADER:
			[array addObject:[NSDictionary
								 dictionaryWithObjectsAndKeys:
									 [NSString
										 stringWithUTF8String:purple_notify_user_info_entry_get_label(user_info_entry)],
									 KEY_KEY, [NSNumber numberWithInteger:AIUserInfoSectionHeader], KEY_TYPE, nil]];

			break;
		case PURPLE_NOTIFY_USER_INFO_ENTRY_SECTION_BREAK:
			[array addObject:[NSDictionary
								 dictionaryWithObjectsAndKeys:[NSNumber numberWithInteger:AIUserInfoSectionBreak],
															  KEY_TYPE, nil]];
			break;
		...
		case PURPLE_NOTIFY_USER_INFO_ENTRY_PAIR: {
			if (purple_notify_user_info_entry_get_label(user_info_entry) &&
				...
				[array addObject:[NSDictionary
									 dictionaryWithObjectsAndKeys:
										 [NSString stringWithUTF8String:purple_notify_user_info_entry_get_label(
																			user_info_entry)],
										 KEY_KEY,
										 processPurpleImages(
											 [NSString stringWithUTF8String:purple_notify_user_info_entry_get_value(
																				user_info_entry)],
											 self),
										 KEY_VALUE, nil]];
				...
				[array
					addObject:[NSDictionary
								  dictionaryWithObject:[NSString
														   stringWithUTF8String:purple_notify_user_info_entry_get_label(
																					user_info_entry)]
												forKey:KEY_KEY]];
			} else if (purple_notify_user_info_entry_get_value(user_info_entry)) {
				NSMutableString *value = [processPurpleImages(
					[NSString stringWithUTF8String:purple_notify_user_info_entry_get_value(user_info_entry)], self)
					mutableCopy];
				[value replaceOccurrencesOfString:@"<br>"
									   withString:@"<br/>"
				...
				for (NSString *valuePair in [value componentsSeparatedByString:@"<br/><b>"]) {
					...
					if (firstEndBold.length > 0) {
						...
						[array
							addObject:[NSDictionary
										  dictionaryWithObjectsAndKeys:
											  [valuePair substringWithRange:NSMakeRange(firstStartBold.length,
																						firstEndBold.location -
																							firstStartBold.length - 1)],
											  KEY_KEY, [valuePair substringFromIndex:NSMaxRange(firstEndBold)],
											  KEY_VALUE, nil]];
					} else {
						[array addObject:[NSDictionary dictionaryWithObject:valuePair forKey:KEY_VALUE]];
					}
				}
				[value release];
			}
	...
	for (i = 0; i < count; i++) {
		...
		if (value && [value rangeOfString:webProfileValue
...
- (void)updateUserInfo:(AIListContact *)theContact withData:(PurpleNotifyUserInfo *)user_info
{
	...
	[theContact setProfileArray:profileContents notify:NotifyLater];
 ...
 *	- For a contact as it is deleted by Purple (e.g. when Sametime refuses an addition because it is known to be
 * invalid)
...
{
	[adium.contactController requestAddContactWithUID:contactUID service:[self _serviceForUID:contactUID] account:self];
}
...
- (NSData *)serversideIconDataForContact:(AIListContact *)contact
{
	...
	if (self.purpleAccount && (buddy = purple_find_buddy(account, [contact.UID UTF8String]))) {
...
- (void)addContact:(AIListContact *)contact toGroup:(AIListGroup *)group
{
	...
	if (![group containsObject:contact]) {
		...
		NSString *alias = [contact.parentContact preferenceForKey:@"Alias" group:PREF_GROUP_ALIASES];
...
- (void)moveListObjects:(NSArray *)objects fromGroups:(NSSet *)oldGroups toGroups:(NSSet *)groups
{
	...
	for (AIListContact *contact in objects) {
		...
		[purpleAdapter moveUID:contact.UID
					 onAccount:self
					fromGroups:sourceMappedNames
					  toGroups:destinationMappedNames
					 withAlias:alias];
...
- (void)authorizationWithDict:(NSDictionary *)infoDict response:(AIAuthorizationResponse)authorizationResponse
{
	if (account) {
		...
		// libpurple will remove its reference to the handle for this request, which is inDict, in response to this
		// callback invocation
		if (callback) {
			[purpleAdapter doAuthRequestCbValue:callback
							  withUserDataValue:[[[infoDict objectForKey:@"userData"] retain] autorelease]];

			/* Retained in -[self authorizationRequestWithDict:].  We kept it around before now in case libpurle wanted
			 * us to close it early, such as because the account disconnected.
			 */
			[infoDict release];
		} else {
...
- (void)removeUser:(NSString *)contactName fromChat:(AIChat *)chat
{
	...
	if (contact.isStranger && ![adium.chatController allGroupChatsContainingContact:contact.parentContact].count &&
		...
		[contact setStatusWithName:nil statusType:AIOfflineStatusType notify:NotifyLater];

		[contact setValue:nil forProperty:@"isOnline" notify:NotifyLater];
		...
		attributes:(NSDictionary *)attributes
{
	...
	if ((alias && !oldAlias) || (!alias && oldAlias) || ![[chat aliasForContact:contact] isEqualToString:alias] ||
	...
	if (contact.isStranger) {
		[contact setStatusWithName:nil
						statusType:((newFlags & AIGroupChatAway) == AIGroupChatAway) ? AIAwayStatusType
...
- (BOOL)rejoinChat:(AIChat *)chat
{
	 ...
	 * Clear the identifier so a new PurpleConversation will be made. The ChatCreationInfo for the chat is still around,
	 * so it can join.
	 */
...
{
	AILog(@"%@ needs an implementation of extractChatCreationDictionaryFromConversation to handle rejoins, bookmarks, "
		  @"and invitations properly",
...
{
	[self setTypingFlagOfChat:chat to:typingState];
}
...
{
	[[chat displayArrayForKey:@"Display Name"] setObject:inTitle withOwner:self];
}
...
- (void)_receivedMessage:(NSAttributedString *)attributedMessage
		 ...
		 fromListContact:(AIListContact *)sourceContact
				   flags:(PurpleMessageFlags)flags
					date:(NSDate *)date
{
	AILogWithSignature(@"Message: %@ inChat: %@ fromListContact: %@ flags: %d date: %@", attributedMessage, chat,
					   sourceContact, flags, date);

	if ((flags & PURPLE_MESSAGE_DELAYED) == PURPLE_MESSAGE_DELAYED) {
		// Display delayed messages as context.

		AIContentContext *messageObject =
			[AIContentContext messageInChat:chat
								 withSource:[sourceContact.UID isEqualToString:self.UID] ? (AIListObject *)self
																						 : (AIListObject *)sourceContact
								destination:self
									   date:date
...
{
	return (account ? ((PURPLE_PLUGIN_PROTOCOL_INFO(purple_find_prpl(purple_account_get_protocol_id(account)))
							->send_attention) != NULL)
					: NO);
}
...
- (BOOL)allowFileTransferWithListObject:(AIListObject *)inListObject
{
	...
	if (prpl_info && prpl_info->send_file)
		return (!prpl_info->can_receive_file ||
				prpl_info->can_receive_file(purple_account_get_connection(account), [inListObject.UID UTF8String]));
	else
		return NO;
}
...
- (void)chat:(AIChat *)inChat isWaitingOnCustomEmoticon:(NSString *)emoticonEquivalent
{
	...
	for (emoticon in inChat.customEmoticons) {
		if ([[emoticon textEquivalents] containsObject:emoticonEquivalent])
			break;
	}
 ...
 */
- (NSString *)_emoticonCachePathForEmoticon:(NSString *)emoticonEquivalent
									   type:(AIBitmapImageFileType)fileType
									 inChat:(AIChat *)inChat
{
	static unsigned long long emoticonID = 0;
	NSString *filename =
		[NSString stringWithFormat:@"TEMP-CustomEmoticon_%@_%@_%qu.%@", [inChat uniqueChatID], emoticonEquivalent,
								   emoticonID++, [NSImage extensionForBitmapImageFileType:fileType]];
...
- (void)chat:(AIChat *)inChat setCustomEmoticon:(NSString *)emoticonEquivalent withImageData:(NSData *)inImageData
{
	...
	NSString *path = [self _emoticonCachePathForEmoticon:emoticonEquivalent
												  ...
												  inChat:inChat];
	[inImageData writeToFile:path atomically:NO];
...
- (void)setPrivacyOptions:(AIPrivacyOption)option
{
	if (account && purple_account_get_connection(account)) {
		...
		if (account->perm_deny != privacyType) {
			...
			AILog(@"Set privacy options for %@ (%p %p) to %i", self, account, purple_account_get_connection(account),
				  account->perm_deny);
...
- (void)connect
{
	...
	if ([self connectivityBasedOnNetworkReachability] && ![self.host length]) {
		// If we use the network for connectivity, and we don't have a host, we need to get ourselves one. Prompt for
		// it!
		[self promptForHostBeforeConnecting];
	} else {
		[self configurePurpleAccountNotifyingTarget:self
										   selector:@selector(continueConnectWithConfiguredPurpleAccount)];
	}
}
...
{
	if (host && *host) {
		[self setPreference:[NSString stringWithUTF8String:host] forKey:KEY_CONNECT_HOST group:GROUP_ACCOUNT_STATUS];
	...
	purple_request_input(
		...
		[[NSString
			stringWithFormat:AILocalizedString(
								 @"%@ (%@) Setup",
								 "first %@ is an account name; second is a service. This is a title for a window"),
							 self.formattedUID, [self.service shortDescription]] UTF8String],
		[AILocalizedString(@"No Server Specified", nil) UTF8String],
		[[NSString
			stringWithFormat:
				AILocalizedString(
					@"No server has been configured for the %@ account %@. Please enter one below to connect", nil),
				[self.service longDescription], self.formattedUID] UTF8String],
		/* default value */ "", /* multiline */ FALSE, /* masked */ FALSE, /* hint */ NULL,
		[AILocalizedString(@"Connect", "Button title to connect; this is a verb") UTF8String],
		G_CALLBACK(prompt_host_ok_cb), [AILocalizedString(@"Cancel", nil) UTF8String],
		G_CALLBACK(prompt_host_cancel_cb),
		/* account */ NULL, /* who */ NULL, /* conv */ NULL, self);
}
...
- (void)continueConnectWithConfiguredProxy
{
	...
	// Set our current status state after filtering its statusMessage as appropriate. This will take us online in the
	// process.
...
- (void)configurePurpleAccountNotifyingTarget:(id)target selector:(SEL)selector
{
	...
	// connect occurs. Once that's done, contextInvocation will be invoked, continuing the configurePurpleAccount
	// process.
 ...
 *
 * target/selector are used rather than a hardcoded callback (or getProxyConfigurationNotifyingTarget: directly) because
 * this allows code reuse between the connect and register processes, which are similar in their need for proxy
 * configuration
 */
...
- (void)accountConnectionProgressStep:(NSNumber *)step percentDone:(NSNumber *)connectionProgressPrecent
{
	...
	[self setValue:[NSNumber numberWithDouble:[connectionProgressPrecent doubleValue] * 100]
		forProperty:@"connectionProgressPercent"
			 notify:NO];
...
- (void)createNewPurpleAccount
{
	...
	//-[SLPurpleCocoaAdapter addAdiumAccount:] should have immediately called back on setPurpleAccount. It's bad if it
	// didn't.
	if (account) {
		AILog(@"Created PurpleAccount 0x%p with UID %@ and protocolPlugin %s", account, self.UID,
			  [self protocolPlugin]);
	} else {
		AILog(@"Unable to create Libpurple account with name %s and protocol plugin %s", self.purpleAccountName,
			  [self protocolPlugin]);
		NSLog(@"Unable to create Libpurple account with name %s and protocol plugin %s", self.purpleAccountName,
			  [self protocolPlugin]);
	}
}
...
- (void)accountConnectionNotice:(NSString *)connectionNotice
{
	[adium.interfaceController
		handleErrorMessage:[NSString stringWithFormat:AILocalizedString(@"%@ (%@) : Connection Notice", nil),
													  self.formattedUID, [service description]]
...
- (AIReconnectDelayType)shouldAttemptReconnectAfterDisconnectionError:(NSString **)disconnectionError
{
	...
	if ([self lastDisconnectionReason] == PURPLE_CONNECTION_ERROR_AUTHENTICATION_FAILED) {
		...
		[self setLastDisconnectionError:
				  AILocalizedString(@"The name you entered is not registered. Check to ensure you typed it correctly.",
									nil)];
		...
		/* This particular message comes with PURPLE_CONNECTION_ERROR_ENCRYPTION_ERROR, which is a 'fatal' error
		 * according to libpurple. Other problems with that message may be fatal, but this one isn't.
		 */
...
- (void)purpleAccountRegistered:(BOOL)success
{
	if (success && [self.service accountViewController]) {
		NSString *username = (NSString *)(purple_account_get_username(account)
											  ? [NSString stringWithUTF8String:purple_account_get_username(account)]
											  : [NSNull null]);
		NSString *pw = (NSString *)(purple_account_get_password(account)
										? [NSString stringWithUTF8String:purple_account_get_password(account)]
										: [NSNull null]);

		[[NSNotificationCenter defaultCenter]
			postNotificationName:AIAccountUsernameAndPasswordRegisteredNotification
						  object:self
						userInfo:[NSDictionary
									 dictionaryWithObjectsAndKeys:username, @"username", pw, @"password", nil]];
	}
}
...
- (NSSet *)supportedPropertyKeys
{
	...
	if (!supportedPropertyKeys) {
		supportedPropertyKeys =
			[[NSMutableSet alloc] initWithObjects:@"idleSince", @"IdleManuallySet", @"textProfile",
												  @"DefaultUserIconFilename", KEY_ACCOUNT_CHECK_MAIL, nil];
...
- (void)updateStatusForKey:(NSString *)key
{
	...
	if (account) {
		...
		if ([key isEqualToString:@"idleSince"]) {
			...
			// Update the mail checking setting if the account is already made (if it isn't, we'll set it when it is
			// made)
			if (account) {
				[purpleAdapter setCheckMail:[self shouldCheckMail] forAccount:self];
			}
		}
	}
}
 ...
 * to the status message.  This method should handle any status whose statusNname this service set as well as any
 * statusName defined in  AIStatusController.h (which will correspond to the services handled by Adium by default). It
 * should also handle a status name not specified in either of these places with a sane default, most likely by loooking
 * at statusState.statusType for a general idea of the status's type.
 *
...
- (NSDictionary *)purpleSongInfoDictionary
{
	...
	if (tuneinfo && [[tuneinfo objectForKey:KEY_ITUNES_PLAYER_STATE] isEqualToString:@"Playing"]) {
		...
		[arguments setObject:([tuneinfo objectForKey:KEY_ITUNES_ALBUM] ? [tuneinfo objectForKey:KEY_ITUNES_ALBUM] : @"")
					  forKey:[NSString stringWithUTF8String:PURPLE_TUNE_ALBUM]];
		[arguments setObject:([tuneinfo objectForKey:KEY_ITUNES_GENRE] ? [tuneinfo objectForKey:KEY_ITUNES_GENRE] : @"")
					  forKey:[NSString stringWithUTF8String:PURPLE_TUNE_GENRE]];
		[arguments
			setObject:([tuneinfo objectForKey:KEY_ITUNES_TOTAL_TIME] ? [tuneinfo objectForKey:KEY_ITUNES_TOTAL_TIME]
																	 : [NSNumber numberWithInteger:-1])
			   forKey:[NSString stringWithUTF8String:PURPLE_TUNE_TIME]];
		[arguments setObject:([tuneinfo objectForKey:KEY_ITUNES_YEAR] ? [tuneinfo objectForKey:KEY_ITUNES_YEAR]
																	  : [NSNumber numberWithInteger:-1])
					  forKey:[NSString stringWithUTF8String:PURPLE_TUNE_YEAR]];
		[arguments
			setObject:([tuneinfo objectForKey:KEY_ITUNES_STORE_URL] ? [tuneinfo objectForKey:KEY_ITUNES_STORE_URL]
																	: @"")
			   forKey:[NSString stringWithUTF8String:PURPLE_TUNE_URL]];

		[arguments setObject:[NSString stringWithFormat:@"%@%@%@", (name ? name : @""), (name && artist ? @" - " : @""),
														(artist ? artist : @"")]
...
- (void)iTunesDidUpdate:(NSNotification *)notification
{
	...
	[purpleAdapter
		setSongInformation:(shouldIncludeNowPlayingInformationInAllStatuses ? [self purpleSongInfoDictionary] : nil)
				 onAccount:self];
}
...
- (void)setStatusState:(AIStatus *)statusState usingStatusMessage:(NSAttributedString *)inStatusMessage
{
	...
	// Get the purple status type from this class or subclasses, which may also potentially modify or nullify our
	// statusMessage
	const char *statusID = [self purpleStatusIDForStatus:statusState arguments:arguments];

	if (![inStatusMessage length] && (statusState.statusType == AIAwayStatusType) && statusState.statusName &&
		(!statusID || ((strcmp(statusID, "away") == 0) && [self shouldSetStatusMessageForDefaultAwayState]))) {
		/* If we don't have a status message, and the status type is away for a non-default away such as "Do Not
		 * Disturb", and we're only setting a default away state becuse we don't know a better one for this service, get
		 * a default description of this away state. This allows, for example, an AIM user to set the "Do Not Disturb"
		 * type provided by her ICQ account and have the away message be set appropriately.
		 */
		inStatusMessage =
			[NSAttributedString stringWithString:[adium.statusController descriptionForStateOfStatus:statusState]];
	}
...
#define MUSICAL_NOTE_AND_SPACE [NSString stringWithUTF8String:"\xe2\x99\xab "]
			...
			temporaryStatusMessage =
				[[[NSMutableAttributedString alloc] initWithString:MUSICAL_NOTE_AND_SPACE] autorelease];
		...
		if ([self shouldSetITMSLinkForNowPlayingStatus]) {
			...
			if (itmsStoreLink) {
				[arguments setObject:itmsStoreLink forKey:@"itmsurl"];
			}
		}
	...
	encodedStatusMessage =
		(inStatusMessage ? [self encodedAttributedString:inStatusMessage forStatusState:statusState] : nil);
	if (encodedStatusMessage) {
		[arguments setObject:encodedStatusMessage forKey:@"message"];
	}
 ...
 * @param isActive An NSNumber with a bool YES if we are activating (going to) the passed state, NO if we are
 * deactivating (going away from) the passed state.
...
{
	[purpleAdapter setStatusID:statusID isActive:isActive arguments:arguments onAccount:self];
}
...
- (void)setAccountIdleSinceTo:(NSDate *)idleSince
{
	...
	[self setValue:([idleSince timeIntervalSinceNow] ? idleSince : nil) forProperty:@"idleSince" notify:NotifyNow];
}
	...
	if (account) {
		...
		if (image && !NSEqualSizes(NSZeroSize, imageSize)) {
			...
			if (prpl_info && (prpl_info->icon_spec.format)) {
				...
				prplScales = (prpl_info->icon_spec.scale_rules & PURPLE_ICON_SCALE_SEND) ||
				...
				if (!buddyIconData) {
					...
					NSImageRep *imageRep =
						[image bestRepresentationForRect:NSMakeRect(0, 0, imageSize.width, imageSize.height)
												 context:nil
												   hints:nil];
					...
					if ([imageRep isKindOfClass:[NSBitmapImageRep class]] &&
						...
						for (i = 0; prpl_formats[i]; i++) {
							if (strcmp(prpl_formats[i], "gif") == 0) {
								...
								AILog(@"%@: Trying to use original GIF data, %lu bytes", self,
									  (unsigned long)[buddyIconData length]);
								...
								while ([buddyIconData length] > maxFileSize &&
									   (newSize.width > 42.0f && newSize.height > 42.0f)) {
								...
								if ([buddyIconData length] > maxFileSize) {
									...
									AILog(@"%@: GIF too large, use a still JPEG of %lu bytes", self,
										  (unsigned long)[buddyIconData length]);
								} else {
									AILog(@"%@: Resized GIF, new file size %lu!", self,
										  (unsigned long)[buddyIconData length]);
								}
					...
					if (!buddyIconData) {
						for (i = 0; prpl_formats[i]; i++) {
							...
							} else if ((strcmp(prpl_formats[i], "jpeg") == 0) ||
									   (strcmp(prpl_formats[i], "jpg") == 0)) {
								buddyIconData = [image JPEGRepresentationWithCompressionFactor:1.0f];
								if (buddyIconData)
									break;

							} else if ((strcmp(prpl_formats[i], "tiff") == 0) ||
									   (strcmp(prpl_formats[i], "tif") == 0)) {
						...
						if (maxFileSize > 0 && ([buddyIconData length] > maxFileSize)) {
							AILog(@"%@: Image %lu is larger than %lu!", self, (unsigned long)[buddyIconData length],
 ...
 *
 * Adds the menu item for act to a growing array of NSMenuItems.  If act has children (a submenu), this method is used
 * recursively to generate the submenu containing each child menu item.
 */
- (void)addMenuItemForMenuAction:(PurpleMenuAction *)act
				  forListContact:(AIListContact *)inContact
					 purpleBuddy:(PurpleBuddy *)buddy
						 toArray:(NSMutableArray *)menuItemArray
				 withServiceIcon:(NSImage *)serviceIcon
{
	NSDictionary *dict;
	NSMenuItem *menuItem;
	NSString *title;
	...
	if (act && act->label && (title = [self titleForContactMenuLabel:act->label forContact:inContact])) {
		...
		if (act->data) {
			dict = [NSDictionary
				dictionaryWithObjectsAndKeys:[NSValue valueWithPointer:act->callback], @"PurpleMenuActionCallback",
											 /* act->data may be freed by purple_menu_action_free() before we use it,
												I'm afraid... */
											 [NSValue valueWithPointer:act->data], @"PurpleMenuActionData",
											 [NSValue valueWithPointer:buddy], @"PurpleBuddy", nil];
		} else {
			dict = [NSDictionary dictionaryWithObjectsAndKeys:[NSValue valueWithPointer:act->callback],
															  @"PurpleMenuActionCallback",
															  [NSValue valueWithPointer:buddy], @"PurpleBuddy", nil];
		}
...
- (NSString *)titleForContactMenuLabel:(const char *)label forContact:(AIListContact *)inContact
{
	if ((strcmp(label, _("Initiate Chat")) == 0) || (strcmp(label, _("Initiate _Chat")) == 0)) {
		return [NSString
			stringWithFormat:AILocalizedString(@"Initiate Multiuser Chat with %@", nil), inContact.formattedUID];
...
- (NSArray *)accountActionMenuItems
{
		...
		if (PURPLE_PLUGIN_HAS_ACTIONS(plugin)) {
			...
			for (l = actions; l; l = l->next) {
				...
				if (l->data) {
					...
					if (action && action->label && (title = [self titleForAccountActionMenuLabel:action->label])) {
						...
						dict = [NSDictionary dictionaryWithObjectsAndKeys:[NSValue valueWithPointer:action->callback],
																		  ...
																		  @"PurplePluginActionCallbackUserData", nil];
	...
	if ([self encrypted] && [self secureConnection]) {
		...
		NSMenuItem *showCertificateMenuItem =
			[[[NSMenuItem alloc] initWithTitle:AILocalizedString(@"Show Server Certificate", nil)
										target:self
...
- (NSString *)titleForAccountActionMenuLabel:(const char *)label
{
	if ((strcmp(label, _("Change Password...")) == 0) || (strcmp(label, _("Change Password")) == 0)) {
		return [AILocalizedString(@"Change Password", "Menu item title for changing the password of an account")
...
- (void)initAccount
{
	NSDictionary *defaults =
		[NSDictionary dictionaryNamed:[NSString stringWithFormat:@"PurpleDefaults%@", self.service.serviceID]
							 forClass:[self class]];

	if (defaults) {
		[adium.preferenceController registerDefaults:defaults forGroup:GROUP_ACCOUNT_STATUS object:self];
	} else {
		AILog(@"Failed to load defaults for %@",
			  [NSString stringWithFormat:@"PurpleDefaults%@", self.service.serviceID]);
	}
...
/*!
 * @brief The account will be deleted, we should ask the user for confirmation. If the prpl supports it, we can also
 * remove the account from the server (if the user wants us to do that)
 */
- (NSAlert *)alertForAccountDeletion
{
	...
	if (prpl_info && prpl_info->unregister_user && [self allowAccountUnregistrationIfSupportedByLibpurple]) {
		return [NSAlert
				 alertWithMessageText:AILocalizedString(@"Delete Account", nil)
...
- (void)alertForAccountDeletion:(id<AIAccountControllerRemoveConfirmationDialog>)dialog didReturn:(NSInteger)returnCode
{
	...
	if (prpl_info && prpl_info->unregister_user) {
		switch (returnCode) {
		case NSAlertOtherReturn:
			// delete & unregister
			if (NSRunAlertPanel(AILocalizedString(@"Delete Account from Server", nil),
								AILocalizedString(@"WARNING! This will delete the account %@ from the Jabber server, "
												  @"and can not be undone.\nAre you sure you want to proceed?",
												  nil),
								AILocalizedString(@"Cancel", nil), AILocalizedString(@"Delete & Unregister", nil), nil,
								([self.formattedUID length] ? self.formattedUID : NEW_ACCOUNT_DISPLAY_TEXT)) ==
...
- (void)unregisteredAccount:(BOOL)success
{
	if (success) {
		 ...
		 */
		[self performSelector:@selector(performDelete) withObject:nil afterDelay:0];
	}
}
						 ...
						 firstTime:(BOOL)firstTime
{
	...
	if ([group isEqualToString:PREF_GROUP_ALIASES]) {
		...
		if ((account != nil) && ([self shouldSetAliasesServerside]) && ([key isEqualToString:@"Alias"])) {

			NSString *alias = [object preferenceForKey:@"Alias" group:PREF_GROUP_ALIASES];
		...
		shouldIncludeNowPlayingInformationInAllStatuses = [[self preferenceForKey:KEY_BROADCAST_MUSIC_INFO
...
- (void)setTypingFlagOfChat:(AIChat *)chat to:(NSNumber *)typingStateNumber
{
	...
	if (currentTypingState != newTypingState) {
		if (newTypingState == AITyping && openPsychicChats && ![chat isOpen]) {
			...
			/*
			 * Use the Libpurple "psychic" tagline. If this is found to be confusing, we should switch to your own
			 * version. The upside of using theirs is that clever gimmicky translations already exist.
			 */
			NSMutableString *forceString =
				[[NSString stringWithUTF8String:_("You feel a disturbance in the force...")] mutableCopy];
			[forceString replaceOccurrencesOfString:@"..." withString:[NSString ellipsis] options:NSLiteralSearch];
			AIContentEvent *newStatusMessage =
				[AIContentEvent eventInChat:chat
		...
		[chat setValue:(newTypingState ? typingStateNumber : nil) forProperty:KEY_TYPING notify:NotifyNow];
	}
}
