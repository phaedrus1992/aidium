									 ...
									 PurpleMessageFlags flags, time_t mtime)
{
	...
	AILog(@"Source: %s \t Name: %s \t MyNick: %s : Message %@", who, purple_conversation_get_name(conv),
		  purple_conv_chat_get_nick(PURPLE_CONV_CHAT(conv)), messageString);
	...
	if ((flags & PURPLE_MESSAGE_SYSTEM) == PURPLE_MESSAGE_SYSTEM || !who) {
		...
		if (normalizedUID.length) {
			messageDict = [NSDictionary dictionaryWithObjectsAndKeys:attributedMessage, @"AttributedMessage",
																	 normalizedUID, @"Source", purpleMessageFlags,
	...
	if ((flags & PURPLE_MESSAGE_SEND) == 0) {
		if (flags & PURPLE_MESSAGE_NOTIFY) {
			...
			[[NSNotificationCenter defaultCenter]
				postNotificationName:Chat_NudgeBuzzOccured
									 ...
									 PurpleMessageFlags flags, time_t mtime)
{
	...
	if (flags & PURPLE_MESSAGE_ERROR) {
		if ([messageString rangeOfString:@"User information not available"].location != NSNotFound) {
			...
			// XXX The user info check only works in English; libpurple should be modified to be better about this
			// useless information spamming
			[pool drain];
			return;
		}
		...
		} else if ([messageString rangeOfString:[NSString stringWithUTF8String:_("Command failed")]].location !=
				   NSNotFound) {
			errorType = AIChatCommandFailed;
		} else if ([messageString rangeOfString:[NSString stringWithUTF8String:_("Wrong number of arguments")]]
					   .location != NSNotFound) {
		 ...
		 * screenname... but I don't know when else it is sent. -evands
		 */
		...
		if (strcmp(message, _("Direct IM established")) == 0) {
			...
			if (isClosingDirectIM) {
				if (strcmp(message, _("The remote user has closed the connection.")) != 0) {
					...
					[adium.contentController displayEvent:messageString ofType:@"directIMDisconnected" inChat:chat];
				}

				[accountLookup(purple_conversation_get_account(conv))
					updateContact:chat.listObject
						 forEvent:[NSNumber numberWithInteger:PURPLE_BUDDY_DIRECTIM_DISCONNECTED]];
				shouldDisplayMessage = FALSE;
			}
		}
...
NSString *get_real_name_for_account_conv_buddy(PurpleAccount *account, PurpleConversation *conv, char *who)
{
	...
	if (prpl_info && prpl_info->get_cb_real_name) {
		...
		uid =
			prpl_info->get_cb_real_name(purple_account_get_connection(account), purple_conv_chat_get_id(convChat), who);
	}
										  ...
										  const char *newAlias)
{
	...
	if (purple_conversation_get_type(conv) == PURPLE_CONV_TYPE_CHAT) {
		...
		[accountLookup(purple_conversation_get_account(conv))
			renameParticipant:get_real_name_for_account_conv_buddy(account, conv, (char *)oldName)
...
static void adiumPurpleConvChatRemoveUsers(PurpleConversation *conv, GList *users)
{
	...
	if (purple_conversation_get_type(conv) == PURPLE_CONV_TYPE_CHAT) {
		...
		[accountLookup(account) removeUsersArray:usersArray fromChat:groupChatLookupFromConv(conv)];
...
static void adiumPurpleConvUpdated(PurpleConversation *conv, PurpleConvUpdateType type)
{
	...
	if (purple_conversation_get_type(conv) == PURPLE_CONV_TYPE_CHAT) {
		...
		case PURPLE_CONV_UPDATE_TOPIC: {
			...
			[accountLookup(purple_conversation_get_account(conv))
				updateTopic:(purple_conv_chat_get_topic(chat)
		...
		case PURPLE_CONV_UPDATE_TITLE:
			[accountLookup(purple_conversation_get_account(conv))
				updateTitle:(purple_conversation_get_title(conv)
		...
		case PURPLE_CONV_UPDATE_FEATURES:
			...
			/*
							[accountLookup(purple_conversation_get_account(conv))
			   mainPerformSelector:@selector(convUpdateForChat:type:) withObject:groupChatLookupFromConv(conv)
...
static PurpleConversationUiOps adiumPurpleConversationOps = {
	adiumPurpleConvCreate, adiumPurpleConvDestroy, adiumPurpleConvWriteChat, adiumPurpleConvWriteIm,
	adiumPurpleConvWriteConv, adiumPurpleConvChatAddUsers, adiumPurpleConvChatRenameUser,
	adiumPurpleConvChatRemoveUsers, adiumPurpleConvUpdateUser,

	adiumPurpleConvPresent, adiumPurpleConvHasFocus,

	/* Custom Smileys */
	adiumPurpleConvCustomSmileyAdd, adiumPurpleConvCustomSmileyWrite, adiumPurpleConvCustomSmileyClose,
...
void adiumPurpleConversation_init(void)
{
	...
	purple_signal_connect_priority(purple_conversations_get_handle(), "conversation-updated", adium_purple_get_handle(),
								   PURPLE_CALLBACK(adiumPurpleConvUpdated), NULL, PURPLE_SIGNAL_PRIORITY_LOWEST);
