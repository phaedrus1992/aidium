...
- (id)initWithPurpleConnection:(PurpleConnection *)_gc
				 ...
				 searchResults:(PurpleNotifySearchResults *)results
					  userData:(gpointer)_user_data
{
	if ((self = [super initWithWindowNibName:@"AMPurpleSearchResultsWindow"])) {
		...
		for (column = results->columns; column; column = g_list_next(column)) {
			...
			NSTableColumn *tcol =
				[[NSTableColumn alloc] initWithIdentifier:[[NSNumber numberWithUnsignedInteger:idx++] stringValue]];
...
- (IBAction)invokeAction:(id)sender
{
	PurpleNotifySearchButton *button =
		[[searchButtons objectForKey:[NSValue valueWithNonretainedObject:sender]] pointerValue];
	if (!button) {
		NSBeep();
		return;
	}
	NSInteger row = [tableview selectedRow];
	NSAssert(INT_MAX >= row, @"Holy crap that's a lot of rows!  GList can't handle that.  Abort.");
	GList *rowptr = NULL;
	if (row != -1)
