 ...
 * @brief Plugin loaded
 *
 * Initialize each libpurple plugin.  These plugins should not do anything within libpurple itself; this should be done
 * in
...
+ (void)pluginDidLoad
{
	...
	for (NSString *libpurplePluginPath in [adium allResourcesForName:@"PlugIns"
		...
		[AICorePluginLoader loadPluginAtPath:libpurplePluginPath confirmLoading:YES pluginArray:libpurplePluginArray];
	}
}
...
{
	// Note that purple_account_new() calls purple_accounts_find() first, returning an existing PurpleAccount if there
	// is one.
...
- (void)initLibPurple
{
	...
	for (NSString *domain in [NSArray
			 arrayWithObjects:@"GLib", @"GModule", @"GLib-GObject", @"GThread", @"Gnt", @"GStreamer", @"stderr", nil]) {
		g_log_set_handler([domain UTF8String], G_LOG_LEVEL_MASK | G_LOG_FLAG_FATAL | G_LOG_FLAG_RECURSION,
						  adium_glib_log, NULL);
	}
	...
	// Initialize the libpurple core; this will call back on the function specified in our core UI ops for us to finish
	...
	if (!adiumPurpleAccount && account) {
		...
		for (adiumPurpleAccount in adium.accountController.accounts) {
			if ([adiumPurpleAccount isKindOfClass:[CBPurpleAccount class]] &&
				...
				[adiumPurpleAccount.UID caseInsensitiveCompare:[NSString stringWithUTF8String:account->username]] ==
					NSOrderedSame) {
				break;
			}
		}
...
AIListContact *contactLookupFromBuddy(PurpleBuddy *buddy)
{
	...
	if (!theContact && buddy) {
		...
		UID = [NSString
			stringWithUTF8String:purple_normalize(purple_buddy_get_account(buddy), purple_buddy_get_name(buddy))];
...
AIChat *imChatLookupFromConv(PurpleConversation *conv)
{
	...
	if (!chat) {
		...
		//		AILog(@"%x purple_conversation_get_name(conv) %s; normalizes to
		//%s",account,purple_conversation_get_name(conv),purple_normalize(account,purple_conversation_get_name(conv)));
		...
		if (!buddy) {
			...
			buddy = purple_buddy_new(account, purple_normalize(account, purple_conversation_get_name(conv)),
									 NULL); // create a PurpleBuddy
		}
		...
		if (!chat) {
			...
			errorString = [NSString
				stringWithFormat:@"conv %p: Got nil chat in lookup for sourceContact %@ (%p ; \"%s\" ; \"%s\") on "
								 @"adiumAccount %@ (%p ; \"%s\")",
								 conv, sourceContact, buddy, (buddy ? purple_buddy_get_name(buddy) : ""),
								 ((buddy && purple_buddy_get_account(buddy) && purple_buddy_get_name(buddy))
									  ? purple_normalize(purple_buddy_get_account(buddy), purple_buddy_get_name(buddy))
									  : ""),
			...
			if (chatName) {
				...
				if (!purpleChat) {
					...
					for (NSString *identifier in chatCreationInfo) {
						...
						if (valueUTF8String) {
							g_hash_table_replace(components, g_strdup([identifier UTF8String]), valueUTF8String);
						}
					}
					...
					if (AIDebugLoggingIsEnabled()) {
						...
						if ((PURPLE_PLUGIN_PROTOCOL_INFO(gc->prpl))->chat_info) {
							...
							for (tmp = list; tmp; tmp = tmp->next) {
								...
								NSString *value =
									[chatCreationInfo objectForKey:[NSString stringWithUTF8String:identifier]];
								if (!value) {
									AILog(@"Danger, Will Robinson! %s is in the proto_info but can't be found in %@",
										  identifier, chatCreationInfo);
								}
					...
					/* Join the chat serverside - the GHashTable components, coupled with the originating
					 * PurpleConnection, now contains all the information the prpl will need to process our request.
					 */
...
{
	NSString *messageImageCacheFilename =
		[NSString stringWithFormat:@"TEMP-Image_%@_%i", adiumAccount.internalObjectID, imageID];
...
NSString *processPurpleImages(NSString *inString, AIAccount *adiumAccount)
{
	...
	// A purple image tag takes the form <IMG ID='12'></IMG> where 12 is the reference for use in PurpleStoredImage*
	// purple_imgstore_get(int)
	...
	while (![scanner isAtEnd]) {
		...
		if ([scanner scanString:targetString intoString:&chunkString]) {
			...
			if (purpleImage) {
				...
				// First make an NSImage, then request a TIFFRepresentation to avoid an obscure bug in the PNG writing
				// routines Exception: PNG writer requires compacted components (bits/component * components/pixel =
				// bits/pixel)
				...
				// If writing the file is successful, write an <IMG SRC="filepath"> tag to our string; the
				// 'scaledToFitImage' class lets us apply CSS to directIM images only
				if ([data writeToFile:imagePath atomically:YES]) {
					[newString
						appendString:[NSString
										 stringWithFormat:@"<IMG CLASS=\"scaledToFitImage\" SRC=\"%@\" ALT=\"%@\">",
														  imagePath, filename]];
				}
						  ...
						  secondary:(const char *)secondary;
{
				...
				isEqualToString:[NSString
									stringWithFormat:[NSString
														 stringWithUTF8String:
															 _("Could not add the buddy %s for an unknown reason.")],
													 "1"]]) {
			/* Rather random error displayed by OSCAR (since forever, as of libpurple 2.4.0) for some clients while
			 * connecting */
			return NULL;
		}
	...
	if ([primaryString rangeOfString:@"did not get sent"].location != NSNotFound) {
		// Oscar send error
		// This may not ever occur as of libpurple 2.4.0; I can't find the phrase 'did not get sent' in any of the code.
		// -evands
		NSString *targetUserName = [[[[primaryString componentsSeparatedByString:@" message to "] objectAtIndex:1]
			componentsSeparatedByString:@" did not get "] objectAtIndex:0];

		errorMessage =
			[NSString stringWithFormat:AILocalizedString(@"Your message to %@ did not get sent", nil), targetUserName];

		if (secondaryString) {
			if ([secondaryString rangeOfString:[NSString stringWithUTF8String:_("Rate")]].location != NSNotFound) {
				description =
					AILocalizedString(@"You are sending messages too quickly; wait a moment and try again.", nil);
			} else if ([secondaryString rangeOfString:[NSString stringWithUTF8String:_("Service unavailable")]]
							   .location != NSNotFound ||
					   [secondaryString rangeOfString:[NSString stringWithUTF8String:_("Not logged in")]].location !=
						   NSNotFound) {
				description = AILocalizedString(@"Connection error.", nil);

			} else if ([secondaryString rangeOfString:[NSString stringWithUTF8String:_("Refused by client")]]
						   .location != NSNotFound) {
				description = AILocalizedString(@"Your message was refused by the other user.", nil);

			} else if ([secondaryString rangeOfString:[NSString stringWithUTF8String:_("Reply too big")]].location !=
					   NSNotFound) {
				description = AILocalizedString(@"Your message was too big.", nil);

			} else if ([secondaryString rangeOfString:[NSString stringWithUTF8String:_("In local permit/deny")]]
						   .location != NSNotFound) {
				description = AILocalizedString(@"The other user is in your deny list.", nil);

			} else if ([secondaryString rangeOfString:[NSString stringWithUTF8String:_("Too evil")]].location !=
					   NSNotFound) {
				description = AILocalizedString(@"Warning level is too high.", nil);

			} else if ([secondaryString rangeOfString:[NSString stringWithUTF8String:_("User temporarily unavailable")]]
						   .location != NSNotFound) {
	...
	[adium.interfaceController
		  handleMessage:([errorMessage length] ? errorMessage : primaryString)
		withDescription:([description length] ? description
									...
									text:(const char *)text
{
	...
	if (!notifyFormattedHTMLDecoder)
		notifyFormattedHTMLDecoder = [AIHTMLDecoder decoder];

	NSString *textString = (text ? [NSString stringWithUTF8String:text] : nil);
	if (textString)
		textString = [[notifyFormattedHTMLDecoder decodeHTML:textString] string];
	...
	[adium.interfaceController
		...
		withDescription:(description ? description : @"")withWindowTitle:(titleString ? titleString : @"")];
...
{
	purple_account_set_register_callback(accountLookupFromAdiumAccount(adiumAccount), adiumPurpleAccountRegisterCb,
										 (__bridge void *)adiumAccount);
...
{
	purple_account_unregister(accountLookupFromAdiumAccount(adiumAccount), purpleUnregisterCb,
							  (__bridge void *)adiumAccount);
}
...
- (BOOL)doCommand:(NSString *)originalMessage fromAccount:(id)sourceAccount inChat:(AIChat *)chat
{
	...
	if (!conv || ([originalMessage length] < 2))
		return NO;
	...
	case PURPLE_CMD_STATUS_FAILED: {
		purple_conv_present_error(purple_conversation_get_name(conv), purple_conversation_get_account(conv),
								  "Command failed");
		didCommand = YES;
		break;
	}
	...
	case PURPLE_CMD_STATUS_WRONG_PRPL:
		/* Ignore this command and let the message send; the user probably doesn't even know what they typed is a
		 * command */
		didCommand = NO;
		break;
	}
							   ...
							   inChat:(AIChat *)chat
{
	...
	if ([originalMessage hasPrefix:@"/"]) {
		didCommand = [self doCommand:originalMessage fromAccount:sourceAccount inChat:chat];
	}
...
- (void)sendNotificationOfType:(AINotificationType)type fromAccount:(id)sourceAccount inChat:(AIChat *)chat
{
	...
	purple_prpl_send_attention(purple_conversation_get_gc(conv), purple_conversation_get_name(conv), type);
}

// Returns YES if the message was sent (and should therefore be displayed).  Returns NO if it was not sent or was
// otherwise used.
...
- (void)sendTyping:(AITypingState)typingState inChat:(AIChat *)chat
{
	...
	if (conv) {
		...
		serv_send_typing(purple_conversation_get_gc(conv), purple_conversation_get_name(conv), purpleTypingState);
	}
}
	 ...
	 withAlias:(NSString *)alias
{
	...
	if (!buddy) {
		 ...
		 * We're working with a new contact, hopefully, so we want to call serv_add_buddy() after modifying the purple
		 * list. This is the order done in add_buddy_cb() in gtkblist.c */
...
- (void)removeUID:(NSString *)objectUID onAccount:(id)adiumAccount fromGroup:(NSString *)groupName
{
	...
	if ((group = purple_find_group(groupUTF8String))) {
		...
		if ((buddy = purple_find_buddy_in_group(account, [objectUID UTF8String], group))) {
			...
			AILog(@"Removing buddy %s from group %s", purple_buddy_get_name(buddy),
			...
			if ((oldGroup = purple_find_group([sourceGroupName UTF8String]))) {
				...
				if ((buddy = purple_find_buddy_in_group(account, [objectUID UTF8String], oldGroup))) {
					...
					if (strcmp(purple_account_get_protocol_id(account), "prpl-yahoo") == 0) {
						 ...
						 * this move. Otherwise, it won't work. However, if we remove it from the old group and it
						 * *isn't* in another group already, Yahoo will force reauthorization, which is ugly.  */
...
- (void)setAlias:(NSString *)alias forUID:(NSString *)UID onAccount:(id)adiumAccount
{
	...
	if (purple_account_is_connected(account)) {
		...
		if (buddy && ((aliasUTF8String && !oldAlias) || (!aliasUTF8String && oldAlias) ||
			...
			//(Because normally it's wasteful to send a nil alias to the account).  We need to manually invoke the
			// update.
			if (oldAlias && !alias) {
				AIListContact *theContact = contactLookupFromBuddy(buddy);

				[adiumAccount updateContact:theContact toAlias:nil];
			}
		}
	}
}
...
{
	// Looking up the conv from the chat will create the PurpleConversation purpleside, joining the chat, opening the
	// server connection, or whatever else is done when a chat is opened.
...
- (void)inviteContact:(AIListContact *)listContact toChat:(AIChat *)chat withMessage:(NSString *)inviteMessage;
{
	...
	if (([adiumAccount isKindOfClass:[CBPurpleAccount class]]) && (conv = convLookupFromChat(chat, adiumAccount)) &&
		...
		serv_chat_invite(purple_conversation_get_gc(conv), purple_conv_chat_get_id(purpleChat),
						 (inviteMessage ? [inviteMessage UTF8String] : ""), [listContact.UID UTF8String]);
	}
}
		  ...
		  onAccount:(id)adiumAccount
{
	...
	AILog(@"Setting status on %p (%s): ID %s, isActive %i, attributes %@", account,
		  purple_account_get_username(account), statusID, [isActive boolValue], arguments);

	purple_account_set_status_list(account, statusID, [isActive boolValue], attrs);
	g_list_free(attrs);

	if (purple_status_is_online(purple_account_get_active_status(account)) && purple_account_is_disconnected(account)) {
