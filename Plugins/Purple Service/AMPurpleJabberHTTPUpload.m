		...
		if ((self = [super init])) {
			...
			if (jabber) {
				purple_signal_connect(jabber, "jabber-receiving-xmlnode", self,
									  PURPLE_CALLBACK(AMPurpleJabberHTTPUpload_received_data_cb),
									  (__bridge void *)self);
...
#pragma mark - Public
	...
	-(BOOL)sendFileTransfer : (ESFileTransfer *)fileTransfer
	{
		...
		_activeFileTransfer = [fileTransfer retain];
...
#pragma mark - Private: Discovery
	...
	-(void)_sendDiscoveryItems
	{
		...
		_discoveryItemsID = [queryID retain];
	...
	-(void)_handleDiscoveryInfoResult : (xmlnode *)query from : (NSString *)fromJID
	{
		...
		for (xmlnode *child = query->child; child; child = child->next) {
			...
			if (var && strcmp(var, [NS_HTTP_UPLOAD UTF8String]) == 0) {
				...
				_uploadServiceJID = [fromJID retain];
...
#pragma mark - Private: Slot Request
	...
	-(void)_requestSlotForTransfer : (ESFileTransfer *)fileTransfer
	{
		...
		NSString *iq = [NSString stringWithFormat:@"<iq type='get' id='%@' to='%@'>"
												  ...
												  @"</iq>",
												  queryID, _uploadServiceJID, NS_HTTP_UPLOAD, escapedFilename, fileSize,
												  contentType];
...
#pragma mark - NSURLSessionTaskDelegate
	...
	-(void)URLSession : (NSURLSession *)session task : (NSURLSessionTask *)task didSendBodyData
		: (int64_t)bytesSent totalBytesSent : (int64_t)totalBytesSent totalBytesExpectedToSend
		...
		: (nullable NSError *)error
	{
		if (error) {
			dispatch_async(dispatch_get_main_queue(), ^{
				[self _handleUploadFailure:[NSString
											   stringWithFormat:@"Upload failed: %@", [error localizedDescription]]];
			});
			return;
		}
