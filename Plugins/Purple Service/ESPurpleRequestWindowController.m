...
+ (ESPurpleRequestWindowController *)showInputWindowWithDict:(NSDictionary *)infoDict
{
	...
	if ((requestWindowController =
			 [[self alloc] initWithWindowNibName:(multiline ? MULTILINE_WINDOW_NIB : SINGLELINE_WINDOW_NIB)
										withDict:infoDict
									   multiline:multiline])) {
...
- (id)initWithWindowNibName:(NSString *)windowNibName withDict:(NSDictionary *)infoDict multiline:(BOOL)multiline
{
	if ((self = [super initWithWindowNibName:windowNibName])) {
		[self showWindowWithDict:[self translatedInfoDict:infoDict] multiline:multiline];
	}
...
- (void)showWindowWithDict:(NSDictionary *)infoDict multiline:(BOOL)multiline
{
	...
	// Buttons
	{
		// Use the supplied OK text, then shift the button left so that the right side remains in the old location in
		// the window
 ...
 * AILocalizedString(@"This is the name that other MSN buddies will see you as.", "Description for the MSN display name
 * setting dialogue.") AILocalizedString(@"Set your home phone number.", "Title for the dialogue prompting for your home
 * phone number") AILocalizedString(@"Set your work phone number.", "Title for the dialogue prompting for your work
 * phone number") AILocalizedString(@"Set your mobile phone number.", "Title for the dialogue prompting for your mobile
 * phone number")
 */
- (NSDictionary *)translatedInfoDict:(NSDictionary *)inDict
{
	...
	[translatedDict setObject:[thisBundle localizedStringForKey:primary value:primary table:nil]
					   forKey:@"Primary Text"];
	[translatedDict setObject:[thisBundle localizedStringForKey:secondary value:secondary table:nil]
					   forKey:@"Secondary Text"];
	[translatedDict setObject:[thisBundle localizedStringForKey:okText value:okText table:nil] forKey:@"OK Text"];
