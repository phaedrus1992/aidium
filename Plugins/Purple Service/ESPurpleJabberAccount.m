...
- (NSSet *)supportedPropertyKeys
{
	...
	if (!supportedPropertyKeys) {
		supportedPropertyKeys = [[NSMutableSet alloc] initWithObjects:@"AvailableMessage", @"Invisible", nil];
...
- (void)configurePurpleAccount
{
	...
	purple_account_set_string(account, "connect_server", (connectServer ? [connectServer UTF8String] : ""));
	...
	/* We have 2 checkboxes in Adium 1.4.1 which combine to provide a single setting within libpurple, for historical
	 * reasons. A later update should have new strings to describe this with a single drop-down labeled "Connection
	 * Security"
	 *
	...
	/* Mac OS X 10.4's cyrus-sasl's PLAIN mech gives us problems.  Is it a bug in the installed library, a bug in its
	 * compilation, or a bug in our linkage against it? I don't know. The result is that the username gets included
	 * twice before the base64 encoding is performed.
	 *
	 * Furthermore, on any version, using the cyrus-sasl PLAIN mech prevents us from following Google Talk best
	 * practices for handling of domain names. This is because we can't add to the <auth> response's attributes:
	 *		xmlns:ga='http://www.google.com/talk/protocol/auth' ga:client-uses-full-bind-result='true'
	 * as per http://code.google.com/apis/talk/jep_extensions/jid_domain_change.html and therefore we won't
	 * automatically resolve changing an
	 * "@gmail.com" to "@googlemail.com" or some other domain name.
	 *
	 * We therefore use the PLAIN implementation in libpurple itself. Libpurple's own DIGEST-MD5 is always used for
	 * compatibility with old OpenFire servers.
	 *
	 * This preference and the changes for it are added via the "libpurple_jabber_avoid_sasl_option_hack.diff" patch we
	 * apply during the build process.
	 */
...
- (AIListContact *)contactWithUID:(NSString *)sourceUID
{
	...
	contact = [adium.contactController existingContactWithService:service account:self UID:sourceUID];
	if (!contact) {
	...
	case 2: // always accept + add
		// add
		{
			...
			if ([groupname length] > 0) {
				AIListContact *contact =
					[adium.contactController contactWithService:self.service
														account:self
															UID:[dict objectForKey:@"Remote Name"]];
...
{
	AIReconnectDelayType shouldAttemptReconnect =
		[super shouldAttemptReconnectAfterDisconnectionError:disconnectionError];
	...
	if (([self lastDisconnectionReason] == PURPLE_CONNECTION_ERROR_CERT_OTHER_ERROR) &&
		...
		// No password specified + above error while we're connecting = behavior of various broken servers. Prompt for a
		// password.
...
- (void)disconnectFromDroppedNetworkConnection
{
	...
	if (PURPLE_CONNECTION_IS_VALID(gc) && !gc->disconnect_timeout) {
...
- (NSString *)titleForContactMenuLabel:(const char *)label forContact:(AIListContact *)inContact
{
	if (strcmp(label, _("Un-hide From")) == 0) {
		...
		return [NSString
			stringWithFormat:AILocalizedString(@"Re-request Authorization from %@", nil), inContact.formattedUID];

	} else if (strcmp(label, _("Cancel Presence Notification")) == 0) {
		return [NSString
			stringWithFormat:AILocalizedString(@"Cancel Presence Notification to %@", nil), inContact.formattedUID];
...
- (NSString *)fallbackAliasForContact:(AIListContact *)contact inChat:(AIChat *)chat
{
	if (contact.isStranger &&
		[contact.UID.lowercaseString rangeOfString:chat.name.lowercaseString].location != NSNotFound) {
...
- (NSMenu *)actionMenuForChat:(AIChat *)chat
{
	...
	menu = [adium.menuController
		contextualMenuWithLocations:
			[NSArray arrayWithObjects:[NSNumber numberWithInteger:Context_Contact_GroupChat_ParticipantAction],
									  [NSNumber numberWithInteger:Context_Contact_Manage], nil]
...
- (const char *)purpleStatusIDForStatus:(AIStatus *)statusState arguments:(NSMutableDictionary *)arguments
{
	...
	case AIAvailableStatusType: {
		if (([statusName isEqualToString:STATUS_NAME_FREE_FOR_CHAT]) ||
			([statusMessageString
				 caseInsensitiveCompare:[adium.statusController
											localizedDescriptionForCoreStatusName:STATUS_NAME_FREE_FOR_CHAT]] ==
			 NSOrderedSame))
	...
	case AIAwayStatusType: {
		if (([statusName isEqualToString:STATUS_NAME_DND]) ||
			([statusMessageString caseInsensitiveCompare:[adium.statusController
															 localizedDescriptionForCoreStatusName:STATUS_NAME_DND]] ==
			 NSOrderedSame) ||
			[statusName isEqualToString:STATUS_NAME_BUSY]) {
			// Note that Jabber doesn't actually support a 'busy' status; if we have it set because some other service
			// supports it, treat it as DND
			statusID = jabber_buddy_state_get_status_id(JABBER_BUDDY_STATE_DND);
				   ...
				   ([statusMessageString
						caseInsensitiveCompare:[adium.statusController
												   localizedDescriptionForCoreStatusName:STATUS_NAME_EXTENDED_AWAY]] ==
					NSOrderedSame))
	...
	[arguments setObject:(priority ? priority : [NSNumber numberWithInteger:0]) forKey:@"priority"];
...
- (void)addContact:(AIListContact *)theContact toGroupName:(NSString *)groupName contactName:(NSString *)contactName
{
	...
	if (atsign.location != NSNotFound)
		...
		[gateways addObject:[NSDictionary
								dictionaryWithObjectsAndKeys:theContact, @"contact", groupName, @"remoteGroup", nil]];
	}
}
		...
		for (NSDictionary *gatewaydict in [[gateways copy] autorelease]) {
			if ([[[gatewaydict objectForKey:@"contact"] UID] isEqualToString:theContact.UID]) {
				[[self purpleAdapter] removeUID:theContact.UID
									  onAccount:self
									  fromGroup:[gatewaydict objectForKey:@"remoteGroup"]];
...
- (void)didConnect
{
	...
	[adhocServer addCommand:@"ping"
				   delegate:(id<AMPurpleJabberAdHocServerDelegate>)[AMPurpleJabberAdHocPing class]
					   name:@"Ping"];
	...
	if ([self enableXMLConsole]) {
		if (!xmlConsoleController)
			xmlConsoleController = [[AMXMLConsoleController alloc] init];
...
- (PurpleSslConnection *)secureConnection
{
	...
	return ((gc && gc->proto_data)
				? ((JabberStream *)purple_account_get_connection(self.purpleAccount)->proto_data)->gsc
				: NULL);
}
...
- (NSArray *)accountActionMenuItems
{
	...
	if ([gateways count] > 0) {
		...
		for (gatewaydict in gateways) {
			...
			NSMenuItem *mitem = [[NSMenuItem alloc] initWithTitle:gateway.UID
														   action:@selector(registerGateway:)
													keyEquivalent:@""];
			...
			NSMenuItem *removeItem =
				[[NSMenuItem alloc] initWithTitle:AILocalizedString(@"Remove gateway", "gateway menu item")
										   action:@selector(removeGateway:)
									keyEquivalent:@""];
	...
	NSMenuItem *discoveryBrowserMenuItem =
		[[NSMenuItem alloc] initWithTitle:AILocalizedString(@"Discovery Browser", nil)
...
- (void)registerGateway:(NSMenuItem *)mitem
{
	if (mitem && [mitem representedObject])
		jabber_register_gateway((JabberStream *)purple_account_get_connection(self.purpleAccount)->proto_data,
								[[[mitem representedObject] UID] UTF8String]);
	else
		NSBeep();
}
...
- (void)removeGateway:(NSMenuItem *)mitem
{
	...
	if ([[NSAlert alertWithMessageText:AILocalizedString(@"Really remove gateway?", nil)
			 ...
			 informativeTextWithFormat:AILocalizedString(
										   ...
										   nil),
									   gateway.UID] runModal] == NSAlertDefaultReturn) {
