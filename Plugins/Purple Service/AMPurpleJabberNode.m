						...
						if (queryJID) {
							AMPurpleJabberNode *newnode = [[AMPurpleJabberNode alloc]
								initWithJID:[NSString stringWithUTF8String:queryJID]
								 ...
								 connection:self.gc];
							// propagate delegates
							newnode.delegates = CFBridgingRelease(CFArrayCreateMutableCopy(
								kCFAllocatorDefault, /*capacity*/ 0, (CFArrayRef)self.delegates));
...
- (id)initWithJID:(NSString *)_jid node:(NSString *)_node name:(NSString *)_name connection:(PurpleConnection *)_gc
{
	if ((self = [super init])) {
		...
		self.gc = _gc;
		self.delegates =
			CFBridgingRelease(CFArrayCreateMutable(kCFAllocatorDefault, /*capacity*/ 0, &nonretainingArrayCallbacks));
