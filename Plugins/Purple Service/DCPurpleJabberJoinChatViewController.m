...
- (NSString *)defaultConferenceServer
{
	...
	if (!(defaultConferenceServer = [account preferenceForKey:KEY_DEFAULT_CONFERENCE_SERVER
...
{
	[account setPreference:inDefaultConferenceServer forKey:KEY_DEFAULT_CONFERENCE_SERVER group:GROUP_ACCOUNT_STATUS];
}
...
- (void)joinChatWithAccount:(AIAccount *)inAccount
{
	...
	chatCreationInfo = [NSMutableDictionary dictionaryWithObjectsAndKeys:room, @"room", server, @"server", nil];

	if (handle) {
		[chatCreationInfo setObject:handle forKey:@"handle"];
	}

	if (password) {
		[chatCreationInfo setObject:password forKey:@"password"];
	}

	[self doJoinChatWithName:[NSString stringWithFormat:@"%@@%@", room, server]
					onAccount:inAccount
			 chatCreationInfo:chatCreationInfo
			 invitingContacts:[self contactsFromNamesSeparatedByCommas:[textField_inviteUsers stringValue]
															 onAccount:inAccount]
