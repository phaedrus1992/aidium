...
+ (void)showNotifyEmailWindowForAccount:(AIAccount *)account
							withMessage:(NSAttributedString *)inMessage
 ...
 */
+ (void *)handleNotifyEmailsForAccount:(AIAccount *)account
								 count:(size_t)count
							  detailed:(BOOL)detailed
							  subjects:(const char **)subjects
								 froms:(const char **)froms
								   tos:(const char **)tos
								  urls:(const char **)urls
{
	...
	titleFont = [fontManager convertFont:[NSFont messageFontOfSize:12] toHaveTrait:NSBoldFontMask];
	...
	if (count == 1) {
		...
		if (haveFroms || haveSubjects) {
			...
			fieldFont = [fontManager convertFont:messageFont toHaveTrait:NSBoldFontMask];
	...
	NSMutableDictionary *infoDict =
		[NSMutableDictionary dictionaryWithObjectsAndKeys:title, @"Title", message, @"Message", nil];
							  ...
							  URLString:(NSString *)inURLString
{
	...
	if (![[NSUserDefaults standardUserDefaults] boolForKey:@"AINoNewMailWindow"]) {
		ESTextAndButtonsWindowController *textAndButtonsWindowController = [[ESTextAndButtonsWindowController alloc]
				initWithTitle:AILocalizedString(@"New Mail", nil)
				defaultButton:nil
			  alternateButton:(inURLString ? AILocalizedString(@"Open Mail in Browser", nil) : nil)otherButton
							 :((mailApplicationName && [mailApplicationName length])
								   ? [NSString
										 stringWithFormat:AILocalizedString(@"Launch %@", nil), mailApplicationName]
								   : nil)
...
+ (void)openURLString:(NSString *)urlString
{
	if ([urlString rangeOfString:[NSString stringWithUTF8String:g_get_tmp_dir()]].location != NSNotFound) {
		 ...
		 * it.
		 */
		err = LSGetApplicationForURL((CFURLRef)[NSURL URLWithString:@"http://www.adium.im"], kLSRolesViewer,
