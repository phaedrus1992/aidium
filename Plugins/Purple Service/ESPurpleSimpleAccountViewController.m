...
+ (void)initialize
{
	if (self == [ESPurpleSimpleAccountViewController class]) {
		bindings = [[NSSet alloc]
			initWithObjects:@"publishStatus", @"useUDP", @"useSIPProxy", @"sipProxy", @"authUser", @"authDomain", nil];
...
{
	NSMutableDictionary *keyToKeyDict = [NSMutableDictionary
		dictionaryWithObjectsAndKeys:KEY_SIMPLE_PUBLISH_STATUS, @"publishStatus", KEY_SIMPLE_USE_UDP, @"useUDP",
									 KEY_SIMPLE_USE_SIP_PROXY, @"useSIPProxy", KEY_SIMPLE_SIP_PROXY, @"sipProxy",
									 KEY_SIMPLE_AUTH_USER, @"authUser", KEY_SIMPLE_AUTH_DOMAIN, @"authDomain", nil];
