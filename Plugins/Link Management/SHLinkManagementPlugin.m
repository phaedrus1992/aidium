...
- (BOOL)validateMenuItem:(NSMenuItem *)menuItem
{
	...
	if (responder && [responder isKindOfClass:[NSTextView class]]) {
		if ([[menuItem title] isEqualToString:RM_LINK_TITLE]) {
			...
			[menuItem
				setTitle:([self textViewSelectionIsLink:(NSTextView *)responder] ? EDIT_LINK_TITLE : ADD_LINK_TITLE)];
...
- (IBAction)editFormattedLink:(id)sender
{
	...
	if (earliestTextView && ![[keyWin windowController] isKindOfClass:[SHLinkEditorWindowController class]]) {
		SHLinkEditorWindowController *linkEditorWindowController =
			[[SHLinkEditorWindowController alloc] initWithTextView:earliestTextView notifyingTarget:nil];
...
- (IBAction)removeFormattedLink:(id)sender
{
	...
	if (earliestTextView) {
		...
		if ([[earliestTextView textStorage] length] && selectedRange.location != NSNotFound &&
