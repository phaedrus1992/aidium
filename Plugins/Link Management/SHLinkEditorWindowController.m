...
- (void)windowDidLoad
{
	...
	[label_linkText
		setLocalizedString:AILocalizedString(@"Link Text:",
	...
	if (textView) {
		...
		if (linkURL) {
			...
			if (linkURL) {
				...
				if (![AHHyperlinkScanner isStringValidURI:linkURL
											  usingStrict:NO
												fromIndex:0U
											   withStatus:NULL
											 schemeLength:NULL]) {
		...
		if (linkURL) {
			...
			tmpString = CFBridgingRelease(
				CFURLCreateStringByReplacingPercentEscapes(kCFAllocatorDefault, (CFStringRef)tmpString, CFSTR("")));
...
- (IBAction)acceptURL:(id)sender
{
	...
	if ((URL = [NSURL URLWithString:urlString])) {
		[self insertLinkTo:URL withText:linkString inView:textView];
...
{
	if ([[textView textStorage] length] && [textView selectedRange].location != NSNotFound &&
...
- (void)insertLinkTo:(NSURL *)linkURL withText:(NSString *)linkTitle inView:(NSTextView *)inView
{
	...
	if (NSMaxRange([inView selectedRange]) == [textStorage length]) {
		NSAttributedString *tmpString = [[NSAttributedString alloc] initWithString:@" " attributes:typingAttributes];
		[[[inView undoManager] prepareWithInvocationTarget:textStorage]
			replaceCharactersInRange:NSMakeRange(NSMaxRange([inView selectedRange]), 1)
				withAttributedString:[[NSAttributedString alloc] initWithString:@"" attributes:typingAttributes]];
		[textStorage appendAttributedString:tmpString];
	}

	// Notify that a change occurred since NSTextStorage won't do it for us
	[[NSNotificationCenter defaultCenter] postNotificationName:NSTextDidChangeNotification object:inView userInfo:nil];
