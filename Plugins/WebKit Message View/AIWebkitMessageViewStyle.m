...
#define APPEND_NEXT_MESSAGE_WITH_SCROLL                                                                                \
	@"checkIfScrollToBottomIsNeeded(); appendNextMessage(\"%@\"); scrollToBottomIfNeeded();"
...
// We set back, when the user finishes editing, the correct topic, which wipes out the existance of the span before. We
// don't need to undo the dbl click action.
#define TOPIC_INDIVIDUAL_WRAPPER                                                                                       \
	@"<span id=\"topicEdit\" ondblclick=\"this.setAttribute('contentEditable', true); this.focus();\">%@</span>"
...
// The old code built the paths itself, which follows the filesystem's case sensitivity, so some noobs named stuff
// wrong. NSBundle is always case sensitive, so those styles broke (they were already broken on case sensitive hfsx)
...
- (NSString *)semiCaseInsensitivePathForResource:(NSString *)res
...
- (BOOL)reloadStyle
{
	 ...
	 * Version 3: main.css is no longer a separate style, it now serves as the base stylesheet and is imported by
	 * default. The default variant is now a separate file in /variants like all other variants. Template.html now
	 * includes appendMessageNoScroll() and appendNextMessageNoScroll() which behave the same as appendMessage() and
	 * appendNextMessage() in Versions 1 and 2 but without scrolling. Version 4: Template.html now includes
	 * replaceLastMessage() Template.html now defines actionMessageUserName and actionMessageBody for display of /me
	 * (actions). If the style provides a custom Template.html, these classes must be defined. CSS can be used to
	 * customize the appearance of actions. HTML filters in are now supported in Adium's content filter system; filters
	 * can assume Version 4 or later.
	 */
	...
	transparentDefaultBackground =
		[[styleBundle objectForInfoDictionaryKey:@"DefaultBackgroundIsTransparent"] boolValue];
	...
	if (tmpName)
		userIconMask = [[NSImage alloc] initWithContentsOfFile:[stylePath stringByAppendingPathComponent:tmpName]];
...
{
	// Our custom background is only transparent if the user has set a custom color with an alpha component less
	// than 1.0
...
- (NSString *)defaultFontFamily
{
	...
	if (!defaultFontFamily)
		defaultFontFamily = [[NSFont systemFontOfSize:0] familyName];
...
- (NSNumber *)defaultFontSize
{
	...
	if (!defaultFontSize)
		defaultFontSize = [NSNumber numberWithInteger:[[NSFont systemFontOfSize:0] pointSize]];
...
@synthesize allowTextBackgrounds, customBackgroundType, customBackgroundColor,
	showIncomingMessageColors = showIncomingColors, showIncomingMessageFonts = showIncomingFonts, customBackgroundPath,
	nameFormat, useCustomNameFormat, showHeader, showUserIcons;
...
- (NSString *)baseTemplateForChat:(AIChat *)chat
{
	...
	if ((styleVersion < 3) && usingCustomTemplateHTML) {
		templateHTML =
			[NSMutableString stringWithFormat:baseHTML,                                           // Template
											  [[NSURL fileURLWithPath:stylePath] absoluteString], // Base path
											  [self pathForVariant:self.activeVariant],           // Variant path
											  headerContent, (footerHTML ? footerHTML : @"")];
	} else {
		templateHTML = [NSMutableString
			stringWithFormat:baseHTML,                                           // Template
							 [[NSURL fileURLWithPath:stylePath] absoluteString], // Base path
							 styleVersion < 3
								 ? @""
...
- (void)_loadTemplates
{
	...
	if ((!baseHTML || [baseHTML length] == 0) && styleVersion >= 1) {
		baseHTML = [NSString stringWithContentsOfUTF8File:[[NSBundle bundleForClass:[self class]]
															  semiCaseInsensitivePathForResource:@"Template"
																						  ofType:@"html"]];
		...
		if ([baseHTML rangeOfString:@"function imageCheck()" options:NSLiteralSearch].location != NSNotFound) {
			...
			[imageSwapFixedBaseHTML
				replaceOccurrencesOfString:@"		function imageCheck() {\n"
											...
											"			}\n"
											"		}"
								withString:@"		function imageCheck() {\n"
											"			var node = event.target;\n"
											"			if(node.tagName.toLowerCase() == 'img' && "
											"!client.zoomImage(node) && node.alt) {\n"
								   ...
								   options:NSLiteralSearch];
			[imageSwapFixedBaseHTML
				replaceOccurrencesOfString:@"		function imageSwap(node) {\n"
											...
											"			alignChat();\n"
											"		}"
								withString:@"		function imageSwap(node) {\n"
	...
	fileTransferHTMLTemplate =
		[[NSString stringWithContentsOfUTF8File:[styleBundle semiCaseInsensitivePathForResource:@"FileTransferRequest"
																						 ofType:@"html"]] mutableCopy];
	if (!fileTransferHTMLTemplate) {
		...
		[fileTransferHTMLTemplate replaceKeyword:@"%message%"
									  withString:@"<p><img src=\"%fileIconPath%\" style=\"width:32px; height:32px; "
												 @"vertical-align:middle;\"></img><input type=\"button\" "
												 @"onclick=\"%saveFileAsHandler%\" value=\"Download %fileName%\"></p>"];
	}
	[fileTransferHTMLTemplate
		replaceKeyword:@"Download %fileName%"
			withString:[NSString stringWithFormat:AILocalizedString(@"Download %@", "%@ will be a file name"),
												  @"%fileName%"]];
...
{
	return
		[NSString stringWithFormat:@"setStylesheet(\"mainStyle\",\"%@\");", [self pathForVariant:self.activeVariant]];
}

- (NSString *)scriptForScrollingAfterAddingMultipleContentObjects
{
	if ((styleVersion >= 3) || !usingCustomTemplateHTML) {
		return @"if (this.AI_viewScrolledOnLoad != undefined) {alignChat(nearBottom());} else "
			   @"{this.AI_viewScrolledOnLoad = true; alignChat(true);}";
	}
...
- (NSMutableString *)_escapeStringForPassingToScript:(NSMutableString *)inString
{
	...
	[inString replaceOccurrencesOfString:@"\\" withString:@"\\\\" options:NSLiteralSearch];

	[inString replaceOccurrencesOfString:@"\"" withString:@"\\\"" options:NSLiteralSearch];

	[inString replaceOccurrencesOfString:@"\n" withString:@"" options:NSLiteralSearch];

	[inString replaceOccurrencesOfString:@"\r" withString:@"<br>" options:NSLiteralSearch];
						  ...
						  similar:(BOOL)contentIsSimilar
{
	...
	AIListObject *theSource =
		([contentSource isKindOfClass:[AIListContact class]] ? [(AIListContact *)contentSource parentContact]
															 : contentSource);
	...
	if (styleVersion >= 4)
		htmlEncodedMessage =
			[adium.contentController filterHTMLString:htmlEncodedMessage
	...
	if ([inString rangeOfString:@"%senderStatusIcon%"].location != NSNotFound) {
		...
		[inString replaceKeyword:@"%senderStatusIcon%" withString:[self statusIconPathForListObject:theSource]];
	}

	// Replaces %localized{x}% with a a localized version of x, searching the style's localizations, and then Adium's
	// localizations
	do {
		...
		if (range.location != NSNotFound) {
			...
			endRange = [inString rangeOfString:@"}%"
									   options:NSLiteralSearch
										 range:NSMakeRange(NSMaxRange(range), [inString length] - NSMaxRange(range))];
			if (endRange.location != NSNotFound && endRange.location > NSMaxRange(range)) {
				NSString *untranslated = [inString
					substringWithRange:NSMakeRange(NSMaxRange(range), (endRange.location - NSMaxRange(range)))];

				NSString *translated = [styleBundle localizedStringForKey:untranslated value:untranslated table:nil];
				...
				[inString safeReplaceCharactersInRange:NSUnionRange(range, endRange) withString:translated];
			}
		}
	} while (range.location != NSNotFound);

	[inString replaceKeyword:@"%userIcons%" withString:(showUserIcons ? @"showIcons" : @"hideIcons")];

	[inString replaceKeyword:@"%messageClasses%"
				  withString:[(contentIsSimilar ? @"consecutive " : @"")
								 stringByAppendingString:[[content displayClasses] componentsJoinedByString:@" "]]];

	[inString replaceKeyword:@"%senderColor%"
				  withString:[NSColor representedColorForObject:contentSource.UID
												withValidColors:self.validSenderColors]];
	...
	BOOL isRTL = ([htmlEncodedMessage rangeOfString:@"<div dir=\"rtl\">"
					  ...
					  .location != NSNotFound);
	[inString replaceKeyword:@"%messageDirection%" withString:(isRTL ? @"rtl" : @"ltr")];
				...
				if (date) {
					if (!timeFormatterCache) {
						...
						[[NSDistributedNotificationCenter defaultCenter]
							addObserver:self
							   selector:@selector(flushTimeFormatterCache:)
								   name:@"AppleDatePreferencesChangedNotification"
								 object:nil];
						[[NSDistributedNotificationCenter defaultCenter]
							addObserver:self
							   selector:@selector(flushTimeFormatterCache:)
								   name:@"AppleTimePreferencesChangedNotification"
								 object:nil];
					}
					NSString *timeFormat = [inString
						substringWithRange:NSMakeRange(NSMaxRange(range), (endRange.location - NSMaxRange(range)))];
					...
					if (!dateFormatter) {
						if ([timeFormat rangeOfString:@"%"].location != NSNotFound) {
							/* Support strftime-style format strings, which old message styles may use */
							dateFormatter = [[NSDateFormatter alloc] initWithDateFormat:timeFormat
	...
	do {
		...
		if (range.location != NSNotFound) {
			...
			if (showUserIcons && userIconPath) {
				...
				replacementString = ([content isOutgoing] ? @"Outgoing/buddy_icon.png" : @"Incoming/buddy_icon.png");
			}
	...
	[inString replaceKeyword:@"%service%" withString:[content.chat.account.service shortDescription]];
	...
	if ([content isKindOfClass:[AIContentMessage class]]) {
		...
		[inString replaceKeyword:@"%status%" withString:@""];

		[inString replaceKeyword:@"%senderScreenName%"
					  withString:[(formattedUID ? formattedUID : displayName) stringByEscapingForXMLWithEntities:nil]];

		[inString replaceKeyword:@"%senderPrefix%" withString:((AIContentMessage *)content).senderPrefix];
		...
		do {
			...
			if (range.location != NSNotFound) {
				...
				if (useCustomNameFormat) {
					...
					// Test both displayName and formattedUID for nil-ness. If they're both nil, the assertion will
					// trip.
					if (!senderDisplay) {
						senderDisplay = displayName;
						if (!senderDisplay) {
							...
							if (!senderDisplay) {
								...
								NSLog(@"Enormous error: we don't have a sender for %@ (%@)", content,
									  [content message]);
				...
				if ([(AIContentMessage *)content isAutoreply]) {
					senderDisplay =
						[NSString stringWithFormat:@"%@ %@", senderDisplay,
												   AILocalizedString(@"(Autoreply)",
																	 "Short word inserted after the sender's name when "
																	 "displaying a message which was an autoresponse")];
				}

				[inString safeReplaceCharactersInRange:range
											withString:[senderDisplay stringByEscapingForXMLWithEntities:nil]];
			}
		} while (range.location != NSNotFound);
		...
		do {
			...
			if (range.location != NSNotFound) {
				NSRange endRange;
				endRange =
					[inString rangeOfString:@"}%"
									options:NSLiteralSearch
									  range:NSMakeRange(NSMaxRange(range), [inString length] - NSMaxRange(range))];
				if (endRange.location != NSNotFound && endRange.location > NSMaxRange(range)) {
					NSString *transparency = [inString
						substringWithRange:NSMakeRange(NSMaxRange(range), (endRange.location - NSMaxRange(range)))];
					...
					if (allowTextBackgrounds && showIncomingColors) {
						...
						if (![thisIsATemporaryString length] && rgb == 0) {
							...
							blue = rgb & 0x0000ff;
							[inString
								safeReplaceCharactersInRange:NSUnionRange(range, endRange)
												  withString:[NSString stringWithFormat:@"rgba(%d, %d, %d, %@)", red,
		...
		if ([content isKindOfClass:[ESFileTransfer class]]) { // file transfers are an AIContentMessage subclass
			...
			[inString replaceKeyword:@"%fileName%" withString:fileName];

			[inString
				replaceKeyword:@"%saveFileHandler%"
			...
			[inString replaceKeyword:@"%saveFileAsHandler%"
						  withString:[NSString stringWithFormat:@"client.handleFileTransfer('SaveAs', '%@')",
																fileTransferID]];

			[inString replaceKeyword:@"%cancelRequestHandler%"
						  withString:[NSString stringWithFormat:@"client.handleFileTransfer('Cancel', '%@')",
																fileTransferID]];
		}
		...
		if ([content isKindOfClass:[AIContentTopic class]]) {
			...
			if (range.location != NSNotFound) {
				[inString safeReplaceCharactersInRange:range
											withString:[NSString stringWithFormat:TOPIC_INDIVIDUAL_WRAPPER,
																				  htmlEncodedMessage]];
			}
		}
		...
		[inString replaceKeyword:@"%senderScreenName%" withString:@""];

		[inString replaceKeyword:@"%senderPrefix%" withString:@""];

		[inString replaceKeyword:@"%sender%" withString:@""];
...
- (NSMutableString *)fillKeywordsForBaseTemplate:(NSMutableString *)inString chat:(AIChat *)chat
{
	...
	[inString replaceKeyword:@"%chatName%" withString:[chat.displayName stringByEscapingForXMLWithEntities:nil]];

	NSString *sourceName = [chat.account.displayName stringByEscapingForXMLWithEntities:nil];
	if (!sourceName)
		sourceName = @" ";
	[inString replaceKeyword:@"%sourceName%" withString:sourceName];

	NSString *destinationName = chat.listObject.displayName;
	if (!destinationName)
		destinationName = chat.displayName;
	[inString replaceKeyword:@"%destinationName%" withString:destinationName];
	...
	[inString replaceKeyword:@"%outgoingColor%"
				  withString:[NSColor representedColorForObject:chat.account.UID
												withValidColors:self.validSenderColors]];
	...
	if (listObject) {
		...
		if (!iconPath && [listObject isKindOfClass:[AIListContact class]] &&
	...
	[inString replaceKeyword:@"%incomingIconPath%" withString:(iconPath ? iconPath : @"incoming_icon.png")];
	...
	[inString replaceKeyword:@"%outgoingIconPath%" withString:(iconPath ? iconPath : @"outgoing_icon.png")];

	NSString *serviceIconPath = [AIServiceIcons pathForServiceIconForServiceID:account.service.serviceID
																		  type:AIServiceIconLarge];

	NSString *serviceIconTag =
		[NSString stringWithFormat:@"<img class=\"serviceIcon\" src=\"%@\" alt=\"%@\" title=\"%@\">",
								   serviceIconPath ? serviceIconPath : @"outgoing_icon.png",
								   [account.service shortDescription], [account.service shortDescription]];

	[inString replaceKeyword:@"%service%" withString:[account.service shortDescription]];

	[inString replaceKeyword:@"%serviceIconImg%" withString:serviceIconTag];

	[inString replaceKeyword:@"%serviceIconPath%" withString:serviceIconPath];

	[inString replaceKeyword:@"%timeOpened%" withString:[timeStampFormatter stringFromDate:[chat dateOpened]]];
	...
	[NSDateFormatter withLocalizedDateFormatterPerform:^(NSDateFormatter *dateFormatter) {
		[inString replaceKeyword:@"%dateOpened%" withString:[dateFormatter stringFromDate:[chat dateOpened]]];
	}];
		...
		if (range.location != NSNotFound) { // a backgroundImage tag is not required
			...
			if (allowsCustomBackground && (customBackgroundPath || customBackgroundColor)) {
						...
						case BackgroundNormal:
							[bodyTag
								appendString:[NSString
												 stringWithFormat:@"background-image: url('%@'); background-repeat: "
																  @"no-repeat; background-attachment:fixed;",
																  customBackgroundPath]];
							break;
						case BackgroundCenter:
							[bodyTag appendString:[NSString
													  stringWithFormat:
							...
							[bodyTag
								appendString:
									[NSString stringWithFormat:
												  @"background-image: url('%@'); -webkit-background-size: 100%% 100%%; "
				...
				if (customBackgroundColor) {
					...
					[bodyTag
						appendString:[NSString stringWithFormat:@"background-color: rgba(%ld, %ld, %ld, %f); ",
																(NSInteger)(red * 255.0), (NSInteger)(green * 255.0),
																(NSInteger)(blue * 255.0), alpha]];
				}
			}
...
{
	NSString *filename =
		[NSString stringWithFormat:@"TEMP-%@%@.tiff", [inObject uniqueID], [NSString randomStringOfLength:5]];
	return [[adium cachesPath] stringByAppendingPathComponent:filename];
}

- (NSString *)statusIconPathForListObject:(AIListObject *)inObject
{
	if (!statusIconPathCache)
		statusIconPathCache = [[NSMutableDictionary alloc] init];
	NSImage *icon = [AIStatusIcons statusIconForListObject:inObject type:AIStatusIconTab direction:AIIconNormal];
	...
	if (!path) {
		path = [[adium cachesPath]
			stringByAppendingPathComponent:[NSString stringWithFormat:@"TEMP-%@%@.tiff", statusName,
																	  [NSString randomStringOfLength:5]]];
