...
- (void)_appendContent:(AIContentObject *)content
...
- (void)messageViewIsClosing
{
	...
	/* The windowScriptObject retained self when we set it as the client in -[AIWebKitMessageViewController
	 * _initWebView]... Unfortunately, (as of 10.4.9) it won't actually release self until the webView deallocates.
	 * We'll do removeWebScriptKey: now in case that works properly later, and do the release of webView here rather
	 * than in dealloc to work around the bug.
	 */
...
- (void)_primeWebViewAndReprocessContent:(BOOL)reprocessContent
{
	...
	NSURL *baseURL =
		[NSURL URLWithString:[NSString stringWithFormat:@"adium://%@/adium", [messageStyle.bundle bundleIdentifier]]];
	...
	if (reprocessContent) {
		...
		// Keep the array of objects waiting to be added, if necessary, to append them after our currently displayed
		// ones
		currentContentQueue = ([contentQueue count] ? [contentQueue copy] : nil);
...
/*!
 * @brief Sets the class 'groupchat' on the #Chat element, to allow styles to modify their appearance based on whether
 * we're in a groupchat
 *
...
- (void)enqueueContentObject:(AIContentObject *)contentObject
{
	 ...
	 * This is NO, for example, when we receive an entire block of message history content so that we can avoid
	 * scrolling after each one.
	 */
...
- (void)_processContentObject:(AIContentObject *)content willAddMoreContentObjects:(BOOL)willAddMoreContentObjects
{
	...
	if ([previousContent isKindOfClass:[AIContentStatus class]] && [content isKindOfClass:[AIContentStatus class]] &&
		[[(AIContentStatus *)previousContent coalescingKey]
	...
	if ([content.type isEqualToString:CONTENT_TOPIC_TYPE]) {
		 ...
		 */
		if (content.postProcessContent && content.chat.isGroupChat &&
			adium.interfaceController.activeChat == content.chat && !nextMessageFocus && nextMessageRegainedFocus) {
...
- (void)saveImageAs:(id)sender
{
	...
	[savePanel beginSheetModalForWindow:[webView window]
			  ...
			  defaultMenuItems:(NSArray *)defaultMenuItems
{
	...
	if (webViewMenuItems) {
		...
		for (menuItem in defaultMenuItems) {
			...
			if ((tag == WebMenuItemTagOpenLinkInNewWindow) || (tag == WebMenuItemTagDownloadLinkToDisk) ||
				(tag == WebMenuItemTagOpenImageInNewWindow) || (tag == WebMenuItemTagDownloadImageToDisk) ||
				(tag == WebMenuItemTagOpenFrameInNewWindow) || (tag == WebMenuItemTagStop) ||
				(tag == WebMenuItemTagReload)) {
				[webViewMenuItems removeObjectIdenticalTo:menuItem];
			} else {
				// This isn't as nice; there's no tag available. Use the localization from WebKit/WebCore, where it
				// seems to be for some other people (#16101), to look at the title.
				if ([[menuItem title]
						isEqualToString:NSLocalizedStringFromTableInBundle(
											@"Open Link", nil, [NSBundle bundleForClass:[WebView class]], nil)] ||
					[[menuItem title] isEqualToString:NSLocalizedStringFromTableInBundle(
														  @"Open Link", nil,
	...
	if ((imageURL = [element objectForKey:WebElementImageURLKey])) {
		...
		//being very careful to only get user icons... a better way would be to put a class "usericon" on the img, but I
		haven't worked out how to do that, so we test for the name of the person in the src, and that it's not an
		emoticon or direct connect image. if([[img getAttribute:@"src"] rangeOfString:internalObjectID].location !=
		NSNotFound && [imgClass rangeOfString:@"emoticon"].location == NSNotFound && [imgClass
		rangeOfString:@"fullSizeImage"].location == NSNotFound && [imgClass rangeOfString:@"scaledToFitImage"].location
		== NSNotFound)
		 */
	}
...
- (void)webView:(WebView *)sender didClearWindowObject:(WebScriptObject *)windowObject forFrame:(WebFrame *)frame
{
	...
	[[webView windowScriptObject] evaluateWebScript:@"document.addEventListener(\"DOMContentLoaded\", function() "
													@"{window.client.$_setDocumentReady()}, false);"];
}
...
- (NSTextView *)textView
{
	...
	while (responder && ![responder isKindOfClass:[NSTextView class]]) {
...
- (NSDragOperation)draggingEntered:(id<NSDraggingInfo>)sender
{
	...
	return ([pasteboard availableTypeFromArray:draggedTypes] ? NSDragOperationCopy : NSDragOperationNone);
}
	...
	if (userIcon) {
		...
		if (!webKitUserIconPath) {
			...
			if ([[webKitUserIcon PNGRepresentation] writeToFile:webKitUserIconPath atomically:YES]) {
				[iconSourceObject setValue:webKitUserIconPath forProperty:KEY_WEBKIT_USER_ICON notify:NO];
			} else {
				AILogWithSignature(@"Warning: Could not write out icon to %@", webKitUserIconPath);
			}
		}

		AILogWithSignature(@"%@'s oldWebKitUserIconPath %@, webKitUserIconPath now %@", inObject, oldWebKitUserIconPath,
						   webKitUserIconPath);
...
#else
		[objectIconPathDict setObject:webKitUserIconPath forKey:iconSourceObject.internalObjectID];
#endif
	}
}
...
- (void)customEmoticonUpdated:(NSNotification *)inNotification
{
	...
	if (imagesCount > 0) {
		...
		NSNumber *shouldScroll = [[webView windowScriptObject] callWebScriptMethod:@"nearBottom" withArguments:nil];
		if (!shouldScroll)
			shouldScroll = [NSNumber numberWithBool:NO];

		if (updatedImage)
			[[webView windowScriptObject] callWebScriptMethod:@"alignChat"
...
- (NSString *)_webKitUserIconPathForObject:(AIListObject *)inObject
{
	NSString *filename = [NSString stringWithFormat:@"%@-%@%@.png", TEMPORARY_FILE_PREFIX, inObject.internalObjectID,
...
{
	if (sel_isEqual(aSelector, @selector(handleAction:forFileTransfer:)) ||
		sel_isEqual(aSelector, @selector(debugLog:)) || sel_isEqual(aSelector, @selector(zoomImage:)) ||
 ...
 * If this method returns nil or you do not implement it, the default name for the selector will be constructed as
 * follows:
 *
...
+ (NSString *)webScriptNameForSelector:(SEL)aSelector
{
	...
	if (sel_isEqual(aSelector, @selector(debugLog:)))
		return @"debugLog";
	if (sel_isEqual(aSelector, @selector(zoomImage:)))
		return @"zoomImage";
	return @"";
}
...
- (BOOL)zoomImage:(DOMHTMLImageElement *)img
{
	...
	[[webView windowScriptObject] callWebScriptMethod:@"alignChat"
										withArguments:[NSArray arrayWithObject:[NSNumber numberWithBool:YES]]];
...
- (void)setChatContentSource:(NSString *)source
{
	if (!webViewIsReady) {
		...
		[self performSelector:@selector(setChatContentSource:) withObject:source afterDelay:0];
	} else {
