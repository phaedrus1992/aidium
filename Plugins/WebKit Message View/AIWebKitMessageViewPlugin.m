...
- (AIWebkitMessageViewStyle *)currentMessageStyleForChat:(AIChat *)chat
{
	...
	if (loadFromGroup && thisStyle) {
		...
		*thisStyle = [AIWebkitMessageViewStyle
			messageViewStyleFromPath:[prefs preferenceForKey:KEY_CURRENT_WEBKIT_STYLE_PATH group:loadFromGroup]];
		if (!*thisStyle) {
			/* If the path isn't cached yet, load the style and then store the path */
			*thisStyle = [AIWebkitMessageViewStyle
				messageViewStyleFromBundle:[self messageStyleBundleWithIdentifier:[prefs
																					  preferenceForKey:KEY_WEBKIT_STYLE
																								 group:loadFromGroup]]];
			if (*thisStyle) {
	...
	if (thisStyle) {
		NSDictionary *fileAttrs =
			[[NSFileManager defaultManager] attributesOfItemAtPath:[[*thisStyle bundle] bundlePath] error:NULL];
...
#pragma mark -
/*!
 * @brief Clears cached style bundle pathes if they are built-in but not bundle-relative; Adium 1.4b18 and prior made
 * these.
 */
- (void)clearHardcodedBuiltInStylePaths
{
	if ([[adium.preferenceController preferenceForKey:KEY_CURRENT_WEBKIT_STYLE_PATH
												group:PREF_GROUP_WEBKIT_REGULAR_MESSAGE_DISPLAY] rangeOfString:@".app"]
			.location != NSNotFound)
		[self resetStylesForType:AIWebkitRegularChat];
