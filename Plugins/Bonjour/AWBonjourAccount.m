...
- (void)userChangedState:(AWEzvContact *)contact
{
	...
	if ([contact status] == AWEzvUndefined) {
		AILogWithSignature(
			@"Warning: Received a status update for a contact with an undefined status. This shouldn't happen.");
	...
	if (![[listContact valueForProperty:@"serverDisplayName"] isEqualToString:contactName]) {
		[listContact setServersideAlias:contactName silently:silentAndDelayed];
	}
...
- (void)user:(AWEzvContact *)contact sentMessage:(NSString *)message withHtml:(NSString *)html
{
	...
	listContact = [adium.contactController existingContactWithService:service account:self UID:contact.uniqueID];
	...
	if ([error isEqualToString:@"Could Not Send"]) {
		...
		[chat setValue:[NSNumber numberWithInt:AIChatMessageSendingUserNotAvailable]
			forProperty:KEY_CHAT_ERROR
				 notify:NotifyNow];
		[chat setValue:nil forProperty:KEY_CHAT_ERROR notify:NotifyNever];
	} else {
		NSLog(@"Bonjour Error (%i): %@", severity, error);
		AILog(@"Bonjour Error (%i): %@", severity, error);
	}
...
- (void)sendTypingObject:(AIContentTyping *)inContentTyping
{
	...
	[libezv sendTypingNotification:(inContentTyping.typingState == AITyping ? AWEzvIsTyping : AWEzvNotTyping) to:to];
}
...
- (void)updateStatusForKey:(NSString *)key
{
	...
	if (areOnline) {
		if ([key isEqualToString:@"idleSince"]) {
			...
			[libezv setStatus:AWEzvIdle withMessage:[self.statusMessage string]];
...
- (NSSet *)supportedPropertyKeys
{
	...
	if (!supportedPropertyKeys) {
		supportedPropertyKeys = [[NSMutableSet alloc]
			initWithObjects:@"isOnline", @"Offline", @"idleSince", @"IdleManuallySet", @"Away", @"AwayMessage", nil];
...
- (void)updateProgressForFileTransfer:(EKEzvFileTransfer *)fileTransfer
							  percent:(NSNumber *)percent
							bytesSent:(NSNumber *)bytesSent
{
	[[ESFileTransfer existingFileTransferWithID:[fileTransfer uniqueID]]
		setPercentDone:[percent floatValue]
			 bytesSent:[bytesSent unsignedLongLongValue]];
}
...
- (void)user:(AWEzvContact *)contact sentFile:(EKEzvFileTransfer *)file
{
	...
	ESFileTransfer *fileTransfer = [adium.fileTransferController newFileTransferWithContact:listContact
																				 forAccount:self
