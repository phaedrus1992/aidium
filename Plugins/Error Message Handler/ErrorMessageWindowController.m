...
- (void)dealloc
{
	[errorTitleArray release];
	errorTitleArray = nil;
	[errorDescArray release];
	errorDescArray = nil;
	[errorWindowTitleArray release];
...
- (void)refreshErrorDialog
{
	...
	CGFloat titleHeightChange =
		[textView_errorTitle frame].size.height - [scrollView_errorTitle documentVisibleRect].size.height;
	...
	CGFloat errorInfoChange =
		[textView_errorInfo frame].size.height - [scrollView_errorInfo documentVisibleRect].size.height;
	...
	if ([errorTitleArray count] == 1) {
		...
		[[self window] setTitle:[NSString stringWithFormat:@"%@ (x%lu)", [errorWindowTitleArray objectAtIndex:0],
														   (unsigned long)[errorTitleArray count]]];
		[button_okay setTitle:AILocalizedString(@"Next", nil)];
		[button_dismissAll
			setTitle:AILocalizedString(@"Dismiss All", @"Used in the error window; closes all open errors.")];
