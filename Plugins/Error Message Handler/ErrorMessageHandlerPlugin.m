			   ...
			   userInfo:(id)userInfo
{
	...
	[NSDateFormatter withLocalizedDateFormatterShowingSeconds:NO
												showingAMorPM:YES
													  perform:^(NSDateFormatter *dateFormatter) {
														  dateString =
															  [[dateFormatter stringFromDate:[NSDate date]] retain];
													  }];
	...
	// Display an alert
	[[ErrorMessageWindowController errorMessageWindowController]
		   displayError:listObject.displayName
		withDescription:(alertText ? [NSString stringWithFormat:@"%@: %@", dateString, alertText] : @"")withTitle
					   :AILocalizedString(@"Contact Alert", nil)];
...
- (NSImage *)imageForEventID:(NSString *)eventID
{
	...
	if (!eventImage)
		eventImage = [[NSImage imageNamed:@"events-error-alert" forClass:[self class]] retain];
	return eventImage;
}
...
{
	return [NSString stringWithFormat:AILocalizedString(@"%lu errors", nil), (unsigned long)count];
}
