...
#define EMOTICON_PACKS_TOOLTIP                                                                                         \
	AILocalizedString(@"Reorder emoticon packs by dragging. Packs are used in the order listed.", nil)
...
- (void)_configureEmoticonListForSelection
{
	...
	if (selectedEmoticonPack) {
		...
		if (rowHeight < EMOTICON_MIN_ROW_HEIGHT)
			rowHeight = EMOTICON_MIN_ROW_HEIGHT;
		if (rowHeight > EMOTICON_MAX_ROW_HEIGHT)
	...
	if (selectedEmoticonPack) {
		...
		[textField_packTitle
			setStringValue:[NSString stringWithFormat:AILocalizedString(@"Emoticons in %@",
																		"Emoticons in <an emoticon pack name>"),
													  [selectedEmoticonPack name]]];
	} else {
...
{
	NSMutableDictionary *attributes = [NSMutableDictionary dictionaryWithObject:[NSColor grayColor]
			   ...
			   row:(NSInteger)row
{
	if (tableView == table_emoticons && [@"Enabled" isEqualToString:[tableColumn identifier]])
		[adium.emoticonController setEmoticon:[[selectedEmoticonPack emoticons] objectAtIndex:row]
									   inPack:selectedEmoticonPack
									  enabled:[object integerValue]];
}
	...
	dropOperation:(NSTableViewDropOperation)op;
{
	...
	NSString *availableType =
		[[info draggingPasteboard] availableTypeFromArray:[NSArray arrayWithObject:EMOTICON_PACK_DRAG_TYPE]];
	...
	for (AIEmoticonPackPreviewController *previewController in emoticonPackPreviewControllers) {
		...
		if ([movedPacks containsObjectIdenticalTo:emoticonPack]) {
			[tableView selectRowIndexes:[NSIndexSet indexSetWithIndex:[emoticonPackPreviewControllers
																		  indexOfObject:previewController]]
...
- (void)tableViewDeleteSelectedRows:(NSTableView *)tableView
{
	...
	NSRange range = [selectedEmoticonPack.path
		rangeOfString:[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"Emoticons"]];
	if (range.length > 0)
		NSBeep();
	else
...
- (void)trashConfirmSheetDidEnd:(NSWindow *)sheet returnCode:(NSInteger)returnCode contextInfo:(void *)contextInfo
{
	...
	for (AIEmoticonPackPreviewController *previewController in
		 [table_emoticonPacks selectedItemsFromArray:emoticonPackPreviewControllers]) {
...
{
	[table_emoticonPacks selectRowIndexes:[NSIndexSet indexSetWithIndex:[emoticonPackPreviewControllers
																			indexOfObject:packController]]
					 byExtendingSelection:NO];
}
