					   ...
					   offset:(NSPointPointer)dragImageOffset
{
	...
	for (i = 0; i < count; i++) {
		...
		NSRect targetFrame =
			NSMakeRect(cellFrame.origin.x - rowRect.origin.x, yOffset, cellFrame.size.width, cellFrame.size.height);
		...
		if ([cell isKindOfClass:[AIGenericViewCell class]]) {
			[(AIGenericViewCell *)cell drawEmbeddedViewWithFrame:targetFrame inView:self];
		} else {
			[cell drawWithFrame:targetFrame inView:self];
		}
