...
- (void)menu:(NSMenu *)menu needsUpdateForMenuItem:(NSMenuItem *)menuItem
{
	...
	if (targetMenuItem) {
		AIListObject *listObject =
			((targetMenuItem == menuItem_inviteToChat) ? [adium.interfaceController selectedListObjectInContactList]
...
- (NSMenu *)groupChatMenuForContact:(AIListContact *)contact
{
	...
	if (contact && ![contact isKindOfClass:[AIListGroup class]] && ![contact isKindOfClass:[AIListBookmark class]]) {
		...
		// Iterate on each service. For an AIListMetacontact, this may be multiple services; for an AIListContact, this
		// will just be a single iteration.
		...
		// Remove the last separator if our new number of items isn't bigger than the previous one (that is, we haven't
		// added any items since the last separator)
		currentNumberOfItems = [menu_chatMenu numberOfItems];
		if ((currentNumberOfItems <= numberOfMenuItems) && (currentNumberOfItems > 0)) {
