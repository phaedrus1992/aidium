...
{
	if (!stringSet)
		stringSet = [[NSMutableSet alloc] init];
...
- (void)addCompletionString:(NSString *)string withImpliedCompletion:(id)impliedCompletion
{
	if (![string isEqualToString:impliedCompletion]) {
		if (!impliedCompletionDictionary)
			impliedCompletionDictionary = [[NSMutableDictionary alloc] init];
...
- (void)textDidChange:(NSNotification *)notification
{
	...
	if (userValueLength > oldUserLength) {
		...
		if (completionValue != nil && [completionValue length] > lastValueLength) {
			...
			[self setStringValue:[userValue
									 stringByAppendingString:[completionValue substringFromIndex:lastValueLength]]];
...
- (NSString *)completionForString:(NSString *)inString
{
	...
	if (length >= minLength) {
		...
		while ((autoString = [enumerator nextObject])) {
			if (([autoString length] > length) && [autoString compare:compString
	...
	if ([sortedArray count] > 0) {
		...
		// When the AICompletingTextfield is modified to be able to provide multiple choices of completions, the entire
		// array can be used later. return sortedArray;
	}
...
- (id)impliedValueForString:(NSString *)aString
{
	...
	if (aString) {
		...
		NSString *impliedCompletionOfImpliedCompletion =
			(NSString *)[impliedCompletionDictionary objectForKey:impliedCompletion];
		 ...
		 * completion
		 */
		if (impliedCompletion && (!impliedCompletionOfImpliedCompletion ||
								  [impliedCompletionOfImpliedCompletion isEqual:impliedCompletion])) {
