...
- (void)addObserver:(id<AIHostReachabilityObserver>)newObserver forHost:(NSString *)host
{
	...
	[self scheduleReachabilityMonitoringForHost:hostCopy observer:newObserver];
}
		...
		if (newObserver == [observers objectAtIndex:i]) {
			if ((!host) || ([host isEqualToString:[hosts objectAtIndex:i]])) {
				...
				SCNetworkReachabilityScheduleWithRunLoop(
					(__bridge SCNetworkReachabilityRef)[reachabilities objectAtIndex:i], CFRunLoopGetCurrent(),
					kCFRunLoopDefaultMode);
				[reachabilities removeObjectAtIndex:i];

				[self removeUnconfiguredHost:host observer:newObserver];
...
- (BOOL)observer:(id)observer isObservingHost:(NSString *)host
{
	...
	if (host && observer) {
		...
		for (unsigned i = 0; i < numObservers; i++) {
			if ((observer == [observers objectAtIndex:i]) && ([host isEqualToString:[hosts objectAtIndex:i]])) {
				isObserving = YES;
				break;
			}
		}
		...
		if (!isObserving && [unconfiguredHostsAndObservers count]) {
			NSDictionary *unconfiguredHostObserverDict =
				[NSDictionary dictionaryWithObjectsAndKeys:observer, @"observer", host, @"host", nil];
...
- (void)reachabilityChanged:(SCNetworkReachabilityRef)reachability
{
	...
	if (i != NSNotFound) {
		...
		for (NSUInteger idx = 0; idx < hosts.count; idx++) {
			...
			SCNetworkReachabilityRef otherReachability =
				(__bridge SCNetworkReachabilityRef)[reachabilities objectAtIndex:idx];
			SCNetworkConnectionFlags flags;

			if (SCNetworkReachabilityGetFlags(otherReachability, &flags) && (flags & kSCNetworkFlagsReachable) &&
...
static void hostReachabilityChangedCallback(SCNetworkReachabilityRef target, SCNetworkConnectionFlags flags, void *info)
{
	...
	NSLog(@"*** hostReachabilityChangedCallback got flags: %c%c%c%c%c%c%c \n",
		  (flags & kSCNetworkFlagsTransientConnection) ? 't' : '-', (flags & kSCNetworkFlagsReachable) ? 'r' : '-',
		  ...
		  (flags & kSCNetworkFlagsIsLocalAddress) ? 'l' : '-', (flags & kSCNetworkFlagsIsDirect) ? 'd' : '-');
#endif
...
static void hostResolvedCallback(CFHostRef theHost, CFHostInfoType typeInfo, const CFStreamError *error, void *info)
{
	...
	if (typeInfo == kCFHostAddresses) {
		...
		if (!addresses || !CFArrayGetCount(addresses)) {
			 ...
			 */
			[self addUnconfiguredHost:host observer:observer];
		} else if (addresses) {
			...
			for (NSData *saData in (__bridge NSArray *)addresses) {
				...
				if ((remoteAddr->sa_family == AF_INET && addedIPv4) ||
					(remoteAddr->sa_family == AF_INET6 && addedIPv6)) {
					continue;
				}
				...
				// Create the pair
				reachabilityRef =
					SCNetworkReachabilityCreateWithAddressPair(NULL, (struct sockaddr *)&localAddr, remoteAddr);

				// Configure our callback
				SCNetworkReachabilitySetCallback(reachabilityRef, hostReachabilityChangedCallback,
												 &reachabilityContext);
				...
				[self gotReachabilityRef:reachabilityRef forHost:host observer:observer];
				...
				if (SCNetworkReachabilityGetFlags(reachabilityRef, &flags)) {
...
#endif
					hostReachabilityChangedCallback(reachabilityRef, flags, (__bridge void *)self);

				} else {
					/* Perform an immediate reachability check, since we've just scheduled checks for future changes
					 * and won't be notified immediately.  We update the hostContext to include our reachabilityRef
					 * before scheduling the info resolution (it's still in our run loop from when we requested the IP
					 * address).
					 */
					CFHostClientContext hostContext = {
						.version = 0,
						.info = (__bridge_retained void *)[NSDictionary
							dictionaryWithObjectsAndKeys:self, @"self", host, @"host", observer, @"observer",
					...
					CFHostSetClient(theHost, hostResolvedCallback, &hostContext);
					CFHostStartInfoResolution(theHost, kCFHostReachability, NULL);
				}
			}
		}
		...
		SCNetworkConnectionFlags flags;
		CFDataRef flagsData = CFHostGetReachability(theHost, NULL);
		CFDataGetBytes(flagsData, CFRangeMake(0, CFDataGetLength(flagsData)), (UInt8 *)&flags);
...
- (void)gotReachabilityRef:(SCNetworkReachabilityRef)reachabilityRef forHost:(NSString *)host observer:(id)observer
{
	...
	[self removeUnconfiguredHost:host observer:observer];
...
- (void)scheduleReachabilityMonitoringForHost:(NSString *)nodename observer:(id)observer
{
	...
	CFHostClientContext hostContext = {
		.version = 0,
		.info = (__bridge_retained void *)[NSDictionary
			dictionaryWithObjectsAndKeys:self, @"self", nodename, @"host", observer, @"observer", nil],
		.retain = CFRetain,
		.release = CFRelease,
		.copyDescription = CFCopyDescription,
	};
	CFHostRef host = CFHostCreateWithName(kCFAllocatorDefault, (__bridge CFStringRef)nodename);
	CFHostSetClient(host, hostResolvedCallback, &hostContext);
 ...
 * holds information on host/observer pairs which we haven't been able to resolve yet.  When the IP configuration
 * changes, we will try again, hoping to get an IP address this time.
 */
- (void)addUnconfiguredHost:(NSString *)host observer:(id)observer
{
	NSDictionary *unconfiguredHostDict =
		[NSDictionary dictionaryWithObjectsAndKeys:observer, @"observer", host, @"host", nil];
	...
	if (addedNewUnconfiguredHost) {
		 ...
		 * need to be negotiated at the router-level before we're actually connected.  In such a case, IPs aren't going
		 * to change... so we'll check one more time, 10 seconds from the last time we get an unconfigured host call,
		 * for connectivity.
		 */
		[NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(queryUnconfiguredHosts) object:nil];
		[self performSelector:@selector(queryUnconfiguredHosts) withObject:nil afterDelay:10.0];
	}
}
...
- (void)removeUnconfiguredHost:(NSString *)host observer:(id)observer
{
	[unconfiguredHostsAndObservers
		removeObject:[NSDictionary dictionaryWithObjectsAndKeys:observer, @"observer", host, @"host", nil]];
...
static void localIPsChangedCallback(SCDynamicStoreRef store, CFArrayRef changedKeys, void *info)
{
	 ...
	 */
	[self performSelector:@selector(queryUnconfiguredHosts) withObject:nil afterDelay:1.0];
}
...
- (void)beginMonitorngIPChanges
{
	if (!ipChangesRunLoopSourceRef) {
		...
		status = CreateIPAddressListChangeCallbackSCF(localIPsChangedCallback, (__bridge void *)self, &storeRef,
													  &ipChangesRunLoopSourceRef);

		// Add it to the run loop so we will receive the notifications
		if ((status == noErr) && ipChangesRunLoopSourceRef) {
			CFRunLoopAddSource(CFRunLoopGetCurrent(), ipChangesRunLoopSourceRef, kCFRunLoopDefaultMode);
		}
...
{
	if (ipChangesRunLoopSourceRef) {
		CFRunLoopRemoveSource(CFRunLoopGetCurrent(), ipChangesRunLoopSourceRef, kCFRunLoopDefaultMode);
 ...
 *
 * When the system wakes, manually reconfigure reachability checking as not all network configurations will report a
 * change.
 */
- (void)systemDidWake:(NSNotification *)notification
{
	...
	for (unsigned i = 0; i < numObservers; i++) {
		...
		[self addObserver:observer forHost:host];
	}
...
// address list changes.
{
	...
	err = MoreSCError(ref);
	if (err == noErr) {
		pattern = SCDynamicStoreKeyCreateNetworkServiceEntity(NULL, kSCDynamicStoreDomainState, kSCCompAnyRegex,
															  kSCEntNetIPv4);
