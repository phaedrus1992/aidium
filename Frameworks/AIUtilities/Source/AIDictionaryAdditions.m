...
+ (NSDictionary *)dictionaryAtPath:(NSString *)path withName:(NSString *)name create:(BOOL)create
{
	...
	NSDictionary *dictionary = [NSDictionary dictionaryWithContentsOfFile:[[path stringByAppendingPathComponent:name]
...
- (BOOL)validateAsPropertyList
{
	...
	for (NSString *key in self) {
		if (![key isKindOfClass:[NSString class]]) {
			NSLog(@"** Dictionary failed validation: %@: Key %@ is a %@ but must be an NSString", self, key,
		...
		if (![value isKindOfClass:[NSString class]] && ![value isKindOfClass:[NSData class]] &&
			...
			![value isKindOfClass:[NSDictionary class]] && ![value isKindOfClass:[NSDate class]]) {
			NSLog(@"** Dictionary failed validation: %@: Value %@ is a %@ but must be a string, data, number, array, "
				  @"dictionary, or date",
	...
	if (plistData) {
		BOOL success =
			[plistData writeToFile:[[path stringByAppendingPathComponent:name] stringByAppendingPathExtension:@"plist"]
						atomically:YES];
		if (!success)
			NSLog(@"%s: Error writing path %@, name %@ (%@)", __PRETTY_FUNCTION__, path, name,
				  [[path stringByAppendingPathComponent:name] stringByAppendingPathExtension:@"plist"]);
...
- (NSDictionary *)dictionaryByTranslating:(NSDictionary *)translation
								   adding:(NSDictionary *)addition
								 removing:(NSSet *)removal
{
	NSDictionary *result;
				...
				includeChangedKeys:(BOOL)flag
{
	if (!other) {
		if (outAddedKeys)
			*outAddedKeys = [self allKeysSet];
		if (outRemovedKeys)
			*outRemovedKeys = [NSSet set];
	} else {
		...
		for (NSString *key in self) {
			...
			if (!otherObj) {
				...
				// Exists in both, objects are not equal, and flag is not NO: Different, and we should put the key in
				// both sets to indicate this.
				[addedKeys addObject:key];
				[removedKeys addObject:key];
			}
		}
		if (outAddedKeys)
			*outAddedKeys = [NSSet setWithSet:addedKeys];
