...
+ (NSToolbarItem *)toolbarItemFromDictionary:(NSDictionary *)theDict withIdentifier:(NSString *)itemIdentifier
{
	...
	if ([item view] != NULL) {
		if ([[item view] respondsToSelector:@selector(copyWithZone:)]) {
			 ...
			 * If the view doesn't conform to NSCopying, use the _same_ view. NSToolbar's copy method will have created
			 * a new NSView and attempted to make it match to the original one.
			 */
