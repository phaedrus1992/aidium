 ...
 *
 * Sent continuously as the mouse moves within the visible view monitored by the <tt>AISmoothTooltipTracker</tt>.  It is
 * initially sent after the mouse hovers for at least a second in a single location within the view. Sent regardless of
 ...
 *
 * Informs the delegate that the mouse has left the view so the tooltip should be hidden. Sent when the mouse leaves the
 * view or it becomes obscured or hidden.
 */
- (void)hideTooltip;
@end
 ...
 *
 * <p>An <tt>AISmoothTooltipTracker</tt> is created for a specific view.  It informs its delegate when the mouse hovers
 * over the view for about a second.</p> <p>The delegate will be informed of the mouse hover even if the application is
 * not currently active (so long as the view is visible to the user).</p> The delegate is updated as the mouse moves
 * (via showTooltipAtPoint:), and is informed when the mouse leaves or the view is obscured or hidden (via
 * hideTooltip)</p> <p>Note: The delegate is not retained.  For maximum stability, the delegate should call
 * setDelegate:nil some time before it deallocs. Not all implementations will -need- this, but it is recommended.</p>
 */
 ...
 */
+ (AISmoothTooltipTracker *)smoothTooltipTrackerForView:(NSView *)inView
										   withDelegate:(id<AISmoothTooltipTrackerDelegate>)inDelegate;
...
/*
 * @brief This should be called when the view for which we are tracking will be removed from its window without the
 * window closing
 *
