...
- (void)drawInteriorWithFrame:(NSRect)cellFrame inView:(NSView *)controlView
{
	...
	if (img) {
		...
		if ((imgSize.height > ourMaxSize.height) || (imgSize.width > ourMaxSize.width)) {
		...
		// Centering
		targetRect = NSOffsetRect(targetRect, AIround((cellFrame.size.width - targetRect.size.width) / 2),
								  AIround((cellFrame.size.height - targetRect.size.height) / 2));
