 ...
 *
 * Notifies the delegate of a new image selected by the user (which may have been set in any of the ways explained in
 * the class description). This may not provide information as worthwhile as
 * imageViewWithImagePicker:didChangeToImageData:, which is the recommended method to implement
 ...
 *
 * Notifies the delegate of an attempt to delete the image.  This may occur by the user pressing delete with the image
 * selected or by the user performing a Cut operation on it. Recommended behavior is to clear the image view or to
 ...
 *
 * By default, the Image Picker will use the same image as the <tt>AIImageViewWithImagePicker</tt> object does. However,
 * if the delegate wishes to supply a different image to be initially displayed in the Image Picker (for example, if a
 * larger or higher resolution image is available), it may implement this method.  If this method returns nil, the
 * imageView's own image will be used just as if the method were not implemented.
 * @param picker The <tt>AIImageViewWithImagePicker</tt> which will display the Image Picker
 * @return An <tt>NSImage</tt> to display in the Image Picker, or nil if the <tt>AIImageViewWithImagePicker</tt>'s own
 ...
 * @brief Requests the name under which to save a file dragged from the image picker to the Finder or another
 * destination
 *
