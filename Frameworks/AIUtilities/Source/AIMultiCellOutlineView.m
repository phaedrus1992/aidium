...
- (void)setContentCell:(NSCell *)cell
{
	...
	[self
		noteHeightOfRowsWithIndexesChanged:[NSIndexSet indexSetWithIndexesInRange:NSMakeRange(0, [self numberOfRows])]];
}
- (NSCell *)contentCell
{
	return contentCell;
...
{
	return ([(id<AIMultiCellOutlineViewDelegate>)[self delegate] outlineView:self isGroup:item] ? groupCell
																								: contentCell);
}
	...
	if (item && [self isExpandable:item] && (viewPoint.x < NSHeight([self frameOfCellAtColumn:0 row:row]))) {
		...
		if ([(id<AIMultiCellOutlineViewDelegate>)[self delegate] outlineView:self isGroup:item]) {
			...
			if (needsExpandCollapseSuppression)
				*needsExpandCollapseSuppression = YES;
			handled = NO;
		}
