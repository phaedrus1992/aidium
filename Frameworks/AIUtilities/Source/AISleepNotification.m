 ...
 * @brief Class to notify when the system goes to sleep and wakes from sleep and optionally place a hold on the sleep
 * process.
 *
 * AISleepNotification posts (on the default NSNotificationCenter) <tt>AISystemWillSleep_Notification</tt> when the
 * system is about to go to sleep, and posts <tt>AISystemDidWake_Notification</tt> when it wakes from sleep.
 *
 ...
 * Through the use of these, a program can preemptively delay sleeping which may occur during, for example, the
 * execution of a lengthy block of code which shouldn't be interrupted, and then remove the delay when execution of the
 * code is complete.
 */
...
+ (void)initialize
{
	...
	if (root_port) {
		...
		NSLog(@"AISleepNotification: Could not retrieve root_port from IORegisterForSystemPower(). Sleep notification "
			  @"disabled.");
	}

	// Observe Hold/continue sleep notification
	[defaultCenter addObserver:self selector:@selector(holdSleep:) name:AISystemHoldSleep_Notification object:nil];
 ...
 */
static void powerServiceInterestCallback(void *refcon, io_service_t service, natural_t messageType,
