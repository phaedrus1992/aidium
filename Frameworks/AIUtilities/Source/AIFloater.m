...
- (id)initWithImage:(NSImage *)inImage styleMask:(unsigned int)styleMask
{
	if ((self = [super init])) {
		...
		panel = [[NSPanel alloc] initWithContentRect:frame styleMask:styleMask backing:NSBackingStoreBuffered defer:NO];
...
- (void)setMaxOpacity:(CGFloat)inMaxOpacity
{
	...
	if (windowIsVisible)
		[self AI_setWindowOpacity:maxOpacity];
}
...
- (void)setVisible:(BOOL)inVisible animate:(BOOL)animate
{
	if (inVisible != windowIsVisible) {
		...
		if (animate) {
			if (!fadeAnimation) {
				NSDictionary *animDict = [NSDictionary
					dictionaryWithObjectsAndKeys:panel, NSViewAnimationTargetKey,
												 inVisible ? NSViewAnimationFadeInEffect : NSViewAnimationFadeOutEffect,
