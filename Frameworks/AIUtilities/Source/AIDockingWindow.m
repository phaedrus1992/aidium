	...
	if (!alreadyMoving && dockingEnabled && ![NSEvent shiftKey]) {
		...
		if (!NSEqualRects(newWindowFrame, dockedWindowFrame)) {
			...
			if (!NSIsEmptyRect(oldWindowFrame)) {
				...
				// If the user is trying to move in the opposite X direction as the docking movement, use the user's
				// movement
				...
				// If the user is trying to move in the opposite Y direction as the docking movement, use the user's
				// movement
...
- (void)toggleToolbarShown:(id)sender
{
	...
	if ([self delegate] && [[self delegate] respondsToSelector:@selector(windowDidToggleToolbarShown:)]) {
		[[self delegate] performSelector:@selector(windowDidToggleToolbarShown:) withObject:self];
	}

	[[NSNotificationCenter defaultCenter] postNotificationName:AIWindowToolbarDidToggleVisibility object:self];
}
