...
{
	return ([[NSMutableString alloc] initWithData:[NSData dataWithContentsOfFile:path] encoding:NSASCIIStringEncoding]);
}
...
- (void)convertNewlinesToSlashes
{
	...
	replacements = [self replaceOccurrencesOfString:@"\r" withString:@" / " options:NSLiteralSearch range:fullRange];
	fullRange.length += (2 * replacements);

	[self replaceOccurrencesOfString:@"\n" withString:@" / " options:NSLiteralSearch range:fullRange];
}
