...
- (void)betterCenter
{
	...
	[self setFrame:NSMakeRect(screen.origin.x + (screen.size.width - frame.size.width) / 2.0f,
							  screen.origin.y + (screen.size.height - frame.size.height) / 1.2f, frame.size.width,
							  frame.size.height)
		   display:NO];
}
 ...
 * @param The NSResponder earliest in the responder chain which matches the passed values, or nil if no such responder
 * exists
 */
- (NSResponder *)earliestResponderWhichRespondsToSelector:(SEL)selector andIsNotOfClass:(Class)classToAvoid
{
	...
	while (responder &&
		   (![responder respondsToSelector:selector] || ((classToAvoid && [responder isKindOfClass:classToAvoid])))) {
 ...
 * @result The NSResponder earliest in the responder chain which is of class targetClass, or nil if no such responder
 * exists
 */
