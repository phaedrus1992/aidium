...
// an enter by the characters inserted (both insert CR, ASCII 10), it also watches and remembers the keys being pressed
// with interpretKeyEvents... When insertText sees a CR, it checks to see what key was pressed to generate that CR, and
// makes a decision to send or not.  Since the sending occurs from within insertText, the returns are processed in the
// correct order with the text, and the problem is eliminated.
//
...
- (void)insertText:(id)aString
{
	...
	if ((sendingEnabled) && ((nextIsReturn && sendOnReturn) || (nextIsEnter && sendOnEnter)) &&
