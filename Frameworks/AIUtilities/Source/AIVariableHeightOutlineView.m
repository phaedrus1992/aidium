...
- (NSImage *)dragImageForRows:(NSUInteger *)buf
 ...
 * @param needsExpandCollapseSuppression Pointer to a BOOL which, on return, will be YES if we need to prevent
 * NSOutlineView from trying to expand/collapse a group.
 *
 * @result YES if the event was handled; NO if it should be processed normally
 */
- (BOOL)handleExpandedStateToggleForEvent:(NSEvent *)theEvent
		   needsExpandCollapseSuppression:(BOOL *)needsExpandCollapseSuppression
{
	...
	if (item && [self isExpandable:item] && (viewPoint.x < NSHeight([self frameOfCellAtColumn:0 row:row]))) {
	...
	if (needsExpandCollapseSuppression)
...
- (void)mouseDown:(NSEvent *)theEvent
{
	...
	if (![self handleExpandedStateToggleForEvent:theEvent
				  needsExpandCollapseSuppression:&needsExpandCollapseSuppression]) {
 ...
 * -[self rectOfRow:] is reasonable cheap once, it can get quite expensive additively. See for example #14463 (700
 * Twitter contacts).
 */
- (BOOL)_ai_drawRow:(NSInteger)row clipRect:(NSRect)rect
{
	if (row >= 0 &&
		row < [self numberOfRows]) { // Somebody keeps calling this method with row = numberOfRows, which is wrong.
		...
		for (tableColumnIndex = 0; tableColumnIndex < count; tableColumnIndex++) {
			...
			// Draw the cell
			if (selected)
				[cell _drawHighlightWithFrame:cellFrame inView:self];
					   ...
					   offset:(NSPointPointer)dragImageOffset
{
	...
	for (i = 0; i < count; i++) {
		...
		for (tableColumnIndex = 0; tableColumnIndex < tableColumnsCount; tableColumnIndex++) {
			...
			[cell drawWithFrame:NSMakeRect(cellFrame.origin.x - rowRect.origin.x, yOffset, cellFrame.size.width,
	...
	if (drawsSelectedRowHighlight && (!drawHighlightOnlyWhenMain || [[self window] isMainWindow])) {
		if ([self drawsGradientSelection]) {
			...
			while (i < bufSize) {
				...
				NSRect highlightRect = NSUnionRect([self rectOfRow:startIndex], [self rectOfRow:lastIndex]);
