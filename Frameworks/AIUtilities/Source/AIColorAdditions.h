 ...
 *
 *	@par	This method loads the file at /usr/share/emacs/ * /etc/rgb.txt. It is not guaranteed to load any specific
 * file matching that glob, but it will load only one.
 *
 ...
 *
 *	@par	This method assumes that the receiver and the other color are both in an RGB color-space. If they're not,
 ...
 *
 *	@par	This method measures the brightness of the receiver using <code>-[NSColor brightnessComponent]</code>, and
 * considers it dark if that value < 0.5.
 *
 ...
 *
 *	@par	This method measures the brightness of the receiver using <code>-[NSColor brightnessComponent]</code>, and
 * considers it medium if 0.35 < that value < 0.65.
 *
 ...
 *
 *	@par	This method subtracts \a amount from the brightness of the receiver (as measured using <code>-[NSColor
 * brightnessComponent]</code>), and returns a new \c NSColor created with the unchanged other component values and the
 * new brightness value.
 *
 *	@par	This method does not adjust the saturation of the receiver. For a method that does, see
 * <code>darkenAndAdjustSaturationBy:</code>.
 *
 ...
 *
 *	@par	This method subtracts \a amount from the brightness of the receiver (as measured using <code>-[NSColor
 * brightnessComponent]</code>) and adds \a amount to its saturation (as measured using <code>-[NSColor
 * saturationComponent]</code>), and returns a new \c NSColor created with the unchanged other component values and the
 * new saturation and brightness values.
 *
 *	@par	This method increases the saturation of the receiver. For a method that doesn't, see <code>darkenBy:</code>.
 *
 *	@par	You would use this method when you want to darken a color while preserving the “color” apparent to the user.
 * The usual reason to do this is to create two ends of a gradient from a single input color.
 *
 *	@param	amount The amount (greater than 0.0 and at most 1.0) to subtract from the receiver's brightness and add to
 ...
 *
 *	@par	The naïve method for inverting a color is to subtract all of its color components from 1.0, but this moves
 * colors not in the center of the color wheel to the opposite side of the wheel—for example, it changes orange into
 * light blue. In other words, it inverts not the brightness (except for shades of gray), but the hue.
 *
 *	@par	This method, on the other hand, inverts the brightness of a color without moving it across the color wheel.
 * For example, dark orange becomes light orange, and vice versa.
 *
 ...
 *
 *	@par	If the receiver is medium (see \c colorIsMedium), this method returns white if the receiver is dark (see \c
 * colorIsDark), or black if it isn't. If the receiver is not medium, this method inverts the receiver and returns the
 * result.
 *
 *	@return	A new color.
 */
 ...
 *
 *	@par	This function exists to help you compute HSL values without first creating an \c NSColor instance. It also
 * gets the job done in only one function call, rather than at least two Objective-C messages.
 *
 *	@par	Hue is expressed as a fraction of a circle. Red is any multiple of 1 (a whole circle), and will be returned
 * as 0.
 *
 *	@param	hue	A pointer to a \c float. If you don't pass \c NULL, this function will assign the hue component here.
 *	@param	saturation	A pointer to a \c float. If you don't pass \c NULL, this function will assign the saturation
 * component here.
 *	@param	luminance	A pointer to a \c float. If you don't pass \c NULL, this function will assign the luminance
 * (brightness) component here.
 ...
 */
void getHueLuminanceSaturationFromRGB(CGFloat *hue, CGFloat *luminance, CGFloat *saturation, CGFloat r, CGFloat g,
									  CGFloat b);
/*!	@brief	Computes the red, green, and blue of a given hue, saturation, and brightness triplet.
 *
 *	@par	This function exists to help you compute RGB values without first creating an \c NSColor instance. It also
 * gets the job done in only one function call, rather than at least two Objective-C messages.
 *
 ...
 */
void getRGBFromHueLuminanceSaturation(CGFloat *r, CGFloat *g, CGFloat *b, CGFloat hue, CGFloat luminance,
									  CGFloat saturation);
 ...
 *
 *	@par	Each of the deltas will be added to that component of the receiver; thus, you may subtract by passing a
 * negative delta.
 *
 ...
 *
 *	@par	Each component is represented by two hexadecimal digits; thus, the result will always be six characters
 * long.
 *
...
/*!	@brief	Returns the red, green, blue, and alpha components of the receiver as a triplet of decimal numbers from 0 to
 * 255.
 *
 *	@par	The alpha component is only included in the result string if it is not 1. If the alpha component <em>is</em>
 * 1, it will be omitted from the result.
 *
 *	@par	The three or four numbers are separated by commas, with no spaces.
 *
 *	@par	This method assumes that the receiver is in an RGB color space. If it isn't, you will get an exception.
 *
 *	@par	To convert the result from this method back to an \c NSColor, use <code>-[NSString(AIColorAdditions)
 * representedColor]</code>.
 *
 ...
 *
 *	@par	If the receiver's alpha component is not 1, the literal will use CSS' <code>rgba()</code> function. If it is
 * 1, the literal will be an HTML-style hex literal (e.g., <code>#ffffff</code>).
 *
 *	@par	The returned string is suitable for splicing into CSS code; for example, <code>[NSString
 * stringWithFormat:@"color: %@;", [myColor CSSRepresentation]]</code>.
 *
 ...
 *
 *	@par	If \a hex is not a valid hexadecimal character, this function returns -1. Remember to check its return
 * value.
 *
 ...
 *
 *	@par	The numbers must be separated by commas, and must range from 0 to 255, inclusive. Their order is red, green,
 * blue, alpha.
 *
 *	@par	This method returns \c nil if the string does not contain exactly three or four numbers, or has anything
 * before or after the numbers (except that whitespace before the first number is skipped).
 *
 *	@par	To create a string suitable for sending this message to, call <code>-[NSColor(AIColorAdditions)
 ...
 * blue, alpha.
 *
 *	@par	Unlike <code>-representedColor</code>, this method forces a specific alpha value in the returned color,
 * ignoring (if it's present) the fourth value in the receiver.
 *
 ...
 *	@par	You do not need to seed a PRNG before calling this method.
 *
 *	@par	If you want the alpha to always be 1 as well, use <code>+randomColor</code> instead. You can change the
 * alpha to another static value using <code>-[NSColor colorWithAlphaComponent:]</code>.
 *
 *	@return	A random color in the calibrated-RGB color space.
 */
 ...
 *
 *	@par	As an extension, this method supports alpha. If \a str contains four or eight characters, the last quarter
 * of the string is the alpha component.
 *
 *	@par	If \a str is shorter than six characters, this method will imagine zeros to fill the shortfall, as WebKit
 * does. For example, “#ff000” (five characters) becomes “#ff0000” (six characters). This does not apply to the
 * abbreviated syntax: a two-character string is illegal.
 *
 *	@par	If \a str is not a valid literal, this method returns \c nil.
 *
 *	@par	This method does not support CSS' <code>rgba()</code> function; therefore, we do not recommend using it as
 * the inverse of <code>-CSSRepresentation</code>.
 *
 ...
 * abbreviated syntax: a two-character string is illegal.
 *
 *	@par	If \a str is not a valid literal, this method returns \a defaultColor. You may pass \c nil for \a
 * defaultColor.
 *
 *	@par	This method does not support CSS' <code>rgba()</code> function; therefore, we do not recommend using it as
 * the inverse of <code>-CSSRepresentation</code>.
 *
 *	@par	If it creates the color from the input string, this method will use the calibrated-RGB color space. If,
 * instead, it returns \a defaultColor, it will not convert it to the calibrated-RGB color space for you: you will get
 * back whatever you passed in.
 *
 *	@param	str	The string to interpret as an HTML color literal.
...
/*!	@brief	Compute a colour for an object based on it's hash, represnted in an HTML color name from within a list of
 * valid colours.
 *
 *	@param	anObject an object for which to compute a matching colour.
 *	@param	validColors an NSArray containing a list of valid colours to return. If nil or empty, the colour will be
 * chosen from a default list.
 *
