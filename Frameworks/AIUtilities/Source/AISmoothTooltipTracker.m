...
#if LOG_TRACKING_INFO
		NSLog(@"[%@ installCursorRect] addTrackingRect %@ on %@ in %@: tag = %i", self, NSStringFromRect(trackingRect),
			  view, [view window], tooltipTrackingTag);
#endif
...
#if LOG_TRACKING_INFO
	if (tooltipTrackingTag != -1) {
		NSLog(@"[%@ removeCursorRect] Remove rect from %@ in %@ : tag = %i", self, view, [view window],
			  tooltipTrackingTag);
	} else {
...
// ------------------------------------------------------------------------------------------- We use a timer to poll
// the location of the mouse.  Why do this instead of using mouseMoved: events?
...
#if LOG_TRACKING_INFO
	NSLog(@"%@: Visible: %i ; Point %@ in %@ = %i", self, [[view window] isVisible],
		  /*		  NSStringFromPoint([[view superview] convertPoint:[[view window]
			 convertPointFromScreen:mouseLocation] fromView:[[view window] contentView]]),*/
		  ...
		  /*NSPointInRect([[view superview] convertPoint:[[view window] convertPointFromScreen:mouseLocation]
			 fromView:[[view window] contentView]],[view frame])*/
		  NSPointInRect([[view window] convertPointFromScreen:mouseLocation],
						[[[view window] contentView] convertRect:[view frame] fromView:[view superview]]));
#endif

	if ([theWindow isVisible] &&
		NSPointInRect([theWindow convertPointFromScreen:mouseLocation],
					  [[theWindow contentView] convertRect:[view frame] fromView:[view superview]]) &&
