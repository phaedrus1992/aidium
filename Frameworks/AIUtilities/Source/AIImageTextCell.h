 ...
 *
 * This NSCell subclass displays in image on the left and one or two lines of text centered vertically in the space
 * remaining for the cell
 */
 ...
 *
 * If non-nil, this string will be drawn underneath the stringValue of the cell.  The two will, together, be vertically
 * centered (when not present, the stringValue alone is vertically centered). It is drawn in with the system font, at
 * size 10.
 */
 ...
 *
 *	@par	If \c YES, the image is drawn after with main string, with \c imageTextPadding points between them. If \c
 * NO, the image is drawn before both strings (if there is a \c subString, the image won't be placed on the baseline of
 * the main string).
 *
 *	@return	\c YES if the image will be drawn after the main string; \c NO if the image will be drawn before both
 * strings.
 */
 ...
 * the main string).
 *
 *	@param	flag	\c YES if you want the image to be drawn after the main string; \c NO if you want the image to be
 * drawn before both strings.
 */
