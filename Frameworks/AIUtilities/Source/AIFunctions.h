 ...
 *	If the point lies on the interior side of that edge, the number returned will be negative, even if the point is
 * outside the rectangle itself. For example, if \c rect.origin.x is 50, and \c rect.size.width is 50, and \c point.x is
 * 25, and \a edge is \c NSMaxXEdge, the result will be -75.0f.
 *
 *	@return The distance between the edge and the point. It is positive if the point is outside the edge, negative if it
 * is inside the edge (even it is outside the rectangle).
 */
...
NSRect AIRectByAligningRect_edge_toRect_edge_(NSRect mobileRect, NSRectEdge mobileRectEdge, NSRect stationaryRect,
 ...
 */
BOOL AIRectIsAligned_edge_toRect_edge_tolerance_(NSRect rect1, NSRectEdge edge1, NSRect rect2, NSRectEdge edge2,
												 CGFloat tolerance);
