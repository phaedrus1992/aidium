 ...
 *
 * This delegate method gives the delegate an opportunity to return an <tt>NSMenu</tt> to be displayed when the user
 * right-clicks (control-clicks) in the outline view.  The passed event can be used to determine where the click
 * occurred to make the menu sensitive to which row or column was clicked.
 ...
 *
 * After reloading data, NSOutlineView collapses all items, which is generally not a desired behavior.
 * <tt>AIOutlineView</tt> provides the delegate an opportunity to specify whether each item should be exapnded or
 * collapsed after a reload. This delegate method should return the correct expanded state of the passed item.
 ...
 *
 * Informs the delegate of a request to delete the selected rows (via the delete key, generally). The delegate may wish
 * to retrieve the currently selected rows and remove them from the data source and subsequently reload the outline
 * view.
 ...
 * implement the find panel UI and filtering
...
/*!
 * @brief Informs the delegate of a request to forward a key down event to the find panel text field, displaying it if
 * necessary
 *
 * Informs the delegate of a request to forward a key down event to the find panel text field,displaying it if
 * necessary. The key down event was recieved by outlineView, and it determined it should be added to the search string
 * @param outlineView The <tt>NSOutlineView</tt> that the user wants to filter
 ...
 *
 * <p>This <tt>NSOutlineView</tt> subclass has several improvements and serves as the base class for the other outline
 * view subclasses in the AIUtilities framework.</p> <p>It supports contextual menu, expanded state, deletion, and
 * dragging-ended notification methods for its delegate (see the <tt>AIOutlineViewDelegate</tt> protocol description for
 * details).</p> <p>It posts AIOutlineViewUserDidExpandItemNotification and AIOutlineViewUserDidCollapseItemNotification
 * to the default NSNotificationCenter when items expand and collapse, respectively; for these notifications, the object
 * is the <tt>AIOutlineView</tt> and the userInfo is an NSDictionary with the changed item in the key @"Object". <p>It
 * supports improved keyboard navigation of the outline view, including supporting the delete key.</p> <p>It adds the
 * use of tab and backtab to cycle through matches when using KFTypeSelectTableView</p> <p>Finally, it fixes a crash
 * when reloadData is called from 'outlineView:setObjectValue:forTableColumn:byItem:' while the last row is edited in a
 * way that will reduce the number of rows in the outline view (crash fix is relevant on all system versions as of OS
 * X 10.3.7).</p>
 */
