...
+ (NSImage *)imageForSSL
{
	...
	if (!SSLIcon) {
		...
		SSLIcon =
			[[NSImage alloc] initByReferencingFile:[securityInterfaceFramework pathForImageResource:@"CertSmallStd"]];
	}
	...
	for (NSImageRep *rep in [self representations]) {
		if ([rep isKindOfClass:NSBitmapImageRepClass]) {
			...
			if (thisWidth >= maxWidth) {
				// Cast explanation: GCC warns about us returning an NSImageRep here, presumably because it could be
				// some other kind of NSImageRep if we don't check the class. Fortunately, we have such a check. This
				// cast silences the warning.
		...
		if (size.width > 0 && size.height > 0) {
			...
			for (unsigned i = 0; i < frameCount; i++) {
				...
				[images addObject:[NSBitmapImageRep
									  imageRepWithData:[bitmap representationUsingType:NSGIFFileType
...
- (NSData *)bestRepresentationByType
{
	...
	if ((bitmap = [[self representations] objectAtIndex:0]) && [bitmap isKindOfClass:[NSBitmapImageRep class]] &&
...
- (void)writeGIFExtensionBlocksInData:(NSMutableData *)data forRepresenation:(NSBitmapImageRep *)bitmap
{
	...
	const char *GIFApplicationExtensionBlock =
		"\x21\xFF\x0B\x4E\x45\x54\x53\x43\x41\x50\x45\x32\x2E\x30\x03\x01\x00\x00\x00";
	...
	while (!NSEqualRanges(blockRange = [data rangeOfData:GIFGraphicControlExtensionBlock
...
- (NSDictionary *)GIFPropertiesForRepresentation:(NSBitmapImageRep *)bitmap
{
	...
	return [NSDictionary
		dictionaryWithObjects:[NSArray arrayWithObjects:frameCount, [NSNumber numberWithUnsignedInt:0], nil]
					  forKeys:[NSArray arrayWithObjects:NSImageFrameCount, NSImageCurrentFrame, nil]];
}
