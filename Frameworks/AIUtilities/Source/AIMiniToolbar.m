	...
	if (identifier != inIdentifier) {
		...
		[[NSNotificationCenter defaultCenter] addObserver:self
												 selector:@selector(toolbarItemsChanged:)
													 name:AIMiniToolbar_ItemsChanged
												   object:identifier];
		[[NSNotificationCenter defaultCenter] addObserver:self
												 selector:@selector(toolbarRefreshItem:)
													 name:AIMiniToolbar_RefreshItem
												   object:nil];
	}
}
...
- (NSArray *)rebuildItems
{
	...
	if (!itemIdentifierArray)
		itemIdentifierArray = [[NSArray alloc] init];
...
- (void)smoothlyArrangeItems
{
	if (!itemsRearranging) {
		...
		[NSTimer scheduledTimerWithTimeInterval:(1.0 / MINI_TOOLBAR_FPS)
										 target:self
									   selector:@selector(arrangeItemsTimer:)
									   userInfo:nil
										repeats:NO];
	}
}
...
- (BOOL)arrangeItems:(NSArray *)targetItemArray absolute:(BOOL)absolute
{
	...
	if (hoverIndex != -1)
		totalWidth += hoverSize.width + MINI_TOOLBAR_ITEM_SPACING;
...
- (void)draggedImage:(NSImage *)image endedAt:(NSPoint)screenPoint operation:(NSDragOperation)operation
{
	...
	if (operation == NSDragOperationNone) {
		...
		[AIAnimatedFloater animatedFloaterWithImage:[NSImage imageNamed:MINI_TOOLBAR_POOF forClass:[self class]]
											   size:puffSize
											 frames:5
											  delay:0.08
												 at:puffOrigin];
...
- (BOOL)performDragOperation:(id<NSDraggingInfo>)sender
{
	...
	if (hoverIndex >= 0 && hoverIndex <= [itemIdentifierArray count]) {
		...
		// Set the frame of the item that was dragged to where the user dropped it, so it will smoothly slide from that
		// position to where it belongs.  This looks cleaner than just 'snapping' the item from where it used to be.
		if (focusedForDrag && draggedItem) {
			...
			[[draggedItem view]
				setFrameOrigin:NSMakePoint(localDrop.x - draggedOffset.width, localDrop.y - draggedOffset.height)];
		}

		// Move/insert the item
		[self
			insertItemWithIdentifier:[[sender draggingPasteboard] stringForType:MINI_TOOLBAR_ITEM_DRAGTYPE]
							 atIndex:dropIndex
					 allowDuplicates:!([pasteboard
										 stringForType:MINI_TOOLBAR_SOURCE])]; // If there is no source, then this item
																			   // was dragged from the customization
																			   // palette - so duplicates are allowed.
