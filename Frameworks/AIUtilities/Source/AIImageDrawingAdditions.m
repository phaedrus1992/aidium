...
- (NSImage *)imageByScalingToSize:(NSSize)size DPI:(CGFloat)dpi
{
	...
	return ([self imageByScalingToSize:NSMakeSize(size.width * backingScaleFactor, size.height * backingScaleFactor)
							  fraction:1.0f
							 flipImage:NO
						proportionally:YES
						allowAnimation:YES]);
				   ...
				   allowAnimation:(BOOL)allowAnimation
{
	...
	if ((NSEqualSizes(originalSize, size)) && (delta == 1.0) && !flipImage) {
		...
		if (allowAnimation &&
			(bestRep = [self bestRepresentationForRect:NSMakeRect(0, 0, self.size.width, self.size.height)
											   context:nil
												 hints:nil]) &&
			...
			for (unsigned i = 0; i < frameCount; i++) {
				// Set current frame
				[(NSBitmapImageRep *)bestRep setProperty:NSImageCurrentFrame
				   ...
				   allowAnimation:(BOOL)allowAnimation
{
	...
	if ((NSEqualSizes(originalSize, size)) && (delta == 1.0) && !flipImage) {
		...
		NSPoint fitFromPoint =
			NSMakePoint((size.width - scaleSize.width) / 2.0f, (size.height - scaleSize.height) / 2.0f);
...
- (NSRect)rectForDrawingInRect:(NSRect)rect atSize:(NSSize)size position:(IMAGE_POSITION)position
{
	...
	case IMAGE_POSITION_LEFT:
		drawRect = NSMakeRect(rect.origin.x, rect.origin.y + (int)((rect.size.height - size.height) / 2.0), size.width,
							  size.height);
		break;
	case IMAGE_POSITION_RIGHT:
		drawRect = NSMakeRect(rect.origin.x + rect.size.width - size.width,
							  rect.origin.y + (int)((rect.size.height - size.height) / 2.0), size.width, size.height);
		break;
	case IMAGE_POSITION_LOWER_LEFT:
		drawRect = NSMakeRect(rect.origin.x, rect.origin.y + (rect.size.height - size.height), size.width, size.height);
		break;
	case IMAGE_POSITION_LOWER_RIGHT:
		drawRect = NSMakeRect(rect.origin.x + (rect.size.width - size.width),
							  rect.origin.y + (rect.size.height - size.height), size.width, size.height);
		break;
	}
