 ...
 *
 * If YES, the scrollView will scroll to the bottom when new content is added (i.e. when the frame of the document view
 * increases), bringing the new data into visibility. Automatic scrolling will only occur if the view was scrolled to
 * the bottom previously; it will not force a scroll to the bottom if the user has scrolled up in the scrollView. The
 * default value is NO.
 ...
 *
 * If YES, keyDown: events which the scrollView receives and which do not cause it to scroll will be passed to the
 * documentView for handling. Defualts to NO.
 ...
 *
 * Some contained views, such as NSTextFields, do not draw focus rings.  This is generally correct, but it may be
 desirable to draw a focus ring around such views anyways.  If <b>inFlag</b> is YES, a focus ring will be drawn when the
 view has focus regardless of its type. The default value is NO.
