...
- (void)adjustColorsToShowOnBackground:(NSColor *)backgroundColor
{
	...
	while (idx < stringLength) {
		...
		if (deltaBrightness >= 0 && deltaBrightness < 0.4f) { // too close
			// change the color
			fontColor = [NSColor colorWithCalibratedHue:[fontColor hueComponent]
...
// adjust the colors in the string so they're visible on the background, adjusting brightness in proportion to the
// original background
...
- (void)convertAttachmentsToStringsUsingPlaceholder:(NSString *)placeholder
{
	if ([self length] && [self containsAttachments]) {
		...
		attachmentRange = [[self string] rangeOfString:attachmentCharacterString
											   options:0
												 range:NSMakeRange(currentLocation, [self length])];
		...
		while (attachmentRange.length != 0) { // if we found an attachment
			...
			attachmentRange =
				[[self string] rangeOfString:attachmentCharacterString
									 options:0
									   range:NSMakeRange(currentLocation, [self length] - currentLocation)];
		}
	}
}
...
{
	return [NSSet setWithObjects:NSFontAttributeName, AIFontFamilyAttributeName, AIFontSizeAttributeName,
								 AIFontWeightAttributeName, AIFontStyleAttributeName, NSForegroundColorAttributeName,
								 NSBackgroundColorAttributeName, NSShadowAttributeName, NSCursorAttributeName,
								 NSUnderlineStyleAttributeName, NSStrikethroughStyleAttributeName,
								 NSSuperscriptAttributeName, nil];
}
+ (NSString *)CSSStringForTextAttributes:(NSDictionary *)attrs
{
	...
	if (!attributeNamesToCSSPropertyNames) {
		attributeNamesToCSSPropertyNames = [[NSDictionary alloc]
			initWithObjectsAndKeys:@"font", NSFontAttributeName, @"font-family", AIFontFamilyAttributeName,
								   @"font-size", AIFontSizeAttributeName, @"font-weight", AIFontWeightAttributeName,
								   @"font-style", AIFontStyleAttributeName, @"color", NSForegroundColorAttributeName,
								   @"background-color", NSBackgroundColorAttributeName, @"text-shadow",
								   NSShadowAttributeName, @"cursor", NSCursorAttributeName, nil];
	}
...
{
	NSAttributedString *string = [[NSAttributedString alloc] initWithString:FONT_HEIGHT_STRING attributes:attributes];
...
{
	NSLog(@"%@", @"**** You are using an out of date external Adium plugin [most likely the SQL Logger]. Please "
				 @"recompile and reinstall the plugin. This will crash in a future release. ****");
	...
	if (length) {
		...
		while (searchRange.location < length) {
			...
			if (URL) {
				...
				[newAttributedString replaceCharactersInRange:searchRange withString:replacementString];
