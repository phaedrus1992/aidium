...
- (void)setPlaceholderString:(NSString *)inPlaceholderString
{
	...
	//	attributes = [NSDictionary dictionaryWithObjectsAndKeys:[NSColor grayColor], NSForegroundColorAttributeName,
	// nil];
	[self setPlaceholder:[[NSAttributedString alloc] initWithString:inPlaceholderString attributes:nil]];
}
...
- (void)setPlaceholder:(NSAttributedString *)inPlaceholder
{
	if (inPlaceholder != placeholder) {
		...
		[tempPlaceholder addAttribute:NSForegroundColorAttributeName
								value:[NSColor grayColor]
								range:NSMakeRange(0, [tempPlaceholder length])];
...
- (void)drawRect:(NSRect)rect
{
	...
	if (placeholder && ([[self string] isEqualToString:@""]) && ([[self window] firstResponder] != self)) {
		NSSize size = [self frame].size;
		NSSize textContainerInset = [self textContainerInset];
		textContainerInset.width += 4;
		textContainerInset.height += 2;

		[placeholder drawInRect:NSMakeRect(textContainerInset.width, textContainerInset.height,
