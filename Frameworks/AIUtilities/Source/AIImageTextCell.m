...
- (NSSize)cellSizeForBounds:(NSRect)cellFrame
{
	...
	if (title != nil) {
		...
		//
		if ([self font]) {
			attributes = [NSDictionary dictionaryWithObjectsAndKeys:paragraphStyle, NSParagraphStyleAttributeName,
		...
		if (subString) {
			...
			attributes = [NSDictionary dictionaryWithObject:[NSFont systemFontOfSize:10] forKey:NSFontAttributeName];
 ...
 *
 *	@par	This is a private method. It should not be called from outside this class. It is intended only for use by
 * drawInteriorWithFrame:.
 *
 *	@par	If the image is taller than the height of \a cellFrame, the image will be drawn scaled down. If the image is
 * shorter, it will be vertically centered.
 *
...
- (void)drawInteriorWithFrame:(NSRect)cellFrame inView:(NSView *)controlView
{
	...
	if (title != nil) {
		...
		// If we don't have a control view, or we do and it's the first responder, draw the text in the
		// alternateSelectedControl text color (white)
		if (highlighted &&
			(highlightWhenNotKey ||
			 ((window = [controlView window]) && ([window isKeyWindow] && ([window firstResponder] == controlView))))) {
		...
		//
		if ([self font]) {
			mainAttributes = [NSDictionary dictionaryWithObjectsAndKeys:paragraphStyle, NSParagraphStyleAttributeName,
																		[self font], NSFontAttributeName, mainTextColor,
																		NSForegroundColorAttributeName, nil];
		} else {
			mainAttributes =
				[NSDictionary dictionaryWithObjectsAndKeys:paragraphStyle, NSParagraphStyleAttributeName, mainTextColor,
														   NSForegroundColorAttributeName, nil];
		}

		attributedMainString = [[NSAttributedString alloc] initWithString:title attributes:mainAttributes];
		...
		if (subString) {
			...
			subStringAttributes =
				[NSDictionary dictionaryWithObjectsAndKeys:paragraphStyle, NSParagraphStyleAttributeName,
														   ...
														   subStringTextColor, NSForegroundColorAttributeName, nil];
		...
		if (imageAfterMainString && (image != nil)) {
			...
			// Note that measuring the size of the attributedMainString should happen here, because we don't want to
			// measure the string (which is expensive) if there isn't an image or we don't need to draw it here.
			...
			[spacingTranslation translateXBy:(imageTextPadding + attributedMainStringSize.width)
										 yBy:-(attributedMainStringSize.height / 2.0f)];
