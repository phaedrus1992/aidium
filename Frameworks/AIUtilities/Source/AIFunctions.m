...
CGFloat AICoordinateForRect_edge_(const NSRect rect, const NSRectEdge edge)
{
	...
	switch (edge) {
	case NSMinXEdge:
		coordinate = NSMinX(rect);
		break;
	case NSMinYEdge:
		coordinate = NSMinY(rect);
		break;
	case NSMaxXEdge:
		coordinate = NSMaxX(rect);
		break;
	case NSMaxYEdge:
		coordinate = NSMaxY(rect);
		break;
	}
...
CGFloat AISignedExteriorDistanceRect_edge_toPoint_(const NSRect rect, const NSRectEdge edge, const NSPoint point)
{
	...
	switch (edge) {
	case NSMinXEdge:
		distanceOutside = rectEdgeCoordinate - point.x;
		break;
	case NSMaxXEdge:
		distanceOutside = point.x - rectEdgeCoordinate;
		break;
	case NSMinYEdge:
		distanceOutside = rectEdgeCoordinate - point.y;
		break;
	case NSMaxYEdge:
		distanceOutside = point.y - rectEdgeCoordinate;
		break;
	}
...
NSRectEdge AIOppositeRectEdge_(const NSRectEdge edge)
{
	...
	switch (edge) {
	case NSMinXEdge:
		oppositeEdge = NSMaxXEdge;
		break;
	case NSMinYEdge:
		oppositeEdge = NSMaxYEdge;
		break;
	case NSMaxXEdge:
		oppositeEdge = NSMinXEdge;
		break;
	case NSMaxYEdge:
		oppositeEdge = NSMinYEdge;
		break;
	}
...
NSRect AIRectByAligningRect_edge_toRect_edge_(NSRect mobileRect, const NSRectEdge mobileRectEdge,
											  const NSRect stationaryRect, const NSRectEdge stationaryRectEdge)
{
	CGFloat alignToCoordinate = AICoordinateForRect_edge_(stationaryRect, stationaryRectEdge);
	switch (mobileRectEdge) {
	case NSMinXEdge:
		mobileRect.origin.x = alignToCoordinate;
		break;
	case NSMinYEdge:
		mobileRect.origin.y = alignToCoordinate;
		break;
	case NSMaxXEdge:
		mobileRect.origin.x = alignToCoordinate - NSWidth(mobileRect);
		break;
	case NSMaxYEdge:
		mobileRect.origin.y = alignToCoordinate - NSHeight(mobileRect);
		break;
	}
...
BOOL AIRectIsAligned_edge_toRect_edge_tolerance_(const NSRect rect1, const NSRectEdge edge1, const NSRect rect2,
												 const NSRectEdge edge2, const CGFloat tolerance)
{
