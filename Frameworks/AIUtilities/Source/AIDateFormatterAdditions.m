...
#define MULTIPLE_WEEKS                                                                                                 \
	AILocalizedStringFromTableInBundle(@"%li weeks", nil, [NSBundle bundleWithIdentifier:AIUTILITIES_BUNDLE_ID], nil)
#define ONE_DAY                                                                                                        \
	AILocalizedStringFromTableInBundle(@"1 day", nil, [NSBundle bundleWithIdentifier:AIUTILITIES_BUNDLE_ID], nil)
#define MULTIPLE_DAYS                                                                                                  \
	AILocalizedStringFromTableInBundle(@"%li days", nil, [NSBundle bundleWithIdentifier:AIUTILITIES_BUNDLE_ID], nil)
#define ONE_HOUR                                                                                                       \
	AILocalizedStringFromTableInBundle(@"1 hour", nil, [NSBundle bundleWithIdentifier:AIUTILITIES_BUNDLE_ID], nil)
#define MULTIPLE_HOURS                                                                                                 \
	AILocalizedStringFromTableInBundle(@"%li hours", nil, [NSBundle bundleWithIdentifier:AIUTILITIES_BUNDLE_ID], nil)
#define ONE_MINUTE                                                                                                     \
	AILocalizedStringFromTableInBundle(@"1 minute", nil, [NSBundle bundleWithIdentifier:AIUTILITIES_BUNDLE_ID], nil)
#define MULTIPLE_MINUTES                                                                                               \
	AILocalizedStringFromTableInBundle(@"%li minutes", nil, [NSBundle bundleWithIdentifier:AIUTILITIES_BUNDLE_ID], nil)
#define ONE_SECOND                                                                                                     \
	AILocalizedStringFromTableInBundle(@"1 second", nil, [NSBundle bundleWithIdentifier:AIUTILITIES_BUNDLE_ID], nil)
#define MULTIPLE_SECONDS                                                                                               \
	AILocalizedStringFromTableInBundle(@"%1.0lf seconds", nil, [NSBundle bundleWithIdentifier:AIUTILITIES_BUNDLE_ID],  \
									   nil)
...
typedef enum { NONE, SECONDS, AMPM, BOTH } StringType;
...
- (id)init
{
	if ((self = [super init])) {
		[[NSDistributedNotificationCenter defaultCenter] addObserver:self
															selector:@selector(flushFormatterCache:)
...
+ (void)withLocalizedDateFormatterShowingSeconds:(BOOL)seconds
								   showingAMorPM:(BOOL)showAmPm
										 perform:(void (^)(NSDateFormatter *))block
{
	dispatch_queue_t localizedFormatterQueue = [self localizedFormatterQueue];
...
+ (NSString *)stringForTimeIntervalSinceDate:(NSDate *)inDate
							  showingSeconds:(BOOL)showSeconds
...
{
	return [self stringForTimeInterval:interval showingSeconds:NO abbreviated:abbreviate approximated:YES];
}
...
{
	return [self stringForTimeInterval:interval showingSeconds:NO abbreviated:NO approximated:NO];
}
 ...
 *@param showSeconds switch to determine if seconds should be shown
 *@param abbreviate switch to chose if w/d/h/ or weeks/days/hours/minutes is used to indicate the unit
 *@param approximate switch to chose if all parts should be shown or only the largest available part. If Hours is the
 * largest available part, Minutes are also shown if applicable.
 *
					   ...
					   approximated:(BOOL)approximate
{
	...
	[NSDate convertTimeInterval:interval toWeeks:&weeks days:&days hours:&hours minutes:&minutes seconds:&seconds];

	// build the strings for the parts
	if (abbreviate) {
		// Note: after checking with a linguistics student, it appears that we're fine leaving it as w, h, etc... rather
		// than localizing.
		...
		weeksString = (weeks == 1) ? ONE_WEEK : [NSString stringWithFormat:MULTIPLE_WEEKS, (long)weeks];
		daysString = (days == 1) ? ONE_DAY : [NSString stringWithFormat:MULTIPLE_DAYS, (long)days];
		hoursString = (hours == 1) ? ONE_HOUR : [NSString stringWithFormat:MULTIPLE_HOURS, (long)hours];
		minutesString = (minutes == 1) ? ONE_MINUTE : [NSString stringWithFormat:MULTIPLE_MINUTES, (long)minutes];
		secondsString = (seconds == 1) ? ONE_SECOND : [NSString stringWithFormat:MULTIPLE_SECONDS, (double)seconds];
	}

	// assemble the parts
	NSMutableArray *parts = [NSMutableArray arrayWithCapacity:5];
	if (approximate) {
		/* We want only one of these. For example, 5 weeks, 5 days, 5 hours, 5 minutes, and 5 seconds should just be "5
		 * weeks". Exception: Hours should display hours and minutes. 5 hours, 5 minutes, and 5 seconds is "5 hours and
		 * 5 minutes".
		 */
		if (weeks)
			...
			if ([span length] >= 2) {
				...
				while (1) {
					...
					if ([format characterAtIndex:([scanner scanLocation] + 1)] == '\'') {
						if (([scanner scanLocation] + 1 < [format length]) &&
							([format characterAtIndex:([scanner scanLocation] + 1)] == '\'')) {
							[newFormat appendString:@"'"];
