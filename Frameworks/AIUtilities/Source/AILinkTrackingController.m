...
#define COPY_LINK                                                                                                      \
	AILocalizedStringFromTableInBundle(@"Copy Link", nil, [NSBundle bundleWithIdentifier:AIUTILITIES_BUNDLE_ID],       \
									   "Copy the link to the clipboard")

@interface AILinkTrackingController ()
- (id)initForView:(NSView *)inControlView
	withTextStorage:(NSTextStorage *)inTextStorage
	  layoutManager:(NSLayoutManager *)inLayoutManager
...
+ (id)linkTrackingControllerForView:(NSView *)inControlView
					withTextStorage:(NSTextStorage *)inTextStorage
					  layoutManager:(NSLayoutManager *)inLayoutManager
					  textContainer:(NSTextContainer *)inTextContainer
{
	return [[self alloc] initForView:inControlView
					 withTextStorage:inTextStorage
...
{
	return [[self alloc] initForView:inTextView
					 withTextStorage:[inTextView textStorage]
					   layoutManager:[inTextView layoutManager]
					   textContainer:[inTextView textContainer]];
}

// Track links in the passed rect.  Returns YES if links exist within our text.  Pass a 0 width/height visible rect to
// stop any link tracking.
...
- (BOOL)handleMouseDown:(NSEvent *)theEvent withOffset:(NSPoint)offset
{
	...
	glyphIndex = [layoutManager glyphIndexForPoint:mouseLoc
	...
	if (charIndex != NSNotFound && charIndex < [textStorage length]) {
		...
		// Check if click is in valid link attributed range, and is inside the bounds of that style range, else fall
		...
		if (linkString != nil && [linkString length] != 0) {
			...
			if (linkURL) {
				...
				// One last check to make sure we're really in the bounds of the link. Useful when the link runs up to
				// the end of the document and a click in the blank area below still pases the style range test above.
...
- (void)_beginCursorTrackingInRect:(NSRect)visibleRect withOffset:(NSPoint)offset
{
	...
	while (NSMaxRange(scanRange) < NSMaxRange(visibleCharRange)) {
		...
		// Get the link URL
		linkURL = [textStorage attribute:NSLinkAttributeName atIndex:NSMaxRange(scanRange) effectiveRange:&scanRange];
		...
		if (linkURL) {
			...
			for (idx = 0; idx < linkCount; idx++) {
				...
				trackedLink =
					[[AIFlexibleLink alloc] initWithTrackingRect:linkRect
															 url:linkURL
				...
				// Install a tracking rect for the link (The userData of each tracking rect is the AIFlexibleLink it
				// covers)
				trackingTag = [controlView addTrackingRect:visibleLinkRect
													 owner:self
												  userData:(__bridge void *)trackedLink
											  assumeInside:NO];
...
- (void)_setMouseOverLink:(AIFlexibleLink *)inHoveredLink atPoint:(NSPoint)inPoint
{
	if (inHoveredLink != nil && mouseOverLink == NO) {
		...
		if (showTooltip && [[inHoveredLink title] caseInsensitiveCompare:[inHoveredLink url]] != NSOrderedSame &&
			[[@"http://" stringByAppendingString:[inHoveredLink title]] caseInsensitiveCompare:[inHoveredLink url]] !=
				NSOrderedSame) {
			...
			[AITooltipUtilities showTooltipWithString:hoveredString
											 onWindow:nil
											  atPoint:inPoint
										  orientation:TooltipAbove]; // Show tooltip
		}
		...
		if (showTooltip) {
			[AITooltipUtilities showTooltipWithString:@""
											 onWindow:nil
											  atPoint:NSMakePoint(0, 0)
										  orientation:TooltipAbove]; // Hide the tooltip
