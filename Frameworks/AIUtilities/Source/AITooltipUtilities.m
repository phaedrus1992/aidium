...
// Tooltips
+ (void)showTooltipWithString:(NSString *)inString
...
+ (void)showTooltipWithAttributedString:(NSAttributedString *)inString
							   onWindow:(NSWindow *)inWindow
								atPoint:(NSPoint)inPoint
							orientation:(AITooltipOrientation)inOrientation
{
				 ...
				 orientation:(AITooltipOrientation)inOrientation
{
	if ((inTitle && [inTitle length]) || (inBody && [inBody length]) || inImage) { // If passed something to display
		...
		if (!(inBody == tooltipBody) || !(inTitle == tooltipTitle) ||
			!(inImage == tooltipImage)) { // we don't exist or something changed

			tooltipTitle = inTitle;

			if (inTitle) {
				[[textView_tooltipTitle textStorage]
					replaceCharactersInRange:NSMakeRange(0, [[textView_tooltipTitle textStorage] length])
						withAttributedString:tooltipTitle];
			} else {
				[[textView_tooltipTitle textStorage]
					deleteCharactersInRange:NSMakeRange(0, [[textView_tooltipTitle textStorage] length])];
			}

			tooltipBody = inBody;
			if (inBody) {
				[[textView_tooltipBody textStorage]
					replaceCharactersInRange:NSMakeRange(0, [[textView_tooltipBody textStorage] length])
						withAttributedString:tooltipBody];
			} else {
				[[textView_tooltipBody textStorage]
					deleteCharactersInRange:NSMakeRange(0, [[textView_tooltipBody textStorage] length])];
			}
			...
			if (fadingOut)
				[tooltipWindow setAlphaValue:0.0f];
			[self _sizeTooltip];
			if (fadingOut)
...
+ (void)_closeTooltip
{
	NSAssert2(!fadeOutAnimation, @"%s: Trying to close tooltip while a tooltip is already fading out! Animation is %@",
			  __PRETTY_FUNCTION__, fadeOutAnimation);
	fadeOutAnimation = [[NSViewAnimation alloc]
		initWithViewAnimations:[NSArray
								   arrayWithObject:[NSDictionary
													   dictionaryWithObjectsAndKeys:tooltipWindow,
...
// This is called when we send stopAnimation to the animation from the
// showTooltipWithTitle:body:image:imageOnRight:onWindow:atPoint:orientation: method.
...
+ (void)_sizeTooltip
{
	...
	if (hasTitle) {
		...
		[[textView_tooltipTitle layoutManager]
			glyphRangeForTextContainer:[textView_tooltipTitle textContainer]]; // void - need to force it to lay out the
																			   // glyphs for an accurate measurement
		tooltipTitleRect =
			[[textView_tooltipTitle layoutManager] usedRectForTextContainer:[textView_tooltipTitle textContainer]];
	} else {
	...
	if (hasBody) {
		// Make sure we're not wrapping by default
		// Set up the tooltip's bounds
		[[textView_tooltipBody layoutManager]
			glyphRangeForTextContainer:[textView_tooltipBody textContainer]]; // void - need to force it to lay out the
																			  // glyphs for an accurate measurement
		tooltipBodyRect =
			[[textView_tooltipBody layoutManager] usedRectForTextContainer:[textView_tooltipBody textContainer]];
	} else {
	...
	CGFloat windowWidth =
		TOOLTIP_INSET * 2 + ((tooltipBodyRect.size.width > tooltipTitleRect.size.width) ? tooltipBodyRect.size.width
																						: tooltipTitleRect.size.width);
	CGFloat windowHeight =
		titleAndBodyMargin + TOOLTIP_INSET * 2 + (tooltipTitleRect.size.height + tooltipBodyRect.size.height);

	// Set the textView's origin
	//  tooltipTitleRect.origin =  NSMakePoint(windowWidth/2 - tooltipTitleRect.size.width/2,TOOLTIP_INSET +
	//  tooltipBodyRect.size.height); //center the title
	tooltipTitleRect.origin =
		NSMakePoint(TOOLTIP_INSET, titleAndBodyMargin + TOOLTIP_INSET + tooltipBodyRect.size.height); // left
	...
	if (tooltipImage) {
		...
		if (imageIsTallerThanTitle) {
			// The image should not overlap the body of the tooltip, so increase the window height (the body has an
			// origin at the bottom-left so will move with the window)
			...
			tooltipTitleRect.origin.y =
				(windowHeight - ((imageSize.height) * 0.5f) - (tooltipTitleRect.size.height * 0.5f));
		}
		...
		if (imageOnRight) {
			...
			[view_tooltipImage setFrameOrigin:NSMakePoint(windowWidth - imageSize.width - TOOLTIP_INSET,
														  windowHeight - imageSize.height - TOOLTIP_INSET)];
		} else {
			...
			[view_tooltipImage
				setFrameOrigin:NSMakePoint(TOOLTIP_INSET, windowHeight - imageSize.height - TOOLTIP_INSET)];
		}
	}
	...
	if (titleAndBodyMargin) {
		...
		[NSBezierPath strokeLineFromPoint:NSMakePoint(TOOLTIP_INSET,
													  (titleAndBodyMargin * 0.5f) + tooltipBodyRect.size.height + 4)
								  toPoint:NSMakePoint(windowWidth - TOOLTIP_INSET,
													  (titleAndBodyMargin * 0.5f) + tooltipBodyRect.size.height + 4)];
