...
- (id)initAllowingCharacters:(NSCharacterSet *)inCharacters
					  length:(NSInteger)inLength
			   caseSensitive:(BOOL)inCaseSensitive
				errorMessage:(NSString *)inErrorMessage;
@end

@implementation AIStringFormatter

+ (id)stringFormatterAllowingCharacters:(NSCharacterSet *)inCharacters
								 length:(NSInteger)inLength
						  caseSensitive:(BOOL)inCaseSensitive
						   errorMessage:(NSString *)inErrorMessage
{
	return [[self alloc] initAllowingCharacters:inCharacters
										 length:inLength
			...
			errorDescription:(NSString **)error
{
	...
	if (characters != nil && [*partialStringPtr length] > 0) {
		NSScanner *scanner =
			[NSScanner scannerWithString:(caseSensitive ? *partialStringPtr : [*partialStringPtr lowercaseString])];
		...
		if (![scanner scanCharactersFromSet:characters intoString:&validSegment]) {
			...
			if (validSegmentLength != partialStringPtrLength) {
				...
				// If the string is valid except for the last character, and the last character is a newline, strip the
				// newline and allow the change
	...
	if (shouldIncreaseErrorCounter) {
		...
		if (errorMessage != nil && errorCount > ERRORS_BEFORE_DIALOG) {
			NSRunAlertPanel(AILocalizedStringFromTableInBundle(
