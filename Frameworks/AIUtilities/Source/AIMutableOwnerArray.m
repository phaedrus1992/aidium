...
- (NSString *)description
{
	NSMutableString *desc =
		[[NSMutableString alloc] initWithFormat:@"<%@: %p: ", NSStringFromClass([self class]), self];
	NSUInteger i = 0;

	for (id object in self) {
		[desc appendFormat:@"(%@:%@:%@)%@", [ownerArray objectAtIndex:i], object, [priorityArray objectAtIndex:i],
						   (object == [contentArray lastObject] ? @"" : @", ")];
		i++;
	}
	[desc appendString:@">"];
...
- (id)objectWithOwner:(id)inOwner
{
	if (ownerArray && contentArray) {
		...
		if (idx != NSNotFound)
			return [contentArray objectAtIndex:idx];
	}
...
- (float)priorityOfObjectWithOwner:(id)inOwner
{
	if (ownerArray && priorityArray) {
		NSUInteger idx = [ownerArray indexOfObject:inOwner];
		if (idx != NSNotFound)
			return [[priorityArray objectAtIndex:idx] floatValue];
	}
	return 0.0f;
}
...
- (id)ownerWithObject:(id)inObject
{
	if (ownerArray && contentArray) {
		...
		if (idx != NSNotFound)
			return [ownerArray objectAtIndex:idx];
	}
...
- (void)setDelegate:(id)inDelegate
{
	...
	delegateRespondsToDidSetObjectWithOwnerPriorityLevel =
		[delegate respondsToSelector:@selector(mutableOwnerArray:didSetObject:withOwner:priorityLevel:)];
}
