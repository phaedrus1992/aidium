 ...
 * @brief Load
 *
 * Install ourself to intercept keyDown: calls so we can stick our delete handling in, and menuForEvent: calls so we can
 * ask our delegate
 */
+ (void)load
{
	// Anything you can do, I can do better...
	method_exchangeImplementations(class_getInstanceMethod([NSTableView class], @selector(keyDown:)),
								   class_getInstanceMethod(self, @selector(keyDown:)));

	method_exchangeImplementations(class_getInstanceMethod([NSTableView class], @selector(menuForEvent:)),
...
- (void)keyDown:(NSEvent *)theEvent
{
	...
	if (pressedChar == NSDeleteFunctionKey || pressedChar == NSBackspaceCharacter ||
		pressedChar == NSDeleteCharacter) { // Delete
...
- (NSMenu *)menuForEvent:(NSEvent *)theEvent
{
	...
	static NSMenu *(*_menu_for_event_method_invoke)(id, Method, NSEvent *) =
		(NSMenu * (*)(id, Method, NSEvent *)) method_invoke;
	return _menu_for_event_method_invoke(self, class_getInstanceMethod([AITableView class], @selector(menuForEvent:)),
										 theEvent);
}
