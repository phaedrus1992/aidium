...
#define VALID_COLORS_ARRAY                                                                                             \
	[[NSArray alloc]                                                                                                   \
		initWithObjects:@"aqua", @"aquamarine", @"blue", @"blueviolet", @"brown", @"burlywood", @"cadetblue",          \
						...
						@"deepskyblue", @"dimgrey", @"dodgerblue", @"firebrick", @"forestgreen", @"fuchsia", @"gold",  \
						@"goldenrod", @"green", @"greenyellow", @"grey", @"hotpink", @"indianred", @"indigo",          \
						@"lawngreen", @"lightblue", @"lightcoral", @"lightgreen", @"lightgrey", @"lightpink",          \
						@"lightsalmon", @"lightseagreen", @"lightskyblue", @"lightslategrey", @"lightsteelblue",       \
						@"lime", @"limegreen", @"magenta", @"maroon", @"mediumaquamarine", @"mediumblue",              \
						@"mediumorchid", @"mediumpurple", @"mediumseagreen", @"mediumslateblue", @"mediumspringgreen", \
						@"mediumturquoise", @"mediumvioletred", @"midnightblue", @"navy", @"olive", @"olivedrab",      \
						@"orange", @"orangered", @"orchid", @"palegreen", @"paleturquoise", @"palevioletred", @"peru", \
						@"pink", @"plum", @"powderblue", @"purple", @"red", @"rosybrown", @"royalblue",                \
						@"saddlebrown", @"salmon", @"sandybrown", @"seagreen", @"sienna", @"silver", @"skyblue",       \
						@"slateblue", @"slategrey", @"springgreen", @"steelblue", @"tan", @"teal", @"thistle",         \
						@"tomato", @"turquoise", @"violet", @"yellowgreen", nil]
...
+ (id)dictionaryWithContentsOfRGBTxtFile:(NSString *)path
{
	...
	char *ch = [data
		mutableBytes]; // we use mutable bytes because we want to tokenise the string by replacing separators with '\0'.
	...
	@autoreleasepool {
		...
		for (unsigned i = 0; i < length; ++i) {
			if (state.inComment) {
				...
				if (state.prevChar != '\n') { // ignore blank lines
					if (!((state.redStart != NULL) && (state.greenStart != NULL) && (state.blueStart != NULL) &&
						  (state.nameStart != NULL))) {
#if COLOR_DEBUG
						NSLog(@"Parse error reading rgb.txt file: a non-comment line was encountered that did not have "
							  @"all four of red (%p), green (%p), blue (%p), and name (%p) - index is %u",
							  state.redStart, state.greenStart, state.blueStart, state.nameStart, i);
#endif
						goto end;
					}
					...
					NSString *name = [NSString stringWithData:[data subdataWithRange:range]
					...
					state.redStart = state.greenStart = state.blueStart = state.nameStart = state.redEnd =
						state.greenEnd = state.blueEnd = NULL;
				} // if (prevChar != '\n')
			} else if ((ch[i] != ' ') && (ch[i] != '\t')) {
				if (state.prevChar == '\n' && ch[i] == '#') {
					state.inComment = YES;
...
+ (NSDictionary *)colorNamesDictionary
{
	if (!RGBColorValues) {
		RGBColorValues =
			[[NSMutableDictionary alloc] initWithObjectsAndKeys:[NSColor colorWithHTMLString:@"#000"], @"black",
																...
																[NSColor colorWithHTMLString:@"#0ff"], @"aqua", nil];
		...
		for (NSString *middlePath in paths) {
			NSString *path = [defaultRGBTxtLocation1
				stringByAppendingPathComponent:[middlePath stringByAppendingPathComponent:defaultRGBTxtLocation2]];
...
- (NSColor *)darkenAndAdjustSaturationBy:(CGFloat)amount
{
	...
	return [NSColor colorWithCalibratedHue:[convertedColor hueComponent]
								saturation:(([convertedColor saturationComponent] == 0.0f)
												? [convertedColor saturationComponent]
												: ([convertedColor saturationComponent] + amount))
								brightness:([convertedColor brightnessComponent] - amount)
									 alpha:[convertedColor alphaComponent]];
}
...
- (NSColor *)adjustHue:(CGFloat)dHue saturation:(CGFloat)dSat brightness:(CGFloat)dBrit
{
	...
	// For some reason, redColor's hue is 1.0f, not 0.0f, as of Mac OS X 10.4.10 and 10.5.2. Therefore, we must
	// normalize any multiple of 1.0 to 0.0. We do this by taking the remainder of hue ÷ 1.
	hue = AIfmod(hue, 1.0f);
...
- (NSString *)stringRepresentation
{
	...
	if (alphaComponent == 1.0) {
		return [NSString stringWithFormat:@"%d,%d,%d", (int)([tempColor redComponent] * 255.0),
										  (int)([tempColor greenComponent] * 255.0),
										  (int)([tempColor blueComponent] * 255.0)];

	} else {
		return [NSString stringWithFormat:@"%d,%d,%d,%d", (int)([tempColor redComponent] * 255.0),
										  (int)([tempColor greenComponent] * 255.0),
										  (int)([tempColor blueComponent] * 255.0), (int)(alphaComponent * 255.0)];
	}
}
 ...
 * @result The float value. Returns 0 as a bailout value if firstChar or secondChar are not valid hexadecimal characters
 * ([0-9]|[A-F]|[a-f]). Also returns 0 if firstChar and secondChar equal 0.
 */
...
+ (id)colorWithHTMLString:(NSString *)str defaultColor:(NSColor *)defaultColor
{
	...
	if ([str hasPrefix:@"rgb"]) {
		...
		if (leftParIndex == NSNotFound || rightParIndex == NSNotFound) {
			NSLog(@"+[NSColor(AIColorAdditions) colorWithHTMLString:] called with unrecognised color function (str is "
				  @"%@); returning %@",
				  str, defaultColor);
			return defaultColor;
		}
		...
		if ([colorComponents count] < 3 || [colorComponents count] > 4) {
			NSLog(@"+[NSColor(AIColorAdditions) colorWithHTMLString:] called with a color function with the wrong "
				  @"number of arguments (str is %@); returning %@",
				  str, defaultColor);
			return defaultColor;
		}
	...
	if ((!strLength) || ([str characterAtIndex:0] != '#')) {
		...
		if (!colorValue)
			colorValue = [colorValues objectForKey:[str lowercaseString]];
		if (!colorValue) {
#if COLOR_DEBUG
			NSLog(@"+[NSColor(AIColorAdditions) colorWithHTMLString:] called with unrecognised color name (str is %@); "
				  @"returning %@",
				  str, defaultColor);
#endif
			return defaultColor;
	...
	char hexStringArray[hexStringArrayLength] = {
...
#if COLOR_DEBUG
		NSLog(@"+[%@ colorWithHTMLString:] called with a string that cannot possibly be a hexadecimal color "
			  @"specification (e.g. #ff0000, #00b, #cc08) (string: %@ input: %@); returning %@",
			  NSStringFromClass(self), colorValue, str, defaultColor);
#endif
		return defaultColor;
	}
