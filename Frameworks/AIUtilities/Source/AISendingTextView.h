 ...
 * since using the regular method of catching returns will not work.  The first return will come in, the current text
 * will be sent, and then the other returns will come in (and nothing will happen since there's no text in the text
 * view).  After the returns are processed, THEN the rest of the text will be inserted as a clump into the text view. To
 * the user this looks like their return was 'missed', since it gets inserted into the text view, and doesn't trigger a
 * send.</p> <p>This fix watches for returns in the insertText method.  However, since it's impossible to distinguish a
 * return from an enter by the characters inserted (both insert /r, 10), it also watches and remembers the keys being
 * pressed with interpretKeyEvents... When insertText sees a /r, it checks to see what key was pressed to generate that
 * /r, and makes a decision to send or not.  Since the sending occurs from within insertText, the returns are processed
 * in the correct order with the text, and the problem is illiminated.</p>
 */
 ...
 *
 * When a send occurs, <b>inTarget</b> will be sent <b>inSelector</b>, which should take one argument which will be the
 * sender.
