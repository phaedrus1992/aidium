...
- (NSRect)frameOfCellAtColumn:(NSInteger)column row:(NSInteger)row
{
	...
	if ([[self delegate] respondsToSelector:@selector(outlineView:extendToEdgeColumn:ofRow:)] &&
		[(id<AIVariableHeightFlexibleColumnsOutlineViewDelegate>)[self delegate] outlineView:self
																		  extendToEdgeColumn:column
																					   ofRow:row]) {
		frameOfCell.size.width =
			[self frame].size.width - frameOfCell.origin.x - AIround(([self intercellSpacing].width) / 2);
	}
...
- (BOOL)_ai_drawRow:(NSInteger)row clipRect:(NSRect)rect;
{
	if (row >= 0 &&
		...
		BOOL delegateRespondsToExtendToEdgeColumn =
			[[self delegate] respondsToSelector:@selector(outlineView:extendToEdgeColumn:ofRow:)];
		...
		for (tableColumnIndex = 0; tableColumnIndex < count; tableColumnIndex++) {
			...
			if (tableColumnIndex == 0) {
				...
				if ([self isExpandable:item]) {
					...
					if ([self isItemExpanded:item]) {
						// Bottom point
						[arrowPath moveToPoint:NSMakePoint(center.x, center.y + FLIPPY_HEIGHT / 2)];
						// Move to top left
						...
						// Bottom left
						[arrowPath
							moveToPoint:NSMakePoint(center.x - (FLIPPY_WIDTH / 2), center.y + (FLIPPY_HEIGHT / 2))];
						// Move to top left
			...
			if (delegateRespondsToExtendToEdgeColumn &&
				[(id<AIVariableHeightFlexibleColumnsOutlineViewDelegate>)[self delegate] outlineView:self
																				  extendToEdgeColumn:tableColumnIndex
																							   ofRow:row]) {
				break;
			}
		}
