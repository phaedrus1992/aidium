...
- (void)drawRect:(NSRect)rect
{
	...
	[transform translateXBy:(self.isHorizontalScroller ? offset / scale : 0.f)
						yBy:(self.isHorizontalScroller ? 0.f : offset / scale)];
	...
	while ((startNum = [enumerator nextObject]) && (stopNum = [enumerator nextObject])) {
		...
		if (self.isHorizontalScroller)
			...
			shadesRect = NSMakeRect(0.f, start, width, (stop - start));
	...
	NSUInteger currentPosition =
		(_currentMark != NSNotFound ? _currentMark : (CGFloat)[self floatValue] * [self contentViewLength]);
	...
	while ((currentMark = [enumerator nextObject])) {
		...
		if (!NSPointInRect(point, knobRect)) {
			if (mark.color) {
				...
				point =
					NSMakePoint((self.isHorizontalScroller ? 0.f : width), (self.isHorizontalScroller ? width : 0.f));
...
- (NSMenu *)menuForEvent:(NSEvent *)event
{
	...
	item = [[NSMenuItem alloc]
		initWithTitle:AILocalizedStringFromTableInBundle(@"Clear All Marks", nil,
														 [NSBundle bundleWithIdentifier:AIUTILITIES_BUNDLE_ID],
	...
	if (self.isHorizontalScroller) {
		item = [[NSMenuItem alloc]
			initWithTitle:AILocalizedStringFromTableInBundle(@"Clear Marks from Here Left", nil,
															 [NSBundle bundleWithIdentifier:AIUTILITIES_BUNDLE_ID],
		...
		item = [[NSMenuItem alloc]
			initWithTitle:AILocalizedStringFromTableInBundle(@"Clear Marks from Here Right", nil,
															 [NSBundle bundleWithIdentifier:AIUTILITIES_BUNDLE_ID],
		...
		item = [[NSMenuItem alloc]
			initWithTitle:AILocalizedStringFromTableInBundle(@"Clear Marks from Here Up", nil,
															 [NSBundle bundleWithIdentifier:AIUTILITIES_BUNDLE_ID],
		...
		item = [[NSMenuItem alloc]
			initWithTitle:AILocalizedStringFromTableInBundle(@"Clear Marks from Here Down", nil,
															 [NSBundle bundleWithIdentifier:AIUTILITIES_BUNDLE_ID],
															 "clear marks from here up contextual menu")
				   action:@selector(clearMarksHereGreater:)
			keyEquivalent:@""];
	...
	item = [[NSMenuItem alloc]
		initWithTitle:AILocalizedStringFromTableInBundle(@"Jump to Previous Mark", nil,
														 [NSBundle bundleWithIdentifier:AIUTILITIES_BUNDLE_ID],
	...
	item = [[NSMenuItem alloc]
		initWithTitle:AILocalizedStringFromTableInBundle(@"Jump to Next Mark", nil,
														 [NSBundle bundleWithIdentifier:AIUTILITIES_BUNDLE_ID],
	...
	item = [[NSMenuItem alloc]
		initWithTitle:AILocalizedStringFromTableInBundle(@"Jump to Focus Mark", nil,
														 [NSBundle bundleWithIdentifier:AIUTILITIES_BUNDLE_ID],
														 "jump to the mark where the last content the user saw ends")
...
- (IBAction)clearMarksHereLess:(id)sender
{
	...
	CGFloat scale = [self scaleToContentView];
	[self removeMarksLessThan:((self.isHorizontalScroller ? where.x - NSMinX(slotRect) : where.y - NSMinY(slotRect)) /
							   scale)];
}
...
- (IBAction)clearMarksHereGreater:(id)sender
{
	...
	CGFloat scale = [self scaleToContentView];
	[self
		removeMarksGreaterThan:((self.isHorizontalScroller ? where.x - NSMinX(slotRect) : where.y - NSMinY(slotRect)) /
								scale)];
}
...
- (void)shiftMarksAndShadedAreasBy:(NSInteger)displacement
{
	...
	if (!(negative && _nearestPreviousMark < ABS(displacement)))
		_nearestPreviousMark += displacement;
	else
		_nearestPreviousMark = NSNotFound;

	if (!(negative && _nearestNextMark < ABS(displacement)))
		_nearestNextMark += displacement;
	else
		_nearestNextMark = NSNotFound;

	if (!(negative && _currentMark < ABS(displacement)))
		_currentMark += displacement;
	else
		_currentMark = NSNotFound;
...
{
	if (self.isHorizontalScroller)
		return (NSWidth([self frame]) / [self knobProportion]);
	else
		return (NSHeight([self frame]) / [self knobProportion]);
}

- (CGFloat)scaleToContentView
{
	if (self.isHorizontalScroller)
		return NSWidth([self rectForPart:NSScrollerKnobSlot]) /
			   NSWidth([[(NSScrollView *)[self superview] contentView] documentRect]);
	else
		return NSHeight([self rectForPart:NSScrollerKnobSlot]) /
			   NSHeight([[(NSScrollView *)[self superview] contentView] documentRect]);
}
...
- (CGFloat)shiftAmountToCenterAlign
{
	CGFloat scale = [self scaleToContentView];
	if (self.isHorizontalScroller)
		return ((NSWidth([self rectForPart:NSScrollerKnobSlot]) * [self knobProportion]) / 2.f) / scale;
	else
		return ((NSHeight([self rectForPart:NSScrollerKnobSlot]) * [self knobProportion]) / 2.f) / scale;
}
@end
