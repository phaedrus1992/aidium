...
- (void)selectTabViewItem:(NSTabViewItem *)tabViewItem
{
	...
	if ([[[self window] toolbar] respondsToSelector:@selector(setSelectedItemIdentifier:)]) {
		[[[self window] toolbar]
			setSelectedItemIdentifier:[NSString stringWithFormat:@"%i", (int)[self indexOfTabViewItem:tabViewItem]]];
	}
	...
	if (tabViewItem != [self selectedTabViewItem]) {
		if (tabViewItem_loading) {
			...
			if (![[self delegate]
					respondsToSelector:@selector(immediatelyShowLoadingIndicatorForTabView:willSelectTabViewItem:)] ||
				[(id<AIToolbarTabViewDelegate>)[self delegate] immediatelyShowLoadingIndicatorForTabView:self
	...
	if (tabViewItem_loading) {
		...
		[progressIndicator_loading performSelector:@selector(stopAnimation:) withObject:self afterDelay:0];
