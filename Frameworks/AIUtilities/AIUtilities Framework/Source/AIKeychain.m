...
static NSString *AIKeychainProtocolString(SecProtocolType protocol) {
	...
	switch (protocol) {
		case kSecProtocolTypeHTTPProxy:	return (__bridge NSString *)kSecAttrProtocolHTTPProxy;
		case kSecProtocolTypeHTTPSProxy:	return (__bridge NSString *)kSecAttrProtocolHTTPSProxy;
		case kSecProtocolTypeFTPProxy:		return (__bridge NSString *)kSecAttrProtocolFTPProxy;
		case kSecProtocolTypeSOCKS:		return (__bridge NSString *)kSecAttrProtocolSOCKS;
		case kSecProtocolTypeRTSPProxy:		return (__bridge NSString *)kSecAttrProtocolRTSPProxy;
		case kSecProtocolTypeFTP:		return (__bridge NSString *)kSecAttrProtocolFTP;
		case kSecProtocolTypeHTTP:		return (__bridge NSString *)kSecAttrProtocolHTTP;
		case kSecProtocolTypeHTTPS:		return (__bridge NSString *)kSecAttrProtocolHTTPS;
		case kSecProtocolTypeIMAP:		return (__bridge NSString *)kSecAttrProtocolIMAP;
		case kSecProtocolTypeSMTP:		return (__bridge NSString *)kSecAttrProtocolSMTP;
		case kSecProtocolTypePOP3:		return (__bridge NSString *)kSecAttrProtocolPOP3;
		case kSecProtocolTypeSSH:		return (__bridge NSString *)kSecAttrProtocolSSH;
		case kSecProtocolTypeLDAP:		return (__bridge NSString *)kSecAttrProtocolLDAP;
		case kSecProtocolTypeIRC:		return (__bridge NSString *)kSecAttrProtocolIRC;
		default: {
			char code[5] = {
...
static OSStatus AIKeychainCopyPassword(NSString *server, NSString *account, NSString *protocol,
                                       NSString *path, UInt16 port, NSString *domain,
                                       ...
                                       NSString **outPassword, SecKeychainItemRef *outItem)
{
	NSMutableDictionary *query = [NSMutableDictionary dictionaryWithObjectsAndKeys:
		...
		kCFBooleanTrue, (__bridge id)kSecReturnData,
		nil];
	if (server)		[query setObject:server forKey:(__bridge id)kSecAttrServer];
	if (account)	[query setObject:account forKey:(__bridge id)kSecAttrAccount];
	if (protocol)	[query setObject:protocol forKey:(__bridge id)kSecAttrProtocol];
	if (domain)		[query setObject:domain forKey:(__bridge id)kSecAttrSecurityDomain];
	if (path)		[query setObject:path forKey:(__bridge id)kSecAttrPath];
	if (port)		[query setObject:[NSNumber numberWithUnsignedShort:port] forKey:(__bridge id)kSecAttrPort];
	...
	if (err == noErr && result) {
		...
		if (outPassword) *outPassword = nil;
	}

	if (outItem) *outItem = NULL; /* SecItem* has no item-ref concept; NULL is the signal for "found" */
	return err;
}

static OSStatus AIKeychainAddPassword(NSString *server, NSString *account, NSString *protocol,
                                      NSString *path, UInt16 port, NSString *domain,
                                      SecAuthenticationType authType,
                                      NSString *password)
{
	NSMutableDictionary *attrs = [NSMutableDictionary dictionaryWithObjectsAndKeys:
		...
		nil];
	if (server)		[attrs setObject:server forKey:(__bridge id)kSecAttrServer];
	if (account)	[attrs setObject:account forKey:(__bridge id)kSecAttrAccount];
	if (protocol)	[attrs setObject:protocol forKey:(__bridge id)kSecAttrProtocol];
	if (domain)		[attrs setObject:domain forKey:(__bridge id)kSecAttrSecurityDomain];
	if (path)		[attrs setObject:path forKey:(__bridge id)kSecAttrPath];
	if (port)		[attrs setObject:[NSNumber numberWithUnsignedShort:port] forKey:(__bridge id)kSecAttrPort];

	return SecItemAdd((__bridge CFDictionaryRef)attrs, NULL);
}

static OSStatus AIKeychainUpdatePassword(NSString *server, NSString *account, NSString *protocol,
                                         NSString *path, UInt16 port, NSString *domain,
                                         SecAuthenticationType authType,
                                         NSString *newPassword)
{
	...
	NSDictionary *update = [NSDictionary dictionaryWithObjectsAndKeys:
		[newPassword dataUsingEncoding:NSUTF8StringEncoding], (__bridge id)kSecValueData,
		nil];

	return SecItemUpdate((__bridge CFDictionaryRef)query, (__bridge CFDictionaryRef)update);
}

static OSStatus AIKeychainDeletePassword(NSString *server, NSString *account, NSString *protocol,
                                         NSString *path, UInt16 port, NSString *domain,
                                         SecAuthenticationType authType)
{
	NSMutableDictionary *query = [NSMutableDictionary dictionaryWithObjectsAndKeys:
...
static NSError *AIKeychainErrorFromStatus(OSStatus err, NSString *funcName) {
	...
	NSDictionary *userInfo = [NSDictionary dictionaryWithObjectsAndKeys:
		funcName ?: @"SecItem*", AIKEYCHAIN_ERROR_USERINFO_SECURITYFUNCTIONNAME,
									  ...
									  error:(out NSError **)outError
{
	...
	OSStatus err = AIKeychainCopyPassword(server, account, protocolStr, path, port, domain, authType, &password, outKeychainItem);

	if (outError) *outError = AIKeychainErrorFromStatus(err, @"SecItemCopyMatching");
...
- (NSDictionary *)dictionaryFromKeychainForServer:(NSString *)server protocol:(SecProtocolType)protocol error:(out NSError **)outError
{
	...
	NSMutableDictionary *query = [NSMutableDictionary dictionaryWithObjectsAndKeys:
		...
		protocolStr, (__bridge id)kSecAttrProtocol,
