...
static NSDate *dateFromHTMLLog(NSString *pathToFile)
{
	...
	if ((openParenRange = [pathToFile rangeOfString:@"(" options:NSBackwardsSearch]).location != NSNotFound) {
		...
		if ((closeParenRange = [pathToFile rangeOfString:@")" options:0 range:openParenRange]).location != NSNotFound) {
			...
			NSString *dateString =
				[pathToFile substringWithRange:NSMakeRange(openParenRange.location + 1,
...
Boolean GetMetadataForHTMLLog(NSMutableDictionary *attributes, NSString *pathToFile)
{
	...
	if ((date = dateFromHTMLLog(pathToFile))) {
		...
		[attributes setObject:date forKey:(NSString *)kMDItemLastUsedDate];
	}
	...
	[attributes setObject:@"Chat log" forKey:(NSString *)kMDItemKind];
	return TRUE;
}
	...
	for (i = 0, j = 0; str2[i]; i++) {
		if (str2[i] == '<') {
			if (cdata_close_tag) {
				...
				if (strncasecmp(str2 + i, cdata_close_tag, strlen(cdata_close_tag)) == 0) {
			...
			if (g_ascii_isspace(str2[k]))
				 ...
				 * to print later. */
				if (strncasecmp(str2 + i, "<a", 2) == 0 && g_ascii_isspace(str2[i + 2])) {
				 ...
				 * address the link was pointing to. */
					...
					if ((hrlen != j - href_st || strncmp(str2 + href_st, href, hrlen)) &&
						(hrlen != j - href_st + 7 || strncmp(str2 + href_st, href + 7, hrlen - 7))) {
						str2[j++] = ' ';
						str2[j++] = '(';
				...
				else if (strncasecmp(str2 + i, "<p>", 3) == 0 || strncasecmp(str2 + i, "<tr", 3) == 0 ||
						 strncasecmp(str2 + i, "<br", 3) == 0 || strncasecmp(str2 + i, "<li", 3) == 0 ||
						 strncasecmp(str2 + i, "<div", 4) == 0 || strncasecmp(str2 + i, "</table>", 8) == 0) {
					str2[j++] = '\n';
				}
