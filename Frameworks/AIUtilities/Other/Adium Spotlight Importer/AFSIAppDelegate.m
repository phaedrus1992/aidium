...
- (void)importChatlogsFromDirectoryTreeRootedAtPath:(NSString *)rootPath
								  numFilesProcessed:(inout NSUInteger *)numFilesProcessedPtr
{
...
- (void)application:(NSApplication *)sender openFiles:(NSArray *)paths
{
	...
	NSBeep();
	NSLog(@"Job's done! Processed %lu files in %f seconds (%f seconds per file)", numFilesProcessed, timeTaken,
		  timeTaken / numFilesProcessed);
}
