...
+ (NSImage *)serviceIconForObject:(AIListObject *)inObject
...
+ (NSImage *)serviceIconForService:(AIService *)service
							  type:(AIServiceIconType)iconType
						 direction:(AIIconDirection)iconDirection
{
...
+ (NSImage *)serviceIconForServiceID:(NSString *)serviceID
								type:(AIServiceIconType)iconType
						   direction:(AIIconDirection)iconDirection
{
	NSImage *serviceIcon;
...
+ (BOOL)setActiveServiceIconsFromPath:(NSString *)inPath
{
	...
	if (serviceIconDict && [[serviceIconDict objectForKey:@"AdiumSetVersion"] intValue] == 1) {
		serviceIconBasePath = [inPath copy];

		serviceIconNames[AIServiceIconSmall] = [[serviceIconDict objectForKey:@"Interface-Small"] copy];

		serviceIconNames[AIServiceIconLarge] = [[serviceIconDict objectForKey:@"Interface-Large"] copy];

		serviceIconNames[AIServiceIconList] = [[serviceIconDict objectForKey:@"List"] copy];
		...
		[[NSNotificationCenter defaultCenter] postNotificationName:AIServiceIconSetDidChangeNotification object:nil];
...
+ (NSImage *)previewMenuImageForIconPackAtPath:(NSString *)inPath
{
	...
	if (iconDict && [[iconDict objectForKey:@"AdiumSetVersion"] intValue] == 1) {
		...
		NSEnumerator *enumerator =
			[[NSArray arrayWithObjects:@"AIM", @"Jabber", @"MSN", @"Yahoo!", nil] objectEnumerator];
		NSString *iconID;
		int xOrigin = 0;
...
+ (NSImage *)defaultServiceIconForType:(AIServiceIconType)type serviceID:(NSString *)serviceID
{
	...
	serviceIconDict =
		[NSDictionary dictionaryWithContentsOfFile:[defaultPath stringByAppendingPathComponent:@"Icons.plist"]];
	if (serviceIconDict && [[serviceIconDict objectForKey:@"AdiumSetVersion"] intValue] == 1) {
