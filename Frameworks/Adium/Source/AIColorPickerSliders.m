 ...
 *
 * In 10.3 through 10.5 (at least), sliderModePopUp provides 4 menu items, each of which corresponds to a particular
 * slider mode and is associated with a shortcut, command-1 through command-4.  Adium uses command-1 through command-4
 * to switch to the first through fourth open chats. Without this class, after the color picker is opened and the slider
 * view is displayed, the next time Adium reconfigures shortcut keys it will be unable to use these 4 as the color
 * picker not only uses them but continues to use them for the rest of the application session.
 *
...
+ (void)load
{
	...
	method_exchangeImplementations(class_getInstanceMethod([NSColorPickerSliders class], @selector(_setupProfileUI)),
								   class_getInstanceMethod(self, @selector(_setupProfileUI)));
}
...
- (void)_setupProfileUI
{
	...
	__setupProfileUI_method_invoke(self,
								   class_getInstanceMethod([AIColorPickerSliders class], @selector(_setupProfileUI)));
