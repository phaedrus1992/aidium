				 ...
				 userInfo:(id)inUserInfo
{
	...
	NSParameterAssert(
		[inTarget respondsToSelector:@selector(presetNameSheetControllerDidEnd:returnCode:newName:userInfo:)]);
...
- (IBAction)okay:(id)sender
{
	...
	if (![target respondsToSelector:@selector(presetNameSheetController:shouldAcceptNewName:userInfo:)] ||
		[target presetNameSheetController:self shouldAcceptNewName:newName userInfo:userInfo]) {
		...
		[self
			configureExplanatoryTextWithString:[NSString stringWithFormat:@"%@\n\n%@", explanatoryText, nameInUseText]];
...
- (void)configureExplanatoryTextWithString:(NSString *)inExplanatoryText
{
	...
	heightChange +=
		[textView_explanatoryText frame].size.height - [scrollView_explanatoryText documentVisibleRect].size.height;
