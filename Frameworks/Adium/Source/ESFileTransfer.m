...
// Init
+ (id)fileTransferWithContact:(AIListContact *)inContact
				   forAccount:(AIAccount *)inAccount
						 type:(AIFileTransferType)inType
{
...
- (id)initWithContact:(AIListContact *)inContact forAccount:(AIAccount *)inAccount type:(AIFileTransferType)inType
{
	...
	if ((self = [super initWithChat:aChat
		...
		[fileTransferDict setObject:[NSValue valueWithNonretainedObject:self] forKey:[self uniqueID]];
	}
...
{
	return (self.fileTransferType == Incoming_FileTransfer || self.fileTransferType == Unknown_FileTransfer)
			   ? (AIListContact *)[self source]
			   : (AIListContact *)[self destination];
}
...
- (NSString *)displayFilename
{
	...
	if (!displayFilename)
		displayFilename = [remoteFilename lastPathComponent];
...
{
	// incoming file transfers should always have a non-account as the source, and outgoing ones should always have an
 ...
 *
 * @param inPercent The percentage complete.  If 0, inBytesSent will be used to calculate the percent complete if
 * possible.
		...
		if (percentDone >= 1.0) {
			...
			if (type == Incoming_FileTransfer) {
				...
				if (FSPathMakeRef((const UInt8 *)[localFilename fileSystemRepresentation], &fsRef, NULL) == noErr) {
					...
					if (err == noErr) {
						...
						if (CFGetTypeID(cfOldQuarantineProperties) == CFDictionaryGetTypeID()) {
							quarantineProperties =
								[[(NSDictionary *)cfOldQuarantineProperties mutableCopy] autorelease];
						} else {
					...
					if (LSSetItemAttribute(&fsRef, kLSRolesAll, kLSItemQuarantineProperties, quarantineProperties) !=
						noErr) {
...
- (BOOL)isStopped
{
	return ((status == Complete_FileTransfer) || (status == Cancelled_Local_FileTransfer) ||
			(status == Cancelled_Remote_FileTransfer) || (status == Failed_FileTransfer));
}
...
- (void)recreateMessage
{
	...
	[self setMessage:[NSAttributedString
						 stringWithString:
							 [NSString
								 stringWithFormat:
									 AILocalizedString(
										 @"%@ requests to send you %@",
										 "This is displayed in the message window when prompting to receive a file. "
										 "The first %@ is the sender; the second %@ is the filename of the file being "
										 "sent. It will be followed by buttons such as 'Save' and 'Cancel'."),
									 [[self contact] formattedUID], filenameDisplay]]];
}
