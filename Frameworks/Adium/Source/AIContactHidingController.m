						 ...
						 firstTime:(BOOL)firstTime
{
	...
	if (!firstTime) {
		[[NSNotificationCenter defaultCenter] postNotificationName:CONTACT_VISIBILITY_OPTIONS_CHANGED_NOTIFICATION
															object:nil];
...
- (BOOL)filterContacts:(NSString *)inSearchString
{
	...
	for (AIListContact *listContact in
		 [adium.contactController.allContacts arrayByAddingObjectsFromArray:adium.contactController.allBookmarks]) {
...
- (BOOL)visibilityOfListObject:(AIListObject *)listObject inContainer:(id<AIContainingObject>)container
{
	...
	if ([listObject conformsToProtocol:@protocol(AIContainingObject)]) {
		...
		return ([(id<AIContainingObject>)listObject visibleCount] > 0 ||
				([listObject boolValueForProperty:@"New Object"] && useContactListGroups));
	}
	...
	BOOL online = listObject.online || [listObject boolValueForProperty:@"signedOff"] ||
				  [listObject boolValueForProperty:@"New Object"];
 ...
 * @result Returns YES if the display name, formatted UID or status message contain inSearchString, or if inSearchString
 * is empty. Otherwise NO.
 */
- (BOOL)evaluatePredicateOnListObject:(AIListObject *)listObject withSearchString:(NSString *)inSearchString
{
 ...
 * @result Returns an NSPredicate to compare a contact to, or if inSearchString is empty, a predicate that matches
 * everything
 */
- (NSPredicate *)createPredicateWithSearchString:(NSString *)inSearchString
{
	...
	while ((tokenType = CFStringTokenizerAdvanceToNextToken(tokenizer)) != kCFStringTokenizerTokenNone) {
		...
		NSPredicate *predicate = [filterPredicateTemplate
			predicateWithSubstitutionVariables:[NSDictionary dictionaryWithObject:keyword forKey:@"KEYWORD"]];
