...
- (NSBezierPath *)bezierPathForDrawingInRect:(NSRect)rect
{
	...
	if (row == 0) {
		if (numberOfRows > 1) {
			// Draw the top corner rounded if this cell is the first cell in the outline view (only possible if its
			// containing group is not being displayed) but not also the last cell
			bezierPath = [NSBezierPath bezierPathWithRoundedTopCorners:rect radius:MOCKIE_RADIUS];

		} else {
			// Draw the entire rect rounded if this cell is the first cell in the outline view (only possible if its
			// containing group is not being displayed) and also the last cell
			bezierPath = [NSBezierPath bezierPathWithRoundedRect:rect radius:MOCKIE_RADIUS];
		}

	} else if (row >= (numberOfRows - 1) || [(id<AIMultiCellOutlineViewDelegate>)self.outlineControlView.delegate
												outlineView:self.outlineControlView
...
// User Icon, clipping to the last bezier path (which should have been part of this same drawing operation) if
// applicable
