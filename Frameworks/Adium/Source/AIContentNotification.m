	...
	notificationType:(AINotificationType)inNotificationType
{
	...
	if ([inSource isKindOfClass:[AIAccount class]]) {
		defaultMessage =
			[NSString stringWithFormat:AILocalizedString(@"You requested %@'s attention",
														 "Message displayed when you send a buzz/nudge/other "
														 "notification. %@ will be the other person's name."),
									   [inDest displayName]];
	} else {
		defaultMessage =
			(inSource ? [NSString stringWithFormat:AILocalizedString(
													   @"%@ wants your attention!",
													   "Message displayed when a contact sends a buzz/nudge/other "
													   "notification. %@ will be the other person's name."),
