...
- (id)initByCompositingStates:(NSArray *)inIconStates
{
	...
	if ((self = [super init])) {
		...
		for (AIIconState *iconState in [inIconStates reverseObjectEnumerator]) {
			...
			if (!baseIconState && ![iconState overlay])
				baseIconState = iconState;
		...
		if (!animatingState)
			animatingState = overlayAnimatingState;
...
- (void)nextFrame
{
	if (animated) {
		...
		if ((currentFrame >= imageArrayCount) && iconRendering_states && iconRendering_baseState &&
...
- (NSImage *)_compositeStates:(NSArray *)iconStateArray
				withBaseState:(AIIconState *)baseState
			   animatingState:(AIIconState *)animatingState
					 forFrame:(NSInteger)frame
{
		...
		if ([iconState overlay]) {
			...
			if ([iconState animated] && animatingStateNumberOfFrames) {
				if (iconState == animatingState) { // Only one state animates at a time
					...
					[imagesToComposite
						addObject:[[iconState imageArray] objectAtIndex:(((frame + 1) / animatingStateNumberOfFrames) *
																		 ([iconState numberOfFrames] - 1))]];
				}
			} else {
