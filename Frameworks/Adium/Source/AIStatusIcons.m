...
+ (NSImage *)statusIconForListObject:(AIListObject *)listObject
								type:(AIStatusIconType)iconType
...
/* Copied from AIStatusController... this is called with a nil statusName frequently, so avoid making lots of extra
 * method calls. */
						   ...
						   direction:(AIIconDirection)iconDirection
{
	...
	if (!statusName)
		statusName = defaultNameForStatusType(statusType);
		...
		if (fileName) {
			...
			if (!statusIcon) {
				...
				if (![defaultStatusName isEqualToString:statusName]) {
					/* If the pack doesn't provide an icon for this specific status name, fall back on and then cache
					 * the default. */
...
+ (BOOL)setActiveStatusIconsFromPath:(NSString *)inPath
{
	...
	if (xtraBundle && ([[xtraBundle objectForInfoDictionaryKey:@"XtraBundleVersion"] intValue] ==
					   1)) // This checks for a new-style xtra
		inPath = [xtraBundle resourcePath];

	NSDictionary *statusIconDict =
		[NSDictionary dictionaryWithContentsOfFile:[inPath stringByAppendingPathComponent:@"Icons.plist"]];

	if (statusIconDict && [[statusIconDict objectForKey:@"AdiumSetVersion"] intValue] == 1) {
		statusIconBasePath = [inPath copy];

		statusIconNames[AIStatusIconTab] = [[statusIconDict objectForKey:@"Tabs"] copy];

		statusIconNames[AIStatusIconList] = [[statusIconDict objectForKey:@"List"] copy];
		...
		[[NSNotificationCenter defaultCenter] postNotificationName:AIStatusIconSetDidChangeNotification object:nil];
...
+ (NSImage *)previewMenuImageForIconPackAtPath:(NSString *)inPath
{
	...
	image = [[NSImage alloc]
		initWithSize:NSMakeSize((PREVIEW_MENU_IMAGE_SIZE + PREVIEW_MENU_IMAGE_MARGIN) * 4, PREVIEW_MENU_IMAGE_SIZE)];
	...
	if (iconDict && [[iconDict objectForKey:@"AdiumSetVersion"] intValue] == 1) {
		...
		[image lockFocus];
		for (NSString *iconID in
			 [NSArray arrayWithObjects:STATUS_NAME_AVAILABLE, STATUS_NAME_AWAY, @"Idle", @"Offline", nil]) {
