...
- (void)dealloc
{
	...
	[idleSince release];
	idleSince = nil;
	[idleReadable release];
	idleReadable = nil;
	[serverDisplayName release];
...
- (void)remoteGroupingChanged
{
	...
	if (self.isStranger != (remoteGroupCount == 0)) {
		[self setValue:[NSNumber numberWithBool:remoteGroupCount > 0] forProperty:@"notAStranger" notify:NotifyLater];
...
- (void)setServersideAlias:(NSString *)alias silently:(BOOL)silent
{
	...
	if (![alias isEqualToString:[self valueForProperty:@"serverDisplayName"]]) {
		...
		[self setValue:alias forProperty:@"serverDisplayName" notify:NotifyLater];
	...
	if (![[displayNameArray objectWithOwner:self.account] isEqualToString:cleanedAlias]) {
		[displayNameArray setObject:cleanedAlias withOwner:self.account priorityLevel:Low_Priority];
...
- (void)setOnline:(BOOL)online notify:(NotifyTiming)notify silently:(BOOL)silent
{
	if (online != self.online) {
		[self setValue:[NSNumber numberWithBool:online] forProperty:@"isOnline" notify:notify];

		if (!silent) {
			[self setValue:[NSNumber numberWithBool:YES]
				forProperty:(online ? @"signedOn" : @"signedOff")notify:notify];
			[self setValue:nil forProperty:(online ? @"signedOff" : @"signedOn")notify:notify];
			[self setValue:nil
				forProperty:(online ? @"signedOn" : @"signedOff")afterDelay:CONTACT_SIGN_ON_OR_OFF_PERSISTENCE_DELAY];
		}
		...
		if (online) {
			...
			// Will always notify
			[self.account removePropertyValuesFromContact:self silently:silent];
		}
	}
}
...
{
	[self setValue:signonDate forProperty:@"Signon Date" notify:notify];
}
/*!
...
- (void)setIdle:(BOOL)inIsIdle sinceDate:(NSDate *)idleSinceDate notify:(NotifyTiming)notify
{
	if (inIsIdle) {
		if (idleSinceDate) {
			[self setValue:idleSinceDate forProperty:@"idleSince" notify:NotifyLater];
		} else {
			// No idleSinceDate means we are Idle but don't know how long, so set to -1
			[self setValue:[NSNumber numberWithInt:-1] forProperty:@"idle" notify:NotifyLater];
		}
	} else {
		[self setValue:nil forProperty:@"idleSince" notify:NotifyLater];
		[self setValue:nil forProperty:@"idle" notify:NotifyLater];
	}

	/* @"idle", for a contact with an IdleSince date, will be changing every minute.  @"isIdle" provides observers a way
	 * to perform an action when the contact becomes/comes back from idle, regardless of whether an IdleSince is
	 * available, without having to do that action every minute for other contacts.
	 */
	[self setValue:[NSNumber numberWithBool:inIsIdle] forProperty:@"isIdle" notify:NotifyLater];
...
- (void)setWarningLevel:(NSInteger)warningLevel notify:(NotifyTiming)notify
{
	if (warningLevel != self.warningLevel) {
		[self setValue:[NSNumber numberWithInteger:warningLevel] forProperty:@"Warning" notify:notify];
	}
}
...
{
	[self setValue:array forProperty:@"ProfileArray" notify:notify];
}
...
{
	[self setValue:profile forProperty:@"textProfile" notify:notify];
}
...
{
	[self setValue:[NSNumber numberWithBool:inIsMobile] forProperty:@"isMobile" notify:notify];
}
...
{
	if (addToPrivacyLists) { // caller of this method wants to actually block or unblock the contact, rather than just
							 // update the property
...
- (AIEncryptedChatPreference)encryptedChatPreferences
{
	...
	if (!prefNumber || ([prefNumber integerValue] == EncryptedChat_Default)) {
		prefNumber = [self.account preferenceForKey:KEY_ENCRYPTED_CHAT_PREFERENCE group:GROUP_ENCRYPTION];
	}
		...
		if (statusName) {
			...
			if (descriptionOfStatus)
				contactListStatusMessage =
					[[[NSAttributedString alloc] initWithString:descriptionOfStatus] autorelease];
		}
	}
...
- (void)removeFromGroup:(AIListObject<AIContainingObject> *)group
{
	if (self.account.online) {
		if (group == adium.contactController.contactList || group == adium.contactController.offlineGroup) {
			[self.account removeContacts:[NSArray arrayWithObject:self] fromGroups:[self.remoteGroups allObjects]];
		} else {
			[self.account removeContacts:[NSArray arrayWithObject:self] fromGroups:[NSArray arrayWithObject:group]];
		}
	}
}
...
- (id)sendScriptCommand:(NSScriptCommand *)command
{
	...
	if (targetAccount) {
		if (self.account != account)
			targetMessagingContact = [adium.contactController contactWithService:self.service
																		 account:account
																			 UID:self.UID];
	...
	if (message && [message length]) {
		...
		if (targetMessagingContact) {
			chat = [adium.chatController openChatWithContact:targetMessagingContact onPreferredAccount:NO];
	...
	if (filePath && [filePath length]) {
		...
		if (!targetFileTransferContact) {
			// Get the target contact.  This could be the same contact, an identical contact on another account,
			// or a subcontact (if we're talking about a metaContact, for example)
			targetFileTransferContact =
				[adium.contactController preferredContactForContentType:CONTENT_FILE_TRANSFER_TYPE forListContact:self];
		}
