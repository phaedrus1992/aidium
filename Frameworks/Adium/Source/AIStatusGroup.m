...
+ (id)statusGroupWithContainedStatusItems:(NSArray *)inContainedObjects
{
	...
	[inContainedObjects makeObjectsPerformSelector:@selector(setContainingStatusGroup:) withObject:statusGroup];
...
- (id)initWithCoder:(NSCoder *)decoder
{
	if ((self = [super initWithCoder:decoder])) {
		if ([decoder allowsKeyedCoding]) {
			...
			// Let 'em know where they stand
			[containedStatusItems makeObjectsPerformSelector:@selector(setContainingStatusGroup:) withObject:self];
...
- (AIStatus *)anyContainedStatus
{
	...
	AIStatusItem *anyStatus =
		([containedStatusItems count] ? [containedStatusItems objectAtIndex:(random() % [containedStatusItems count])]
									  : nil);
	...
	if ([anyStatus isKindOfClass:[AIStatusGroup class]]) {
		...
		// XXX if we found an empty status group, we should look elsewhere if possible, iterating through the list or
		// something.
	}
 ...
 *
 * This should not be used for the root group, as it doesn't include the temporary and built-in items or the Custom...
 * items.
 */
- (NSMenu *)statusSubmenuNotifyingTarget:(id)target action:(SEL)selector
{
	...
	while ((statusState = [enumerator nextObject])) {
		...
		if ([statusState isKindOfClass:[AIStatus class]]) {
			...
			/* AIStatusGroup */
			[menuItem setSubmenu:[(AIStatusGroup *)statusState statusSubmenuNotifyingTarget:target action:selector]];
		}
...
- (NSUInteger)moveStatusItem:(AIStatusItem *)statusState toIndex:(NSUInteger)destIndex
{
	...
	if (destIndex > sourceIndex)
		destIndex -= 1;
	if (destIndex > [containedStatusItems count])
...
NSComparisonResult statusArraySort(id objectA, id objectB, void *context)
{
	...
	// We treat Invisible statuses as being the same as Away for purposes of the menu
	if (statusTypeA == AIInvisibleStatusType)
		statusTypeA = AIAwayStatusType;
	if (statusTypeB == AIInvisibleStatusType)
		statusTypeB = AIAwayStatusType;
