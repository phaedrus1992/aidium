...
- (CGFloat)cellWidth
{
	...
	if (extendedStatusVisible && idleTimeVisible && !idleTimeIsBelow && [listObject valueForProperty:@"idleReadable"]) {
		...
		NSAttributedString *idleAttString = [[NSAttributedString alloc] initWithString:idleTimeString
...
- (void)drawContentWithFrame:(NSRect)rect
{
	...
	// Far Left
	if (statusIconPosition == LIST_POSITION_FAR_LEFT)
		rect = [self drawStatusIconInRect:rect position:IMAGE_POSITION_LEFT];
	if (serviceIconPosition == LIST_POSITION_FAR_LEFT)
		rect = [self drawServiceIconInRect:rect position:IMAGE_POSITION_LEFT];

	// User Icon [Left]
	if (userIconPosition == LIST_POSITION_LEFT)
		rect = [self drawUserIconInRect:rect position:IMAGE_POSITION_LEFT];

	// Left
	if (statusIconPosition == LIST_POSITION_LEFT)
		rect = [self drawStatusIconInRect:rect position:IMAGE_POSITION_LEFT];
	if (serviceIconPosition == LIST_POSITION_LEFT)
		rect = [self drawServiceIconInRect:rect position:IMAGE_POSITION_LEFT];

	// Far Right
	if (statusIconPosition == LIST_POSITION_FAR_RIGHT)
		rect = [self drawStatusIconInRect:rect position:IMAGE_POSITION_RIGHT];
	if (serviceIconPosition == LIST_POSITION_FAR_RIGHT)
		rect = [self drawServiceIconInRect:rect position:IMAGE_POSITION_RIGHT];

	// User Icon [Right]
	if (userIconPosition == LIST_POSITION_RIGHT)
		rect = [self drawUserIconInRect:rect position:IMAGE_POSITION_RIGHT];

	// Right
	if (statusIconPosition == LIST_POSITION_RIGHT)
		rect = [self drawStatusIconInRect:rect position:IMAGE_POSITION_RIGHT];
	if (serviceIconPosition == LIST_POSITION_RIGHT)
		rect = [self drawServiceIconInRect:rect position:IMAGE_POSITION_RIGHT];
	...
	if (idleTimeIsBelow && statusMessageIsBelow) {
		rect = [self drawUserExtendedStatusInRect:rect
									  withMessage:(useStatusMessageAsExtendedStatus
													   ...
													   : [listObject valueForProperty:@"extendedStatus"])drawUnder:YES];
		...
		[self drawUserExtendedStatusInRect:rect
							   withMessage:(useStatusMessageAsExtendedStatus
												? [listObject statusMessageString]
												: [listObject valueForProperty:@"extendedStatus"])drawUnder:NO];
	} else {
		...
		if (!statusMessageIsBelow && statusMessageVisible) {
			rect = [self drawUserExtendedStatusInRect:rect withMessage:[listObject statusMessageString] drawUnder:NO];
		}
...
- (NSRect)drawUserIconInRect:(NSRect)inRect position:(IMAGE_POSITION)position
{
	...
	if (userIconVisible) {
		...
		if (position == IMAGE_POSITION_LEFT)
			rect.origin.x += USER_ICON_LEFT_PAD;
		...
		if ((statusIconPosition == LIST_POSITION_BADGE_LEFT) || (statusIconPosition == LIST_POSITION_BADGE_RIGHT) ||
			(serviceIconPosition == LIST_POSITION_BADGE_LEFT) || (serviceIconPosition == LIST_POSITION_BADGE_RIGHT)) {
			drawRect = [image rectForDrawingInRect:inRect atSize:userIconSize position:position];
		...
		if (position == IMAGE_POSITION_LEFT)
			rect.origin.x += USER_ICON_RIGHT_PAD;
...
- (NSRect)drawStatusIconInRect:(NSRect)rect position:(IMAGE_POSITION)position
{
	if (statusIconsVisible) {
		...
		rect = [image drawInRect:rect atSize:NSMakeSize(0, 0) position:position fraction:1.0f];
...
- (NSRect)drawServiceIconInRect:(NSRect)rect position:(IMAGE_POSITION)position
{
	if (serviceIconsVisible) {
		...
		if (!isBadge) {
			if (position == IMAGE_POSITION_LEFT)
				rect.origin.x += SERVICE_ICON_LEFT_PAD;
		...
		if (!isBadge) {
			if (position == IMAGE_POSITION_LEFT)
				rect.origin.x += SERVICE_ICON_RIGHT_PAD;
	...
	if (extendedStatusVisible && (drawUnder || [self textAlignment] != NSCenterTextAlignment)) {
		if (string) {
			...
			NSDictionary *attributes =
				([self cellIsSelected] ? [self statusAttributesInverted] : [self statusAttributes]);
			...
			if (nameSize.width > drawRect.size.width)
				nameSize = rect.size;
			...
			if (drawUnder) {
				...
				offset =
					(labelFontHeight - statusFontHeight) + ([[self font] descender] - [[self statusFont] descender]);
			}
