...
- (NSSet *)_informObserversOfObjectStatusChange:(AIListObject *)inObject
									   withKeys:(NSSet *)modifiedKeys
										 silent:(BOOL)silent;
...
//  Silent: Silences all events, notifications, sounds, overlays, etc. that would have been associated with this status
//  change
...
- (void)_performDelayedUpdates:(NSTimer *)timer
{
	...
	if (updatesOccured) {
		...
		if (delayedAttributeChanges) {
			if (!shouldSort && [[AISortController activeSortController]
								   shouldSortForModifiedAttributeKeys:delayedModifiedAttributeKeys]) {
				shouldSort = YES;
			}
		...
		if (postAttributesChangesComplete) {
			...
			[[NSNotificationCenter defaultCenter]
				postNotificationName:ListObject_AttributeChangesComplete
							  object:nil
							userInfo:[NSDictionary dictionaryWithObject:delayedModifiedAttributeKeys forKey:@"Keys"]];
		}
	}
...
- (void)unregisterListObjectObserver:(id)inObserver
{
	...
	if (informingObservers) {
		if (!removedContactObservers)
			removedContactObservers = [[NSMutableSet alloc] init];
...
- (void)updateAllListObjectsForObserver:(id<AIListObjectObserver>)inObserver
{
	...
	for (AIListBookmark *listBookmark in [(AIContactController *)adium.contactController bookmarkEnumerator]) {
		...
		if (attributes)
			[self listObjectAttributesChanged:listBookmark modifiedKeys:attributes];
	}
	...
	for (AIListGroup *listGroup in [(AIContactController *)adium.contactController groupEnumerator]) {
		...
		if (attributes)
			[self listObjectAttributesChanged:listGroup modifiedKeys:attributes];
	}
	...
	for (AIAccount *account in adium.accountController.accounts) {
		...
		if (attributes)
			[self listObjectAttributesChanged:account modifiedKeys:attributes];
	}
										 ...
										 silent:(BOOL)silent
{
	...
	for (NSValue *observerValue in [contactObservers copy]) {
		...
		if (newKeys) {
			if (!attrChange)
				attrChange = [[NSMutableSet alloc] init];
