 ...
 * @param inMessageHeader A plain <tt>NSString</tt> which will be displayed as a bolded header for the message.  Hidden
 * if nil.
 ...
 * @param inTarget The target to send the selector <tt>textAndButtonsWindowDidEnd:(NSWindow *)window
 * returnCode:(AITextAndButtonsReturnCode)returnCode userInfo:(id)userInfo</tt> when the sheet ends.
...
/*!
 * @brief Change and show a text and buttons window which will notify a target when a button is clicked or the window is
 * closed.
 *
 * The buttons have titles of defaultButton, alternateButton, and otherButton.
 * The buttons are laid out on the lower-right corner of the window, with defaultButton on the right, alternateButton on
...
- (void)setKeyEquivalent:(NSString *)keyEquivalent
			modifierMask:(unsigned int)mask
			   forButton:(AITextAndButtonsWindowButton)windowButton
{
	NSButton *button = nil;
	switch (windowButton) {
...
- (void)configureWindow
{
	...
	if (messageHeader) {
		...
		heightChange +=
			[textView_messageHeader frame].size.height - [scrollView_messageHeader documentVisibleRect].size.height;
	...
	// Set the message, then change the window size accordingly
	{
		...
		messageHeightChange =
			[textView_message frame].size.height - [scrollView_message documentVisibleRect].size.height;
	...
	if (alternateButton) {
		...
		/* For NSButtons, sizeToFit is 8 pixels smaller than the HIG recommended size  */
		newFrame.size.width += 8;
		/* Only use integral widths to keep alignment correct; round up as an extra pixel of whitespace never hurt
		 * anybody */
		newFrame.size.width = AIround(NSWidth(newFrame) + 0.5f);
		if (newFrame.size.width < 90)
			newFrame.size.width = 90;
...
- (BOOL)textView:(NSTextView *)aTextView doCommandBySelector:(SEL)aSelector
{
	...
	AILogWithSignature(@"escape? %i newline? %i", [[button_alternate keyEquivalent] isEqualToString:@"\E"],
					   [[button_default keyEquivalent] isEqualToString:@"\n"]);

	if (aSelector == @selector(cancelOperation:) && [[button_alternate keyEquivalent] isEqualToString:@"\E"]) {
