...
+ (NSAppleEventDescriptor *)aliasListDescriptorWithArray:(NSArray *)anArray
{
	...
	unsigned int theIndex, theNumOfParam;
	...
	if (theNumOfParam > 0) {
		...
		for (theIndex = 0; theIndex < theNumOfParam; theIndex++) {
			...
			NSAssert1(theAliasDesc != nil, @"Could not get an alias NSAppleEventDescriptor for %@",
					  [anArray objectAtIndex:theIndex]);
...
+ (id)descriptorWithURL:(NSURL *)aURL
{
	return [self descriptorWithDescriptorType:typeFileURL
										 data:[NSData dataWithBytes:(void *)aURL length:sizeof(NSURL)]];
}
...
+ (NSAppleEventDescriptor *)aliasDescriptorWithFile:(id)aFile
{
	...
	if ([aFile getFSRef:&theReference] == YES && FSNewAliasMinimal(&theReference, &theAliasHandle) == noErr) {
		...
		theAppleEventDescriptor =
			[self descriptorWithDescriptorType:typeAlias
										  data:[NSData dataWithBytes:*theAliasHandle
															  length:GetHandleSize((Handle)theAliasHandle)]];
...
{
	return [self descriptorWithDescriptorType:typeShortInteger
										 data:[NSData dataWithBytes:&aValue length:sizeof(aValue)]];
}
// typeLongInteger
/*
 * +descriptorWithLong:
...
{
	return [self descriptorWithDescriptorType:typeLongInteger
										 data:[NSData dataWithBytes:&aValue length:sizeof(aValue)]];
}
// typeInteger
/*
...
+ (id)descriptorWithNumber:(NSNumber *)aNumber
{
	...
	struct {
		...
		DescType descType;
		unsigned short size;
	} theTypes[] = {{@encode(float), typeIEEE32BitFloatingPoint, sizeof(float)},
	...
	for (theIndex = 0; theDescriptor == nil && theTypes[theIndex].objCType != NULL; theIndex++) {
		if (strcmp(theTypes[theIndex].objCType, theType) == 0) {
			...
			theDescriptor = [self descriptorWithDescriptorType:theTypes[theIndex].descType
														 bytes:theBuffer
														length:theTypes[theIndex].size];
		}
	}
...
+ (id)descriptorWithArray:(NSArray *)anArray
{
	...
	if (theNumOfParam > 0) {
		...
		for (theIndex = 0; theIndex < theNumOfParam; theIndex++)
			[theEventList insertDescriptor:[self descriptorWithObject:[anArray objectAtIndex:theIndex]]
								   atIndex:theIndex + 1];
	}
 ...
 */
+ (NSAppleEventDescriptor *)recordDescriptorWithObjects:(id *)anObjects
											   keywords:(AEKeyword *)aKeywords
												  count:(unsigned int)aCount
{
	...
	if ((theDescriptor = [self recordDescriptor]) != nil) {
		...
		for (theIndex = 0; theIndex < aCount; theIndex++) {
			[theDescriptor setDescriptor:[NSAppleEventDescriptor descriptorWithObject:anObjects[theIndex]]
							  forKeyword:aKeywords[theIndex]];
		}
	}
	return theDescriptor;
}
...
+ (NSAppleEventDescriptor *)recordDescriptorWithDictionary:(NSDictionary *)aDictionary
{
	...
	if (aDictionary != nil && (theDescriptor = [self recordDescriptor]) != nil) {
		...
		while ((theKey = [theEnumerator nextObject])) {
			...
			[theDescriptor setDescriptor:[NSAppleEventDescriptor descriptorWithObject:[aDictionary objectForKey:theKey]]
							  forKeyword:[theKey unsignedIntValue]];
		}
	}
	return theDescriptor;
}
...
+ (id)descriptorWithDictionary:(NSDictionary *)aDictionary
{
	...
	[theRecordDescriptor setDescriptor:[NSAppleEventDescriptor userRecordDescriptorWithDictionary:aDictionary]
...
{
	NSAppleEventDescriptor *theDescriptor = nil, *theRecordDescriptor = nil;
	va_list theArgList;
	va_start(theArgList, anObject);
...
{
	NSAppleEventDescriptor *theRecordDescriptor = [self recordDescriptor];
	[theRecordDescriptor setDescriptor:[NSAppleEventDescriptor userRecordDescriptorWithObjectAndKeys:anObject
																						   arguments:anArgList]
							forKeyword:keyASUserRecordFields];
	return theRecordDescriptor;
}
...
+ (NSAppleEventDescriptor *)userRecordDescriptorWithObjectAndKeys:(id)anObject arguments:(va_list)anArgList
{
	...
	if (theUserRecord) {
		...
		do {
			...
			[theUserRecord insertDescriptor:[NSAppleEventDescriptor descriptorWithString:[theKey description]]
									atIndex:theIndex++];
 ...
 */
+ (NSAppleEventDescriptor *)userRecordDescriptorWithObjects:(id *)anObject
													   keys:(NSString **)aKeys
													  count:(unsigned int)aCount
{
	NSAppleEventDescriptor *theUserRecord = [self listDescriptor];
	if (theUserRecord) {
		...
		for (theIndex = 0; theIndex < aCount; theIndex++) {
			...
			[theUserRecord insertDescriptor:[NSAppleEventDescriptor descriptorWithString:aKeys[theIndex]]
									atIndex:theIndex + 1];
			[theUserRecord insertDescriptor:[NSAppleEventDescriptor descriptorWithObject:anObject[theIndex]]
									atIndex:theIndex + 2];
		}
	}
	...
	if ([aDictionary count] > 0 && (theUserRecord = [self listDescriptor]) != nil) {
		...
		while ((theKey = [theEnumerator nextObject]) != nil) {
			[theUserRecord insertDescriptor:[NSAppleEventDescriptor descriptorWithString:[theKey description]]
									atIndex:theIndex++];
			[theUserRecord
				insertDescriptor:[NSAppleEventDescriptor descriptorWithObject:[aDictionary objectForKey:theKey]]
						 atIndex:theIndex++];
		}
	}
...
- (NSDictionary *)dictionaryValueFromRecordDescriptor
{
	...
	for (theIndex = 1; theIndex <= theNumOfItems; theIndex++) {
		...
		id theObject = theKeyword == keyASUserRecordFields ? [self descriptorForKeyword:keyASUserRecordFields]
...
- (NSDictionary *)dictionaryValue
{
	...
	NSMutableDictionary *theDictionary =
		theNumOfItems ? [NSMutableDictionary dictionaryWithCapacity:theNumOfItems / 2] : nil;

	for (theIndex = 1; theIndex + 1 <= theNumOfItems; theIndex += 2) {
		[theDictionary setObject:[[theUserRecordFields descriptorAtIndex:theIndex + 1] objectValue]
						  forKey:[[theUserRecordFields descriptorAtIndex:theIndex] stringValue]];
	}
...
- (NSURL *)urlValue
{
	...
	case typeAlias: //	alias record
	{
		...
		if (theError == noErr &&
			FSResolveAlias(NULL, (AliasHandle)theAliasHandle, &theTarget, &theWasChanged) == noErr) {
...
- (NSValue *)value
{
	...
	case typeOSAErrorRange: {
		...
		Size theActualSize;
		short int theStart, theEnd;
		if (AEGetParamPtr([self aeDesc], keyOSASourceStart, typeShortInteger, &theTypeCode, (void *)&theStart,
						  sizeof(short int), &theActualSize) == noErr &&
			AEGetParamPtr([self aeDesc], keyOSASourceEnd, typeShortInteger, &theTypeCode, (void *)&theEnd,
						  sizeof(short int), &theActualSize) == noErr) {
	...
	case typeRangeDescriptor: {
		...
		Size theActualSize;
		short int theStart, theEnd;
		if (AEGetParamPtr([self aeDesc], keyAERangeStart, typeShortInteger, &theTypeCode, (void *)&theStart,
						  sizeof(short int), &theActualSize) == noErr &&
			AEGetParamPtr([self aeDesc], keyAERangeStop, typeShortInteger, &theTypeCode, (void *)&theEnd,
						  sizeof(short int), &theActualSize) == noErr) {
			theValue = [NSValue valueWithRange:NSMakeRange(theStart, theEnd - theStart)];
		}
		break;
...
- (id)objectValue
{
	...
	case typeAERecord: //	list of keyword-specified
		theResult = [self numberOfItems] == 1 && [self keywordForDescriptorAtIndex:1] == keyASUserRecordFields
						? [self dictionaryValue]
						: [self dictionaryValueFromRecordDescriptor];
		break;
	case typeAlias: //	alias record
	case typeFileURL:
	...
	case cEventIdentifier: {
		...
		theResult =
			[NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithUnsignedInt:theValues[0]], @"EventClass",
													   [NSNumber numberWithUnsignedInt:theValues[1]], @"EventID", nil];
		break;
	}
	case typeNull:
...
{
	return aTargetDescriptor ? [self appleEventWithEventClass:kCoreEventClass
													  eventID:kAEOpenApplication
											 targetDescriptor:aTargetDescriptor
...
{
	NSAppleEventDescriptor *theEvent = nil, *theEventList = nil;
...
{
	return [[[NSAppleEventDescriptor alloc] initWithSubroutineName:aRoutineName
										   argumentsListDescriptor:aParam] autorelease];
}
...
+ (id)descriptorWithSubroutineName:(NSString *)aRoutineName argumentsArray:(NSArray *)aParamArray
{
	return [[[NSAppleEventDescriptor alloc]
		 initWithSubroutineName:aRoutineName
		argumentsListDescriptor:aParamArray ? [NSAppleEventDescriptor descriptorWithArray:aParamArray] : nil]
		autorelease];
}
 ...
 */
+ (id)descriptorWithSubroutineName:(NSString *)aRoutineName
							labels:(AEKeyword *)aLabels
				   argumentObjects:(id *)anObjects
							 count:(unsigned int)aCount
{
	return [[[self alloc] initWithSubroutineName:aRoutineName labels:aLabels arguments:anObjects
										   count:aCount] autorelease];
}
...
+ (id)descriptorWithSubroutineName:(NSString *)aRoutineName
							labels:(AEKeyword *)aLabels
			   argumentDescriptors:(NSAppleEventDescriptor **)aParam
							 count:(unsigned int)aCount
{
	return [[[self alloc] initWithSubroutineName:aRoutineName labels:aLabels argumentDescriptors:aParam
										   count:aCount] autorelease];
}
...
+ (id)descriptorWithSubroutineName:(NSString *)aRoutineName labelsAndArguments:(AEKeyword)aKeyWord, ...
{
	...
	theDescriptor = [[[self alloc] initWithSubroutineName:aRoutineName labelsAndArguments:aKeyWord
												arguments:theArgList] autorelease];
	va_end(theArgList);
	return theDescriptor;
}
...
{
	return [self initWithSubroutineName:aRoutineName
				argumentsListDescriptor:aParamArray ? [NSAppleEventDescriptor descriptorWithArray:aParamArray] : nil];
}
...
- (id)initWithSubroutineName:(NSString *)aRoutineName argumentsListDescriptor:(NSAppleEventDescriptor *)aParam
{
	if (self = [self initWithEventClass:kASAppleScriptSuite
					   ...
					   targetDescriptor:[NSAppleEventDescriptor currentProcessDescriptor]
							   returnID:kAutoGenerateReturnID
						  transactionID:kAnyTransactionID]) {
		[self setParamDescriptor:[NSAppleEventDescriptor
									 descriptorWithCString:[[aRoutineName lowercaseString] lossyCString]]
 ...
 */
- (id)initWithSubroutineName:(NSString *)aRoutineName
					  labels:(AEKeyword *)aLabels
				   arguments:(id *)anObjects
					   count:(unsigned int)aCount
{
	if (self = [self initWithEventClass:kASAppleScriptSuite
		...
		for (theIndex = 0; theIndex < aCount; theIndex++) {
			if (aLabels[theIndex] == keyASPrepositionGiven &&
				...
				[self setParamDescriptor:[NSAppleEventDescriptor userRecordDescriptorWithDictionary:anObjects[theIndex]]
							  forKeyword:keyASUserRecordFields];
				...
				[self setParamDescriptor:[NSAppleEventDescriptor descriptorWithObject:anObjects[theIndex]]
							  forKeyword:aLabels[theIndex]];
			}
		}
	}
 ...
 */
- (id)initWithSubroutineName:(NSString *)aRoutineName
		  labelsAndArguments:(AEKeyword)aKeyWord
				   arguments:(va_list)anArgList
{
	if (self = [self initWithEventClass:kASAppleScriptSuite
								eventID:kASPrepositionalSubroutine
					   targetDescriptor:[NSAppleEventDescriptor currentProcessDescriptor]
		...
		do {
			...
			if (aKeyWord == keyASPrepositionGiven) {
				[self setParamDescriptor:[NSAppleEventDescriptor
											 descriptorWithObject:[NSAppleEventDescriptor
																	  userRecordDescriptorWithObjectAndKeys:theObject
																								  arguments:anArgList]]
							  forKeyword:keyASUserRecordFields];
