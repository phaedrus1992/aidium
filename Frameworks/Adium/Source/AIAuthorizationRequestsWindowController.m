...
- (void)toolbarWillAddItem:(NSNotification *)notification
{
	...
	if ([[item itemIdentifier] isEqualToString:AUTHORIZE]) {
		...
		[menu addItemWithTitle:AUTHORIZE target:self action:@selector(authorize:) keyEquivalent:@""];

		[menu addItemWithTitle:AUTHORIZE_ADD target:self action:@selector(authorizeAdd:) keyEquivalent:@""];
		...
		[menu addItemWithTitle:DENY target:self action:@selector(deny:) keyEquivalent:@""];

		[menu addItemWithTitle:DENY_BLOCK target:self action:@selector(denyBlock:) keyEquivalent:@""];
		...
		[menu addItemWithTitle:IGNORE target:self action:@selector(ignore:) keyEquivalent:@""];
 ...
 * - (void)authorizationWindowController:(NSWindowController *)inWindowController authorizationWithDict:(NSDictionary
 * *)infoDict response:(AIAuthorizationResponse)authorizationResponse
 */
...
- (void)getInfo:(id)sender
{
	...
	AIListContact *contact =
		[[selectedItem objectForKey:@"Account"] contactWithUID:[selectedItem objectForKey:@"Remote Name"]];
...
- (void)rebuildHeights
{
	...
	for (NSInteger row = 0; row < requests.count; row++) {
		...
		NSAttributedString *mainTitle = [[NSAttributedString alloc]
			initWithString:[self tableView:tableView objectValueForTableColumn:tableColumn row:row]
		...
		if (reason) {
			...
			NSAttributedString *subStringTitle =
				[[NSAttributedString alloc] initWithString:[[requests objectAtIndex:row] objectForKey:@"Reason"]
...
- (id)tableView:(NSTableView *)aTableView objectValueForTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)rowIndex
{
	...
	if ([identifier isEqualToString:@"icon"]) {
		...
		if (accounts.count > 1) {
			...
			return [NSString stringWithFormat:AILocalizedString(@"%@ on the account %@", nil), finalDisplay,
											  ((AIAccount *)[[requests objectAtIndex:rowIndex] objectForKey:@"Account"])
...
- (NSMenu *)tableView:(NSTableView *)inTableView menuForEvent:(NSEvent *)theEvent
{
	...
	if (inTableView.selectedRowIndexes.count == 1) {
		[menu addItemWithTitle:GET_INFO target:self action:@selector(getInfo:) keyEquivalent:@""];
