 ...
 *
 * userinfo is an NSDictionary with keys KEY_PREVIOUS_INSPECTED_OBJECT and KEY_NEW_INSPECTED_OBJECT and objects of class
 * AIListObject
 */
...
typedef enum { AINormalWindowLevel = 0, AIFloatingWindowLevel = 1, AIDesktopWindowLevel = 2 } AIWindowLevel;
...
forward of the Contact List, if necessary). *	@li	Else, if any chat window has unviewed content, bring foremost the
chat window and chat tab with the most recent unviewed content. *	@li	Else, if all chat windows are minimized,
unminimize one of them. *	@li	If the application is hidden, unhide it.
 *
 *	@return	A value suitable for returning from the \c NSApplication delegate method
<code>applicationShouldHandleReopen:hasVisibleWindows:
</code>. Specifically: \c YES if AppKit should perform its usual response to the event; \c NO if AppKit should do
nothing.
 */
 ...
 *
 * If multiple objects are selected, this returns the first one. If possible, use
 * arrayOfSelectedListObjectsInContactList and handle multiple selections.
 */
 ...
 *
 * @param inEntry The object which will be queried for information when building a tooltip. Must conform to the
 * AIContactListTooltipEntry protocol.
 * @param isSecondary Most tooltips are secondary; this means they are displayed in the bottom half of the tooltip which
...
- (void)openInterface;
- (void)closeInterface;
- (id)openChat:(AIChat *)chat
	inContainerWithID:(NSString *)containerID
			 withName:(NSString *)containerName
