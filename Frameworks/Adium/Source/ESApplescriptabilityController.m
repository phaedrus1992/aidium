...
- (void)setMyIdleTime:(NSTimeInterval)timeInterval
{
	[[NSNotificationCenter defaultCenter]
		postNotificationName:Adium_RequestSetManualIdleTime
					  object:(timeInterval ? [NSNumber numberWithDouble:timeInterval] : nil)userInfo:nil];
}

- (NSData *)defaultImageData
{
	return ([adium.preferenceController preferenceForKey:KEY_USER_ICON group:GROUP_ACCOUNT_STATUS]);
}
- (void)setDefaultImageData:(NSData *)newDefaultImageData
{
	[adium.preferenceController setPreference:newDefaultImageData forKey:KEY_USER_ICON group:GROUP_ACCOUNT_STATUS];
}
...
- (void)setMyStatus:(AIStatus *)newStatus
{
	if ([newStatus isKindOfClass:[AIStatus class]]) {
		...
		NSLog(@"Applescript error: Tried to set status to %@ which is of class %@.  This method expects an object of "
			  @"class %@.",
			  newStatus, NSStringFromClass([newStatus class]), NSStringFromClass([AIStatus class]));
	}
}
...
/*!
 * @brief Run an AppleScript, optionally calling a function with arguments, and notifying a target/selector with its
