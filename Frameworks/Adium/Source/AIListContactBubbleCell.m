...
- (void)drawBackgroundWithFrame:(NSRect)rect
{
	if (![self cellIsSelected]) {
		...
		if (!labelColor)
			labelColor = [self backgroundColor];
		...
		if (drawWithGradient) {
			...
			NSGradient *gradient =
				[[[NSGradient alloc] initWithStartingColor:labelColor
											   endingColor:[labelColor darkenAndAdjustSaturationBy:0.4f]] autorelease];
...
- (void)drawSelectionWithFrame:(NSRect)cellFrame
{
	if ([self cellIsSelected]) {
		...
		lastBackgroundBezierPath =
			[[NSBezierPath bezierPathWithRoundedRect:[self bubbleRectForFrame:cellFrame]] retain];
