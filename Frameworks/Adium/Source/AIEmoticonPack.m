...
- (AIEmoticonPack *)initFromPath:(NSString *)inPath
{
	if ((self = [super init])) {
		...
		if (xtraBundle && ([[xtraBundle objectForInfoDictionaryKey:@"XtraBundleVersion"] intValue] == 1)) {
			...
			//New style xtras store the same info, but it's in Contents/Resources/ so that we can have an info.plist
		file and use NSBundle. emoticonLocation = [[xtraBundle resourcePath] retain];
		}
		 */
...
- (void)loadEmoticons
{
	...
	if (infoDict) {
		...
		switch (version) {
		case 0:
			[self loadProteusEmoticons:infoDict];
			break;
		case 1:
			[self loadAdiumEmoticons:[infoDict objectForKey:EMOTICON_LIST] localizedStrings:localizedInfoDict];
			break;
		default:
			break;
		}
 ...
 *
 * @param emoticons A dictionary whose keys are file names and objects are themselves dictionaries with equivalent and
 * name information.
 */
- (void)loadAdiumEmoticons:(NSDictionary *)emoticons localizedStrings:(NSDictionary *)localizationDict
{
	...
	while ((fileName = [enumerator nextObject])) {
		...
		if ([dict isKindOfClass:[NSDictionary class]]) {
			...
			[emoticonArray
				addObject:[AIEmoticon emoticonWithIconPath:[bundle pathForImageResource:fileName]
...
// Upgrading
// ------------------------------------------------------------------------------------------------------------ Methods
// for opening and converting old format Adium emoticon packs
		...
		if ([[fileName lastPathComponent] characterAtIndex:0] != '.' &&
			...
			if (isDirectory) {
				...
				if (emoticonStrings && imagePath) {
					...
					[emoticonDict
						setObject:[NSDictionary dictionaryWithObjectsAndKeys:emoticonStrings, EMOTICON_EQUIVALENTS,
...
- (NSString *)_imagePathForEmoticonPath:(NSString *)inPath
{
	...
	while ((fileName = [enumerator nextObject])) {
		if ([fileName hasPrefix:@"Emoticon"])
...
- (NSArray *)_equivalentsForEmoticonPath:(NSString *)inPath
{
	...
	NSString *equivString = [NSMutableString stringWithContentsOfURL:equivFileURL
...
- (NSString *)_stringWithMacEndlines:(NSString *)inString
{
	...
	while (charRange.length != 0) {
		if (!newString)
			newString = [[inString mutableCopy] autorelease];
...
{
	return (
		[NSString stringWithFormat:@"[%@: %@, ServiceClass %@]", [super description], [self name], self.serviceClass]);
}
