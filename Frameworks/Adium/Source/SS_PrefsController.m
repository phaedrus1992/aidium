...
#define Last_Pane_Defaults_Key                                                                                         \
	[[[NSBundle mainBundle] bundleIdentifier] stringByAppendingString:@"_Preferences_Last_Pane_Defaults_Key"]
...
- (id)initWithPanesSearchPath:(NSString *)path bundleExtension:(NSString *)ext
{
	if ((self = [self init])) {
		...
		if (searchPath) {
			NSEnumerator *enumerator = [[NSBundle pathsForResourcesOfType:bundleExtension
															  inDirectory:searchPath] objectEnumerator];
			NSString *panePath;
...
- (void)createPreferencesWindowAndDisplay:(BOOL)shouldDisplay
{
	...
	[self
		debugLog:[NSString stringWithFormat:
							   @"Could not load last-used preference pane \"%@\". Trying to load another pane instead.",
							   lastViewName]];
	...
	[self debugLog:[NSString stringWithFormat:@"Could not load any valid preference panes. The preference pane bundle "
											  @"extension was \"%@\" and the search path was: %@",
											  bundleExtension, searchPath]];
			...
			if (!paneClass) {
				...
				if ([paneClass conformsToProtocol:@protocol(SS_PreferencePaneProtocol)] &&
					...
					[self debugLog:[NSString
									   stringWithFormat:
										   @"Did not load bundle: %@ because its Principal Class is either not an "
										   @"NSObject subclass, or does not conform to the PreferencePane Protocol.",
										   paneBundle]];
				}
			} else {
				[self debugLog:[NSString stringWithFormat:@"Did not load bundle: %@ because its Principal Class was "
														  @"already used in another Preference pane.",
														  paneBundle]];
			}
		} else {
			[self debugLog:[NSString stringWithFormat:@"Could not obtain name of Principal Class for bundle: %@",
													  paneBundle]];
		}
	} else {
...
- (BOOL)loadPrefsWithIdentifier:(NSString *)name display:(BOOL)disp
{
	if (!prefsWindow) {
		NSBeep();
		[self debugLog:[NSString stringWithFormat:@"Could not load \"%@\" preference pane because the Preferences "
												  @"window seems to no longer exist.",
												  name]];
		return NO;
	}
	...
	if (!tempPane) {
		[self debugLog:[NSString
						   stringWithFormat:@"Could not load preference pane \"%@\", because that pane does not exist.",
											name]];
		return NO;
	}
	...
	if (!prefsView) {
		[self debugLog:[NSString stringWithFormat:@"Could not load \"%@\" preference pane because its view could not "
	...
	if (newFrame.size.width < minimumWidthForToolbar)
		newFrame.size.width = minimumWidthForToolbar;
...
- (void)createPrefsToolbar
{
	...
	for (identifier in panesOrder) {
		if ([preferencePanes objectForKey:identifier] != nil) {
			...
			[item setPaletteLabel:paneName]; // item's label in the "Customize Toolbar" sheet (not relevant here, but we
											 // set it anyway)
			...
			[self debugLog:[NSString stringWithFormat:@"Could not create toolbar item for preference pane \"%@\", "
													  @"because that pane does not exist.",
													  identifier]];
		}
	}

	NSString *bundleIdentifier = [[NSBundle mainBundle] bundleIdentifier];
	prefsToolbar = [[NSToolbar alloc]
		initWithIdentifier:[bundleIdentifier stringByAppendingString:@"_Preferences_Toolbar_Identifier"]];
	...
	if ((prefsToolbarItems && ([prefsToolbarItems count] > 1)) || alwaysShowsToolbar) {
		...
		[self debugLog:@"Not showing toolbar in Preferences window because there is only one preference pane loaded. "
					   @"You can override this behaviour using -[setAlwaysShowsToolbar:YES]."];
	}
	...
	for (name in newPanesOrder) {
		if (([preferencePanes objectForKey:name] != nil) || ([name isEqual:NSToolbarSeparatorItemIdentifier])) {
			...
			[self debugLog:[NSString stringWithFormat:@"Did not add preference pane \"%@\" to the toolbar ordering "
													  @"array, because that pane does not exist.",
													  name]];
		}
	}
}
