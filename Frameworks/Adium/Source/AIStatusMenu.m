...
/*!
 * @brief The delegate is just too good for the menu items we've created; it will create all of the ones it wants on its
 * own
 */
		...
		if (account) {
			...
			if (menuItemStatusState) {
				...
				while (!foundCorrectStatusState && (statusState = [activeStatusStatesEnumerator nextObject])) {
					 ...
					 */
					foundCorrectStatusState =
						(![flatStatusSet containsObject:statusState] && ([menuItem tag] == statusState.statusType));
				}
	...
	if (isOptionClick || (([sender state] == NSOnState) && (statusItem.statusType != AIOfflineStatusType))) {
		...
		if (account) {
			...
			if (shouldRebuild) {
				...
				[[NSNotificationCenter defaultCenter] postNotificationName:AIStatusStateArrayChangedNotification
																	object:nil];
			}
...
- (IBAction)selectCustomState:(id)sender
{
	...
	/* If we are going to a custom state of a different type, we don't want to prefill with baseStatusState as it
	 * stands. Instead, we load the last used status of that type.
	 */
...
+ (NSMenu *)staticStatusStatesMenuNotifyingTarget:(id)target selector:(SEL)selector
{
	...
	if (!target && !selector) {
		// Need to set a target and action for items with submenus (AIStatusGroups) to be selectable... so if we're not
		// given one, set one.
		target = self;
