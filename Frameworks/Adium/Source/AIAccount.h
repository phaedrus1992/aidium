...
@class AIListContact, AIChat, AIContentObject, ESFileTransfer, AIStatus, AIContentMessage, AIContentNotification,
	AIContentTyping;
...
// Privacy
typedef enum { AIPrivacyTypePermit = 0, AIPrivacyTypeDeny } AIPrivacyType;
...
typedef enum { AIAuthorizationNoResponse = 0, AIAuthorizationDenied, AIAuthorizationAllowed } AIAuthorizationResponse;
 ...
 *
 * @param inWindowController The window controller which closed; an account may have kept track of what windows were
 * showing its authorization prompts
 ...
 * was no response
 */
- (void)authorizationWithDict:(NSDictionary *)__attribute__((ns_consumed)) infoDict
					 response:(AIAuthorizationResponse)authorizationResponse;
