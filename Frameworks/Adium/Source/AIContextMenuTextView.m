...
+ (NSMenu *)defaultMenu
{
	...
	NSMenu *adiumMenu = [adium.menuController
		contextualMenuWithLocations:[NSArray arrayWithObjects:[NSNumber numberWithInt:Context_TextView_LinkEditing],
...
- (void)textDidChange:(NSNotification *)notification
{
	if (([self selectedRange].location == 0) &&
		([self selectedRange].length == 0)) { // remove attributes if we're changing text at (0,0)
