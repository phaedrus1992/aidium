 ...
 */
- (id)initWithContactListView:(AIListOutlineView *)inContactListView
...
- (void)dealloc
{
	...
	contactListView = nil;
	[scrollView_contactList release];
...
- (void)setContactListRoot:(ESObjectWithProperties<AIContainingObject> *)newContactListRoot
{
	if (contactList != newContactListRoot) {
		[contactList release];
		contactList = [newContactListRoot retain];
	}
...
- (void)performExpandItem:(NSNotification *)notification
{
	...
	NSAssert([listObject conformsToProtocol:@protocol(AIContainingObject)],
			 @"Attempted to expand a non-containing object.");
...
- (void)performCollapseItem:(NSNotification *)notification
{
	...
	NSAssert([listObject conformsToProtocol:@protocol(AIContainingObject)],
			 @"Attempted to collapse a non-containing object.");
...
- (void)updateLayoutFromPrefDict:(NSDictionary *)prefDict andThemeFromPrefDict:(NSDictionary *)themeDict
{
	...
	if (windowStyle != AIContactListWindowStyleContactBubbles_Fitted) {
		...
		[contentCell
			setStatusIconsVisible:(allowIcons ? [[prefDict objectForKey:KEY_LIST_LAYOUT_SHOW_STATUS_ICONS] boolValue]
											  : NO)];
		[contentCell
			setServiceIconsVisible:(allowIcons ? [[prefDict objectForKey:KEY_LIST_LAYOUT_SHOW_SERVICE_ICONS] boolValue]
											   : NO)];
		[contentCell
			setExtendedStatusVisible:
				NO /*(allowIcons ? [[prefDict objectForKey:KEY_LIST_LAYOUT_SHOW_EXT_STATUS] boolValue] : NO)*/];
	...
	pillowsOrPillowsFittedWindowStyle = (windowStyle == AIContactListWindowStyleContactBubbles ||
	...
	if (pillowsOrPillowsFittedWindowStyle) {
		...
		[(AIListContactBubbleCell *)contentCell
			setDrawWithGradient:[[prefDict objectForKey:KEY_LIST_LAYOUT_CONTACT_BUBBLE_GRADIENT] boolValue]];
		...
		[(AIListGroupBubbleCell *)groupCell
			setHideBubble:[[prefDict objectForKey:KEY_LIST_LAYOUT_GROUP_HIDE_BUBBLE] boolValue]];
	} else {
		// Background
		[contactListView
			setUsesAlternatingRowBackgroundColors:[[themeDict objectForKey:KEY_LIST_THEME_GRID_ENABLED] boolValue]];
		[contactListView setBackgroundFade:[[themeDict objectForKey:KEY_LIST_THEME_BACKGROUND_FADE] floatValue]];
		[contactListView
			setBackgroundColor:[[themeDict objectForKey:KEY_LIST_THEME_BACKGROUND_COLOR] representedColor]];
...
- (void)listObjectAttributeChangesComplete:(NSNotification *)notification
{
	...
	if (object && [object isKindOfClass:[AIListContact class]] && ([(AIListContact *)object parentContact] != object)) {
		for (AIProxyListObject *proxyObject in
			 [[[(AIListContact *)object parentContact].proxyObjects copy] autorelease]) {
			[contactListView redisplayItem:proxyObject];
		}
	}
}
...
- (id)outlineView:(NSOutlineView *)outlineView child:(NSInteger)idx ofItem:(AIProxyListObject *)item
{
	...
	if (item) {
		...
		proxyListObject = [AIProxyListObject proxyListObjectForListObject:contactList inListObject:nil];
...
- (BOOL)outlineView:(NSOutlineView *)outlineView expandStateOfItem:(AIProxyListObject *)item
{
	return !item || ((id<AIContainingObject>)(item.listObject)).expanded ||
		   [item.listObject boolValueForProperty:@"ExpandedByFiltering"];
}
...
- (NSMenu *)contextualMenuForListObject:(AIListObject *)listObject
{
	...
	NSArray *locationsArray = [NSArray
		arrayWithObjects:[NSNumber numberWithInt:(isGroup ? Context_Group_Manage : Context_Contact_Manage)],
	...
	return [adium.menuController contextualMenuWithLocations:locationsArray forListObject:listObject];
}
							  ...
							  item:(AIProxyListObject *)item
{
	...
	if (outlineView == contactListView) {
		...
		[self outlineView:contactListView willDisplayCell:cell forTableColumn:tableColumn item:item];
...
- (void)setDragItems:(NSNotification *)notification
{
	...
	if (((id<AIContainingObject>)contactList).countOfContainedObjects == 0) {
		[[NSNotificationCenter defaultCenter] postNotificationName:DetachedContactListIsEmpty object:contactListView];
	}
}
		 ...
		 childIndex:(NSInteger)idx
{
	...
	[[NSNotificationCenter defaultCenter] postNotificationName:@"AIListControllerDragEnded" object:nil];
 ...
 * @param index The index within item into which the drag would currently drop. It may be a 0-based index inside item or
 * may be NSOutlineViewDropOnItemIndex.
...
- (void)showTooltipAtPoint:(NSPoint)screenPoint
{
	...
	if ([hoveredObject isKindOfClass:[AIListContact class]] ||
		([hoveredObject isKindOfClass:[AIListGroup class]] && ![(id<AIContainingObject>)hoveredObject isExpanded])) {
...
- (AIListObject *)contactListItemAtScreenPoint:(NSPoint)screenPoint
{
	...
	if (showTooltips &&
		([NSApp isActive] || (showTooltipsInBackground && ![contactListView.window hidesOnDeactivate]))) {
		...
		if (viewPoint.x > NSMinX(contactListFrame) && viewPoint.x < NSMaxX(contactListFrame)) {
			listObject =
				((AIProxyListObject *)[contactListView itemAtRow:[contactListView rowAtPoint:viewPoint]]).listObject;
		}
	}
