...
{
	if ((self = [super init])) {
		topSpacing = bottomSpacing = leftSpacing = rightSpacing = 0;
		topPadding = bottomPadding = leftPadding = rightPadding = 0;
...
@synthesize rightPadding, leftPadding, topPadding, bottomPadding, indentation, rightSpacing, leftSpacing, topSpacing,
	bottomSpacing;
// Drawing
// --------------------------------------------------------------------------------------------------------------
#pragma mark Drawing
...
- (NSRect)drawDisplayNameWithFrame:(NSRect)inRect
{
	...
	NSRect rect = inRect;
	if (nameSize.width > rect.size.width)
		nameSize.width = rect.size.width;
	if (nameSize.height > rect.size.height)
...
{
	NSString *label =
		([self shouldShowAlias] ? [[proxyObject listObject] longDisplayName]
								: ([proxyObject listObject].formattedUID ? [proxyObject listObject].formattedUID
...
- (BOOL)cellIsSelected
{
	return ([self isHighlighted] && [[self.outlineControlView window] isKeyWindow] &&
...
// The background color for this cell.  This will either be [controlView backgroundColor] or [controlView
// alternatingGridColor]
