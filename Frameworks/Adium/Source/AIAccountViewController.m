...
- (id)init
{
	...
	if ((self = [super init])) {
		...
		if (!view_setup)
			[ourBundle loadNibFile:@"AccountSetup" externalNameTable:nameTable withZone:nil];
		if (!view_profile)
			[ourBundle loadNibFile:@"AccountProfile" externalNameTable:nameTable withZone:nil];
		if (!view_options)
			[ourBundle loadNibFile:@"AccountOptions" externalNameTable:nameTable withZone:nil];
		if (!view_privacy)
			[ourBundle loadNibFile:@"AccountPrivacy" externalNameTable:nameTable withZone:nil];
...
{
	if (popUp_encryption) {
		[popUp_encryption setMenu:[adium.contentController encryptionMenuNotifyingTarget:nil withDefault:NO]];
 ...
 * @brief Privacy View
 *
 * Returns the account privacy view.  This view is for privacy options.  The default view provides options for
 * encryption (which is supported at present by all Gaim-provided protocols) and for sending the Typing status.
 */
- (NSView *)privacyView
{
	return view_privacy;
...
- (void)configureForAccount:(AIAccount *)inAccount
{
	if (account != inAccount) {
		...
		// UID Label
		// Must use AILocalized...FromTableInBundle() because this class is intended to be subclasses, and if the
		// subclass is in a different bundle, AILocalized...String fails.
		[textField_accountUIDLabel
			setStringValue:[[service userNameLabel]
							   stringByAppendingString:AILocalizedStringFromTableInBundle(
														   @":", nil,
														   [NSBundle bundleForClass:[AIAccountViewController class]],
														   "Colon which will be appended after a label such as 'User "
														   "Name', before an input field")]];

		// UID
		NSString *formattedUID = account.formattedUID;
		[textField_accountUID setStringValue:(formattedUID ? account.formattedUID : @"")];
		[textField_accountUID
			setFormatter:[AIStringFormatter
							 stringFormatterAllowingCharacters:[service allowedCharactersForAccountName]
														length:[service allowedLengthForAccountName]
												 caseSensitive:[service caseSensitive]
												  errorMessage:AILocalizedStringFromTableInBundle(
																   @"The characters you're entering are not valid for "
																   @"an account name on this service.",
																   nil,
																   [NSBundle
																	   bundleForClass:[AIAccountViewController class]],
																   nil)]];
		...
		NSString *alias = [[[account preferenceForKey:KEY_ACCOUNT_DISPLAY_NAME
												group:GROUP_ACCOUNT_STATUS] attributedString] string];
		[textField_alias setStringValue:(alias ? alias : @"")];
		[[textField_alias cell]
			setPlaceholderString:[[[adium.preferenceController preferenceForKey:KEY_ACCOUNT_DISPLAY_NAME
...
- (void)saveConfiguration
{
	...
	if (textField_accountUID) {
		...
		if (![account.UID isEqualToString:newUID] || ![account.formattedUID isEqualToString:newUID]) {
	...
	// Connect Port
	[account setPreference:([textField_connectPort intValue] ? [NSNumber numberWithInt:[textField_connectPort intValue]]
															 : nil)
	...
	[account setPreferences:changedPrefDict inGroup:GROUP_ACCOUNT_STATUS];
}
...
- (void)setValue:(id)value forKey:(NSString *)key
{
	...
	if (prefKey) {
		// If this is a key for which we have an Adium preferences mapping, set the value for saving in
		// saveConfiguration
...
- (id)valueForKey:(NSString *)key
{
	...
	if (prefKey) {
		...
		if (!value)
			value = [account preferenceForKey:prefKey group:GROUP_ACCOUNT_STATUS];
		return value;
	} else {
...
- (void)localizeStrings
{
	[label_password setLocalizedString:AILocalizedStringFromTableInBundle(
										   @"Password:", nil, [NSBundle bundleForClass:[AIAccountViewController class]],
										   "Label for the password field in the account preferences")];
	[label_typing
		setLocalizedString:
			AILocalizedStringFromTableInBundle(
				@"Typing:", nil, [NSBundle bundleForClass:[AIAccountViewController class]],
				"Label beside the 'let others know when you are typing' checkbox in the account preferences")];
	[checkBox_sendTyping setLocalizedString:AILocalizedStringFromTableInBundle(
												@"Let others know when you are typing", nil,
												[NSBundle bundleForClass:[AIAccountViewController class]],
												"Text of the typing preference checkbox in the account preferences")];
	[label_encryption
		setLocalizedString:AILocalizedStringFromTableInBundle(@"Encryption:", nil,
															  [NSBundle bundleForClass:[AIAccountViewController class]],
															  "Label besides the encryption preference menu")];
	[label_alias
		setLocalizedString:AILocalizedStringFromTableInBundle(
							   @"Alias:", nil, [NSBundle bundleForClass:[AIAccountViewController class]], nil)];
	[label_port setLocalizedString:AILocalizedStringFromTableInBundle(
									   @"Port:", nil, [NSBundle bundleForClass:[AIAccountViewController class]],
									   "Label for the port field in the account preferences")];
	[label_server
		setLocalizedString:AILocalizedStringFromTableInBundle(
							   @"Login Server:", nil, [NSBundle bundleForClass:[AIAccountViewController class]],
