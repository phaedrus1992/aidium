...
#define ACCOUNT_CONNECT_PARENS_MENU_TITLE                                                                              \
	AILocalizedString(@"%@ (Connecting)", "Account Name (Connecting) - shown for an account while it is connecting")
...
- (void)setDelegate:(id<AIAccountMenuDelegate>)inDelegate
{
	...
	includeDisabledAccountsMenu =
		([delegate respondsToSelector:@selector(accountMenuShouldIncludeDisabledAccountsMenu:)] &&
...
- (NSArray *)buildMenuItems
{
	...
	if (includeDisabledAccountsMenu) {
		...
		if (includeAddAccountsMenu) {
			...
			NSMenuItem *menuItem =
				[[NSMenuItem allocWithZone:[NSMenu menuZone]] initWithTitle:AILocalizedString(@"Add Account", nil)
																	 target:self
		...
		if ([disabledAccountMenu numberOfItems]) {
			NSMenuItem *menuItem =
				[[NSMenuItem allocWithZone:[NSMenu menuZone]] initWithTitle:AILocalizedString(@"Disabled Accounts", nil)
																	 target:self
																	 action:@selector(dummyAction:)
															  keyEquivalent:@""
														  representedObject:nil];
	...
	if (submenuType) {
		...
		[statusMenu performSelector:@selector(rebuildMenu) withObject:nil afterDelay:0];
	}
...
{
	if ([listObject isKindOfClass:[AIAccount class]] && ![(AIAccount *)listObject enabled]) {
...
- (void)_updateMenuItem:(NSMenuItem *)menuItem
{
	...
	if (account) {
		...
		/* The default font size for menu items in the main menu seems to be 1 pt larger than the font size produced by
		 * [NSFont menuFontOfSize:0.0f]: specifically, as of Mac OS X 10.4.10, it is 14 pt. This seems to be an AppKit
		 * bug.
		 */
		NSDictionary *titleAttributes = [NSDictionary
			dictionaryWithObjectsAndKeys:[NSFont menuFontOfSize:(useSystemFont
																	 ? [NSFont systemFontSizeForControlSize:controlSize]
																	 : 14.0f)],
										 NSFontAttributeName,
		...
		if ([account encrypted]) {
			...
			if (!securityInterfaceFramework)
				securityInterfaceFramework =
					[NSBundle bundleWithPath:@"/System/Library/Frameworks/SecurityInterface.framework"];
			...
			NSMutableAttributedString *SSLIconText =
			...
			[SSLIconText addAttribute:NSBaselineOffsetAttributeName
								value:[NSNumber numberWithFloat:-3.0f]
								range:NSMakeRange(0, [SSLIconText length])];
...
- (NSString *)_titleForAccount:(AIAccount *)account
{
	...
	if (!accountTitle || ![accountTitle length])
		accountTitle = NEW_ACCOUNT_DISPLAY_TEXT;
	...
	if (account.enabled) {
		if (showTitleVerbs) {
			if ([account boolValueForProperty:@"isConnecting"] || [account valueForProperty:@"waitingToReconnect"]) {
				...
				titleFormat =
					(account.online ? ACCOUNT_DISCONNECT_ACTION_MENU_TITLE : ACCOUNT_CONNECT_ACTION_MENU_TITLE);
			}
...
- (NSSet *)updateListObject:(AIListObject *)inObject keys:(NSSet *)inModifiedKeys silent:(BOOL)silent
{
	if ([inObject isKindOfClass:[AIAccount class]]) {
		...
		} else if ([inModifiedKeys containsObject:@"isOnline"] || [inModifiedKeys containsObject:@"isConnecting"] ||
		...
		if (submenuType && [inObject.service isSocialNetworkingService] &&
			[inModifiedKeys containsObject:@"isOnline"]) {
			menuItem = [self menuItemForAccount:(AIAccount *)inObject];

			if (menuItem) {
				[menuItem setSubmenu:socialNetworkingSubmenuForAccount((AIAccount *)inObject, [menuItem target],
...
- (void)editAccount:(id)sender
{
	[[NSNotificationCenter defaultCenter] postNotificationName:@"AIEditAccount" object:[sender representedObject]];
}
	...
	if ((submenu = [menuItem submenu])) {
		...
		while ((submenuItem = [enumerator nextObject])) {
			...
			if ((status = [[submenuItem representedObject] objectForKey:@"AIStatus"])) {
				newRepresentedObject =
					[[NSDictionary alloc] initWithObjectsAndKeys:status, @"AIStatus", account, @"AIAccount", nil];
			} else {
				// Custom status items don't have an associated AIStatus.
				newRepresentedObject = [[NSDictionary alloc] initWithObjectsAndKeys:account, @"AIAccount", nil];
			}
...
static NSMenu *socialNetworkingSubmenuForAccount(AIAccount *account, id target, SEL action, id self)
{
	 ...
	 */
	onlineOfflineItem = [[NSMenuItem allocWithZone:[NSMenu menuZone]]
			initWithTitle:(account.online ? AILocalizedString(@"Disconnect", nil) : AILocalizedString(@"Connect", nil))
				   target:target
				   action:action
			keyEquivalent:@""
		representedObject:account];
...
NSMenu *statusMenuForAccountMenuItem(NSArray *menuItemArray, NSMenuItem *accountMenuItem, BOOL addOriginalItems,
									 id self)
{
	...
	if ([account.service isSocialNetworkingService]) {
		accountSubmenu =
			socialNetworkingSubmenuForAccount(account, [accountMenuItem target], [accountMenuItem action], self);
	...
	if (account.enabled) {
		enableDisableItem =
			[[NSMenuItem allocWithZone:[NSMenu menuZone]] initWithTitle:AILocalizedString(@"Disable", nil)
																 target:self
																 action:@selector(toggleAccountEnabled:)
														  keyEquivalent:@""
													  representedObject:account];
		...
		enableDisableItem =
			[[NSMenuItem allocWithZone:[NSMenu menuZone]] initWithTitle:AILocalizedString(@"Enable", nil)
																 target:self
																 action:@selector(toggleAccountEnabled:)
														  keyEquivalent:@""
													  representedObject:account];
...
- (void)statusMenu:(AIStatusMenu *)inStatusMenu didRebuildStatusMenuItems:(NSArray *)menuItemArray
{
	...
	if (accountMenuItemsCount <= 1)
		return;

	for (i = 0; i < accountMenuItemsCount; i++) {
		/* Add the original items the last time; all other times, add copies of the items.
		 * An NSMenuItem can only be in one menu, so we have to make copies for all but one; without this specifier,
		 * we'd create one more copy than necessary, which is	inefficient.
		 */
		...
		if ([[menuItem representedObject] isKindOfClass:[AIAccount class]]) {
			...
			// The menu item is not for an AIAccount directly. If it has a submenu of AIAccount-representing
			// NSMenuItems, handle those.
			...
			if ((thisItemSubmenu = [menuItem submenu])) {
				...
				for (j = 0; j < thisItemSubmenuCount; j++) {
					...
					if ([[menuItem representedObject] isKindOfClass:[AIAccount class]]) {
						...
						accountSubmenu =
							statusMenuForAccountMenuItem(menuItemArray, menuItem, reallyAddOriginalItems, self);
