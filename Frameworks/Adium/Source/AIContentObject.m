...
- (BOOL)isSimilarToContent:(AIContentObject *)inContent
{
	if (source == [inContent source] && [[self type] compare:[inContent type]] == NSOrderedSame &&
		...
		[[self displayClasses] containsObject:@"autoreply"] ==
			[[inContent displayClasses] containsObject:@"autoreply"]) {
 ...
 *
 * This existed for AppleScript support of obtaining the message string in 1.1 and below.  I don't believe it worked
 * -eds. It will likely no longer be necessary with the GSoC 2007 Applescripting changes.  I've removed it from the
 * public API but left it here for now. -eds
 *
...
{
	return [NSString stringWithFormat:@"{%@ :<Source=%@> <Destination=%@> <Message=%@>}", [super description],
									  [self source], [self destination], [self message]];
}
