 ...
 * @param contactsToInvite An array of AIListContacts which will be invited to the chat once Chat_DidOpen is posted for
 * it (once it is open)
	 ...
	 withInvitationMessage:(NSString *)invitationMessage
{
	...
	chat = [adium.chatController chatWithName:inName identifier:nil onAccount:inAccount chatCreationInfo:inInfo];
	...
	if ([namesSeparatedByCommas length]) {
		...
		if ([contactNames count]) {
			...
			for (aContactName in contactNames) {
				...
				UID = [inAccount.service normalizeUID:[self impliedCompletion:aContactName]
...
- (NSDragOperation)doDraggingEntered:(id<NSDraggingInfo>)sender
{
	...
	if ([pboard availableTypeFromArray:[NSArray arrayWithObject:@"AIListObject"]]) {
		...
		// If so, get the ID's
		if ([[pboard availableTypeFromArray:[NSArray arrayWithObject:@"AIListObjectUniqueIDs"]]
				isEqualToString:@"AIListObjectUniqueIDs"]) {
		...
		if ([[pboard availableTypeFromArray:[NSArray arrayWithObject:@"AIListObjectUniqueIDs"]]
			...
			for (uniqueID in dragItemsUniqueIDs) {
				...
				if ((listContact = [self validContact:uniqueID withService:account.service])) {
					...
					if ([oldValue length] &&
						![[oldValue substringFromIndex:([oldValue length] - 1)] isEqualToString:@","]) {
