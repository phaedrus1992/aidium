...
, AIDelayedContentFilter, AIHTMLContentFilter;
 ...
 * @param context An object, such as an AIListContact or an AIAccount, potentially relevant to filtration. May be
 * anything, so check its class as needed.
 * @result The filtered attributed string, which may be the same as attributedString, or nil if the attributed string
 * should be discared.
 */
 ...
 * AIHTMLContentFilters only see strings just as the WKMV is about to display them. This allows outputting custom html
 * for display
 */
@protocol AIHTMLContentFilter
/*!
 ...
 * @param content The AIContentObject associated with this HTML, allowing access to the source and destination for the
 * message
 ...
 *
 * [adium.contentController delayedFilterDidFinish:uniqueID:] should be called with the eventual result if this method
 * returns YES. If the filter eventually fails, this method MUST be called with the original inAttributedString.
 *
 * @param inAttributedString NSAttributedString to filter
 * @param context An object, such as an AIListContact or an AIAccount, potentially relevant to filtration. May be
 * anything, so check its class as needed.
 * @param uniqueID A uniqueID which will be passed back to [adium.contentController delayedFilterDidFinish:uniqueID:]
 * when this filtration is complete.
 *
