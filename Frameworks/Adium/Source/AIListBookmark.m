...
{
	if ((self = [super initWithUID:inUID account:inAccount service:inService])) {
...
- (id)initWithChat:(AIChat *)inChat
{
	if ((self = [self initWithUID:[NSString stringWithFormat:@"Bookmark:%@", inChat.uniqueChatID]
		...
		if ([inChat valueForProperty:KEY_TOPIC]) {
			[self setStatusMessage:[NSAttributedString stringWithString:[inChat valueForProperty:KEY_TOPIC]]
							notify:NotifyNow];
		}
...
- (id)initWithCoder:(NSCoder *)decoder
{
	...
	if ((self = [self
			 initWithUID:[decoder decodeObjectForKey:@"UID"]
				 account:myAccount
...
{
	AIChat *chat = [adium.chatController existingChatWithName:[self name] onAccount:self.account];
...
{
	if (!internalObjectID) {
		NSAssert(self.account != nil, @"Null list bookmark account - make sure you didn't try to touch the "
...
/*!
 * @brief For a newly created bookmark, set the group that -restoreGrouping will move us to. This is saved, so has no
 * use on existing bookmarks
 */
- (void)setInitialGroup:(AIListGroup *)inGroup
{
	[self setPreference:inGroup.UID forKey:KEY_CONTAINING_OBJECT_UID group:PREF_GROUP_OBJECT_STATUS_CACHE];
}
...
- (void)addContainingGroup:(AIListGroup *)inGroup
{
	...
	if ((!savedGroupUID || ![groupUID isEqualToString:savedGroupUID]) &&
		...
		[self setPreference:groupUID forKey:KEY_CONTAINING_OBJECT_UID group:PREF_GROUP_OBJECT_STATUS_CACHE];
	}
}
...
- (void)restoreGrouping
{
	...
	NSString *savedGroupUID = [self preferenceForKey:KEY_CONTAINING_OBJECT_UID group:PREF_GROUP_OBJECT_STATUS_CACHE]
								  ?: AILocalizedString(@"Bookmarks", nil);
...
- (AIChat *)openChatWithoutActivating
{
	if (self.account.joiningGroupChatRequiresCreationDictionary && !self.chatCreationDictionary) {
		if (NSRunAlertPanel(
				AILocalizedString(@"Unable to join bookmarked chat", nil),
				AILocalizedString(@"The bookmark %@ does not contain enough information and can not be used. Please "
				...
				AILocalizedStringFromTable(@"Cancel", @"Buttons", nil), nil,
...
- (void)claimChatIfOurs:(AIChat *)chat
{
	if ([self chatIsOurs:chat]) {
		chat.displayName = self.displayName;
		[self setStatusMessage:[NSAttributedString stringWithString:([chat valueForProperty:KEY_TOPIC] ?: @"")]
						notify:NotifyNow];
	}
}
...
- (BOOL)chatIsOurs:(AIChat *)chat
{
	return (chat && [chat.name isEqualToString:[self.account.service normalizeChatName:self.name]] &&
					   ...
					   context:(void *)context
{
	if ([keyPath isEqualToString:@"isOnline"] && object == self.account) {
		// If an account is just initially signing on, a -setOnline:notify:silently will still broadcast an event for
		// the contact. The initial delay an account (usually) sets is done after they're set as online, so these
		// bookmarks would always fire. Thus, we have to use the secondary, silent notification so that the online gets
		// propogated without the events.
...
- (NSSet *)updateChat:(AIChat *)inChat keys:(NSSet *)inModifiedKeys silent:(BOOL)silent
{
	if ([self chatIsOurs:inChat]) {
		...
		if ([inModifiedKeys containsObject:KEY_UNVIEWED_CONTENT] ||
			[inModifiedKeys containsObject:KEY_UNVIEWED_MENTION]) {
			[self _updateUnreadMessagesStatusForChat:inChat];
		}
	}
...
- (void)_updateUnreadMessagesStatusForChat:(AIChat *)inChat
{
	...
	if (inChat.unviewedMentionCount) {
		...
		if (inChat.unviewedMentionCount > 1) {
			statusMessage =
				[NSString stringWithFormat:AILocalizedString(@"%lu mentions, %lu messages",
															 "Status message for a bookmark (>1 mention, >1 messages)"),
										   (unsigned long)inChat.unviewedMentionCount,
										   (unsigned long)inChat.unviewedContentCount];
		} else if (inChat.unviewedContentCount > 1) {
			statusMessage =
				[NSString stringWithFormat:AILocalizedString(@"1 mention, %lu messages",
															 "Status message for a bookmark (1 mention, >1 messages)"),
										   (unsigned long)inChat.unviewedContentCount];
		} else {
			statusMessage =
				AILocalizedString(@"1 mention, 1 message", "Status message for a bookmark (1 mention, 1 message)");
		}
		...
		if (inChat.unviewedContentCount > 1) {
			statusMessage = [NSString
				stringWithFormat:AILocalizedString(@"%lu messages", "Status message for a bookmark (>1 messages)"),
								 (unsigned long)inChat.unviewedContentCount];
		} else {
...
{
	return
		[NSString stringWithFormat:@"<%@:%p %@ - %@ on %@ in %@>", NSStringFromClass([self class]), self,
								   self.formattedUID, [self chatCreationDictionary], self.account, self.remoteGroups];
}
