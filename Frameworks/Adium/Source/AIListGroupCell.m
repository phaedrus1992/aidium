...
- (NSRect)drawGroupCountWithFrame:(NSRect)inRect
{
	...
	if ([listObject valueForProperty:@"countText"]) {
		...
		if (countSize.width + GROUP_COUNT_PADDING > rect.size.width)
			countSize.width = rect.size.width;
...
- (NSMutableDictionary *)labelAttributes
{
	...
	[labelAttributes setObject:leftParagraphStyleWithTruncatingMiddle forKey:NSParagraphStyleAttributeName];
...
- (void)drawBackgroundGradientInRect:(NSRect)inRect
{
	...
	if (gradientL < backgroundL) { // Seal the top
		...
		[backgroundColor set];
		[NSBezierPath
			fillRect:NSMakeRect(inRect.origin.x, inRect.origin.y + inRect.size.height - 1, inRect.size.width, 1)];
	}
	...
	if (drawsGradientEdges) {
		...
		[NSBezierPath
			fillRect:NSMakeRect(inRect.origin.x + inRect.size.width - 1, inRect.origin.y, 1, inRect.size.height)];
	}
}
