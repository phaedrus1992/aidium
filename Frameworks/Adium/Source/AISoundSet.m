...
- (BOOL)_loadSoundSetFromPath:(NSString *)inPath
{
	...
	if (![[NSFileManager defaultManager] fileExistsAtPath:soundPlistPath]) {
		if (![self _upgradeTextBasedSoundSet:inPath]) {
			...
			NSString *newFolder =
				[[currentFolder stringByDeletingLastPathComponent] stringByAppendingPathComponent:newFolderName];
			...
			NSRunAlertPanel(AILocalizedString(@"Sound set upgrade failed", nil),
							AILocalizedString(@"This version of Adium uses a new format for sound sets. Adium was not "
	...
	if (success) {
		...
		if (version == 1) {
			...
			if (!locations)
				locations = [NSArray arrayWithObject:inPath];
			...
			for (NSString *key in [localSounds keyEnumerator]) {
				[(NSMutableDictionary *)sounds
					setObject:[[self _fullPathForSoundAtLocalPath:[localSounds objectForKey:key]
			...
			NSRunAlertPanel(AILocalizedString(@"Cannot open sound set", nil),
							AILocalizedString(@"The sound set %@ is version %i, and this version of Adium does not "
											  @"know how to handle that; perhaps try a later version of Adium.",
											  nil),
...
- (BOOL)_upgradeTextBasedSoundSet:(NSString *)setPath
{
	...
	if (success) {
		...
		if (success) {
			...
			oldSetString = [NSString
				stringWithContentsOfURL:[NSURL fileURLWithPath:[tempSetPath
																   stringByAppendingPathComponent:
																	   [setName stringByAppendingPathExtension:@"txt"]]]
		...
		if (success) {
			NSDictionary *infoDict =
				[NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithInt:1], SOUNDSET_VERSION, oldSetInfo,
														   SOUNDSET_INFO, newSounds, SOUNDSET_SOUNDS, nil];
...
{
	return [NSString stringWithFormat:@"<%@: %p - %@: %lu sounds at %@>", NSStringFromClass([self class]), self,
									  self.name, (unsigned long)[self.sounds count], sourcePath];
}
@end
