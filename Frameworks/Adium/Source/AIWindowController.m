...
- (NSRect)savedFrameFromString:(NSString *)frameString
{
	...
	if (windowFrame.size.width < minFrameSize.width)
		windowFrame.size.width = minFrameSize.width;
	if (windowFrame.size.height < minFrameSize.height)
		windowFrame.size.height = minFrameSize.height;
	if (windowFrame.size.width > maxFrameSize.width)
		windowFrame.size.width = maxFrameSize.width;
	if (windowFrame.size.height > maxFrameSize.height)
		windowFrame.size.height = maxFrameSize.height;

	// Don't allow the window to shrink smaller than its toolbar
	NSRect contentFrame = [NSWindow contentRectForFrameRect:windowFrame styleMask:[[self window] styleMask]];
	...
	if (NSMaxX(windowFrame) < NSMinX(screenBoundariesRect))
		windowFrame.origin.x = NSMinX(screenBoundariesRect);
	if (NSMinX(windowFrame) > NSMaxX(screenBoundariesRect))
		windowFrame.origin.x = NSMaxX(screenBoundariesRect) - NSWidth(windowFrame);
	if (NSMaxY(windowFrame) < NSMinY(screenBoundariesRect))
		windowFrame.origin.y = NSMinY(screenBoundariesRect);
	if (NSMinY(windowFrame) > NSMaxY(screenBoundariesRect))
		windowFrame.origin.y = NSMaxY(screenBoundariesRect) - NSHeight(windowFrame);
 ...
 * This allows saving a separate, unique saved frame for each new combination of monitor resolutions and relative
 * positions.
 */
 ...
 *
 * Subclasses have no business overriding this method.  See adiumFrameAutosaveName for the right place to determine the
 * name under which the frame is stored.
 *
...
- (NSString *)savedFrameString
{
	...
	if (key) {
		...
		if (!frameString) {
			...
			frameString = [adium.preferenceController
				preferenceForKey:[NSString stringWithFormat:@"%@-%lu", key, (unsigned long)[[NSScreen screens] count]]
						   group:PREF_GROUP_WINDOW_POSITIONS];
			if (!frameString) {
				// Fall back on the single screen preference if necessary (this is effectively a preference upgrade).
				frameString = [adium.preferenceController preferenceForKey:key group:PREF_GROUP_WINDOW_POSITIONS];
			}
		}
	}
...
- (NSString *)stringWithSavedFrame
{
	 ...
	 * we need to shift the origin to make up for the change in height. This is the bit that 10.5's -[NSWindow
	 * stringWithSavedFrame] gets wrong.
	 */
	if (NSMinY(frame) > NSMinY([[window screen] visibleFrame]))
		frame.origin.y += toolbarHeight;

	return [NSString stringWithFormat:@"%.0f %.0f %.0f %.0f %.0f %.0f %.0f %.0f", NSMinX(frame), NSMinY(frame),
									  NSWidth(frame), NSHeight(frame), NSMinX(screenFrame), NSMinY(screenFrame),
									  NSWidth(screenFrame), NSHeight(screenFrame)];
}
