...
- (AIListContact *)contactFromTextField
{
	...
	// It is not on our contact list, create a contact if possible and start a new chat, otherwise, tell the user they
...
- (void)AI_configureTextFieldForAccount:(AIAccount *)account
{
	...
	for (AIListContact *contact in adium.contactController.allContacts) {
		if (account == nil || ([contact.service.serviceClass isEqualToString:account.service.serviceClass] &&
							   (![contact isKindOfClass:[AIMetaContact class]] ||
								[(AIMetaContact *)contact uniqueContainedObjectsCount]))) {
...
{
	NSString *accountID = [adium.preferenceController
		preferenceForKey:[NSString stringWithFormat:@"AccountPlusFieldLastAccountID:%@", [self lastAccountIDKey]]
...
- (void)_saveConfiguredAccount
{
	[adium.preferenceController
		setPreference:[[[popUp_service selectedItem] representedObject] internalObjectID]
...
{
	// Controls
	[button_cancel setLocalizedString:AILocalizedStringFromTableInBundle(
										  @"Cancel", nil,
										  [NSBundle bundleForClass:[AIAccountPlusFieldPromptController class]], nil)];
...
{
	if ([aNotification object] == nil || [aNotification object] == textField_handle) {
	...
	for (AIAccount *account in adium.accountController.accounts) {
		if ([self accountMenu:inAccountMenu shouldIncludeAccount:account]) {
			...
			if (numberOfOnlineAccounts > 1) {
				anyItem = [[NSMenuItem alloc]
					initWithTitle:AILocalizedStringFromTableInBundle(
									  @"Any", nil, [NSBundle bundleForClass:[AIAccountPlusFieldPromptController class]],
									  nil)
						   action:nil
					keyEquivalent:@""];
				break;
