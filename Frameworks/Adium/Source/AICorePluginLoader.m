...
- (void)loadPlugins
{
	...
	// If the Adium version has increased since our last run, warn the user that their external plugins may no longer
	// work
	NSString *lastVersion = [[NSUserDefaults standardUserDefaults] objectForKey:CONFIRMED_PLUGINS_VERSION];
	if (!lastVersion || [adium compareVersion:[NSApp applicationVersion] toVersion:lastVersion] == NSOrderedAscending) {
	...
	NSString *internalPluginsPath = [[[[NSBundle mainBundle] bundlePath]
		stringByAppendingPathComponent:DIRECTORY_INTERNAL_PLUGINS] stringByExpandingTildeInPath];
 ...
 * @param inPluginArray May be nil.  If non-nil, an NSMutableArray to fill with an instance of the principal class
 * (AIPlugin conforming) of each plugin which loads.
 */
+ (void)loadPluginAtPath:(NSString *)pluginPath
		  confirmLoading:(BOOL)confirmLoading
			 pluginArray:(NSMutableArray *)inPluginArray
{
#ifdef PLUGIN_LOAD_TIMING
	...
	if (![self confirmPluginArchitectureAtPath:pluginPath]) {
		NSRunInformationalAlertPanel(
			[NSString
				stringWithFormat:AILocalizedString(@"Plugin %@ Will be Disabled",
												   "%@ will be the name of a plugin. This is the title of the dialogue "
												   "shown when an plugin is loaded on an unsupported architecture."),
	...
	if (confirmLoading &&
		(![self confirmMinimumVersionMetForPluginAtPath:pluginPath] || ![self confirmPluginAtPath:pluginPath]))
		return;

	if (![self allDependenciesMetForPluginAtPath:pluginPath]) {
		if (!deferredPluginPaths)
			deferredPluginPaths = [[NSMutableArray alloc] init];
	...
	@try {
		if ((pluginBundle = [NSBundle bundleWithPath:pluginPath])) {
			...
			if ([self pluginIsBlacklisted:pluginBundle]) {
				NSRunInformationalAlertPanel(
					[NSString
						stringWithFormat:AILocalizedString(@"Plugin %@ Will be Disabled",
														   "%@ will be the name of a plugin. This is the title of the "
														   "dialogue shown when an plugin is blacklisted."),
										 [[pluginPath lastPathComponent] stringByDeletingPathExtension]],
					AILocalizedString(@"This plugin is known to be incompatible with AdiumY %@.",
									  "%@ will be a version number of Adium"),
					AILocalizedString(@"Disable", nil), nil, nil, [NSApp applicationVersion]);
				[self disablePlugin:pluginPath];
				return;
			}
			...
			if (principalClass) {
				...
				NSLog(@"Failed to obtain principal class from plugin \"%@\" (\"%@\")! infoDictionary: %@",
					  [pluginPath lastPathComponent], pluginPath, [pluginBundle infoDictionary]);
			}
		...
		if (confirmLoading) {
			...
			[self disablePlugin:pluginPath];
			NSRunCriticalAlertPanel([NSString stringWithFormat:@"Error loading %@", [[pluginPath lastPathComponent]
																						stringByDeletingPathExtension]],
									@"An external plugin failed to load and has been disabled.  Please relaunch AdiumY",
									@"Quit", nil, nil);
			[NSApp terminate:nil];
		}
	}
	...
	if (![[NSUserDefaults standardUserDefaults] boolForKey:@"AdiumYAutoConfirmExternalPlugins"] &&
		...
		if (NSRunInformationalAlertPanel(
				[NSString stringWithFormat:AILocalizedString(@"Disable %@?",
															 "%@ will be the name of a plugin. This is the title of "
															 "the dialogue shown when an unknown plugin is loaded"),
										   [[pluginPath lastPathComponent] stringByDeletingPathExtension]],
				AILocalizedString(
					@"External plugins may cause crashes and odd behavior after updating AdiumY.  Disable "
					@"this plugin if you experience any issues.",
					nil),
				AILocalizedString(@"Disable", nil), AILocalizedString(@"Continue", nil), nil) == NSAlertDefaultReturn) {
			// Disable this plugin
			[self disablePlugin:pluginPath];
			loadPlugin = NO;
			...
			confirmed = (confirmed ? [confirmed arrayByAddingObject:[pluginPath lastPathComponent]]
								   : [NSArray arrayWithObject:[pluginPath lastPathComponent]]);
...
+ (BOOL)confirmMinimumVersionMetForPluginAtPath:(NSString *)pluginPath
{
	NSString *minimumVersionOfPlugin =
		[[[NSBundle bundleWithPath:pluginPath] infoDictionary] objectForKey:@"AIMinimumAdiumVersionRequirement"];
	...
	NSComparisonResult versionComparison = [adium compareVersion:appVersion toVersion:minimumVersionOfPlugin];

	if (versionComparison == NSOrderedAscending) {
		NSString *pluginName = [[pluginPath lastPathComponent] stringByDeletingPathExtension];

		NSRunAlertPanel([NSString stringWithFormat:@"Could not load %@", pluginName],
						@"%@ requires AdiumY %@ or later, but you have AdiumY %@. Please upgrade AdiumY to use %@",
						AILocalizedString(@"Disable", nil), nil, nil, pluginName, minimumVersionOfPlugin, appVersion,
						pluginName);
		[self disablePlugin:pluginPath];
		return NO;
	}
...
+ (BOOL)allDependenciesMetForPluginAtPath:(NSString *)pluginPath
{
	NSArray *dependencies =
		[[[NSBundle bundleWithPath:pluginPath] infoDictionary] objectForKey:@"AIPluginDependencies"];
...
+ (void)disablePlugin:(NSString *)pluginPath
{
	...
	NSString *disabledPath =
		[[basePath stringByDeletingLastPathComponent] stringByAppendingPathComponent:EXTERNAL_DISABLED_PLUGIN_FOLDER];

	[[NSFileManager defaultManager] createDirectoryAtPath:disabledPath
