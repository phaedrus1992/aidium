									...
									includeSubject:(BOOL)includeSubject
{
	...
	if (includeSubject) {
		description = [NSString stringWithFormat:AILocalizedString(@"%@ requested authorization",
																   "Event: <A contact's name> requested authorization"),
												 listObject.formattedUID];

	} else {
		description =
			AILocalizedString(@"requested authorization",
							  "Event: requested authorization (follows a contact's name displayed as a header)");
	}
...
- (NSImage *)imageForEventID:(NSString *)eventID
{
	...
	if (!eventImage)
		eventImage = [[NSImage imageNamed:@"default-icon" forClass:[self class]] retain];
	return eventImage;
}
