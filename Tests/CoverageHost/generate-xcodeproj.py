...
# main.m — minimal Cocoa app (empty, just serves as the test host)
...
'''
with open(os.path.join(PROJECT_DIR, "main.m"), "w") as f:
    f.write(main_m)

# CoverageHostTest.m — exercises AIUtilities functions for coverage data
test_m = '''#import <XCTest/XCTest.h>
...
@end
'''
with open(os.path.join(PROJECT_DIR, "CoverageHostTest.m"), "w") as f:
    f.write(test_m)

# CoverageHost-Info.plist (app host)
host_plist = {
...
with open(os.path.join(PROJECT_DIR, "CoverageHost-Info.plist"), "wb") as f:
    plistlib.dump(host_plist, f, sort_keys=False)

# CoverageHostTests-Info.plist (test bundle)
test_plist = {
    "CFBundleDevelopmentRegion": "en",
    "CFBundleExecutable": "$(EXECUTABLE_NAME)",
    "CFBundleIdentifier": "$(PRODUCT_BUNDLE_IDENTIFIER)",
...
with open(os.path.join(PROJECT_DIR, "CoverageHostTests-Info.plist"), "wb") as f:
    plistlib.dump(test_plist, f, sort_keys=False)
