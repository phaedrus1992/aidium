...
#define TEST_ATTR(name, attrStr, attrName, rangeLoc, rangeLen, checkBlock)                                             \
	do {                                                                                                               \
...
void runBoldTests(NSFont *baseFont)
{
	...
	// Basic bold: *hello*
	NSAttributedString *result = [AMPurpleJabberMessageStylingParser attributedStringFromStyledBody:@"*hello*"
																							   font:baseFont];
...
void runItalicTests(NSFont *baseFont)
{
	...
	NSAttributedString *result = [AMPurpleJabberMessageStylingParser attributedStringFromStyledBody:@"_hello_"
																							   font:baseFont];
...
void runNestingTests(NSFont *baseFont)
{
	...
	NSAttributedString *result = [AMPurpleJabberMessageStylingParser attributedStringFromStyledBody:@"_*hello*_"
																							   font:baseFont];
...
void runNestedBlockquoteTests(NSFont *baseFont)
{
	...
	NSAttributedString *result = [AMPurpleJabberMessageStylingParser attributedStringFromStyledBody:@"> > deeply nested"
																							   font:baseFont];
	TEST(@"nested blockquote: content is 'deeply nested'", stringAtRangeEquals(result, 0, 13, @"deeply nested"));

	// Bold inside nested blockquote
	result = [AMPurpleJabberMessageStylingParser attributedStringFromStyledBody:@"> > *bold inside*" font:baseFont];
	TEST(@"nested blockquote: bold inside nested quote", hasBoldTrait(result, 0, 11));
	TEST(@"nested blockquote: bold content is 'bold inside'", stringAtRangeEquals(result, 0, 11, @"bold inside"));
}
...
void runDepthLimitTests(NSFont *baseFont)
{
	...
	NSAttributedString *result = [AMPurpleJabberMessageStylingParser attributedStringFromStyledBody:deepNest
																							   font:baseFont];
...
void runLanguageHintTests(NSFont *baseFont)
{
	...
	NSAttributedString *result = [AMPurpleJabberMessageStylingParser attributedStringFromStyledBody:body font:baseFont];
	TEST(@"language hint: code content is 'int x = 1;'", stringAtRangeEquals(result, 0, 10, @"int x = 1;"));
	TEST(@"language hint: has monospace font", hasMonospaceFont(result, 0, 10));
	...
	TEST(@"language hint: 'objectivec' not in output", hintRange.location == NSNotFound);
}
...
void runRTLTests(NSFont *baseFont)
{
	...
	NSAttributedString *result = [AMPurpleJabberMessageStylingParser attributedStringFromStyledBody:arabicBody
																							   font:baseFont];
	NSArray *writingDir = [result attribute:NSWritingDirectionAttributeName atIndex:0 effectiveRange:NULL];
	TEST(@"RTL: Arabic text gets writing direction attribute", writingDir != nil);

	// LTR text should NOT get writing direction attribute
	result = [AMPurpleJabberMessageStylingParser attributedStringFromStyledBody:@"hello" font:baseFont];
	writingDir = [result attribute:NSWritingDirectionAttributeName atIndex:0 effectiveRange:NULL];
...
void runBlockquoteStylingTests(NSFont *baseFont)
{
	...
	NSAttributedString *result = [AMPurpleJabberMessageStylingParser attributedStringFromStyledBody:@"> hello"
																							   font:baseFont];
	NSParagraphStyle *para = [result attribute:NSParagraphStyleAttributeName atIndex:0 effectiveRange:NULL];
...
int main(int argc, const char *argv[])
{
	...
	printf("\n======================================\n");
	printf("Results: %lu passed, %lu failed out of %lu\n", (unsigned long)g_testsPassed, (unsigned long)g_testsFailed,
